...
> -   バージョン v8.1.0 から v8.5.2 では、TiDB および TiDB Dashboardのテレメトリ機能が削除されます。
> -   バージョン v8.5.3 から v8.5.6 では、TiDBはテレメトリ機能を再度導入しました。ただし、テレメトリ関連の情報はローカルにのみ記録され、ネットワーク経由でPingCAPにデータが送信されなくなりました。
> -   v8.5.7以降、TiDB および TiDB Dashboardのテレメトリ機能は非推奨です。
