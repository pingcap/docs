...
- 4.3.1 TiKV RocksDB encounters `write stall`.
    ...
    A TiKV instance has two RocksDB instances, one in `data/raft` to save the Raft log, another in `data/db` to save the real data. You can check the specific cause for stall by running `grep "Stalling" RocksDB` in the log. The RocksDB log is a file starting with `LOG`, and `LOG` is the current log. `write stall` is a RocksDB-native performance degradation mechanism. When write stall occurs in RocksDB, the system performance drops significantly. Prior to v5.2.0, TiDB tries to stop further write requests by returning a `ServerIsBusy` error directly to the client, but this could lead to a sharp decrease in QPS performance. Since v5.2.0, TiKV has introduced a new flow control mechanism that suppresses writes by delaying write requests dynamically in the scheduling layer to replace the previous mechanism of returning `server is busy` to clients when encountering `write stall`. The new flow control mechanism is enabled by default, and TiKV will automatically disable the `write stall` mechanism in `KvDB` and `RaftDB` (except memtable). However, when the number of pending requests exceeds a certain threshold, the flow control mechanism will still take effect, beginning to reject some or all write requests and returning `server is busy` errors to clients. For more information and thresholds, please refer to the [Flow Control Configuration Explanation](/tikv-configuration-file#storageflow-control).
    ...
    - Too many `pending compaction bytes` causes stall. The disk I/O fails to keep up with the write operations in business peaks. You can mitigate this problem by increasing the `soft-pending-compaction-bytes-limit` and `hard-pending-compaction-bytes-limit` of the corresponding CF.
        ...
        - The default value of `[storage.flow-control] soft-pending-compaction-bytes-limit` is `192GB`. If the pending compaction bytes reaches the threshold, RocksDB slows down the write speed. You can set `[storage.flow-control] soft-pending-compaction-bytes-limit` to `384GB`.

        - The default value of `[storage.flow-control] hard-pending-compaction-bytes-limit` is `1024GB`. If the pending compaction bytes reaches the threshold (this is not likely to happen, because RocksDB slows down the write after the pending compaction bytes reaches `soft-pending-compaction-bytes-limit`), RocksDB stops the write operation (by returning `ServerIsBusy` Error). You can set `[storage.flow-control] hard-pending-compaction-bytes-limit` to `2048GB`.<!-- See [case-275](https://github.com/pingcap/tidb-map/blob/master/maps/diagnose-case-study/case275.md) in Chinese.-->
