...
- 3.1.2 TiDB DDL job hangs or executes slowly (use `admin show ddl jobs` to check DDL progress)
    ...
    - Cause 1: TiDB introduces [metadata lock](/metadata-lock.md) in v6.3.0 and enables it by default in v6.5.0. If the tables involved in the DDL operation have intersections with the tables involved in the uncommitted transaction, the DDL operation is blocked until the transaction is committed or rolled back.
