...
> - TiDB が特定のクエリを実行する際に MPP モードを選択しているかどうかを確認するには、 [MPPモードの説明文](/explain-mpp.md#explain-statements-in-the-mpp-mode)を参照してください。 `EXPLAIN`文の出力に`ExchangeSender`および`ExchangeReceiver`オペレーターが含まれている場合、MPP モードが使用されています。
