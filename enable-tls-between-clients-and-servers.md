...
-   認証:(オプション）クライアントはサーバーのIDを検証でき、サーバーはクライアントのIDを検証して中間者攻撃を回避できます
...
TiDB v5.2.0以降、 `Ssl_server_not_after`および`Ssl_server_not_before`のステータス変数を使用して、証明書の有効性の開始日と終了日を監視できます。
