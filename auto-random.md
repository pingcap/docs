...
- Specify this attribute for the primary key column **ONLY** of integer type. Otherwise, an error might occur. In addition, when the primary key attribute is `NONCLUSTERED`, `AUTO_RANDOM` is not supported even on the integer primary key. For more details about the primary key of `CLUSTERED`, refer to [clustered index](/clustered-indexes.md).
