...
Start the surviving PD node using the `--force-new-cluster` startup parameter, and ensure that the node uses the original data directory. You can specify it explicitly in the command line with `--data-dir`, or configure `data-dir` in `conf/pd.toml` in advance. For example:
...
> - If you specify the original data directory in both `conf/pd.toml` and `--data-dir` in the command line, `data-dir` in the `conf/pd.toml` configuration takes precedence.
