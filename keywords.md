---
title: Keywords
summary: この記事では、TiDBのキーワード、予約語と非予約語の違いについて紹介します。キーワードはSQLステートメント内で特別な意味を持つ単語であり、一部は識別子として直接使用できる非予約キーワード、一部は特別な処理が必要な予約キーワードです。予約キーワードを識別子として使用するには、バッククォートで囲む必要があります。特別な場合では、予約キーワードが.区切り文字とともに使用される場合、バッククォートは必要ありません。TiDBのキーワードリストも提供されています。
---
