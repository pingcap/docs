...
キーワードとは、 [`SELECT`](/sql-statements/sql-statement-select.md) 、 [`UPDATE`](/sql-statements/sql-statement-update.md) 、 [`DELETE`](/sql-statements/sql-statement-delete.md)などのSQL文において特別な意味を持つ単語のことです。キーワードの中には、識別子として直接使用できるものがあり、これらは**非予約キーワード**と呼ばれます。一方、識別子として使用する前に特別な処理が必要なものもあり、これらは**予約キーワード**と呼ばれます。
...
予約語ではないキーワードはバッククォートを必要としません。例えば`BEGIN`や`END`などは、次のステートメントで識別子として正常に使用できます。
...
特別な場合として、予約語を`.`区切り文字とともに使用する場合は、バッククォートは不要です。
...
[`tidb_enable_window_function`](/system-variables.md#tidb_enable_window_function)システム変数を使用して、 のキーワードが[ウィンドウ関数](/functions-and-operators/window-functions.md)ツリーで有効になるかどうかを制御できます。 `tidb_enable_window_function`を`OFF`に設定すると、ウィンドウ関数内の単語はキーワードとして扱われなくなります。

## キーワードリスト {#keyword-list}

以下のリストは、TiDB のキーワードを示しています。予約語は`(R)`でマークされています。 の[ウィンドウ関数](/functions-and-operators/window-functions.md)語は`(R-Window)`でマークされています。
