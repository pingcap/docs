...
コンピューティングレイヤーが依存するストレージソリューションについては、本ドキュメントではTiKVの行ベースのストレージ構造のみを紹介します。OLAPサービスについては、TiDBはTiKVの拡張機能として列指向のストレージソリューション[TiFlash](/tiflash/tiflash-overview.md)を導入しています。
