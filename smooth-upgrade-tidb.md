...
-   アップグレードする前に、次の制限事項を考慮してください。
    ...
    -   TiDB Distributed eXecution Framework (DXF) が有効になっている場合は、 [`tidb_enable_dist_task`](/system-variables.md#tidb_enable_dist_task-new-in-v710) ～ `OFF` (デフォルト値) を設定して無効にします。進行中の分散`ADD INDEX`および`IMPORT INTO`タスクがすべて完了していることを確認してください。あるいは、これらのタスクをキャンセルし、アップグレードが完了するまで待って再開することもできます。そうしないと、アップグレード中の`ADD INDEX`操作によってデータ インデックスの不整合が発生する可能性があります。
