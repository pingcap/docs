...
TiDBでは、SQLオプティマイザーは、テーブルを結合する順序と、特定のSQLステートメントの最も効率的な結合アルゴリズムを決定する必要があります。このドキュメントの例は、次のサンプルデータに基づいています。
...
結合する必要のある推定行数が少ない場合（通常は10000行未満）、インデックス結合方式を使用することをお勧めします。この結合方法は、MySQLで使用される主要な結合方法と同様に機能します。次の例では、演算子`├─TableReader_28(Build)`は最初にテーブル`t1`を読み取ります。一致する行ごとに、TiDBはテーブル`t2`をプローブします。
...
インデックス結合はメモリ使用量が効率的ですが、多数のプローブ操作が必要な場合は、他の結合方法よりも実行に時間がかかる場合があります。次のクエリも検討してください。

```sql
SELECT * FROM t1 INNER JOIN t2 ON t1.id=t2.t1_id WHERE t1.pad1 = 'value' and t2.pad1='value';
```

内部結合操作では、TiDBは結合の並べ替えを実装し、最初に`t1`または`t2`のいずれかにアクセスする場合があります。 TiDBが`build`ステップを適用する最初のテーブルとして`t1`を選択すると、TiDBはテーブル`t2`をプローブする前に、述語`t1.col = 'value'`でフィルタリングできると仮定します。述語`t2.col='value'`のフィルターは、表`t2`の各プローブに適用されます。これは、他の結合方法よりも効率が低い可能性があります。

インデックス結合は、ビルド側が小さく、プローブ側が事前にインデックス付けされていて大きい場合に効果的です。インデックス結合のパフォーマンスがハッシュ結合よりも悪く、SQLオプティマイザーによって選択されない次のクエリについて考えてみます。
...
`HashJoin_27`の実行プロセスの場合、TiDBは次の操作を順番に実行します。
...
`EXPLAIN`結果テーブルの`operator info`列には、クエリが内部結合か外部結合か、結合の条件など、 `HashJoin_27`に関するその他の情報も記録されます。上記の例では、クエリは内部結合であり、結合条件`equal:[eq(test.t1.id, test.t2.id)]`はクエリ条件`WHERE t1.id = t2.id`に部分的に対応しています。次の例の他のJoin演算子の演算子情報は、これに似ています。
...
マージ結合は、結合の両側がソートされた順序で読み取られるときに適用される特別な種類の結合です。これは、*効率的なジッパーマージ*に似ていると説明できます。データは結合の`Build`側と`Probe`側の両方で読み取られるため、結合操作はストリーミング操作のように機能します。マージ結合はハッシュ結合よりもはるかに少ないメモリを必要としますが、並行して実行されません。
...
2.  片側のデータを読み取り`Probe` 。
3.  `Probe`側のデータの各行が`Build`側の完全な結合グループと一致するかどうかを比較します。同等の条件とは別に、同等ではない条件があります。ここでの「一致」とは、主に、同等でない条件が満たされているかどうかを確認することを指します。結合グループは、すべての結合キー間で同じ値を持つデータを参照します。
