...
TiDB では、SQL オプティマイザーは、テーブルをどの順序で結合するか、および特定の SQL ステートメントに対して最も効率的な結合アルゴリズムは何かを決定する必要があります。このドキュメントの例は、次のサンプル データに基づいています。
...
インデックス結合はメモリ使用量の点では効率的ですが、多数のプローブ操作が必要な場合、他の結合方法よりも実行が遅くなる可能性があります。次のクエリも検討してください。

```sql
SELECT * FROM t1 INNER JOIN t2 ON t1.id=t2.t1_id WHERE t1.pad1 = 'value' and t2.pad1='value';
```

内部結合操作では、 TiDB は結合の並べ替えを実装し、最初に`t1`または`t2`にアクセスする可能性があります。 TiDB が`build`ステップを適用する最初のテーブルとして`t1`を選択し、その後、テーブル`t2`をプローブする前に述語`t1.pad1 = 'value'`でフィルター処理できると仮定します。 述語`t2.pad1='value'`のフィルターはテーブル`t2`の各プローブに適用されますが、他の結合方法よりも効率が低い可能性があります。
...
> **注記：**
>
> 上記の例では、SQL オプティマイザーは、インデックス結合よりもパフォーマンスの悪いハッシュ結合プランを選択します。クエリの最適化は[NP完全問題](https://en.wikipedia.org/wiki/NP-completeness)で、最適ではないプランが選択される場合があります。これが頻繁に実行されるクエリである場合は、 [SQL プラン管理](/sql-plan-management.md)使用してヒントをクエリにバインドすることをお勧めします。これは、アプリケーションが TiDB に送信するクエリにヒントを挿入するよりも管理が簡単です。
...
ハッシュ結合操作では、TiDB はハッシュ テーブル内の結合の`Build`側のデータを読み取ってキャッシュし、次に結合の`Probe`側のデータを読み取り、ハッシュ テーブルを調べて必要な行にアクセスします。ハッシュ結合は、インデックス結合よりも多くのメモリを必要としますが、結合する必要がある行が多数ある場合は、はるかに高速に実行されます。ハッシュ結合演算子は TiDB でマルチスレッド化されており、並列で実行されます。
...
-   [`tidb_mem_quota_query`](/system-variables.md#tidb_mem_quota_query) (デフォルト値: 1GB) - クエリのメモリクォータを超えた場合、TiDB はメモリを節約するためにハッシュ結合の`Build`の演算子をディスクに書き出そうとします。
...
TiDB は、ハッシュ結合のパフォーマンスを最適化し、実行速度を大幅に向上させるランタイム フィルター機能を提供します。具体的な最適化の使用方法については、 [ランタイムフィルター](/runtime-filter.md)参照してください。

## マージ結合 {#merge-join}

マージ結合は、結合の両側がソートされた順序で読み取られる場合に適用される特殊な結合です。これは、*効率的なジッパー マージ*に似ていると言えます。結合の`Build`側と`Probe`側の両方でデータが読み取られるため、結合操作はストリーミング操作のように機能します。マージ結合はハッシュ結合よりもはるかに少ないメモリしか必要としませんが、並列実行されません。
