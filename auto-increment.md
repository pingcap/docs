...
2.  クライアントはインスタンス`A`にステートメント`INSERT INTO t (c) (1)`を送信します。このステートメントでは`id`の値が指定されていないため、ID は`A`に割り当てられます。現在、 `A` `[1, 30000]`のIDをキャッシュしているため、AUTO_INCREMENT IDの値として`2`割り当て、ローカルカウンタを`1`増加させる可能性があります。このとき、ID `2`のデータが既にデータベースに存在するため、 `Duplicated Error`エラーが返されます。
...
TiDBの以前のバージョンでは、AUTO_INCREMENT IDのキャッシュサイズはユーザーにとって透過的でした。v3.0.14、v3.1.2、v4.0.rc-2以降、TiDBは`AUTO_ID_CACHE`テーブルオプションを導入し、ユーザーがAUTO_INCREMENT IDを割り当てるためのキャッシュサイズを設定できるようになりました。
