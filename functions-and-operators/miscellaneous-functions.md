...
TiDB 支持大部分在 MySQL 8.0 中可用的 [杂项函数](https://dev.mysql.com/doc/refman/8.0/en/miscellaneous-functions.html)。
...
| [`SLEEP()`](#sleep)                      | 暂停指定秒数。注意，对于 [{{{ .starter }}}](https://docs.pingcap.com/tidbcloud/select-cluster-tier#tidb-cloud-serverless) 集群，`SLEEP()` 函数有最大支持时间限制，为 300 秒。  |
...
`ANY_VALUE()` 函数返回一组值中的任意一个值。通常在需要在 `SELECT` 语句中包含非聚合列且配合 `GROUP BY` 子句使用的场景中。
...
在上述示例中，TiDB 对第一个 `SELECT` 语句返回错误，因为 `id` 列是非聚合列且未包含在 `GROUP BY` 子句中。为解决此问题，第二个 `SELECT` 查询使用 `ANY_VALUE()` 来从每个分组中获取任意值，并使用 `GROUP_CONCAT()` 将每个分组中的所有 `id` 值连接成一个字符串。这种方式可以在不改变 SQL 模式的情况下，获取每个分组的一个值和所有值。

### BIN_TO_UUID()

`BIN_TO_UUID()` 和 `UUID_TO_BIN()` 可用于在文本格式 UUID 和二进制格式之间转换。这两个函数都接受两个参数。
...
在上述示例中，`UPDATE` 语句将 `c1` 列的值设置为列的默认值（为 `5`）加上 `3`，结果为 `8`。
...
`INET_ATON()` 函数将点分十进制表示的 IPv4 地址转换为二进制版本，以便高效存储。
...
`INET6_ATON()` 类似于 [`INET_ATON()`](#inet_aton)，但可以处理 IPv6 地址。
...
`INET6_NTOA()` 类似于 [`INET_NTOA()`](#inet_ntoa)，但可以处理 IPv6 地址。
...
上述语句使用 `NAME_CONST()`，而以下语句使用推荐的列别名方式。
...
| 名称 | 描述  |
|:------------|:-----------------------------------------------------------------------------------------------|
| [`UUID_SHORT()`](https://dev.mysql.com/doc/refman/8.0/en/miscellaneous-functions.html#function_uuid-short)            | 提供一个在某些假设下唯一的 UUID，但这些假设在 TiDB 中不存在 [TiDB #4620](https://github.com/pingcap/tidb/issues/4620) |
