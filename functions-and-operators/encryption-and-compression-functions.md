...
`SHA2(str, n)`関数は[SHA-2](https://en.wikipedia.org/wiki/SHA-2)ファミリーのアルゴリズムを使用してハッシュを計算します。`n`引数はアルゴリズムを選択するために使用されます。`SHA2()`は、引数のいずれかが`NULL`の場合、または`n`で選択されたアルゴリズムが不明またはサポートされていない場合、 `NULL`返します。
