...
| `TIDB_PARSE_TSO(num)` | The `TIDB_PARSE_TSO` function can be used to extract the physical timestamp from a TiDB TSO timestamp. See also: [`tidb_current_ts`](/system-variables.md#tidb_current_ts). |
...
| `VITESS_HASH(str)` | Returns the hash of a string that is compatible with Vitess' HASH function. This is intended to help in migrations from Vitess. |
...
Plans can be found in encoded form in the slow query log. The `TIDB_DECODE_PLAN()` function is then used to reverse the plan into a human-readable form.
