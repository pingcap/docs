...
| [`TIDB_DECODE_SQL_DIGESTS()`](#tidb_decode_sql_digests) | クラスター内の一連の SQL ダイジェストに対応する正規化された SQL文 (フォーマットと引数のない形式) を照会します。                                                                                                                                                                                |
...
ユーザー`user1`を作成し、リソースグループ`rg1`と`rg2` 2つのリソースグループを作成し、ユーザー`user1`リソースグループ`rg1`にバインドします。
...
- 制限事項:
    ...
    - FastPlan プロセスを実行できないため、オプティマイザのパフォーマンスに影響します。
