...
TiDBは、UNION、EXCEPT、INTERSECT演算子を使用した3つの集合演算をサポートしています。集合の最小単位は[`SELECT`文](/sql-statements/sql-statement-select.md)です。
...
TiDB は`UNION DISTINCT`と`UNION ALL`両方の演算子をサポートします。`UNION DISTINCT`は結果セットから重複レコードを削除し、 `UNION ALL`は重複を含むすべてのレコードを保持します。TiDB では`UNION DISTINCT`はデフォルトで使用されます。
...
クエリ`UNION DISTINCT`と`UNION ALL`例はそれぞれ次のようになります。
