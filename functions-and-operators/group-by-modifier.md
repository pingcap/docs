...
summary: TiDB v7.4.0以降、GROUP BY句はWITH ROLLUP修飾子をサポートします。これにより、複数の列からのデータの集計と要約が可能になります。WITH ROLLUP修飾子を使用すると、集計結果内の他の高レベルのディメンションからのスーパーサマリー情報を表示する追加の行を取得できます。これにより、高度なデータ分析とレポートの生成が容易になります。TiDBは、GROUP BYリストに列がNある場合、クエリ結果をN+1のグループに集約します。
---
