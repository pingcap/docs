...
- Grouping method:
    ...
    - Subsequent grouping dimensions start from the right end of the grouping list and decrease one element at a time to form new groups.

- Aggregation summaries: on each dimension, the query performs aggregation operations, and then aggregates the results of this dimension with the results of all previous dimensions. This means you can get aggregated data at different dimensions, from detailed to overall.
...
For example, you can use `NULL` in the `HAVING` clause to filter and view the aggregated results for the 2-dimensional groups only:
...
The following is an example of how to use the `GROUPING()` function:
...
From this output, you can determine the aggregation dimension of a row directly from the results of `grp_year` and `grp_month`, which prevents interference from native `NULL` values in the `year` and `month` grouping expressions.
