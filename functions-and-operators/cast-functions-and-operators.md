...
| `FLOAT(n)`           | 浮動小数点数`n`はオプションで、 `0`から`53`の範囲で指定します。      | いいえ                  |
| `JSON`               | JSON                                      | いいえ                  |
| `REAL`               | 浮動小数点数                                    | はい                   |
...
- TiDBは`CAST()`に対する`AT TIME ZONE`をサポートしていません。詳細については[#51742](https://github.com/pingcap/tidb/issues/51742)を参照してください。
