...
This statement performs a distributed restore from a backup archive previously produced by a [`BACKUP` statement](/common/sql-statements/sql-statement-backup.md).
...
The `RESTORE` statement is blocking, and will finish only after the entire restore task is finished, failed, or canceled. A long-lasting connection should be prepared for running `RESTORE`. The task can be canceled using the [`KILL TIDB QUERY`](/common/sql-statements/sql-statement-kill.md) statement.
...
* [SHOW RESTORES](/common/sql-statements/sql-statement-show-backups.md)
