...
The `BACKUP` statement is blocked until the entire backup task is finished, failed, or canceled. A long-lasting connection should be prepared for executing `BACKUP`. The task can be canceled using the [`KILL TIDB QUERY`](/common/sql-statements/sql-statement-kill.md) statement.

Only one `BACKUP` and [`RESTORE`](/common/sql-statements/sql-statement-restore.md) task can be executed at a time. If a `BACKUP` or `RESTORE` statement is already being executed on the same TiDB server, the new `BACKUP` execution will wait until all previous tasks are finished.
...
* [SHOW BACKUPS](/common/sql-statements/sql-statement-show-backups.md)
