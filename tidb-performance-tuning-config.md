...
| [`tidb_analyze_column_options`](/system-variables.md#tidb_analyze_column_options-new-in-v830)| Collect statistics for all the columns to avoid suboptimal execution plans causing by missing column statistics, since the defalut behaviour will only collect statistics for [predicate columns](/statistics.md#collect-statistics-on-some-columns). | This can cause more resource usage for the analyze job compared to the default value |
