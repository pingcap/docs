...
> **Note:**
>
> - When you enable Raft Engine for the first time, TiKV transfers its original data from RocksDB to Raft Engine. Therefore, you need to wait an extra tens of seconds to start TiKV.
> - The data format of Raft Engine in TiDB v5.4.0 is not compatible with previous versions. Therefore, when you need to downgrade a TiDB cluster from v5.4.0 to earlier versions, **before** downgrading, disable Raft Engine by setting `enable` to `false` and restarting TiKV for the configuration to take effect.
