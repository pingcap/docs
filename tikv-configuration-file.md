...
- 例えば、 `[a,e)`リージョンのバックアップファイルのサイズが`sst-max-size`より大きい場合、そのファイルは`[a,b)` 、 `[b,c)` 、 `[c,d)`および`[d,e)`の領域を持つ複数のファイルにバックアップされ、 `[a,b)` 、 `[b,c)` 、 `[c,d)`のサイズは`sst-max-size`と同じ (またはわずかに大きい) です。
