...
+ Note: The TTL feature is only available for the RawKV interface for now. You can only configure this feature when creating a new cluster because different interfaces use different data formats in the bottom layer. If you modify this item on an existing cluster, the cluster reports errors when starting.
+ Default value: `false`
