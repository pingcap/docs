...
-   デフォルト値: 256MiB
-   注: 通常、値`file-size-limit`は、外部ストレージに表示されるバックアップ ファイルのサイズよりも大きくなります。これは、バックアップ ファイルが外部ストレージにアップロードされる前に圧縮されるためです。
...
-   リージョンの解決済み TSレイテンシーがこのしきい値を超えると、異常検出プロセスがトリガーされます。この時点で、Resolved TSレイテンシーが 3 x [四分位範囲](https://en.wikipedia.org/wiki/Interquartile_range)を超えるリージョンは、ロックの解決が遅いと見なされ、TiKV-CDC をトリガーしてリージョンのデータ変更を再サブスクライブし、ロック リソース ステータスをリセットします。
-   デフォルト値: `60s`
...
-   TiKV は、前の期間のタイムスタンプの消費に応じて、キャッシュされたタイムスタンプの数を調整します。ローカルにキャッシュされたタイムスタンプの使用率が低い場合、TiKV はキャッシュされたタイムスタンプの数を`renew-batch-min-size`に達するまで徐々に減らします。アプリケーションで大量のバースト書き込みトラフィックが頻繁に発生する場合は、必要に応じてこのパラメーターをより大きな値に設定できます。このパラメーターは、単一の tikv サーバーのキャッシュ サイズであることに注意してください。パラメーターを大きすぎる値に設定し、クラスターに多くの tikv サーバーが含まれている場合、TSO の消費が速すぎます。
