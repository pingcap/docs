...
+ Default value: `2MiB` (Before v8.5.7, the default value is `0`)
+ Minimum value: `0`
+ Unit: B|KiB|MiB|GiB
...
> **Note:**
>
> This configuration item was introduced to address backup failures caused by S3 rate limiting. Starting from TiDB v6.1.1, configure this value with caution. Setting it too large might cause large upload parts to fail or time out when the network is unstable.
...
### `enable-network-io-collection` <span class="version-mark">New in v8.5.7</span>
...
### `bg-cpu-throttle-threshold` <span class="version-mark">New in v8.5.7</span>
...
### `fg-cpu-throttle-threshold` <span class="version-mark">New in v8.5.7</span>
...
### `bg-compaction-pressure-threshold` <span class="version-mark">New in v8.5.7</span>

+ Specifies the threshold, as a percentage of [`storage.flow-control.soft-pending-compaction-bytes-limit`](#soft-pending-compaction-bytes-limit), at which background write I/O throttling begins. Below this threshold, background write I/O increases toward [`bg-write-io-ceiling`](#bg-write-io-ceiling-new-in-v857). At or above this threshold, TiKV scales background write I/O linearly down toward [`bg-write-io-floor`](#bg-write-io-floor-new-in-v857) as compaction pressure approaches 100%.
+ Default value: `70.0`
+ Unit: percentage (%)

### `bg-write-io-ceiling` <span class="version-mark">New in v8.5.7</span>

+ Specifies the maximum write I/O rate allowed for background tasks when compaction pressure is below [`bg-compaction-pressure-threshold`](#bg-compaction-pressure-threshold-new-in-v857).
+ Default value: `"100GB"`
+ Unit: bytes per second

### `bg-write-io-floor` <span class="version-mark">New in v8.5.7</span>
...
### `enable-fair-scheduling` <span class="version-mark">New in v8.5.7</span>
...
### `enable-read-admission-control` <span class="version-mark">New in v8.5.7</span>

+ Controls whether to enable admission control for read requests. When enabled, read requests from over-baseline resource groups are delayed or rejected with `SchedTooBusy` when CPU utilization exceeds [`fg-cpu-throttle-threshold`](#fg-cpu-throttle-threshold-new-in-v857).
+ Default value: `false`

### `enable-write-admission-control` <span class="version-mark">New in v8.5.7</span>

+ Controls whether to enable admission control for write requests. When enabled, write requests from over-baseline resource groups are delayed or rejected with `SchedTooBusy` when CPU utilization exceeds [`fg-cpu-throttle-threshold`](#fg-cpu-throttle-threshold-new-in-v857).
+ Default value: `false`

### `historical-usage-window-mins` <span class="version-mark">New in v8.5.7</span>
...
### `baseline-burst-pct` <span class="version-mark">New in v8.5.7</span>
...
### `admission-max-delayed-count` <span class="version-mark">New in v8.5.7</span>
