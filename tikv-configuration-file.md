...
### simple <code>simplify-metrics</code> <span class="version-mark">v6.1.1 の新機能</span> {#code-simplify-metrics-code-span-class-version-mark-new-in-v6-1-1-span}
...
> -   TiDB v5.4.0 のRaft Engineのデータ形式は、以前の TiDB バージョンと互換性がありません。したがって、TiDB クラスターを v5.4.0 から以前のバージョンにダウングレードする必要がある場合は、ダウングレードする**前**に、 Raft Engineを`enable`から`false`に設定して無効にし、TiKV を再起動して構成を有効にします。
