...
`t1`と`t2`については、これらのテーブルは行識別子としてクラスター化インデックスを使用していないため、 `_tidb_rowid`に対してクエリを実行できます。
...
-   パーティション化された非クラスター化テーブルでは、 `_tidb_rowid`値はパーティション間で一意であることが保証されません。`EXCHANGE PARTITION`実行した後、異なるパーティションに同じ`_tidb_rowid`値を持つ行が含まれる可能性があります。
