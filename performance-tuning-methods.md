...
This document describes a tuning by database time approach, and illustrates how to use the TiDB [Performance Overview dashboard](/grafana-performance-overview-dashboard.md) for performance analysis and tuning.
...
- If the bottleneck is in TiDB, further identify the exact module in the distributed system based by database time overview, color-based performance data, key metrics, resource utilization, and top-down latency breakdowns.

### Is TiDB the bottleneck?

- If the average idle time of TiDB connections in transactions is higher than the average SQL processing latency, the database is not to blame for the transaction latency of applications. The database time takes only a small part of the user response time, indicating that the bottleneck is outside of the database.
...
The Performance Overview dashboard provides the following three stacked area graphs. They help you understand database load profile and quickly identify the bottleneck causes in terms of statements, sql phase, and TiKV or PD request type during SQL execution.
...
The following three panels in Performance Overview present the application load profile, how the application interacts with TiDB, and whether the application fully utilizes TiDB's [prepared plan cache](/sql-prepared-plan-cache.md).
...
- Queries Using Plan Cache OPS: The count that the TiDB cluster hits the prepared plan cache per second. prepared plan cache only supports the `prepared statement` command. When prepared plan cache is enabled in TiDB, the following three scenarios will occur:
    ...
    - No prepared plan cache is hit: The number of plan cache hit per second is 0. The application is using query interface, or cached plans are cleaned up by calling the StmtClose command after each StmtExecute execution.
...
**Example 2: prepared plan cache unavailable for query commands in read-only OLTP load**
...
**Example 3: prepared plan cache unavailable with prepared statement enabled for OLTP load**
...
> **Note:**
>
> As of TiDB v6.0.0, you can prevent the `StmtClose` command from clearing cached execution plans via the global variable (`set global tidb_ignore_prepared_cache_close_stmt=on;`). In this way, the subsequent execution can hit the prepared plan cache.
...
Obviously TiDB obviously consumes more CPU, which is near the bottleneck threshold of 8 CPUs. It is recommended that you scale out the TiDB.
...
Obviously TiKV consumes more CPU, which is expected because TPC-C is a write-intensive scenario. It is recommended that you scale out the TiKV to improve performance.
...
- If the application load is not read-only and contains transactions, by comparing the average query latency with `avg-in-txn`, you can determine the proportion in processing transactions for inside and outside the database, and identify the bottleneck in user response time. 
- If the application load is read-only or autocommit mode is on, you can compare the average query latency with `avg-not-in-txn`.
...
- The network latency from the application server to the database is high. For example, the network latency is high because in public-cloud deployments the applications and the TiDB cluster are not in the same region, or the dns load balancer and the TiDB cluster are not in the same region.
...
In this TPC-C workload, the average time of `parse`, `compile` and `execute` phases are 7.39 us, 38.1 us, and 12.8 ms, respectively. The `execute` phase is the bottleneck of `query` latency.

#### KV and TSO Request Duration

TiDB interacts with PD and TiKV in the `execute` phase. As shown in the following figure, when processing SQL request, TiDB request TSOs before entering `parse` and `compile` phase. The PD Client does not block the caller, but returns a `TSFuture` and asynchronously sends and receives the TSO requests in the background. Once the PD client finishes handling the TSO requests, it returns `TSFuture`. The holder of the `TSFuture` needs to call the Wait method to get the final TSOs. After TiDB finishes `parse` and `compile` phase, it enters the `execute` phase, where two situations might occur:
...
The TSO wait time is recorded as `TSO WAIT` and the network time of the TSO request is recorded as `TSO RPC`. After the TSO wait is complete, TiDB executor usually send read or write reuqests to TiKV.
