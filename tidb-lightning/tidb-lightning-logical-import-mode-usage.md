...
競合データとは、PK列またはUK列に同じデータを持つレコードが2つ以上存在する状態を指します。論理インポートモードでは、 [`conflict.strategy`](/tidb-lightning/tidb-lightning-configuration.md#tidb-lightning-task)の設定項目を設定することで、競合データの処理戦略を構成できます。TiDB Lightningは、この戦略に基づいて、異なるSQL文を使用してデータをインポートします。
