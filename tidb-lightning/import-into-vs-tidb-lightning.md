...
[TiDB グローバルソート](/tidb-global-sort.md)を使用する場合、大容量のローカルディスクをマウントする必要はありません。TiDB Global Sort は Amazon S3 をストレージとして使用できます。インポートタスクが完了すると、グローバルソート用に Amazon S3 に保存された一時データは自動的に削除され、ストレージコストを節約できます。
...
グローバルソートのストレージとして Amazon S3 を使用した場合、 `IMPORT INTO`のパフォーマンステスト結果は次のとおりです。
