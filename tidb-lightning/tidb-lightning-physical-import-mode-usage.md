...
したがって、TiDB クラスターにレイテンシの影響を受けやすいアプリケーションがあり、同時実行性が低い場合は、クラスターへのデータのインポートにTiDB Lightningを使用し**ないこと**を強くお勧めします。これはオンライン申請に重大な影響を与えます。
...
-   **水平方向のスケーリングを実現するには、平行インポート機能を使用します。**
