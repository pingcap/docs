...
- If you want to run multiple TiDB Lightning instances simultaneously to import data into the same TiDB cluster, you can enable [Parallel Import](/tidb-lightning/tidb-lightning-distributed-import.md) to coordinate the import. If each instance imports different tables, the Parallel Import option is not required. However, if multiple instances import data into the same table, enabling Parallel Import is necessary to prevent conflicts and ensure data integrity.
