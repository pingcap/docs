...
2. TiDB Lightning creates table schemas in the target database and fetches the metadata.
    ...
    If you set `add-index-by-sql` to `true`, `tidb-lightning` adds indexes via the SQL interface, and drops all secondary indexes from the target table before importing the data. The default values is `false`, wich is consistent with earlier versions.
