...
| path | ✓  | ✓   |    | エラーを含むファイルのパス                                                               |
| offset | ✓  | ✓   |    | ファイル内でエラーが見つかったバイト位置                                                        |
| error | ✓  | ✓   |    | エラーメッセージ                                                                    |
| context | ✓  |     |    | エラーを囲むテキスト                                                                  |
| index_name |    |     | ✓  | 競合している一意キーの名前。主キーが競合している場合は`'PRIMARY'`になります。                                 |
| key_data |    |     | ✓  | エラーの原因となった行のフォーマットされたキーハンドル。この内容は人間による参照のみを目的としており、機械による読み取りを意図したものではありません。 |
| row_data |    | ✓   | ✓  | エラーの原因となったフォーマットされた行データ。この内容は人間による参照のみを目的としており、機械による読み取りを意図したものではありません。     |
| raw_key |    |     | ✓  | 競合するKVペアのキー                                                                 |
| raw_value |    |     | ✓  | 競合するKVペアの値                                                                  |
| raw_handle |    |     | ✓  | 競合した行の行ハンドル                                                                 |
| raw_row |    |     | ✓  | 競合行のエンコードされた値                                                               |
