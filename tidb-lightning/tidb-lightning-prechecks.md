...
| Check if PITR is enabled or Changefeed tasks are running in the cluster | >= 6.5.0 | The physical import mode is incompatible with PITR and Changefeed. If PITR is enabled or a Changefeed task is running, TiDB Lightning automatically exits with an error. If you are certain that the tables to be imported do not require PITR or Changefeed replication, you can skip this check to proceed with the import. |
