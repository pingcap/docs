...
ログバックアップは、監視メトリックの収集に[Prometheus](https://prometheus.io/)使用をサポートしています。現在、すべての監視メトリックはTiKVに組み込まれています。
...
| **tikv_log_backup_task_status**                        | ゲージ    | ログバックアップタスクのステータス。`0`は実行中、 `1`は一時停止中、 `2`はエラーを意味します。<br/> `task :: string`                                                                |
