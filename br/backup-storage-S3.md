...
Before backup or restoration using S3, you need to configure the privileges to access S3.
...
- Before running the `br backup` command, configure `access-key` and `secret-access-key` to access S3, and set `-send-credentials-to-tikv=true` to pass the access key from BR to each TiKV.
...
Considering that the access key in a command is vulnerable to leakage, you are advised to associate IAM role to EC2 instances to access S3.
