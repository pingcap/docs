...
> When the upstream cluster has **many Regions** and a **short flush interval**, PITR generates a large number of small files. This increases batching and dispatching overhead during restore. To raise the number of files processed per batch, you can **moderately** increase the following parameters:
>
> - `pitr-batch-size`: cumulative **bytes per batch** (default **16 MiB**).
> - `pitr-batch-count`: **number of files per batch** (default **8**).
>
> These two thresholds are evaluated independently. Whichever threshold is reached first closes the current batch and starts the next, while the other threshold is ignored for that batch.
