...
>
> 外部storageには、単一のレプリカのKVデータのみが格納されます。そのため、外部storageのデータサイズは、クラスターに復元された実際のデータサイズを表すものではありません。BRは、クラスターに設定されたレプリカ数に応じてすべてのレプリカを復元します。レプリカの数が多いほど、実際に復元できるデータ量も多くなります。テストでは、すべてのクラスターのデフォルトのレプリカ数は3です。全体的な復元パフォーマンスを向上させるには、TiKV設定ファイルの[`import.num-threads`](/tikv-configuration-file.md#import)項目とBRコマンドの[`pitr-concurrency`](/br/use-br-command-line-tool.md#common-options)オプションを変更します。
