...
ログバックアップを開始するには、 `tiup br log start`を実行します。クラスターは 1 回につき 1 つのログバックアップ タスクのみ実行できます。
...
-   `name` : ログバックアップ タスクの名前。
-   `status` : ログバックアップ タスクのステータス`NORMAL` 、 `PAUSED` 、 `ERROR`を含む)。
-   `start` : ログバックアップ タスクの開始タイムスタンプ。
