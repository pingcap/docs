...
| `role-arn` | When you need to access data on Amazon S3 of a third-party account using [an IAM role](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles.html), you can specify the corresponding [Amazon Resource Name (ARN)](https://docs.aws.amazon.com/general/latest/gr/aws-arns-and-namespaces.html) of the IAM role with the `role-arn` parameter, such as `arn:aws:iam::888888888888:role/my-role`. For more information about using an IAM role to access data on Amazon S3 of a third-party account, see [AWS documentation](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_common-scenarios_third-party.html). |
| `external-id` | When you access data on Amazon S3 of a third-party account, you might need to specify a correct [external ID](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_create_for-user_externalid.html) to make sure you can assume [the IAM role](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles.html). `external-id` can be any random string generated by the third party that manages the S3 resources. `external-id` is optional, which means if the third party does not specify an external ID for the IAM role, you can assume the role and access the corresponding S3 data without providing this parameter. |
...
If the access key and secret access key are not specified and no ARN for the IAM (`role-arn`) or external ID (`external-id`) is provided, the migration tools try to infer these keys from the environment in the following order:
...
| `--s3.role-arn` | When you need to access data on Amazon S3 of a third-party account using [an IAM role](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles.html), you can specify the corresponding [Amazon Resource Name (ARN)](https://docs.aws.amazon.com/general/latest/gr/aws-arns-and-namespaces.html) of the IAM role with the `role-arn` parameter, such as `arn:aws:iam::888888888888:role/my-role`. For more information about using an IAM role to access data on Amazon S3 of a third-party account, see [AWS documentation](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_common-scenarios_third-party.html). |
| `--s3.external-id` | When you access data on Amazon S3 of a third-party account, you might need to specify a correct [external ID](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_create_for-user_externalid.html) to make sure you can assume [the IAM role](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles.html). `external-id` can be any random string generated by the third party that manages the S3 resources. `external-id` is optional, which means if the third party does not specify an external ID for the IAM role, you can assume the role and access the corresponding S3 data without providing this parameter. |
