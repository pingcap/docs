...
- Minimum privileges for TiKV and BR to access the backup directories during restore: `s3:ListBucket`, `s3:GetObject` and `s3:PutObject`(BR will write the checkpoint information to the `/checkpoints` subdirectory under the backup directory; When BR restores the log backup files, it will write the rewrite id map to the `/pitr_id_maps` subdirectory under the backup directory.)
