...
> - Starting from BR v8.3.0, the snapshot restore task introduces available disk space checks for TiKV and TiFlash. This includes verifying whether TiKV and TiFlash have sufficient disk space based on the size of SST files to be restored at the beginning of task, and verifying whether TiKV (v8.3.0 or later) has sufficient disk space before downloading each SST file. If the space is insufficient, the restore task will fail with an error. You can skip the check in the beginning of task by setting `--check-requirements=false`, but the disk space check before TiKV downloading SST files cannot be skipped.
