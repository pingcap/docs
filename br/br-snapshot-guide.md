...
> - Starting from BR v8.3.0, the snapshot restore task introduces available disk space checks for TiKV and TiFlash: at the beginning of the task, BR verifies whether TiKV and TiFlash have sufficient disk space based on the size of SST files to be restored; for TiKV v8.3.0 or later version, BR verifies whether TiKV has sufficient disk space before downloading each SST file. If the space is insufficient, the restore task will fail with an error. You can skip the check in the beginning of the task by setting `--check-requirements=false`, but the disk space check before TiKV downloading each SST file cannot be skipped.
