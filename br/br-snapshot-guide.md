...
このドキュメントでは、br コマンドラインツール (以下`br`と呼びます) を使用して TiDB スナップショットをバックアップおよび復元する方法について説明します。データをバックアップおよび復元する前に、 [brコマンドラインツールをインストールします](/br/br-use-overview.md#deploy-and-use-br)必要があります。
...
- バックアップデータの復元速度は、クラスタ構成、デプロイメント、および実行中のアプリケーションに大きく左右されます。スナップショット復元のパフォーマンスと影響は、さまざまなユーザーシナリオによって異なるため、実際の環境でテストすることをお勧めします。
