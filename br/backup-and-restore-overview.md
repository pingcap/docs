...
バックアップと復元を実行する前に、BRはTiDBクラスタのバージョンとBRのバージョンを比較してチェックします。メジャーバージョンの不一致（たとえば、BRv4.xとTiDBv5.x）がある場合、BRは終了するようにリマインダーを表示します。バージョンチェックを強制的にスキップするには、 `--check-requirements=false`を設定します。
