...
`br`コマンドは、サブコマンド、オプション、パラメータで構成されます。サブコマンドとは、 `-`または`--`を含まない文字列です。オプションとは、 `-`または`--`で始まる文字列です。パラメータとは、サブコマンドまたはオプションの直後に続く文字列で、サブコマンドまたはオプションに渡されます。
...
- `--pd` : PDサービスアドレスを指定するオプション。 `"${PD_IP}:2379"`は`--pd`のパラメータです。
...
`tiup br backup`および`tiup br restore`には次のサブコマンドが含まれます。
...
- `--status-addr` : `br`がPrometheusに統計を提供するリスニングアドレスを指定します。
