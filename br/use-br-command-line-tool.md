...
その他のオプションの説明については、 [すべてのクラスタデータをバックアップする](#use-br-command-line-to-back-up-cluster-data)を参照してください。
...
>
> -   データは複数のピアに複製されます。 SSTを取り込むとき、これらのファイルは*すべての*ピアに存在する必要があります。これは、単一ノードからの読み取りで十分なバックアップとは異なります。
