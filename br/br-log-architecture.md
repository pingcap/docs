...
本文介绍了以 Backup & Restore (BR) 工具为例的 TiDB 日志备份和点-in-时间恢复（PITR）架构及流程。
...
* **local metadata**：表示由单个 TiKV 节点备份的元数据，包括 local checkpoint ts、global checkpoint ts 和备份文件信息。
* **local checkpoint ts**（在 local metadata 中）：表示在该 TiKV 节点中，所有在 local checkpoint ts 之前生成的日志已被备份到目标存储。
* **global checkpoint ts**：表示所有 TiKV 节点中在 global checkpoint ts 之前生成的日志已被备份到目标存储。TiDB Coordinator 通过收集所有 TiKV 节点的 local checkpoint ts 计算此时间戳，并报告给 PD。
* **TiDB Coordinator**：由 TiDB 节点选举产生，负责收集和计算整个日志备份任务的进度（global checkpoint ts）。该组件为无状态设计，故障后由存活的 TiDB 节点选举出新的 Coordinator。
...
2. TiKV 监控日志备份任务的创建和更新。
   ...
   * **Fetch log backup task**：每个 TiKV 节点的 log backup observer 从 PD 获取日志备份任务，然后备份指定范围内的日志数据。

3. 日志备份观察者持续备份 KV 变更日志。

   * **Read kv change data**：读取 KV 变更数据，并将变更日志保存到 [backup files in custom format](#log-backup-files)。
   ...
   * **Configure GC**：请求 PD 阻止未备份（大于 local checkpoint ts）的数据被 TiDB GC 机制回收。
...
3. BR 还原日志备份数据。
   ...
   * **Request TiKV to restore data**：创建日志还原请求并发送给对应的 TiKV 节点。日志还原请求包含待还原的日志备份数据信息。
...
- `{store_id}.ts` 文件：每次每个 TiKV 节点上传日志备份数据时更新，存储 global checkpoint ts。`{store_id}` 为 TiKV 节点的存储 ID。
...
- `backupmeta`：存储备份元数据。文件名中的 `resolved_ts` 表示备份进度，意味着该 TSO 之前的数据已全部备份。但注意此 TSO 仅反映某些分片的进度。
- `global_checkpoint`：代表全局备份进度，记录可以通过 `br restore point` 还原的最新时间点。
- `{date}/{hour}`：存储对应日期和小时的备份数据。清理存储时，建议使用 `br log truncate`，而非手动删除数据。因为元数据引用了此目录中的数据，手动删除可能导致还原失败或还原后数据不一致。
