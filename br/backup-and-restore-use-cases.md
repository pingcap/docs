...
summary: TiDBのバックアップと復元に関する使用例を紹介します。TiDBはスナップショットバックアップ、ログバックアップ、およびPITRを提供し、AWSでの本番クラスターにデプロイする際の推奨ハードウェアリソースも示します。バックアップポリシーの決定やログバックアップ、スナップショットバックアップ、PITRの実行方法、古いデータのクリーンアップ方法も説明します。
---
...
PITR を使用するには、TiDB クラスター &gt;= v6.2.0 をデプロイし、 BR をTiDB クラスターと同じバージョンに更新する必要があります。このドキュメントでは例として v7.5.1 を使用します。
