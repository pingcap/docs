...
summary: ログバックアップを SST 形式に圧縮することで、ポイントインタイム リカバリ (PITR) の効率を向上させる方法を学習します。
---
...
ログバックアップを手動で圧縮するには、 `tikv-ctl`と`br` 2 つのツールが必要です。
...
> -   `--storage`の値は、ログバックアップ タスクの`log status`コマンドの出力と一致する必要があります。
