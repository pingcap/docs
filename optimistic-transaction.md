...
3. The client issues a write request.
    ...
    TiDB checks whether the written data satisfies constraints (to ensure the data types are correct, the NOT NULL constraint is met, and so on.). **Valid data is stored in the private memory of this transaction in TiDB**.
