...
Starting from TiDB v8.5.7 and v9.0.0, you can also use the system variable [`tidb_opt_enable_alternative_logical_plans`](/system-variables.md#tidb_opt_enable_alternative_logical_plans-new-in-v857-and-v900) to optimize such scenarios. After you enable this variable, if the candidate plan after [correlated subquery decorrelation](/correlated-subquery-optimization.md) does not generate an equivalent `IndexJoin` candidate with the same access direction as the original correlated subquery, the optimizer additionally keeps a "non-decorrelated" candidate plan, evaluates both "decorrelated" and "non-decorrelated" candidate plans, and finally selects the [execution plan](/explain-subqueries.md) with the lower cost.
