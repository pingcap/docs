...
2. Name the CSV files as follows:
    ...
    - TiDB Cloud supports importing the following compressed file formats: `.gz`, `.lz4`, `.zstd`, `.xz`, `.lzo` and `.snappy`. If you want to import compressed CSV files, name the files in the `${db_name}.${table_name}.csv.${compress}` format. For example, if you want to import the `trips.csv.gz` file to the `bikeshare.trips` table, you can name the file as `bikeshare.trips.csv.gz`.
