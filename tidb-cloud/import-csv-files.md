...
> - If there is a changefeed in a Dedicated Tier cluster, you cannot create an import task for it, because the current import task uses the Local-backend import mode. In this mode, the imported data does not generate change logs, so the changefeed cannot detect the imported data.
