...
> **Note:**
>
> トラフィックの可視性を向上させるため、{{{ .starter }}} と {{{ .essential }}} では、詳細な実行情報において AWS PrivateLink 経由の接続に対する実際のクライアント IP アドレスが表示されるようになりました。現在、この機能はパブリックプレビューであり、AWS リージョン `Frankfurt (eu-central-1)` でのみ利用できます。
