...
summary: 了解如何使用 `tidbcloud_serverless_cluster` 资源来创建和修改 TiDB Cloud Essential 实例。
---

# 使用 `tidbcloud_serverless_cluster` 资源

本文档介绍如何使用 `tidbcloud_serverless_cluster` 资源管理 [TiDB Cloud Essential](/tidb-cloud/select-cluster-tier.md#essential) 实例。

你还将学习如何通过 `tidbcloud_projects` 数据源获取所需的信息。

`tidbcloud_serverless_cluster` 资源的功能包括：

- 创建 TiDB Cloud Essential 实例
- 修改 TiDB Cloud Essential 实例
- 导入 TiDB Cloud Essential 实例
- 删除 TiDB Cloud Essential 实例
...
每个 TiDB Cloud Essential 实例都属于一个项目。在创建 TiDB Cloud Essential 实例之前，你需要获取要创建 TiDB Cloud Essential 实例的项目 ID。如果未指定 `project_id`，则会使用默认项目。
...
## 创建 TiDB Cloud Essential 实例 {#create-a-essential-instance}

你可以使用 `tidbcloud_serverless_cluster` 资源创建 TiDB Cloud Essential 实例。

1. 为 TiDB Cloud Essential 实例创建一个目录并进入该目录。
...
2. 创建一个 `cluster.tf` 文件。
    ...
    - 对于资源详情，你可以根据项目 ID 和 [`tidbcloud_serverless_cluster` 规范](https://registry.terraform.io/providers/tidbcloud/tidbcloud/latest/docs/resources/serverless_cluster)进行配置。要创建 TiDB Cloud Essential 实例，必须在资源详情中指定 `auto_scaling` 属性。
...
3. 运行 `terraform apply` 命令。应用资源时，不建议使用 `terraform apply --auto-approve`。
    ...
    - 你可以检查配置与状态之间的差异。
    - 你还可以看到此次 `apply` 的结果。它将添加一个新资源，不会更改或销毁任何资源。
    - `known after apply` 表示你将在执行 `apply` 后获得相应的值。

4. 如果计划看起来没有问题，输入 `yes` 继续：
...
5. 使用 `terraform show` 或 `terraform state show tidbcloud_serverless_cluster.${resource-name}` 命令检查资源状态。前者会显示所有资源和数据源的状态。
...
## 修改 TiDB Cloud Essential 实例 {#modify-a-essential-instance}

对于 TiDB Cloud Essential 实例，你可以使用 Terraform 管理资源。可修改的参数包括：

- `display_name`：TiDB Cloud Essential 实例的显示名称。
- `auto_scaling`：TiDB Cloud Essential 实例的自动扩缩容配置。
- `endpoints.public.disabled`：是否禁用公共端点。
- `automated_backup_policy.start_time`：自动备份开始的 UTC 时间，格式为 `HH:mm`。

要修改 TiDB Cloud Essential 实例，你可以修改 `tidbcloud_serverless_cluster` 资源的配置，然后使用 `terraform apply` 命令应用更改。例如，你可以按如下方式修改 `display_name` 和 `auto_scaling`：
...
然后，你可以使用 `terraform show` 或 `terraform state show tidbcloud_serverless_cluster.${resource-name}` 命令检查资源状态。前者会显示所有资源和数据源的状态。
...
## 导入 TiDB Cloud Essential 实例 {#import-a-essential-instance}

对于未由 Terraform 管理的 TiDB Cloud Essential 实例，你可以通过导入将其纳入 Terraform 管理。
...
3. 检查并应用生成的配置。
    ...
    检查生成的配置文件，确保其满足你的需求。你也可以选择将该文件的内容移动到你偏好的位置。
...
现在，你可以使用 Terraform 管理已导入的 TiDB Cloud Essential 实例。

## 删除 TiDB Cloud Essential 实例 {#delete-a-essential-instance}

要删除 TiDB Cloud Essential 实例，你可以删除 `tidbcloud_serverless_cluster` 资源的配置，然后使用 `terraform apply` 命令销毁该资源：
