...
- [获取 TiDB Cloud Terraform 提供者](/tidb-cloud/terraform-get-tidbcloud-provider.md) v0.4.0 或更高版本
...
3. 运行 `terraform apply` 命令。建议不要在应用资源时使用 `terraform apply --auto-approve`。
    ...
    - 你可以检查配置与状态之间的差异。
    - 你还可以查看此次 `apply` 的结果。它会添加一个新资源，没有资源会被修改或销毁。
...
4. 如果计划中的内容都没有问题，输入 `yes` 继续：
    ...
    tidbcloud_sql_user.example：2 秒后创建完成
...
1. 在用于 [创建 SQL 用户](#创建 SQL 用户) 的 `sql_user.tf` 文件中，修改 `password`、`builtin_role` 和 `custom_roles`（如果适用）。
...
2. 运行 `terraform apply` 命令：
    ...
    Terraform 将执行以下操作：
      ...
      # tidbcloud_sql_user.example 将就地更新
      ~ resource "tidbcloud_sql_user" "example" {
          + auth_method  = (应用后确定)
...
3. 如果计划中的内容都没有问题，输入 `yes` 继续：

    ```shell
      输入一个值：yes

    tidbcloud_sql_user.example：修改中...
    tidbcloud_sql_user.example：2 秒后修改完成

    应用完成！资源：0 个已添加，1 个已更改，0 个已销毁。
    ```

4. 使用 `terraform state show tidbcloud_sql_user.${资源名}` 检查状态：
...
内置角色已更改为 `role_readonly`。由于密码是敏感值，未显示。

## 导入 SQL 用户

对于未由 Terraform 管理的 TiDB Cloud SQL 用户，你可以通过导入方式让 Terraform 管理它。

例如，可以按如下方式导入未由 Terraform 创建的 SQL 用户：

1. 添加导入块到你的 `.tf` 文件中，替换 `example` 为你想要的资源名，替换 `${id}` 为 `cluster_id,user_name` 格式：
...
2. 生成新的配置文件
    ...
    这样会在当前目录生成 `generated.tf` 文件，包含导入资源的配置。但由于未设置必需的参数 `password`，提供者会报错。你可以在生成的配置文件中替换 `password` 参数的值。
...
3. 审查并应用生成的配置
    ...
    应用完成！资源：导入 1 个，新增 0 个，修改 0 个，销毁 0 个。
    ```
...
```shell
  ...
  tidbcloud_sql_user.example：3 秒后销毁完成

  应用完成！资源：0 个已添加，0 个已更改，1 个已销毁。
```
