...
TiDB Cloudは、金融サービス、eコマース、Enterpriseアプリケーション、ゲームなど、さまざまな業界の1500社以上のグローバル企業でミッションクリティカルなユースケースを支えてきたTiDBの開発チームによってサポートされています。TiDB Cloudは、ユーザーごとに無料の基本サポートプランを提供しており、より高度なサービスをご希望の場合は有料プランにアップグレードできます。詳細については、 [TiDB Cloudサポート](/tidb-cloud/tidb-cloud-support.md)をご覧ください。
