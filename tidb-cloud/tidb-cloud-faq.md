...
对于 {{{ .starter }}} 或 Essential 实例，连接实例的步骤简化如下：

1. 设置数据库用户和登录凭证。
2. 选择 SQL 客户端，在 TiDB Cloud UI 上获取自动生成的连接字符串，然后通过 SQL 客户端使用该字符串连接 {{{ .starter }}} 或 Essential 实例。

更多信息请参见 [连接 TiDB Cloud {{{ .starter }}} 或 Essential 实例](/tidb-cloud/connect-to-tidb-cluster-serverless.md)。
