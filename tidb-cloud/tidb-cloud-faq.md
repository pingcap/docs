...
不支持。如果你需要在备份保留期内将数据恢复到任意时间点，可以使用 PITR（时间点恢复）。更多信息请参见 [在 TiDB Cloud Dedicated 集群中使用 PITR](/tidb-cloud/backup-and-restore.md#turn-on-auto-backup) 或 [在 TiDB Cloud Essential 实例中使用 PITR](/tidb-cloud/backup-and-restore-serverless.md#restore)。
...
在 TiDB Cloud 中，你可以根据需要使用 TiDB Cloud Dedicated 集群、{{{ .starter }}} 实例或 {{{ .essential }}} 实例。

对于 TiDB Cloud Dedicated 集群，TiDB Cloud 通过以下措施确保集群安全：

- 为每个集群创建独立的子账户和 VPC。
- 设置防火墙规则以隔离外部连接。
- 为每个集群创建服务端 TLS 证书和组件级 TLS 证书，以加密集群传输中的数据。
- 为每个集群提供 IP 访问规则，确保只有允许的源 IP 地址可以访问你的集群。

对于 {{{ .starter }}} 和 {{{ .essential }}} 实例，TiDB Cloud 通过以下措施确保实例安全：

- 为每个实例创建独立的子账户。
- 设置防火墙规则以隔离外部连接。
- 提供实例服务端 TLS 证书，以加密实例传输中的数据。
...
### 如何连接到 TiDB 中的数据库？ {#how-do-i-connect-to-my-database-in-tidb}
...
2. 设置数据库用户和登录凭证。
3. 下载并为集群服务端配置 TLS。
4. 选择一个 SQL 客户端，获取 TiDB Cloud UI 中显示的自动生成连接字符串，然后通过该字符串使用 SQL 客户端连接到集群。

更多信息请参见 [连接到你的 TiDB Cloud Dedicated 集群](/tidb-cloud/connect-to-tidb-cluster.md)。

</div>

<div label="{{{ .starter }}}/Essential">

对于 {{{ .starter }}} 或 Essential 实例，连接到实例的步骤简化如下：

1. 设置数据库用户和登录凭证。
2. 选择一个 SQL 客户端，获取 TiDB Cloud UI 中显示的自动生成连接字符串，然后通过该字符串使用 SQL 客户端连接到你的 {{{ .starter }}} 或 Essential 实例。

更多信息请参见 [连接到你的 {{{ .starter }}} 或 Essential 实例](/tidb-cloud/connect-to-tidb-cluster-serverless.md)。
