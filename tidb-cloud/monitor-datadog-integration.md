...
| tidb_cloud.db_query_per_second             | ゲージ      | タイプ: 選択|挿入|...<br/>クラスター名: `<cluster name>`<br/>インスタンス: tidb-0|tidb-1…<br/>コンポーネント: `tidb`                        | すべての TiDB ノードで 1秒あたりに実行される SQL文の数。ステートメントの種類 ( `SELECT` 、 `INSERT` 、または`UPDATE` ) ごとにカウントされます。                                                                                                                                                                                                                     |
