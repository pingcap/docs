---
title: TiDB Cloud Essential 的数据库审计日志（Beta）
...
> **Note:**
>
> 目前，数据库审计日志功能仅支持按需开通。若需申请该功能，请点击 [TiDB Cloud 控制台](https://tidbcloud.com) 右下角的 **?**，然后点击 **Request Support**。在 **Description** 字段填写 “Apply for TiDB Cloud Essential database audit logging”，并点击 **Submit**。
...
你可以将审计日志存储在 TiDB Cloud，并下载到本地。审计日志会在 365 天后过期并被删除。如需更长的保留周期，请联系 [TiDB Cloud Support](/tidb-cloud/tidb-cloud-support.md)。

### Amazon S3

若需将审计日志存储在 Amazon S3，你需要提供以下信息：
...
若需将审计日志存储在 Google Cloud Storage，你需要提供以下信息：
...
若需将审计日志存储在 Azure Blob Storage，你需要提供以下信息：
...
> **Note:**
>
> `AUDIT` 事件类别及其子类别始终会被记录在审计日志中，无法被过滤。
...
以 Amazon S3 存储为例。若需启用审计日志并将日志存储在 Amazon S3，执行以下命令：
...
若需通过 TiDB Cloud CLI 更新审计日志设置，执行以下命令：
...
若需通过 TiDB Cloud CLI 禁用审计日志，执行以下命令：
...
若需创建过滤规则，请定义你希望在审计日志中捕获的用户和事件。你可以根据需要指定用户、事件类别、表和状态码，实现个性化日志记录。
...
若需通过 TiDB Cloud CLI 创建过滤规则，执行以下命令：
...
若需通过 TiDB Cloud CLI 编辑过滤规则，执行以下命令：
...
3. 在 **DB Audit Logging** 页面，找到你要禁用的过滤规则，关闭开关以禁用该过滤规则。

</div>

<div label="CLI">

若需通过 TiDB Cloud CLI 禁用过滤规则，执行以下命令：
...
> - 若需获取某一天（如 2025 年 1 月 1 日）的所有日志，请设置 `--start-date 2025-01-01` 和 `--end-date 2025-01-02`。在某些情况下，你可能需要下载所有日志文件并根据 `TIME` 字段进行排序。
...
4. 若需下载审计日志，从列表中选择一个或多个日志，然后点击 **Download**。

</div>

<div label="CLI">

若需通过 TiDB Cloud CLI 下载审计日志，执行以下命令：
...
当事件类别为 `QUERY` 或其子类别时，审计日志包含以下信息：
...
当事件类别为 `CONNECTION` 或其子类别时，审计日志包含以下信息：

| 字段           | 描述                                                                                   |
|-----------------|-----------------------------------------------------------------------------------------------|
| `CURRENT_DB`      | 当前数据库名称。当事件类别包含 DISCONNECT 时不记录该信息。 |
...
当事件类别为 `AUDIT` 或其子类别时，审计日志包含以下信息：
...
TiDB Cloud Essential 不保证审计日志的顺序，这意味着你可能需要检查所有日志文件以查找最新事件。若需按时间顺序排序日志，可以使用审计日志中的 `TIME` 字段。