...
你可以使用 `tidbcloud_dedicated_private_endpoint_connection` 资源创建一个 TiDB Cloud Dedicated 私有端点连接。
...
2. 创建一个 `private_endpoint_connection.tf` 文件：
    ...
    - 获取 TiDB Cloud Dedicated 私有端点连接规格信息，请参见 [tidbcloud_private_endpoint_connection (资源)](https://registry.terraform.io/providers/tidbcloud/tidbcloud/latest/docs/resources/dedicated_private_endpoint_connection)。
...
3. 运行 `terraform apply` 命令。建议不要在应用资源时使用 `terraform apply --auto-approve`。
    ...
    - `known after apply` 表示在 `apply` 后你会得到对应的值。
...
4. 如果计划中的内容都没有问题，输入 `yes` 继续：
    ...
    tidbcloud_dedicated_private_endpoint_connection.example: 创建中...
    tidbcloud_dedicated_private_endpoint_connection.example: 创建完成，耗时10秒

    应用完成！资源：1个已添加，0个已更改，0个已销毁。
    ```

5. 使用 `terraform show` 或 `terraform state show tidbcloud_dedicated_private_endpoint_connection.${资源名}` 命令检查资源状态。前者显示所有资源和数据源的状态。
...
对于未由 Terraform 管理的 TiDB Cloud Dedicated 私有端点连接，可以通过导入开始用 Terraform 管理。
...
3. 审查并应用生成的配置。
    ...
    然后运行 `terraform apply` 以导入你的基础设施。应用后，示例输出如下：

    ```shell
    tidbcloud_dedicated_private_endpoint_connection.example: 导入中... [id=aws-1934187953894000000,example]
    tidbcloud_dedicated_private_endpoint_connection.example: 导入完成 [id=aws-19341879538940000000,example]

    应用完成！资源：1个已导入，0个已添加，0个已更改，0个已销毁。
    ```
...
要删除一个 TiDB Cloud Dedicated 私有端点连接，可以删除 `tidbcloud_dedicated_private_endpoint_connection` 资源的配置，然后运行 `terraform apply` 来销毁资源：

```shell
  $ terraform apply
  tidbcloud_dedicated_private_endpoint_connection.example: 刷新状态...
  ...
  tidbcloud_dedicated_private_endpoint_connection.example: 1秒后完成销毁

  应用完成！资源：0个已添加，0个已更改，1个已销毁。
```
