...
- **持久性** 意味着一旦事务提交，即使发生系统故障也会保持提交状态。TiKV 使用持久化存储来保证持久性。
...
Direct Customer（直客）指直接从 PingCAP 购买 TiDB Cloud 并直接支付账单的最终客户。与 [MSP 客户](#msp-customer) 区分。
...
自 v5.0 起，TiDB 通过 TiFlash 节点引入了大规模并行处理（MPP，Massively Parallel Processing）架构，将大型 Join 查询的执行负载分摊到多个 TiFlash 节点。当启用 MPP 模式时，TiDB 会根据成本判断是否使用 MPP 框架进行计算。在 MPP 模式下，Join 键会在计算过程中通过 Exchange 操作重新分布，将计算压力分散到每个 TiFlash 节点，从而加速计算。更多信息，参见 [使用 TiFlash MPP 模式](/tiflash/use-tiflash-mpp-mode.md)。

### MSP Customer

MSP 客户（Managed Service Provider Customer）指通过 MSP 渠道购买 TiDB Cloud 并通过 MSP 支付账单的最终客户。与 [直客](#direct-customer) 区分。
...
项目成员是被邀请加入组织一个或多个项目的用户。项目成员可以管理集群、网络访问、备份等资源。
...
Request Unit（RU，请求单位）是用于衡量单次数据库请求所消耗资源量的单位。单个请求消耗的 RU 数量取决于多种因素，如操作类型或检索/修改的数据量。更多信息，参见 [TiDB Cloud Serverless 计费详情](https://www.pingcap.com/tidb-cloud-serverless-pricing-details)。

## S

### Spending limit

Spending limit（消费上限）指你每月愿意为某个工作负载支付的最大金额。它是一种成本控制机制，允许你为 TiDB Cloud Serverless 集群设置预算。对于 [可扩展集群](/tidb-cloud/select-cluster-tier.md#scalable-cluster-plan)，消费上限至少需设置为 $0.01。此外，若可扩展集群符合条件，可享有免费额度，集群会优先消耗免费额度。
...
存储在线事务处理（OLTP）数据的存储节点。为高可用性，按 3 的倍数（如 3、6、9）扩容，其中两个节点作为副本。增加 TiKV 节点数量可以提升整体吞吐量。
