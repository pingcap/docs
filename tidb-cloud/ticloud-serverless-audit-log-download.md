...
TiDB Cloud Essential クラスターからデータベース監査ログファイルをダウンロードします。
...
| --concurrency int          | 同時ダウンロード数。デフォルト値は`3`です。                              | いいえ | インタラクティブモードと非インタラクティブモードの両方で動作します。 |
|  --force             | 確認なしで監査ログをダウンロードします。                                 | いいえ | インタラクティブモードと非インタラクティブモードの両方で動作します。 |
