...
3.  **「Google Cloud Storage からのデータのインポート」**ページで、次のソース データ情報を設定します。
...
データのインポートの進行状況が**「完了」と**表示されたら、サンプル データとデータベース スキーマがTiDB Cloud Serverless のデータベースに正常にインポートされています。
