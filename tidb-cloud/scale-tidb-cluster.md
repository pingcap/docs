...
> - Before decreasing the vCPU, you need to ensure that the current node storage of TiKV or TiFlash does not exceed the maximum node storage corresponding to the target vCPU as specified in [TiDB Cloud TiKV/TiFlash Storage Size](https://docs.pingcap.com/tidbcloud/size-your-cluster/#tiflash-node-storage). If the current storage of any component exceeds the maximum, decreasing the vCPU is prohibited.
