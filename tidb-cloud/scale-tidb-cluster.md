...
To scale a running cluster, take the following steps:
...
## Change the vCPU size or decrease the storage size

When a cluster is running, you cannot change its vCPU size or decrease its storage size. To make such changes, take either of the following methods:
...
- Method 1: Backup and restore
    ...
    You need to [create a latest backup of the cluster](/tidb-cloud/backup-and-restore.md#manual-backup), [delete the cluster](/tidb-cloud/delete-tidb-cluster.md), and then change the vCPU or storage size when you [restore the deleted cluster](/tidb-cloud/backup-and-restore.md#restore-a-deleted-cluster). Before taking this method, make sure the following impacts are acceptable:
...
- Method 2: Create a support ticket
