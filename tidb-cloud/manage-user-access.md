...
組織レベルでは、 TiDB Cloud は4 つのロールを定義しており、 `Organization Owner`を招待したり、メンバーに組織ロールを付与したりできます。
...
> **注記：**
>
> -   ロール`Organization Console Audit Manager` （ `Organization Console Audit Admin`から名称変更）は、データベース監査ログではなく、 TiDB Cloudコンソールの監査ログを管理するために使用されます。データベース監査を管理するには、プロジェクトレベルでロール`Project Owner`を使用してください。
...
プロジェクト レベルでは、 TiDB Cloud は3 つのロールを定義し、 `Project Owner`を招待したり、メンバーにプロジェクト ロールを付与したりできます。

> **注記：**
>
> -   `Organization Owner`はすべてのプロジェクトに対する`Project Owner`のすべての権限を持っているため、 `Organization Owner`プロジェクト メンバーを招待し、メンバーにプロジェクト ロールを付与することもできます。
...
| プロジェクト内のすべてのTiDB Cloud Starter クラスターに対して[支出限度額](/tidb-cloud/manage-serverless-spend-limit.md)管理します。 | ✅               | ❌                                | ❌                               | ❌                |
...
| エンドポイントの使用や作成など、データ読み取り専用の操作については[データサービス](/tidb-cloud/data-service-overview.md)管理して、データを読み取ってください。 | ✅               | ✅                                | ✅                               | ❌                |
| データの読み取りおよび書き込み操作については[データサービス](/tidb-cloud/data-service-overview.md)管理します。                         | ✅               | ✅                                | ❌                               | ❌                |
| [SQLエディター](/tidb-cloud/explore-data-with-chat2query.md)を使用してクラスター データをビュー。                          | ✅               | ✅                                | ✅                               | ❌                |
| [SQLエディター](/tidb-cloud/explore-data-with-chat2query.md)を使用してクラスターデータを変更および削除します。                    | ✅               | ✅                                | ❌                               | ❌                |
...
1.  [TiDB Cloudコンソール](https://tidbcloud.com)で、左上隅のコンボボックスをクリックします。所属する組織とプロジェクトのリストが表示されます。
    ...
    > **ヒント：**
    >
    > -   現在特定のクラスターのページにいる場合は、左上隅のコンボ ボックスをクリックした後、コンボ ボックス内の ← をクリックして組織とプロジェクトのリストに戻る必要があります。
...
5.  招待するユーザーのメール アドレスを入力し、ユーザーの組織ロールを選択します。
...
8.  招待されたメールアドレスがTiDB Cloudアカウントに登録されていない場合、ユーザーはアカウント作成のためのサインアップページに移動します。メールアドレスがTiDB Cloudアカウントに登録済みの場合、ユーザーはサインインページに移動し、サインインするとアカウントは自動的に組織に参加します。
...
1.  [TiDB Cloudコンソール](https://tidbcloud.com)で、左上隅のコンボボックスをクリックします。所属する組織とプロジェクトのリストが表示されます。
    ...
    >
    > -   現在特定のクラスターのページにいる場合は、左上隅のコンボ ボックスをクリックした後、コンボ ボックス内の ← をクリックして組織とプロジェクトのリストに戻る必要があります。
    > -   複数のプロジェクトのメンバーである場合は、コンボ ボックス内の対象プロジェクト名をクリックして、プロジェクトを切り替えることができます。
...
ロール`Organization Owner`の場合は、組織内の任意のプロジェクトの名前を変更できます。ロール`Project Owner`の場合は、プロジェクトの名前を変更できます。
