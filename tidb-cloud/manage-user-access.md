...
TiDB Cloud 使用基于组织、项目和资源的层级结构来帮助你管理用户和 TiDB 部署。

- 组织是顶层实体（例如公司或客户），你可以用它来管理 TiDB Cloud 账户（包括一个管理账户和任意数量的成员账户）、[项目](#projects)和[资源](/tidb-cloud/tidb-cloud-glossary.md#tidb-cloud-resource)。
- 项目是 TiDB Cloud 资源的容器。

    - 对于 {{{ .starter }}} 和 Essential 实例，项目是一个可选的逻辑容器，这意味着你既可以将这些实例分组到项目中，也可以将这些实例保留在组织级别。
    - 对于 {{{ .dedicated }}} 集群，项目与基础设施绑定且是必需的，这意味着出于管理目的，必须将 {{{ .dedicated }}} 集群分组到项目中。
- TiDB Cloud 中的资源可以是 TiDB X 实例（例如 {{{ .starter }}} 或 {{{ .essential }}}），也可以是 {{{ .dedicated }}} 集群。

如果你是组织所有者，可以在组织中创建多个项目。

- 对于 TiDB X 实例，你既可以将它们分组到项目中，也可以将它们直接保留在组织级别。
...
- 要访问某个组织，用户必须是该组织的成员。
- 要访问组织中的某个项目，用户至少必须拥有该组织中该项目的读取权限。
- 要访问某个特定的 TiDB X 实例，用户可以通过项目角色或实例角色获得访问权限。
- 要访问 TiDB Cloud Dedicated 集群，用户必须拥有该集群所在项目的读取权限。

关于用户角色和权限的更多信息，请参见[用户角色](#user-roles)。

### 组织

一个组织可以包含多个项目，以及未分组到任何项目中的 TiDB X 实例。
...
| 拥有该组织下所有项目的 `Project Owner` 权限，以及该组织下所有 TiDB X 实例的所有 TiDB X 实例角色权限。 | ✅ | ❌ | ❌ | ❌ | ❌ |
...
> - 对于 TiDB X 项目和 TiDB Dedicated 项目，项目角色控制对项目中资源的访问。对于 TiDB Dedicated 项目，项目角色还控制 Dedicated 特有的项目设置。
> - 项目角色不适用于 TiDB X 虚拟项目，因为 TiDB X 虚拟项目不提供任何管理能力。要管理未分组到任何 TiDB X 项目中的特定 TiDB X 实例的 RBAC，请使用[实例角色](#instance-roles)。
...
| 管理项目中的资源操作，例如创建、修改、移动和删除该项目类型支持的实例或集群。 | ✅ | ❌ | ❌ | ❌ |
...
TiDB X 实例支持实例级角色，因此你可以仅为单个 TiDB X 实例授予访问权限，而无需为项目中的所有资源授予相同的访问权限。

> **注意：**
>
> - 实例角色仅适用于 {{{ .starter }}} 和 {{{ .essential }}}。TiDB Cloud Dedicated 集群不支持实例角色。
> - `Organization Owner` 自动拥有组织中所有 TiDB X 实例的全部权限。
> - 每个实例角色默认继承 `Organization Viewer` 角色的所有权限。
> - 项目角色和实例角色是叠加的。用户既可以从项目角色继承访问权限，也可以在单个实例上拥有更具体的角色。
...
当你想管理项目中的所有资源时，请使用项目角色；当你只想授予对特定 TiDB X 实例的访问权限时，请使用实例角色。
...
> **注意：**
>
> 如果成员被移除出组织，该成员也会被移除出所有项目，并失去该组织中的所有实例访问权限。
...
本节介绍如何重命名项目，以及如何邀请和移除项目成员。要了解如何创建或管理项目，请参见[管理项目](/tidb-cloud/manage-projects-and-resources.md#manage-tidb-cloud-projects)。
...
1. 在 TiDB Cloud 控制台中，进入你组织的 [**My TiDB**](https://tidbcloud.com/tidbs) 页面，然后点击 **Project view** 标签页。

    > **提示：**
    >
    > 如果你属于多个组织，请先使用左上角下拉框切换到目标组织。

2. 在项目视图中，找到目标项目对应的表格，点击表格右上角的 **...**，然后点击 **Rename**。
...
1. 在 TiDB Cloud 控制台中，进入你组织的 [**My TiDB**](https://tidbcloud.com/tidbs) 页面，然后点击 <svg width="1em" height="1em" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" stroke-width="1.5" class="tiui-icon Folder " style="width: calc(1.125rem * var(--mantine-scale)); height: calc(1.125rem * var(--mantine-scale));"><path d="M8.66671 4.66667L7.92301 3.17928C7.70898 2.7512 7.60195 2.53715 7.44229 2.38078C7.30109 2.24249 7.13092 2.13732 6.94409 2.07287C6.73282 2 6.49351 2 6.0149 2H3.46671C2.71997 2 2.3466 2 2.06139 2.14532C1.8105 2.27316 1.60653 2.47713 1.4787 2.72801C1.33337 3.01323 1.33337 3.3866 1.33337 4.13333V4.66667M1.33337 4.66667H11.4667C12.5868 4.66667 13.1469 4.66667 13.5747 4.88465C13.951 5.0764 14.257 5.38236 14.4487 5.75869C14.6667 6.18651 14.6667 6.74656 14.6667 7.86667V10.8C14.6667 11.9201 14.6667 12.4802 14.4487 12.908C14.257 13.2843 13.951 13.5903 13.5747 13.782C13.1469 14 12.5868 14 11.4667 14H4.53337C3.41327 14 2.85322 14 2.42539 13.782C2.04907 13.5903 1.74311 13.2843 1.55136 12.908C1.33337 12.4802 1.33337 11.9201 1.33337 10.8V4.66667Z" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="inherit"></path></svg> 图标进入项目视图。

    > **提示：**
    >
    > 如果你属于多个组织，请先使用左上角下拉框切换到目标组织。

2. 在项目视图中，找到目标项目对应的表格，点击表格右上角的 **...**，然后点击 **Invite**。

3. 在显示的对话框中，输入要邀请用户的邮箱地址，然后为该用户选择一个项目角色。
...
6. 如果被邀请邮箱尚未注册 TiDB Cloud 账户，用户会被引导至注册页面创建账户；如果邮箱已注册 TiDB Cloud 账户，用户会被引导至登录页面。登录后，账户会自动加入该项目。
...
## 管理实例访问 {#manage-instance-access}

### 授予对 TiDB X 实例的访问权限 {#grant-access-to-a-tidb-x-instance}
...
3. 在 **Users** 页面，找到目标成员所在行，点击该行中的 **...**，然后点击 **Edit Role**。
    ...
    > **提示：**
    >
    > 如果该用户尚未加入你的组织，请点击右上角的 **Invite User**，并按照[邀请用户加入你的组织](#invite-a-user-to-your-organization)中的步骤为该用户授予实例角色。

4. 在 **Edit Role** 页面中的 **Instance access** 部分，点击 **Add Role and Select Instance**，然后为用户授予角色并选择目标 TiDB X 实例。

5. 点击 **Save**。

### 移除用户的实例访问权限

如果你拥有 `Organization Owner` 或 `Project Owner` 角色，则可以移除用户的实例访问权限。

如需移除用户的实例访问权限，请执行以下步骤：

1. 在 [TiDB Cloud 控制台](https://tidbcloud.com)中，使用左上角的组合框切换到目标组织。

2. 在左侧导航栏中，点击 **Organization Settings** > **Users**。

3. 在 **Users** 页面中，找到目标成员所在行，点击该行中的 **...**，然后点击 **Edit Role**。
...
如需在 TiDB Cloud 中修改用户角色，请执行以下步骤：

1. 在 [TiDB Cloud 控制台](https://tidbcloud.com)中，使用左上角的组合框切换到目标组织。

2. 在左侧导航栏中，点击 **Organization Settings** > **Users**。

3. 在 **Users** 页面中，找到目标用户所在行，点击该行中的 **...**，然后点击 **Edit Role**。

    - 如果你拥有 `Organization Owner` 角色，则可以修改目标用户的组织角色、项目角色和实例角色。
    - 如果你拥有 `Project Owner` 角色，则可以修改目标用户的项目角色和实例角色。
