...
- 项目是 TiDB Cloud 资源的容器。
    ...
    - 对于 {{{ .starter }}} 和 Essential 实例，项目是一个可选的逻辑容器，这意味着你可以将这些实例分组到一个项目中，也可以将这些实例保留在组织级别。
    - 对于 {{{ .dedicated }}} 集群，项目是基础设施绑定的且必需的，这意味着 {{{ .dedicated }}} 集群必须分组到项目中进行管理。
- TiDB Cloud 中的资源可以是 TiDB X 实例（例如 {{{ .starter }}} 或 {{{ .essential }}}）或 {{{ .dedicated }}} 集群。
...
- **TiDB X 项目**：此项目类型仅用于 TiDB X 实例（{{{ .starter }}} 和 {{{ .essential }}}）。它帮助你按项目管理 TiDB X 实例的 RBAC。在 [**My TiDB**](https://tidbcloud.com/tidbs) 页面上创建项目时，TiDB X 项目是默认的项目类型。
- **TiDB X 虚拟项目**：此项目是虚拟的，不提供任何管理功能。它充当不属于任何项目的 TiDB X 实例（{{{ .starter }}} 和 {{{ .essential }}}）的虚拟容器，以便这些实例可以通过 TiDB Cloud API 使用项目 ID 进行访问。每个组织都有一个唯一的虚拟项目 ID。你可以从 TiDB Cloud API 的 [List all accessible projects](https://docs.pingcap.com/tidbcloud/api/v1beta/#tag/Project/operation/ListProjects) 端点获取此 ID。
