...
各TiDB Cloud Dedicatedクラスタはプロジェクトに属します。TiDB Cloud Dedicatedクラスタを作成する前に、クラスタを作成するプロジェクトのIDを取得する必要があります。`project_id`指定されていない場合は、デフォルトのプロジェクトが使用されます。
...
Terraform によって管理されていない TiDB クラスターの場合は、インポートすることで Terraform の管理下に置くことができます。
