...
このドキュメントでは、TiDBバージョン7.5.0を搭載したTiDB Cloud DedicatedクラスターのTPC-Cパフォーマンステストの手順と結果について説明します。このレポートは、TiDB Self-Managedバージョン7.5.0クラスターのパフォーマンスに関する参考資料としてもご利用いただけます。
