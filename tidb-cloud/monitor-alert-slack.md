...
1.  まだお持ちでない場合は、 [Slackアプリを作成する](https://api.slack.com/apps/new)。 **Create New App**をクリックし、 **[最初から]**を選択します。名前を入力し、アプリを関連付けるワークスペースを選択して、 **[アプリの作成] を**クリックします。
2.  アプリの設定ページに移動します。[アプリの管理ダッシュボード](https://api.slack.com/apps)から設定をロードできます。
3.  **「受信Webhook」**タブをクリックし、 **Activate Incoming Webhooks**を**オン**に切り替えます。
