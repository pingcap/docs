...
TiDB Cloud 目前仅支持自建 Kafka 的 Private Connect，不支持与 MSK、Confluent Kafka 或其他 Kafka SaaS 服务的直接集成。若需通过 Private Connect 连接这些 Kafka SaaS 服务，你可以部署一个 [kafka-proxy](https://github.com/grepplabs/kafka-proxy) 作为中间层，将 Kafka 服务以自建 Kafka 的方式暴露。详细示例可参考 [Set Up Self-Hosted Kafka Private Service Connect by Kafka-proxy in Google Cloud](/tidb-cloud/setup-self-hosted-kafka-private-service-connect.md#set-up-self-hosted-kafka-private-service-connect-by-kafka-proxy)。该方案适用于所有 Kafka SaaS 服务。
...
TiDB Cloud 目前仅支持自建 Kafka 的 Private Connect，不支持与 MSK、Confluent Kafka 或其他 Kafka SaaS 服务的直接集成。若需通过 Private Connect 连接这些 Kafka SaaS 服务，你可以部署一个 [kafka-proxy](https://github.com/grepplabs/kafka-proxy) 作为中间层，将 Kafka 服务以自建 Kafka 的方式暴露。
...
2. 根据你的 Kafka 认证配置，选择 **Authentication** 选项。
    ...
    - 如果 Kafka 需要认证，选择对应的认证类型，并填写 Kafka 账号的 **user name** 和 **password**。
...
6. 如果你选择 **Avro** 作为数据格式，页面会显示一些 Avro 专属配置。你可以按如下方式填写：
    ...
    - 在 **Schema Registry** 区域，填写你的 schema registry 端点。如果启用 **HTTP Authentication**，会显示用户名和密码字段，并自动填入你的 TiDB <CustomContent plan="dedicated">集群</CustomContent><CustomContent plan="premium">实例</CustomContent>端点和密码。
...
7. 在 **Topic Distribution** 区域，选择分发模式，并根据所选模式填写 topic 名称配置。
    ...
    - **Distribute changelogs by table to Kafka Topics**
        ...
        如果你希望为每个表创建独立的 Kafka topic，请选择此模式。这样，每个表的 Kafka 消息会被发送到专属的 Kafka topic。你可以通过设置 topic 前缀、数据库名与表名之间的分隔符以及后缀自定义 topic 名称。例如，分隔符设置为 `_` 时，topic 名称格式为 `<Prefix><DatabaseName>_<TableName><Suffix>`。
    ...
    - **Distribute changelogs by database to Kafka Topics**
        ...
        如果你希望为每个数据库创建独立的 Kafka topic，请选择此模式。这样，每个数据库的 Kafka 消息会被发送到专属的 Kafka topic。你可以通过设置 topic 前缀和后缀自定义数据库的 topic 名称。
    ...
    - **Send all changelogs to one specified Kafka Topic**
        ...
        如果你希望所有变更日志共用一个 Kafka topic，请选择此模式。这样，变更订阅中的所有 Kafka 消息都会被发送到同一个 Kafka topic。你可以在 **Topic Name** 字段定义 topic 名称。

8. 在 **Partition Distribution** 区域，你可以决定每条 Kafka 消息发送到哪个分区。你可以为所有表统一定义一个分区分发器，或为不同表定义不同的分区分发器。TiDB Cloud 提供四种分发器类型：

    - **Distribute changelogs by primary key or index value to Kafka partition**

        如果你希望将表的 Kafka 消息分发到不同分区，请选择此方式。行变更日志的主键或索引值将决定其被分发到哪个分区。该方式有助于分区均衡，并保证行级有序。

    - **Distribute changelogs by table to Kafka partition**

        如果你希望表的所有 Kafka 消息都发送到同一个分区，请选择此方式。行变更日志的表名将决定其被分发到哪个分区。该方式保证表内有序，但可能导致分区不均衡。

    - **Distribute changelogs by timestamp to Kafka partition**

        如果你希望将 Kafka 消息随机分发到不同分区，请选择此方式。行变更日志的 commitTs 将决定其被分发到哪个分区。该方式有助于分区均衡，并保证每个分区内有序。但同一数据项的多次变更可能被分发到不同分区，不同消费者的消费进度可能不同，可能导致数据不一致。因此，消费者需在消费前按 commitTs 对多分区数据进行排序。

    - **Distribute changelogs by column value to Kafka partition**

        如果你希望将表的 Kafka 消息分发到不同分区，请选择此方式。行变更日志的指定列值将决定其被分发到哪个分区。该方式保证每个分区内有序，并确保相同列值的变更日志被分发到同一分区。
