...
> **ノート：**
>
> [開発者層のクラスター](/tidb-cloud/select-cluster-tier.md#developer-tier)の場合、changefeed 機能は使用できません。
...
3.  Kafka で TLS 暗号化が有効になっており、Kafka 接続に TLS 暗号化を使用する場合は、 **TLS 暗号化**オプションを有効にします。

4.  Kafka 認証構成に従って、**認証**オプションを選択します。

    -   Kafka が認証を必要としない場合は、デフォルトのオプション**Disable**のままにします。
    -   Kafka で認証が必要な場合は、対応する認証タイプを選択し、認証のために Kafka アカウントのユーザー名とパスワードを入力します。

5.  [**次へ**] をクリックして、設定した構成を確認し、次のページに進みます。
...
3.  **Avro**をデータ形式として選択すると、Avro 固有の設定がページに表示されます。これらの構成は次のように入力できます。
    ...
    -   **Handle Decimal**および<strong>Handle Bigint Unsigned</strong>構成で、 TiDB Cloudが Kafka メッセージの decimal および Bigint データ型を処理する方法を指定します。
    -   [**スキーマ レジストリ]**領域で、スキーマ レジストリ エンドポイントを入力します。 <strong>HTTP 認証</strong>を有効にする場合は、スキーマ レジストリのユーザー名とパスワードも入力する必要があります。

4.  [**トピックの配布]**領域で、配布モードを選択し、デフォルトの Kafka トピック名を指定して、対応するトピックのプレフィックス、セパレータ、およびサフィックスを設定します。

    配布モードは、changefeed が Kafka トピックをテーブルごと、データベースごとに作成する方法、またはすべての変更ログに対して 1 つのトピックを作成する方法を制御します。データ形式として**Avro**を選択した場合は、[<strong>配布モード]</strong>ドロップダウン リストで [<strong>テーブルごとに変更ログを Kafka トピック</strong>に配布] モードのみを選択できます。

    -   **テーブルごとに変更ログを Kafka トピックに配布する**

        変更フィードでテーブルごとに専用の Kafka トピックを作成する場合は、このモードを選択します。次に、テーブルのすべての Kafka メッセージが専用の Kafka トピックに送信されます。トピックのプレフィックス、セパレータ、およびサフィックスを設定することで、テーブルのトピック名を定義できます。たとえば、セパレータを`_`に設定すると、トピック名は`<Prefix>_<DatabaseName>_<TableName>_<Suffix>`の形式になります。
    ...
    -   **データベースごとに変更ログを Kafka トピックに配布する**
        ...
        変更フィードでデータベースごとに専用の Kafka トピックを作成する場合は、このモードを選択します。次に、データベースのすべての Kafka メッセージが専用の Kafka トピックに送信されます。トピックの接頭辞、区切り文字、接尾辞を設定して、データベースのトピック名を定義できます。たとえば、セパレータを`_`に設定すると、トピック名は`<Prefix>_<DatabaseName>_<Suffix>`の形式になります。

    -   **指定された 1 つの Kafka トピックにすべての変更ログを送信します**

        変更フィードですべての変更ログに対して 1 つの Kafka トピックを作成する場合は、このモードを選択します。次に、変更フィード内のすべての Kafka メッセージが 1 つの Kafka トピックに送信されます。**デフォルトのトピック名**フィールドでトピック名を定義できます。
