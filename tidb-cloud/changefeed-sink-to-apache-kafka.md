...
summary: 本文档介绍如何创建变更订阅（changefeed），将数据从 TiDB Cloud 流式同步到 Apache Kafka。内容包括限制、前提条件，以及为 Apache Kafka 配置变更订阅的步骤。该过程涉及设置网络连接、为 Kafka ACL 授权添加权限，以及配置变更订阅规范。
---
...
- 对于每个 TiDB Cloud <CustomContent plan="dedicated">集群</CustomContent><CustomContent plan="premium">实例</CustomContent>，最多可以创建 100 个变更订阅。
...
TiDB Cloud 目前仅支持自建 Kafka 的 Private Connect，不支持与 MSK、Confluent Kafka 或其他 Kafka SaaS 服务的直接集成。若需通过 Private Connect 连接这些 Kafka SaaS 服务，你可以部署一个 [kafka-proxy](https://github.com/grepplabs/kafka-proxy) 作为中间层，将 Kafka 服务以自建 Kafka 的形式暴露。详细示例可参考 [Set Up Self-Hosted Kafka Private Service Connect by Kafka-proxy in Google Cloud](/tidb-cloud/setup-self-hosted-kafka-private-service-connect.md#set-up-self-hosted-kafka-private-service-connect-by-kafka-proxy)。该方案适用于所有 Kafka SaaS 服务。
...
2. 修改 Apache Kafka 服务关联的安全组的入站规则。
    ...
    你必须将 TiDB Cloud 集群所在区域的 CIDR 添加到入站规则中。该 CIDR 可在 **VPC Peering** 页面找到。这样可以允许来自 TiDB 集群的流量访问 Kafka broker。
...
2. 修改 Apache Kafka 所在 VPC 的 ingress 防火墙规则。
    ...
    你必须将 TiDB Cloud 集群所在区域的 CIDR 添加到 ingress 防火墙规则中。该 CIDR 可在 **VPC Peering** 页面找到。这样可以允许来自 TiDB 集群的流量访问 Kafka broker。
...
要在 TiDB Cloud Premium 实例中为变更订阅创建私有端点，请参考 [Set Up Private Endpoint for Changefeeds](/tidb-cloud/set-up-sink-private-endpoint.md)。

TiDB Cloud 目前仅支持自建 Kafka 的 Private Connect，不支持与 MSK、Confluent Kafka 或其他 Kafka SaaS 服务的直接集成。若需通过 Private Connect 连接这些 Kafka SaaS 服务，你可以部署一个 [kafka-proxy](https://github.com/grepplabs/kafka-proxy) 作为中间层，将 Kafka 服务以自建 Kafka 的形式暴露。
...
为了允许 TiDB Cloud 变更订阅将数据流式同步到 Apache Kafka 并自动创建 Kafka topic，请确保在 Kafka 中添加了以下权限：
...
10. 端点创建完成后，登录 [Azure portal](https://portal.azure.com/)，接受连接请求。
11. 返回 [TiDB Cloud 控制台](https://tidbcloud.com)，确认你已接受连接请求。TiDB Cloud 会测试连接，测试通过后进入下一页面。
...
6. 如果你选择 **Avro** 作为数据格式，页面会显示一些 Avro 专属配置。你可以按如下方式填写：
    ...
    - 在 **Schema Registry** 区域，填写你的 schema registry 端点。如果启用 **HTTP Authentication**，会显示用户名和密码字段，并自动填充为你的 TiDB <CustomContent plan="dedicated">集群</CustomContent><CustomContent plan="premium">实例</CustomContent>端点和密码。
...
7. 在 **Topic Distribution** 区域，选择分发模式，并根据模式填写 topic 名称配置。
    ...
    - **Distribute changelogs by table to Kafka Topics**
        ...
        如果你希望为每个表创建专用的 Kafka topic，请选择此模式。这样，某个表的所有 Kafka 消息都会发送到专用的 Kafka topic。你可以通过设置 topic 前缀、数据库名与表名之间的分隔符以及后缀自定义 topic 名称。例如，分隔符设置为 `_` 时，topic 名称格式为 `<Prefix><DatabaseName>_<TableName><Suffix>`。
    ...
    - **Distribute changelogs by database to Kafka Topics**
        ...
        如果你希望为每个数据库创建专用的 Kafka topic，请选择此模式。这样，某个数据库的所有 Kafka 消息都会发送到专用的 Kafka topic。你可以通过设置 topic 前缀和后缀自定义数据库的 topic 名称。
    ...
    - **Send all changelogs to one specified Kafka Topic**
        ...
        如果你希望所有变更日志共用一个 Kafka topic，请选择此模式。这样，变更订阅中的所有 Kafka 消息都会发送到同一个 Kafka topic。你可以在 **Topic Name** 字段定义 topic 名称。
...
8. 在 **Partition Distribution** 区域，你可以决定 Kafka 消息将被发送到哪个分区。你可以为所有表定义 **单一分区分发器**，也可以为不同表定义 **不同的分区分发器**。TiDB Cloud 提供四种分发器类型：
    ...
    - **Distribute changelogs by primary key or index value to Kafka partition**
        ...
        如果你希望将某个表的 Kafka 消息分发到不同分区，请选择此分发方式。行变更日志的主键或索引值将决定其被分发到哪个分区。该方式有助于分区均衡，并保证行级有序。

    - **Distribute changelogs by table to Kafka partition**

        如果你希望将某个表的 Kafka 消息全部发送到同一个分区，请选择此分发方式。行变更日志的表名将决定其被分发到哪个分区。该方式保证表级有序，但可能导致分区不均衡。

    - **Distribute changelogs by timestamp to Kafka partition**

        如果你希望将 Kafka 消息随机分发到不同分区，请选择此分发方式。行变更日志的 commitTs 将决定其被分发到哪个分区。该方式有助于分区均衡，并保证每个分区内有序。但同一数据项的多次变更可能被分发到不同分区，不同消费者的消费进度可能不同，可能导致数据不一致。因此，消费者需要在消费前按 commitTs 对多分区数据进行排序。

    - **Distribute changelogs by column value to Kafka partition**

        如果你希望将某个表的 Kafka 消息分发到不同分区，请选择此分发方式。行变更日志的指定列值将决定其被分发到哪个分区。该方式保证每个分区内有序，并确保相同列值的变更日志被分发到同一分区。
...
10. 在 **Split Event** 区域，选择是否将 `UPDATE` 事件拆分为单独的 `DELETE` 和 `INSERT` 事件，或保持原始 `UPDATE` 事件。详见 [Split primary or unique key UPDATE events for non-MySQL sinks](https://docs.pingcap.com/tidb/stable/ticdc-split-update-behavior/#split-primary-or-unique-key-update-events-for-non-mysql-sinks)。
