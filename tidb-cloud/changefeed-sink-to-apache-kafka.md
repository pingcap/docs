...
- 如果待复制的表没有主键或非空唯一索引，在复制过程中缺少唯一约束可能导致在某些重试场景中重复数据被下游插入。
...
- Private Connect：适合避免 VPC CIDR 冲突并满足安全合规，但会产生额外的 [Private Data Link 费用](/tidb-cloud/tidb-cloud-billing-ticdc-rcu.md#private-data-link-cost)。
...
- 如果你的 Apache Kafka 服务托管在 AWS 上，请参考 [在 AWS 中设置自托管 Kafka Private Link 服务](/tidb-cloud/setup-aws-self-hosted-kafka-private-link-service.md)，确保网络连接配置正确。设置完成后，在 TiDB Cloud 控制台提供以下信息以创建 changefeed：

    - Kafka 广告监听器模式中的 ID
    - 端点服务名称
    - 引导端口

- 如果你的 Apache Kafka 服务托管在 Google Cloud 上，请参考 [在 Google Cloud 中设置自托管 Kafka Private Service Connect](/tidb-cloud/setup-self-hosted-kafka-private-service-connect.md)，确保网络连接配置正确。设置完成后，在 TiDB Cloud 控制台提供以下信息以创建 changefeed：

    - Kafka 广告监听器模式中的 ID
    - 服务附件
    - 引导端口

- 如果你的 Apache Kafka 服务托管在 Azure 上，请参考 [在 Azure 中设置自托管 Kafka Private Link 服务](/tidb-cloud/setup-azure-self-hosted-kafka-private-link-service.md)，确保网络连接配置正确。设置完成后，在 TiDB Cloud 控制台提供以下信息以创建 changefeed：
...
如果你希望通过 Public IP 访问你的 Kafka 服务，请为所有 Kafka broker 分配 Public IP 地址。
...
为了让 TiDB Cloud changefeed 能够向 Apache Kafka 流式传输数据并自动创建 Kafka 主题，需在 Kafka 中添加以下权限：

- 为主题资源类型添加 `Create` 和 `Write` 权限。
- 为集群资源类型添加 `DescribeConfigs` 权限。

例如，如果你的 Kafka 集群在 Confluent Cloud，可以参考 [资源](https://docs.confluent.io/platform/current/kafka/authorization.html#resources) 和 [添加 ACLs](https://docs.confluent.io/platform/current/kafka/authorization.html#adding-acls) 获取更多信息。
...
6. 点击 **Next** 测试网络连接。测试成功后，会跳转到下一页。

</div>
<div label="Private Link (AWS)">

1. 在 **Connectivity Method** 中选择 **Private Link**。
2. 授权 [AWS Principal](https://docs.aws.amazon.com/IAM/latest/UserGuide/reference_policies_elements_principal.html#principal-accounts) 让 TiDB Cloud 创建端点以供你的端点服务使用。网页提示中会提供 AWS Principal 信息。
...
11. TiDB Cloud 会创建 **Private Link** 端点，可能需要几分钟时间。
12. 端点创建完成后，登录云服务提供商控制台，接受连接请求。
13. 返回 [TiDB Cloud 控制台](https://tidbcloud.com)，确认已接受连接请求。TiDB Cloud 会测试连接，成功后进入下一页。

</div>
<div label="Private Service Connect (Google Cloud)">

1. 在 **Connectivity Method** 中选择 **Private Service Connect**。
2. 在 [设置 Google Cloud 中自托管 Kafka Private Service Connect](/tidb-cloud/setup-self-hosted-kafka-private-service-connect.md) 的 **Network** 部分，确保在 **Kafka Advertised Listener Pattern** 中填写相同的唯一 ID。
3. 填写在 [设置 Google Cloud 中自托管 Kafka Private Service Connect] 中配置的 **Service Attachment**。
...
10. TiDB Cloud 会创建 **Private Service Connect** 端点，可能需要几分钟。
11. 端点创建完成后，登录云服务提供商控制台，接受连接请求。
12. 返回 [TiDB Cloud 控制台](https://tidbcloud.com)，确认已接受连接请求。TiDB Cloud 会测试连接，成功后进入下一页。

</div>
<div label="Private Link (Azure)">

1. 在 **Connectivity Method** 中选择 **Private Link**。
2. 在创建 changefeed 之前，授权 TiDB Cloud 的 Azure 订阅或允许任何拥有你的别名的人访问你的 Private Link 服务。网页提示中的 **Reminders before proceeding** 提供 Azure 订阅信息。关于 Private Link 服务的可见性，详见 [控制服务暴露](https://learn.microsoft.com/en-us/azure/private-link/private-link-service-overview#control-service-exposure)。
3. 确保在 [在 Azure 中设置自托管 Kafka Private Link 服务](/tidb-cloud/setup-azure-self-hosted-kafka-private-link-service.md) 的 **Network** 部分，填写相同的唯一 ID。
...
9. 如果启用了 TLS 加密，开启 **TLS Encryption** 选项。
10. 点击 **Next** 测试网络连接。测试成功后，跳转到下一页。
11. TiDB Cloud 会创建 **Private Link** 端点，可能需要几分钟。
...
1. 自定义 **Table Filter**，筛选你想要复制的表。规则语法请参考 [table filter rules](/table-filter.md)。
    ...
    - **Tables without valid keys**：显示缺少主键或唯一键的表。这些表在复制时存在挑战，因为缺少唯一标识符可能导致下游处理重复事件时数据不一致。建议在启动复制前，为这些表添加唯一键或主键，或通过筛选规则排除这些表，例如用规则 `"!test.tbl1"` 排除表 `test.tbl1`。

2. 自定义 **Event Filter**，筛选你想要复制的事件。

    - **Tables matching**：设置事件过滤器应用的表。规则语法与前述 **Table Filter** 区域相同。每个 changefeed 最多可添加 10 条事件过滤规则。
    - **Ignored events**：设置事件过滤器排除的事件类型。

3. 自定义 **Column Selector**，选择事件中的列，只将相关数据变更发送到下游。

    - **Tables matching**：指定列选择器适用的表。未匹配规则的表，全部列都会发送。
    - **Column Selector**：指定匹配表的哪些列会被发送到下游。
...
4. 在 **Data Format** 区域，选择你希望的 Kafka 消息格式。
    ...
    - Open Protocol：一种行级数据变更通知协议，提供监控、缓存、全文索引、分析引擎和不同数据库之间的主从复制的数据源。详见 [Open Protocol data format](https://docs.pingcap.com/tidb/stable/ticdc-open-protocol)。
...
5. 若要在 Kafka 消息中添加 TiDB-extension 字段，开启 **TiDB Extension** 选项。

    有关 TiDB-extension 字段的详细信息，参见 [TiDB extension fields in Avro data format](https://docs.pingcap.com/tidb/stable/ticdc-avro-protocol) 和 [TiDB extension fields in Canal-JSON data format](https://docs.pingcap.com/tidb/stable/ticdc-canal-json#tidb-extension-field)。
...
6. 如果选择 **Avro** 作为数据格式，页面会显示一些 Avro 相关配置。你可以按如下方式填写：
    ...
    - 在 **Schema Registry** 区域，填写你的 schema registry 端点。如果启用 **HTTP Authentication**，会显示用户名和密码字段，并自动填充你的 TiDB 集群端点和密码。

7. 在 **Topic Distribution** 区域，选择分发模式，并根据模式填写主题名配置。

    如果选择 **Avro** 作为数据格式，**Distribution Mode** 下拉列表中只能选择 **Distribute changelogs by table to Kafka Topics** 模式。

    分发模式控制 changefeed 如何创建 Kafka 主题，可以按表、按数据库，或为所有变更日志创建一个主题。

    - **Distribute changelogs by table to Kafka Topics**

        若希望每个表对应一个专属 Kafka 主题，选择此模式。此时，某个表的所有 Kafka 消息会发送到专属主题。可以通过设置主题前缀、数据库名与表名的分隔符以及后缀来自定义主题名。例如，若分隔符为 `_`，主题名格式为 `<Prefix><DatabaseName>_<TableName><Suffix>`。
    ...
    - **Distribute changelogs by database to Kafka Topics**
        ...
        若希望每个数据库对应一个专属 Kafka 主题，选择此模式。此时，某个数据库的所有 Kafka 消息会发送到专属主题。可以通过设置主题前缀和后缀来自定义主题名。对于非行事件（如 Resolved Ts Event），也可以在 **Default Topic Name** 指定主题。

    - **Send all changelogs to one specified Kafka Topic**

        若希望所有变更日志都发送到同一个 Kafka 主题，选择此模式。可以在 **Topic Name** 字段中定义主题名。

8. 在 **Partition Distribution** 区域，决定 Kafka 消息的分区策略。可以选择 **为所有表使用单一分区调度器**，或 **为不同表使用不同的分区调度器**。TiDB Cloud 提供四种调度器类型：
    ...
    - **Distribute changelogs by table to Kafka partition**
        ...
        若希望每个表的 Kafka 消息都在同一分区，选择此方式。行变更的表名决定消息的分区。这保证了表的有序性，但可能导致分区不均衡。
    ...
    - **Distribute changelogs by column value to Kafka partition**
        ...
        若希望按列值将表的 Kafka 消息分发到不同分区，选择此方式。行变更的指定列值决定消息的分区。这保证了每个分区的有序性，并确保相同列值的变更都在同一分区。
