...
TiDB Cloudチェンジフィードは、 TiDB Cloudから他のデータサービスにデータの変更を複製するのに役立つ継続的なデータ ストリームです。
...
TiDB Cloudでは、テーブルフィルター (レプリケートするテーブルを指定する) とイベント フィルター (INSERT や DELETE などの特定の種類のイベントを含めるか除外するか) を定義することで、レプリケーションをカスタマイズできます。
