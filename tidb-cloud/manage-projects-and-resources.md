...
- TiDB X 实例，即一种面向服务的 TiDB Cloud 产品，基于 [TiDB X architecture](/tidb-cloud/tidb-x-architecture.md) 构建，例如 <CustomContent plan="starter,essential,dedicated">{{{ .starter }}} 或 Essential</CustomContent><CustomContent plan="premium">{{{ .starter }}}, Essential, 或 Premium</CustomContent> 实例
- 一个 {{{ .dedicated }}} 集群
