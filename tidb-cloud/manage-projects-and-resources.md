...
- TiDB X instance，即基于 [TiDB X architecture](/tidb-cloud/tidb-x-architecture.md) 构建的面向服务的 TiDB Cloud 产品，例如 {{{ .starter }}}、Essential 或 Premium instance
- {{{ .dedicated }}} cluster
...
- 对于 TiDB X instances，项目是可选的，这意味着你既可以将这些实例归入某个项目，也可以将它们保留在组织级别。
- 对于 {{{ .dedicated }}} clusters，项目是必需的。
...
默认情况下，[**My TiDB**](https://tidbcloud.com/tidbs) 页面显示资源视图，其中会展示你在当前组织中有权限访问的所有资源。

如果你的组织中有很多 instances 或 clusters，可以使用页面顶部的筛选器快速找到所需内容。

要查看某个 TiDB Cloud 资源的详细信息，请点击目标资源名称进入其概览页面。
...
要执行更多操作并管理特定 TiDB Cloud 资源的设置，请点击目标资源名称进入其概览页面。
...
在项目视图中，你可以看到自己在该组织中所属的项目：

- 不属于任何项目的 TiDB X instances 会显示在名为 `Out of project` 的表中。
- 属于特定项目的 TiDB X instances 会显示在对应的 TiDB X 项目表中。
- TiDB Cloud Dedicated clusters 会显示在对应的 Dedicated 项目表中。这些表的文件夹图标中带有 **D**，用于标识 **Dedicated** 项目类型。
...
>
> - 免费试用用户无法创建新项目。
> - 对于 TiDB X instances，创建项目是可选的。对于 TiDB Cloud Dedicated clusters，你必须使用默认项目或创建新项目来管理它们。

如果你具有 `Organization Owner` 角色，则可以在组织中创建项目。
...
3. 根据你要为哪种类型的 TiDB Cloud 资源创建项目，执行以下操作之一：
    ...
    - 如果该项目用于 TiDB X instances，点击 **Confirm**。

        > **Note:**
        >
        > 对于 {{{ .premium }}} instances，加密是按实例而不是按项目配置的。创建实例后，你可以启用 [Dual-Layer Data Encryption](/tidb-cloud/premium/dual-layer-data-encryption-premium.md)，在默认的存储层加密之上增加数据库层加密。

    - 如果该项目用于 {{{ .dedicated }}} clusters，选择 **Create for Dedicated Cluster** 选项，为项目配置 [Customer-Managed Encryption Keys (CMEK)](/tidb-cloud/tidb-cloud-encrypt-cmek-aws.md) 和 [maintenance window](/tidb-cloud/configure-maintenance-window.md)，然后点击 **Confirm**。
...
2. 在项目视图中，找到目标项目，然后按如下方式进行管理：
    ...
    - 对于 TiDB Dedicated 项目，你还可以点击目标项目所在行中的 <MDSvgIcon name="icon-project-settings" /> 图标，按项目管理 {{{ .dedicated }}} clusters 的设置，例如网络、维护、告警订阅和加密访问。

### 在项目之间移动 TiDB X instance {#move-a-tidb-x-instance-between-projects}

如果你具有 `Organization Owner` 或 `Project Owner` 角色，则可以将 TiDB X instance 移动到某个项目中，或移出所有项目。

> **Note:**
>
> 只有 TiDB X instances 支持在 TiDB X 项目之间移动以及移出所有 TiDB X 项目。TiDB Cloud Dedicated clusters 不支持在项目之间移动。

要移动 TiDB X instance，请按以下步骤操作：

1. 在 [TiDB Cloud console](https://tidbcloud.com) 中，进入你所在组织的 [**My TiDB**](https://tidbcloud.com/tidbs) 页面，然后点击 **Project view** 标签页。

2. 在项目视图中，展开包含待移动 TiDB X instance 的项目文件夹，点击目标 TiDB X instance 的 **...**，然后点击 **Move**。

    > **Tip:**
    >
    > 如果 TiDB X instance 不属于任何项目，它会显示在 **Out of project** 文件夹中。

3. 在显示的对话框中，执行以下操作之一：

    - 要将 TiDB X instance 移动到某个项目，选择 **To a project**，然后从下拉列表中选择目标项目。
    - 要将 TiDB X instance 移出所有项目，选择 **Outside any project**。
