...
> **Note:**
        ...
        > - {{{ .premium }}} インスタンスでは、暗号化はプロジェクト単位ではなくインスタンス単位で構成されます。インスタンスの作成後、[Dual-Layer Data Encryption](/tidb-cloud/premium/dual-layer-data-encryption-premium.md) を有効にして、デフォルトのストレージレイヤー暗号化の上にデータベースレイヤー暗号化を追加できます。
