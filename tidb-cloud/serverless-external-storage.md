...
> **Note:**
>
> 仅当集群的云服务商为 AWS 时，才支持通过 Role ARN 访问 Amazon S3。如果你使用其他云服务商，请改用 AWS 访问密钥。更多信息，参见 [使用 AWS 访问密钥配置 Amazon S3 访问](#configure-amazon-s3-access-using-an-aws-access-key)。
...
1. 创建 IAM 用户。更多信息，参见 [创建 IAM 用户](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_users_create.html#id_users_create_console)。

2. 使用你的 AWS 账号 ID 或账号别名，以及 IAM 用户名和密码登录 [IAM 控制台](https://console.aws.amazon.com/iam)。

3. 创建访问密钥。更多信息，参见 [为 IAM 用户创建访问密钥](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_access-keys.html#Using_CreateAccessKey)。
...
1. 在 Google Cloud [服务账号页面](https://console.cloud.google.com/iam-admin/serviceaccounts)点击 **CREATE SERVICE ACCOUNT** 创建服务账号。更多信息，参见 [创建服务账号](https://cloud.google.com/iam/docs/creating-managing-service-accounts)。
...
3. 在 **Shared access signature** 页面，按如下方式创建具有所需权限的服务 SAS token。更多信息，参见 [创建服务 SAS token](https://docs.microsoft.com/en-us/azure/storage/common/storage-sas-overview)。

    1. 在 **Allowed services** 区域，选择 **Blob** 服务。
    2. 在 **Allowed Resource types** 区域，选择 **Container** 和 **Object**。
    3. 在 **Allowed permissions** 区域，按需选择权限。
...
1. 创建 RAM 用户并获取 AccessKey 对。更多信息，参见 [创建 RAM 用户](https://www.alibabacloud.com/help/en/ram/user-guide/create-a-ram-user)。
    
    在 **Access Mode** 区域，选择 **Using permanent AccessKey to access**。

2. 创建具有所需权限的自定义策略。更多信息，参见 [创建自定义策略](https://www.alibabacloud.com/help/en/ram/user-guide/create-a-custom-policy)。
...
3. 将自定义策略附加到 RAM 用户。更多信息，参见 [为 RAM 用户授权](https://www.alibabacloud.com/help/en/ram/user-guide/grant-permissions-to-the-ram-user)。