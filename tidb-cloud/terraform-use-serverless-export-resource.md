...
本文描述了如何使用 `tidbcloud_serverless_export` 资源管理 [TiDB Cloud Serverless](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 数据导出任务。
...
3. 运行 `terraform apply` 命令。建议在应用资源时不要使用 `terraform apply --auto-approve`。
    ...
    - 你可以检查配置与状态之间的差异。
    - 你还可以查看此次 `apply` 的结果。它会添加一个新资源，没有资源会被更改或销毁。
...
1. 在你的 `.tf` 文件中添加导入块。
    ...
    在你的 `.tf` 文件中添加以下导入块，替换 `example` 为你想要的资源名，替换 `${id}` 为 `cluster_id,export_id` 格式：
