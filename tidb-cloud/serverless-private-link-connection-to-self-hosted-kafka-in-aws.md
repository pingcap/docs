...
1. The private link connection connect to your endpoint service, with different ports.
2. Endpoint service forwards requests to your load balancers.
3. Load balancers forward requests to different Kafka brokers based on the port mapping.

## Prerequisites
   
1. Ensure that you have the following authorization to set up a Kafka in your own AWS account.
...
5. In the **Attach Domains** dialog choose the **TiDB Cloud Managed** domain type: The domains will be auto generated by TiDB Cloud, copy the unique name for the later use and click **Attach Domains** to confirm.

</div>

<div label="CLI">

First use dry run to preview the domains to be attached, it will output a unique-name for the next step, copy it for the later use.
