...
| TiKV storage utilization exceeds 80% | Consider increasing the node number or node storage size for TiKV to increase your storage capacity. When usage of TiKV exceeds 80%, latency might increase, and higher usage might cause requests to fail. |
| TiFlash storage utilization exceeds 80% | Consider increasing the node number or node storage size for TiFlash to increase your storage capacity. Usage of TiFlash above 80% indicates disk exhaustion and requests might fail.|
