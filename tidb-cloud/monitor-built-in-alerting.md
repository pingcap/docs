...
> -   一部のアラート ルールはデフォルトで無効になっています。必要に応じて有効にできます。

### リソース使用量アラート {#resource-usage-alerts}
...
| TiDB ノード全体の最大メモリ使用率が 10 分間で 70% を超えました        | クラスター内に[ホットスポット](/tidb-cloud/tidb-cloud-sql-tuning-overview.md#hotspot-issues)があるかどうかを確認するか、TiDB のノード番号またはノード サイズを増やして、現在のワークロードのメモリ使用率を減らすことを検討してください。   |
| TiKV ノード全体の最大メモリ使用率が 10 分間で 70% を超えました        | クラスター内に[ホットスポット](/tidb-cloud/tidb-cloud-sql-tuning-overview.md#hotspot-issues)があるかどうかを確認するか、TiKV のノード番号またはノード サイズを増やして、現在のワークロードのメモリ使用率を減らすことを検討してください。   |
| TiDB ノード全体の最大 CPU 使用率が 10 分間で 80% を超えました      | クラスター内に[ホットスポット](/tidb-cloud/tidb-cloud-sql-tuning-overview.md#hotspot-issues)があるかどうかを確認するか、TiDB のノード番号またはノード サイズを増やして、現在のワークロードの CPU 使用率を減らすことを検討してください。 |
| TiKV ノード全体の最大 CPU 使用率が 10 分間で 80% を超えました      | クラスター内に[ホットスポット](/tidb-cloud/tidb-cloud-sql-tuning-overview.md#hotspot-issues)があるかどうかを確認するか、TiKV のノード番号またはノード サイズを増やして、現在のワークロードの CPU 使用率を減らすことを検討してください。 |
