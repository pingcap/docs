...
| TiKV storage utilization exceeds 80% | Consider increasing the node number or node storage size for TiKV to increase your storage capacity. The storage usage of TiKV exceeding 80% can cause latency spikes, and higher usage might cause requests to fail. |
| TiFlash storage utilization exceeds 80% | Consider increasing the node number or node storage size for TiFlash to increase your storage capacity. When the storage usage of all TiFlash nodes reaches 80%, executing a DDL statement to add a TiFlash replica hangs indefinitely.|
