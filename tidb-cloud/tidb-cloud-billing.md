...
Infrequent Access (IA) を使用した行ベースのストレージでは、データは **remote object storage** に保存されます。次の SQL ステートメントを使用して、テーブルの **Storage Class** を `IA` に設定できます。
...
- **TiDB Cloud-level encryption:** クラウドプロバイダーの暗号化に加えて、TiDB Cloud は顧客管理暗号化キー (CMEK) またはエスクローキーを使用して、自動的に第 2 の暗号化レイヤーを適用します。
...
2.  TiDB Cloud製品ページで、 **Continue to Subscribe**クリックします。注文ページが表示されます。

3.  注文ページで**「購読」**をクリックし、次に**Set Up your Account**クリックしてください。TiDB Cloudのサインアップページに移動します。
