...
組織で`Organization Owner`または`Organization Billing Manager`のロールを担っている場合は、 TiDB Cloudアカウントを AWS 請求アカウントまたは Google Cloud 請求アカウントにリンクできます。それ以外の場合は、このセクションをスキップしてください。
