...
3. In the editor, you can either simply type `--` followed by your instructions to let AI automatically generate SQL queries, or write SQL queries manually.
    ...
    For example, if you would like to create a new table `test.t` with two columns (column `id` and column `name`), you can type `-- create a new table t with id and name` and press **Enter**. Then, AI will generate a SQL statement accordingly. For the generated statement, you can accept it by pressing **Tab** and then further edit it if needed, or reject it by pressing **Esc**.
