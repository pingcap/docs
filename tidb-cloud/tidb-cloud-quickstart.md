...
## 第 4 步：连接到你的 {{{ .starter }}} 实例 {#step-4-connect-to-your-starter-instance}

TiDB 与 MySQL 协议高度兼容，因此你可以使用大多数 MySQL 工具、驱动程序和 ORM 连接到 TiDB。以下步骤以 MySQL CLI 客户端 `mysql` 为例。

1. 打开 [**My TiDB**](https://tidbcloud.com/tidbs) 页面，然后点击目标 {{{ .starter }}} 实例的名称，进入其实例概览页面。
...
MySQL CLI 客户端将打开并连接到你的 {{{ .starter }}} 实例。
