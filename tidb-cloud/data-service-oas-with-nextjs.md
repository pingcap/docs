...
- [一个 {{{ .starter }}} 实例](/tidb-cloud/create-tidb-cluster-serverless.md) 或 [一个 {{{ .dedicated }}} 集群](/tidb-cloud/create-tidb-cluster.md)。
...
首先，在你的 {{{ .starter }}} 实例或 {{{ .dedicated }}} 集群中创建表 `test.repository`，并插入一些示例数据。以下示例插入了由 PingCAP 开发的一些开源项目，作为演示数据。
...
数据插入完成后，前往 [TiDB Cloud 控制台](https://tidbcloud.com)的 [**Data Service**](https://tidbcloud.com/project/data-service) 页面。创建一个关联到你的 {{{ .starter }}} 实例或 {{{ .dedicated }}} 集群的 Data App，为该 Data App 创建一个 API key，然后在 Data App 中创建一个 `GET /repositories` 的 endpoint。该 endpoint 对应的 SQL 语句如下，用于查询 `test.repository` 表中的所有行：
...
2. 在 `hello-repos` 项目目录下，将 `app/page.tsx` 的内容替换为以下代码，该代码会从 `GET /repositories` endpoint 获取数据并进行渲染：
    ...
    > **注意：**
    >
    > 如果你的 Data App 关联的 {{{ .starter }}} 实例或 {{{ .dedicated }}} 集群分布在不同区域，你会在下载的 OpenAPI 规范文件的 `servers` 部分看到多个项。此时，你还需要在 `config` 对象中配置 endpoint 路径，如下所示：
    >
    >  ```js
    >  const config = new Configuration({
    >      username: process.env.TIDBCLOUD_DATA_SERVICE_PUBLIC_KEY,
