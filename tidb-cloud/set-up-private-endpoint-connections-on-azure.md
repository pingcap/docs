...
Azure プライベート エンドポイント接続機能は、プライベート エンドポイントを自動的に検出できます。つまり、Azure ポータルで[Azureプライベートエンドポイントの作成](#step-2-create-an-azure-private-endpoint)を行った後、 TiDB Cloudコンソールの [ **Azure プライベート エンドポイント接続の作成**] ダイアログで**[キャンセル]**をクリックしても、作成されたエンドポイントを**[ネットワーク]**ページで表示できます。キャンセルが意図的でない場合は、エンドポイントの設定を続行してセットアップを完了できます。キャンセルが意図的な場合は、TiDB Cloudコンソールでエンドポイントを直接削除できます。
