...
Azure プライベートエンドポイント接続機能は、プライベートエンドポイントを自動的に検出できます。つまり、 [Azureプライベートエンドポイントの作成](#step-2-create-an-azure-private-endpoint)Azure ポータルで、 TiDB Cloudコンソールの [ **Azure プライベートエンドポイント接続の作成**] ダイアログで**[キャンセル]** をクリックしても、作成されたエンドポイントを**Networking**ページで表示できます。キャンセルが意図的でない場合は、エンドポイントの設定を続行してセットアップを完了できます。キャンセルが意図的な場合は、TiDB Cloudコンソールでエンドポイントを直接削除できます。
