...
TiDB では、 [ベクトルデータ型](/tidb-cloud/vector-search-data-types.md)列に対する近似最近傍 (ANN) 検索にベクトル検索インデックスを作成して利用できます。ベクトル検索インデックスを使用すると、ベクトル検索クエリを数ミリ秒で完了できます。
...
> **注記：**
>
> ベクトル検索インデックスは[TiDB サーバーレス](/tidb-cloud/select-cluster-tier.md#tidb-serverless)クラスターに対してのみ使用できます。
