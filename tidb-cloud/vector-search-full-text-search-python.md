...
summary: 全文检索允许你通过精确关键词检索文档。在 RAG（检索增强生成）场景下，你可以将全文检索与向量检索结合使用，以提升检索质量。
aliases: ['/tidb/stable/vector-search-full-text-search-python']
---

# 使用 Python 进行全文检索

与关注语义相似度的 [向量检索](/vector-search/vector-search-overview.md) 不同，全文检索允许你通过精确关键词检索文档。在 RAG（检索增强生成）场景下，你可以将全文检索与向量检索结合使用，以提升检索质量。
...
2. 点击右上角的 **Connect**，会弹出连接对话框，显示连接参数。
   ...
   注意，上述示例仅用于演示。你需要将参数替换为你自己的值，并妥善保管。
