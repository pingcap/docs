...
本文档的示例使用了跨多个 MySQL 实例的复杂分片迁移任务，并涉及处理自增主键冲突的场景。该示例场景同样适用于在单个 MySQL 实例内合并不同分片表的数据。
...
- 如果一个表的数据被分成多个 CSV 文件，需为这些 CSV 文件添加数字后缀。例如，`${db_name}.${table_name}.000001.csv` 和 `${db_name}.${table_name}.000002.csv`。数字后缀可以不连续，但必须递增。你还需要在数字前补零，确保所有后缀长度一致。
...
3. 在 **Import Data from Amazon S3** 页面，填写以下信息：
    ...
    - **Data Format**：选择 **CSV**。
    - **Folder URI**：填写源数据的 bucket URI。本示例可使用对应表的二级目录 `s3://dumpling-s3/store/sales/`，这样 TiDB Cloud 可以一次性将所有 MySQL 实例的数据导入并合并到 `store.sales`。
