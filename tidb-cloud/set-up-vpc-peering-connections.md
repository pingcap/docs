...
3. 在左侧导航栏，点击 **Project Settings** 下的 **Network Access**。
4. 在 **Network Access** 页面，点击 **Project CIDR** 标签页，然后根据你的云服务商选择 **AWS** 或 **Google Cloud**。
5. 在右上角，点击 **Create CIDR**。在 **Create AWS CIDR** 或 **Create Google Cloud CIDR** 对话框中指定区域和 CIDR 值，然后点击 **Confirm**。
...
2. 在项目视图中，找到目标项目，然后点击该项目的 <MDSvgIcon name="icon-project-settings" />。
3. 在左侧导航栏，点击 **Project Settings** 下的 **Network Access**。
4. 在 **Network Access** 页面，点击 **VPC Peering** 标签页，然后点击 **AWS** 子标签。

    默认会显示 **VPC Peering** 配置。

5. 在右上角，点击 **Create VPC Peering**，选择 **TiDB Cloud VPC Region**，然后填写你现有 AWS VPC 的相关信息：
...
6. 点击 **Create** 发送 VPC Peering 请求，然后在 **VPC Peering** > **AWS** 标签页查看 VPC Peering 信息。新建的 VPC Peering 状态为 **System Checking**。

7. 若要查看新建 VPC Peering 的详细信息，在 **Action** 列点击 **...** > **View**。将显示 **VPC Peering Details** 页面。
...
2. 在项目视图中，找到目标项目，然后点击该项目的 <MDSvgIcon name="icon-project-settings" />。
3. 在左侧导航栏，点击 **Project Settings** 下的 **Network Access**。
4. 在 **Network Access** 页面，点击 **VPC Peering** 标签页，然后点击 **Google Cloud** 子标签。

    默认会显示 **VPC Peering** 配置。

5. 在右上角，点击 **Create VPC Peering**，选择 **TiDB Cloud VPC Region**，然后填写你现有 Google Cloud VPC 的相关信息：
...
6. 点击 **Create** 发送 VPC Peering 请求，然后在 **VPC Peering** > **Google Cloud** 标签页查看 VPC Peering 信息。新建的 VPC Peering 状态为 **System Checking**。
...
1. 在 [**My TiDB**](https://tidbcloud.com/tidbs) 页面，点击目标 TiDB Cloud Dedicated 集群的名称，进入其概览页面。
