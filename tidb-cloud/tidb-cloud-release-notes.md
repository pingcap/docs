...
- **TiDB Cloud Dedicated**
    ...
    - 支持用户自定义控制 [TiDB Cloud Dedicated](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) changefeed 中 `UPDATE` 事件的拆分方式。
        ...
        有关拆分行为的更多信息，请参见 [为非 MySQL sink 拆分主键或唯一键的 `UPDATE` 事件](https://docs.pingcap.com/tidb/stable/ticdc-split-update-behavior/#split-primary-or-unique-key-update-events-for-non-mysql-sinks)。
...
- **TiDB Cloud Essential**
    ...
    - 新创建的 [TiDB Cloud Essential](/tidb-cloud/select-cluster-tier.md#essential) 集群默认启用区域级高可用性，你可以在集群创建过程中根据需要切换为可用区级高可用性。
...
- **TiDB Cloud Starter**
    ...
    - 在 [TiDB Cloud Starter](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 中引入 Auto Embedding（Beta），让你无需额外配置即可轻松将文本转换为向量。该功能可加速在 TiDB Cloud 中开发语义搜索、RAG、重排序和分类等场景，减少集成成本。
        ...
        - **与 AWS Bedrock 的原生集成**：支持 Amazon Titan 和 Cohere 文本嵌入模型，享有免费配额。
...
- **TiDB Cloud Essential**
    ...
    TiDB Cloud Essential 自 2025 年 5 月起在阿里云新加坡区域开启有限公测。本次是 Essential 首次正式纳入发布说明。目前，Essential 在阿里云上的功能集与 Starter 保持一致。

    体验方式：

    - 在 [TiDB Cloud 控制台](https://tidbcloud.com/) 创建集群时选择阿里云作为云服务商，即可看到 Essential 选项。
    - 你也可以通过 [阿里云 Marketplace 上的 TiDB 产品页](https://www.alibabacloud.com/en/marketplace/tidb?_p_lc=1) 访问 Essential。

  下一步计划将扩展阿里云的区域覆盖，并增加对 AWS 的支持。

    如果你在预览期间体验了阿里云上的 Essential，可以通过 Web 控制台反馈，也欢迎加入我们的 [Slack 社区](https://tidbcommunity.slack.com/archives/CH7TTLL7P) 或 [Discord 社区](https://discord.gg/ukhXbn69Nx) 交流。
...
- 增强版 Datadog 和 New Relic 集成现已开放预览。
  ...
  该预览版本现已面向未配置 Datadog 或 New Relic 集成的组织开放。对于已配置 Datadog 或 New Relic 集成的组织，我们将在下月主动联系你，协商合适的迁移方案和时间表。
...
- 优化 [TiDB Cloud Dedicated](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 中 TiKV 的扩缩容流程，提升集群稳定性。
    ...
    当你 [变更 TiKV 节点的 vCPU 和内存规格](/tidb-cloud/scale-tidb-cluster.md#change-vcpu-and-ram) 时，TiDB Cloud 会自动检测集群内部服务是否需要扩容以支持新配置。

    - 若需要扩容，TiDB Cloud 会在操作前提示你确认。
...
- [TiDB Cloud Serverless](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 数据库审计日志（beta）现可按需申请。该功能允许你在日志中记录用户访问详情（如执行的 SQL 语句）。
...
- 重构左侧导航栏，提升整体导航体验。
    ...
    - 左侧导航栏的入口会根据组合框的当前选择动态调整，帮助你聚焦最相关的功能。
...
- [TiDB Cloud Dedicated](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 现已在 Microsoft Azure 上公测。
    ...
    - 在 Azure 上，所有 AWS 和 Google Cloud 支持的核心功能均已支持。
    - 目前 Azure 支持的区域包括 East US 2、日本东部和东南亚，后续将支持更多区域。
...
- TiKV [Standard](/tidb-cloud/size-your-cluster.md#standard-storage) 和 [Performance](/tidb-cloud/size-your-cluster.md#performance-and-plus-storage) 存储定价正式发布。
    ...
    优惠期自 **2025 年 6 月 5 日 00:00 UTC** 起结束，届时价格恢复为标准价。更多 TiDB Cloud Dedicated 价格信息请参见 [TiDB Cloud Dedicated 价格详情](https://www.pingcap.com/tidb-dedicated-pricing-details/#node-cost)。
...
- [TiDB Cloud Dedicated](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群的 changefeed 现已支持将数据流式同步到 [Apache Pulsar](https://pulsar.apache.org)。
    ...
    该功能使你能够将 TiDB Cloud Dedicated 集群与更多下游系统集成，满足更多数据集成需求。使用该功能需确保集群版本为 v7.5.1 或更高。
...
- [TiDB Node Groups](/tidb-cloud/tidb-node-group-overview.md) 功能现已在托管于 AWS 和 Google Cloud 的 [TiDB Cloud Dedicated](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群中正式发布（GA）。
  ...
  更多优势请参见 [技术博客](https://www.pingcap.com/blog/tidb-cloud-node-groups-scaling-workloads-predictable-performance/)。快速上手请参见 [管理 TiDB Node Group](/tidb-cloud/tidb-node-group-management.md)。
...
- 在托管于 AWS 的 [TiDB Cloud Dedicated](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群中引入 [Standard 存储](/tidb-cloud/size-your-cluster.md#standard-storage) 类型的 TiKV 节点。
    ...
    **主要优势：**
    ...
    - **成本效益**：与之前的存储类型相比，在更具竞争力的价格下提供更高性能。
    ...
    **可用性：**
    ...
    Standard 存储的价格与 Basic 存储不同。更多信息请参见 [价格说明](https://www.pingcap.com/tidb-dedicated-pricing-details/)。
...
- 支持为部署在 Google Cloud 的 [TiDB Cloud Dedicated](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群创建 TiDB 节点组，提升资源管理灵活性。
    ...
    更多信息请参见 [TiDB Node Group 概览](/tidb-cloud/tidb-node-group-overview.md)。
...
- 推出 Connected Care，TiDB Cloud 的全新支持服务。
  ...
  更多信息请参见 [Connected Care 概览](/tidb-cloud/connected-care-overview.md)。
...
- [TiDB Cloud Serverless](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 集群现已支持单次导入本地 CSV 文件最大 250 MiB（此前为 50 MiB）。
...
- 支持通过 Private Connect（beta）将 [TiDB Cloud Dedicated](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群连接到 AWS 和 Google Cloud 上的通用 Kafka。
    ...
    Private Connect 利用云服务商的 Private Link 或 Private Service Connect 技术，使 TiDB Cloud VPC 内的 changefeed 能通过私有 IP 连接到客户 VPC 内的 Kafka，就像这些 Kafka 直接部署在 TiDB Cloud VPC 内一样。该功能有助于避免 VPC CIDR 冲突，满足安全合规要求。
...
- Kafka changefeed 新增可配置选项：
    ...
    - 支持使用 Debezium 协议。Debezium 是一种数据库变更捕获工具，将每个捕获的数据库变更转换为事件消息并发送到 Kafka。更多信息请参见 [TiCDC Debezium 协议](https://docs.pingcap.com/tidb/v8.1/ticdc-debezium)。
...
- [TiDB Cloud Serverless](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 集群区域级高可用性（beta）。
    ...
    - 关键 OLTP 组件（如 PD 和 TiKV）在多个可用区冗余部署。
    - 主区故障时自动故障转移，最小化服务中断。
  
  目前该功能仅在 AWS 东京（ap-northeast-1）区域可用，且仅可在集群创建时启用。
