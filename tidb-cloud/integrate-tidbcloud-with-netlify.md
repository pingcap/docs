...
上記のセクションで説明したサンプルアプリは、Netlifyのサーバーレス関数上で動作します。このセクションでは[TiDB Cloud Serverless Driver](/develop/serverless-driver.md)でエッジ関数を使用する方法を示します。 レス エッジ関数はNetlifyが提供する機能で、Netlify CDNのエッジでサーバーレス関数を実行できます。
