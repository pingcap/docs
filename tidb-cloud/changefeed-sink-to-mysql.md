...
**Sink to MySQL**コネクタは、特定のタイムスタンプ以降の増分データのみをTiDB Cloud DedicatedクラスターからMySQLにシンクできます。TiDB Cloud Dedicatedクラスターに既にデータが存在する場合は、 **Sink to MySQL**を有効にする前に、 TiDB Cloud Dedicatedクラスターの既存データをエクスポートしてMySQLにロードすることができます。
