...
4.  オプション：**詳細設定**セクションで、追加設定を構成します。
    ...
    -   **Max Messages per Batch**と**Max Publish Delay**：Pulsarに送信されるイベントメッセージのバッチ処理を指定します。**Max Messages per Batch**数は、バッチあたりの最大メッセージ数を設定し、**Max Publish Delay**は、バッチを送信する前の最大待機時間を設定します。
