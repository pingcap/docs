...
# TiDB v7.5.0 のTiDB CloudTPC-C パフォーマンス テスト レポート {#tidb-cloud-tpc-c-performance-test-report-for-tidb-v7-5-0}

このドキュメントでは、TiDB バージョン v7.5.0 を使用した TiDB 専用クラスターの TPC-C パフォーマンス テストの手順と結果を説明します。このレポートは、TiDB セルフホスト v7.5.0 クラスターのパフォーマンスの参考としても使用できます。

## テストの概要 {#test-overview}

このテストは、オンライン トランザクション処理 (OLTP) シナリオにおける TiDB v7.5.0 の TPC-C パフォーマンスを示すことを目的としています。
