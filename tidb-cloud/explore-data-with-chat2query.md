...
1. Write SQL queries.
    ...
    - If AI is enabled, simply press **⌘ + I** followed by your instructions and press **Enter** to let AI generate SQL queries automatically.
        ...
        For a SQL query generated by Chat2Query, click the **Accept** button to approve and further edit the query as needed. If the query does not meet your requirements, click the **Discard** button to reject it. Alternatively, click **Regenerate** if you wish to receive new query attempts from Chat2Query.
...
In SQL Editor, you can rewrite existing SQL queries to optimize performance, fix errors, or meet other specific requirements using Chat2Query.

1. Select the SQL query you want to rewrite.

2. Use the following keyboard shortcuts based on your operating system to invoke Chat2Query for rewriting:

    - **⌘ + I** on macOS
    - **Ctrl + I** on Windows/Linux

    Press `Enter` after your instructions to let AI handle the rewrite.

3. After invoking Chat2Query, you will be presented with options:

    - **Accept**: Click this button to accept the suggested rewrite and proceed to further edit the query if needed.
    - **Discard**: If the suggested rewrite does not meet your expectations, click this button to discard it.
    - **Regenerate**: Click this button if you want Chat2Query to attempt another rewrite based on your feedback or further instructions.

> **Note:**
> 
> Chat2Query utilizes advanced AI algorithms to suggest optimizations and corrections. However, it's recommended to review the suggestions thoroughly before finalizing the queries.
