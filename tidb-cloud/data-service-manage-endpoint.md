...
5.  デプロイを確定するには、 **Deploy**をクリックしてください。エンドポイントが正常にデプロイされると、 **「エンドポイントがデプロイされました」**というメッセージが表示されます。
