...
1. 进入项目的 [**Data Service**](https://tidbcloud.com/project/data-service) 页面。
...
3. 在对话框中，执行以下操作：
    ...
    4. （可选）**Auto-Deploy Endpoint** 选项（默认禁用）用于控制是否直接部署生成的 endpoint。启用后，将跳过草稿 review 流程，生成的 endpoint 会立即部署，无需进一步人工 review 或审批。
...
5. 检查新生成 endpoint 的名称、SQL 语句、属性和参数。
    ...
    - Endpoint 名称：生成的 endpoint 名称格式为 `/<所选表名>`，request method（如 `GET`、`POST`、`PUT`）显示在 endpoint 名称前。例如，若所选表名为 `sample_table`，所选 operation 为 `POST (Create)`，则生成的 endpoint 显示为 `POST /sample_table`。
        ...
        - 若已存在相同 request method 和 endpoint 名称的 endpoint，TiDB Cloud Data Service 会在生成的 endpoint 名称后追加 `_dump_<随机字母>`。例如，`/sample_table_dump_EUKRfl`。

    - SQL 语句：TiDB Cloud Data Service 会根据表的列定义和所选 endpoint operation 自动为生成的 endpoint 编写 SQL 语句。你可以点击 endpoint 名称，在页面中间区域查看其 SQL 语句。
    - Endpoint 属性：TiDB Cloud Data Service 会根据你的选择自动配置 endpoint 路径、request method、timeout 和 tag。你可以在页面右侧面板查看属性。
    - Endpoint 参数：TiDB Cloud Data Service 会为生成的 endpoint 自动配置参数。你可以在页面右侧面板查看参数。
...
Data Service 提供了一个包含预定义系统 endpoint 的 endpoint 库，你可以直接将其添加到 Data App，减少 endpoint 开发工作量。目前，库中仅包含 `/system/query` endpoint，你只需在预定义的 `sql` 参数中传入 SQL 语句，即可 execute 任意 SQL 语句。
...
3. 要将 `/system/query` endpoint 添加到 Data App，请切换 **Execute Query** 开关为 **Added**。

    > **提示：**
    >
    > 若要从 Data App 移除已添加的预定义 endpoint，请切换 **Execute Query** 开关为 **Removed**。
...
4. 点击 **Save**。
    ...
    添加的系统 endpoint 会显示在 endpoint 列表顶部。

5. 检查新 endpoint 的名称、SQL 语句、属性和参数。

    > **注意：**
    >
    > `/system/query` endpoint 功能强大且通用，但也可能带来破坏性风险。请谨慎使用，并确保 query 安全、合理，以防止意外后果。

    - Endpoint 名称：endpoint 名称和路径为 `/system/query`，request method 为 `POST`。
    - SQL 语句：`/system/query` endpoint 默认不带任何 SQL 语句。你可以在页面中间的 SQL editor 中编写所需 SQL 语句。注意，写在 `/system/query` endpoint SQL editor 中的 SQL 语句会保存在 SQL editor 里，便于后续开发和测试，但不会保存到 endpoint 配置中。
    - Endpoint 属性：在页面右侧的 **Properties** 标签页中可以查看 endpoint 属性。与其他自定义 endpoint 不同，系统 endpoint 仅支持自定义 `timeout` 和 `max rows` 属性。
    - Endpoint 参数：在页面右侧的 **Params** 标签页中可以查看 endpoint 参数。`/system/query` endpoint 的参数为自动配置，无法修改。

## 开发 endpoint

对于每个 endpoint，你可以编写要在 TiDB cluster 上 execute 的 SQL 语句，定义 SQL 语句的参数，或管理名称和版本。
...
在 endpoint 详情页右侧面板，点击 **Properties** 标签页可查看和配置 endpoint 的属性。
...
- **Path**：用户访问 endpoint 时使用的路径。
    ...
    - 路径长度必须小于 64 个字符。
    - request method 与 path 的组合在同一个 Data App 内必须唯一。
    - 路径仅允许字母、数字、下划线（`_`）、斜杠（`/`）和用大括号包裹的参数（如 `{var}`）。每个路径必须以斜杠（`/`）开头，以字母、数字或下划线（`_`）结尾。例如：`/my_endpoint/get_id`。
    - 用 `{ }` 包裹的参数仅允许字母、数字和下划线（`_`），且每个参数必须以字母或下划线（`_`）开头。

    > **注意：**
    >
    > - 路径中每个参数必须单独占一层，不支持前缀或后缀。
    >
    >    合法路径：```/var/{var}``` 和  ```/{var}```
    >
...
- **Timeout(ms)**：endpoint 的超时时间，单位为毫秒。
- **Max Rows**：endpoint 可操作或 return 的最大行数。
- **Tag**：用于标识一组 endpoint 的 tag。
- **Pagination**：仅当 request method 为 `GET` 且 endpoint 的最后一条 SQL 语句为 `SELECT` 操作时可用。启用 **Pagination** 后，你可以在调用 endpoint 时通过 query 参数指定 `page` 和 `page_size` 进行分页，例如：`https://<region>.data.tidbcloud.com/api/v1beta/app/<App ID>/endpoint/my_endpoint/get_id?page=<Page Number>&page_size=<Page Size>`。更多信息，参见 [调用 endpoint](#call-an-endpoint)。

    > **注意：**
    >
    > - 如果 request 未包含 `page` 和 `page_size` 参数，则默认行为是单页 return **Max Rows** 属性指定的最大行数。
    > - `page_size` 必须小于等于 **Max Rows** 属性，否则会 return 错误。

- **Cache Response**：仅当 request method 为 `GET` 时可用。启用 **Cache Response** 后，TiDB Cloud Data Service 可在指定的 TTL（time-to-live）期间 cache 你的 `GET` request 返回的 response。
- **Time-to-live(s)**：仅在启用 **Cache Response** 时可用。用于指定 cache response 的 TTL（秒）。在 TTL 期间，若再次发起相同的 `GET` request，Data Service 会直接 return cache response，而不是再次从目标数据库获取数据，从而提升 query performance。
- **Batch Operation**：仅当 request method 为 `POST` 或 `PUT` 时可见。启用 **Batch Operation** 后，你可以在单个 request 中操作多行数据。例如，在 [调用 endpoint](#call-an-endpoint) 时，通过 curl 命令的 `--data-raw` 选项将数据对象数组放入 `items` 字段，即可在单个 `POST` request 中插入多行数据。
...
1. 选择 cluster。
    ...
    > **注意：**
    >
    > 仅显示已关联到 Data App 的 cluster。要管理已关联的 cluster，参见 [管理已关联数据源](/tidb-cloud/data-service-manage-data-app.md#manage-linked-data-sources)。
...
3. 编写 SQL 语句。
    ...
    在 SQL editor 中，你可以编写如表连接 query、复杂 query、aggregate function 等语句。你也可以直接输入 `--` 加指令，让 AI 自动生成 SQL 语句。

    若要定义参数，可以在 SQL 语句中以变量占位符（如 `${ID}`）插入。例如：`SELECT * FROM table_name WHERE id = ${ID}`。然后，你可以点击右侧面板的 **Params** 标签页，修改参数定义和测试值。更多信息，参见 [参数](#configure-parameters)。

    定义数组参数时，参数会在 SQL 语句中自动转换为用逗号分隔的多个值。为确保 SQL 语句有效，你需要在某些 SQL 语句（如 `IN`）中为参数加括号（`()`）。例如，定义数组参数 `ID`，测试值为 `1,2,3`，则应使用 `SELECT * FROM table_name WHERE id IN (${ID})` 进行 query。
...
4. 运行 SQL 语句。
    ...
    如果 SQL 语句中包含参数，请确保已在右侧 **Params** 标签页设置测试值或默认值，否则会 return 错误。
    ...
    - 若 editor 中仅有一条语句，按 **⌘ + Enter** 或点击 <svg width="1rem" height="1rem" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M6.70001 20.7756C6.01949 20.3926 6.00029 19.5259 6.00034 19.0422L6.00034 12.1205L6 5.33028C6 4.75247 6.00052 3.92317 6.38613 3.44138C6.83044 2.88625 7.62614 2.98501 7.95335 3.05489C8.05144 3.07584 8.14194 3.12086 8.22438 3.17798L19.2865 10.8426C19.2955 10.8489 19.304 10.8549 19.3126 10.8617C19.4069 10.9362 20 11.4314 20 12.1205C20 12.7913 19.438 13.2784 19.3212 13.3725C19.307 13.3839 19.2983 13.3902 19.2831 13.4002C18.8096 13.7133 8.57995 20.4771 8.10002 20.7756C7.60871 21.0812 7.22013 21.0683 6.70001 20.7756Z" fill="currentColor"></path></svg>**Run** 运行。

    - 若 editor 中有多条语句，想顺序运行其中一条或多条，将光标放在目标语句上或用光标选中目标语句行，然后按 **⌘ + Enter** 或点击 **Run**。

    - 若要顺序运行 editor 中所有语句，按 **⇧ + ⌘ + Enter**，或用光标选中所有语句行后点击 **Run**。
    ...
    - 若 editor 中仅有一条语句，按 **Ctrl + Enter** 或点击 <svg width="1rem" height="1rem" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M6.70001 20.7756C6.01949 20.3926 6.00029 19.5259 6.00034 19.0422L6.00034 12.1205L6 5.33028C6 4.75247 6.00052 3.92317 6.38613 3.44138C6.83044 2.88625 7.62614 2.98501 7.95335 3.05489C8.05144 3.07584 8.14194 3.12086 8.22438 3.17798L19.2865 10.8426C19.2955 10.8489 19.304 10.8549 19.3126 10.8617C19.4069 10.9362 20 11.4314 20 12.1205C20 12.7913 19.438 13.2784 19.3212 13.3725C19.307 13.3839 19.2983 13.3902 19.2831 13.4002C18.8096 13.7133 8.57995 20.4771 8.10002 20.7756C7.60871 21.0812 7.22013 21.0683 6.70001 20.7756Z" fill="currentColor"></path></svg>**Run** 运行。

    - 若 editor 中有多条语句，想顺序运行其中一条或多条，将光标放在目标语句上或用光标选中目标语句行，然后按 **Ctrl + Enter** 或点击 **Run**。

    - 若要顺序运行 editor 中所有语句，按 **Shift + Ctrl + Enter**，或用光标选中所有语句行后点击 **Run**。
...
- **Required**：指定参数在 request 中是否必填。对于 path 参数，该配置为必填且不可修改。其他参数默认非必填。
- **Type**：指定参数的数据 type。对于 path 参数，仅支持 `STRING` 和 `INTEGER`。其他参数支持 `STRING`、`NUMBER`、`INTEGER`、`BOOLEAN` 和 `ARRAY`。
...
- **Default Value**：指定参数的默认值。
    ...
    - 确保值可转换为参数的 type，否则 endpoint 会 return 错误。
    - 若未为参数设置测试值，测试 endpoint 时会使用默认值。
- **Location**：参数的位置。该属性不可修改。
    - 对于 path 参数，该属性为 `Path`。
    - 其他参数，若 request method 为 `GET` 或 `DELETE`，该属性为 `Query`；若为 `POST` 或 `PUT`，该属性为 `Body`。

在 **Test Values** 区域，你可以查看和设置测试参数。这些值在测试 endpoint 时作为参数值使用。确保值可转换为参数的 type，否则 endpoint 会 return 错误。
...
1. 进入项目的 [**Data Service**](https://tidbcloud.com/project/data-service) 页面。
2. 在左侧面板，点击目标 Data App 名称以查看其 endpoint。
3. 点击要测试的 endpoint 名称，查看其详情。
4. （可选）若 endpoint 包含参数，需先设置测试值。
...
测试完成后，你可以在页面底部看到 JSON 格式的 response。关于 JSON response 的详细说明，参见 [endpoint 的 response](#response)。
...
4. 若 Data App 启用了 **Review Draft**，会弹出对话框供你 review 修改内容。你可以根据 review 选择是否放弃修改。
5. 点击 **Deploy** 确认部署。部署成功后会提示 **Endpoint has been deployed**。
...
4. 在对话框中，选择你要使用的环境和认证方式，然后复制代码示例。
    ...
    - 环境：根据需要选择 **Test Environment** 或 **Online Environment**。**Online Environment** 仅在 endpoint 部署后可用。
    - 认证方式：选择 **Basic Authentication** 或 **Digest Authentication**。
        - **Basic Authentication** 以 base64 编码文本传输 API key。
        - **Digest Authentication** 以加密方式传输 API key，更加安全。

      与 **Basic Authentication** 相比，**Digest Authentication** 的 curl 代码多了一个 `--digest` 选项。

    以下是启用 **Batch Operation** 并使用 **Digest Authentication** 的 `POST` request curl 代码示例：
    ...
    > - 你也可以不指定 region，直接请求全局域名 `data.tidbcloud.com`。此时，TiDB Cloud Data Service 会在内部将 request 重定向到目标 region，但可能会带来额外延时。如果选择此方式，调用 endpoint 时请确保在 curl 命令中添加 `--location-trusted` 选项。
...
5. 将代码示例粘贴到你的应用中，根据需要修改后运行。
    ...
    - 若 endpoint 的 request method 为 `GET` 且已启用 **Pagination**，你可以通过修改 `page=<Page Number>` 和 `page_size=<Page Size>` 的值进行分页。例如，获取第 2 页且每页 10 条数据，使用 `page=2` 和 `page_size=10`。
    - 若 endpoint 的 request method 为 `POST` 或 `PUT`，请根据要操作的数据行填写 `--data-raw` 选项。
    ...
    - 若 endpoint 包含参数，调用时需指定参数值。

### Response

调用 endpoint 后，你会看到 JSON 格式的 response。更多信息，参见 [Data Service 的 response 和状态码](/tidb-cloud/data-service-response-and-status-code.md)。

## 下线 endpoint

> **注意：**
>
> 如果你已[将 Data App 连接到 GitHub](/tidb-cloud/data-service-manage-github-connection.md)并启用 **Auto Sync & Deployment**，下线该 Data App 的 endpoint 也会删除 GitHub 上该 endpoint 的配置。
