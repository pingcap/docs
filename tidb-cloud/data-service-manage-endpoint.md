...
-   **バッチ操作**: このプロパティは、リクエスト メソッドが`POST`または`PUT`の場合にのみ表示されます。**バッチ操作**を有効にすると、1 つのリクエストで複数の行を操作できます。たとえば、エンドポイントを呼び出すときに、curl コマンドの`--data-raw`オプションにデータ オブジェクトの配列を追加することで、単一の`POST`リクエストに複数行のデータを挿入できます。
...
-   **Enum** : (オプション) パラメーターの有効な値を指定します。パラメーターのタイプが`STRING` 、 `INTEGER` 、または`NUMBER`の場合にのみ使用できます。
