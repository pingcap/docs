...
3.  ダイアログで、次の操作を行います。
    ...
    2.  エンドポイントを生成するには、少なくとも 1 つの HTTP 操作 ( `GET (Retrieve)`など) を選択し`PUT (Update)` `POST (Create)`
        ...
        選択したテーブルに[ベクトルデータ型](/tidb-cloud/vector-search-data-types.md)が含まれている場合、 **「ベクトル検索操作」**オプションを有効にしてベクトル距離関数を選択すると、選択した距離関数に基づいてベクトル距離を自動的に計算するベクトル検索エンドポイントを生成できます。サポートされている[ベクトル距離関数](/tidb-cloud/vector-search-functions-and-operators.md)関数は次のとおりです。
