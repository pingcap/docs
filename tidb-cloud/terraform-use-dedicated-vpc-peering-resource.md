...
summary: 了解如何使用 TiDB Cloud Dedicated VPC peering 资源创建和修改 TiDB Cloud Dedicated VPC peering。
---
...
你可以使用 `tidbcloud_dedicated_vpc_peering` 资源创建 TiDB Cloud Dedicated VPC 对等连接。

以下示例演示如何创建一个 TiDB Cloud Dedicated VPC 对等连接。
...
4. 如果一切看起来正常，输入 `yes` 继续：
    ...
    tidbcloud_dedicated_vpc_peering.example: 正在创建...
    tidbcloud_dedicated_vpc_peering.example: 仍在创建中... [已用时 10 秒]
    ```
...
对于未由 Terraform 管理的 TiDB Cloud Dedicated VPC 对等连接，你可以通过导入来管理。

例如，可以导入未由 Terraform 创建的 VPC 对等连接。

1. 在你的 `.tf` 文件中添加导入块，用于新建的 TiDB Cloud Dedicated VPC 对等连接资源。

    将以下导入块添加到你的 `.tf` 文件中，替换 `example` 为你想要的资源名，替换 `${id}` 为 `cluster_id,vpc_peering_id` 格式：
...
3. 审查并应用生成的配置。
    ...
    ```shell
    tidbcloud_dedicated_vpc_peering.example: 导入中... [id=aws-1934187953894000000,example]
    tidbcloud_dedicated_vpc_peering.example: 导入完成 [id=aws-19341879538940000000,example]

    应用完成！已导入资源：1 个，新增：0 个，变更：0 个，删除：0 个。
    ```
...
```shell
  ...
  tidbcloud_dedicated_vpc_peering.example: 1 秒后销毁完成
