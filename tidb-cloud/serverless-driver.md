...
传统的基于 TCP 的 MySQL 驱动并不适合 serverless 函数，因为它们期望建立长连接、持久的 TCP 连接，这与 serverless 函数的短生命周期特性相矛盾。此外，在 [Vercel Edge Functions](https://vercel.com/docs/functions/edge-functions) 和 [Cloudflare Workers](https://workers.cloudflare.com/) 等边缘环境中，可能缺乏完整的 TCP 支持和完整的 Node.js 兼容性，这些驱动甚至可能无法工作。

[TiDB Cloud serverless driver (Beta)](https://github.com/tidbcloud/serverless-js) 针对 JavaScript，可以让你通过 HTTP 连接到 TiDB Cloud Starter 或 TiDB Cloud Essential 集群，而 HTTP 通常被 serverless 环境所支持。借助该驱动，你可以从边缘环境连接到 TiDB Cloud Starter 或 TiDB Cloud Essential 集群，并在保持与传统基于 TCP 的 MySQL 驱动类似开发体验的同时，减少 TCP 带来的连接开销。
...
| `url`        | string   | N/A           | 数据库的 URL，格式为 `mysql://[username]:[password]@[host]/[database]`，其中 `database` 可省略（如果你打算连接默认数据库）。                                                                                                                                                                                                                                  |
...
| `decoders`   | object   | `{}`          | 一组键值对，允许你为不同的列类型自定义解码过程。在每对中，你可以指定列类型作为 key，并指定相应的函数作为 value。该函数以 TiDB Cloud serverless driver 返回的原始字符串值为参数，并返回解码后的值。                                                                                                                     |
...
当配置了 `url` 后，无需单独配置 `host`、`username`、`password` 和 `database`。以下代码等价：
...
| `decoders`   | object | `{}`              | 一组键值对，允许你为不同的列类型自定义解码过程。在每对中，你可以指定列类型作为 key，并指定相应的函数作为 value。该函数以 TiDB Cloud serverless driver 返回的原始字符串值为参数，并返回解码后的值。如果你在连接级别和 SQL 级别都配置了 `decoders`，则连接级别中 key 不同的键值对会合并到 SQL 级别生效；如果同一个 key（即列类型）在两级都配置了，则以 SQL 级别为准。 |
...
// 你也可以在 SQL 级别配置 decoder 选项，以覆盖连接级别相同 key 的解码器。
conn.execute(`select ...`, [], {
  decoders: {
    // ...
  }
...
支持 DDL 以及以下 SQL 语句：`SELECT`、`SHOW`、`EXPLAIN`、`USE`、`INSERT`、`UPDATE`、`DELETE`、`BEGIN`、`COMMIT`、`ROLLBACK` 和 `SET`。
...
- 对于 TiDB Cloud Essential 集群，计费遵循 [TiDB Cloud Essential 计费](/tidb-cloud/tidb-cloud-billing.md#pricing-for-essential) 模型。
