...
TiDB Cloudは、クライアントとTiDB Cloudクラスタ間のTLS接続において、 [レッツエンクリプト](https://letsencrypt.org/)の証明書を証明機関（CA）として使用します。TiDB Cloud証明書の有効期限が切れると、クラスタの通常の動作や確立されたTLSセキュア接続に影響を与えることなく、自動的にローテーションされます。
