...
在向某个区域添加 VPC 对等请求之前，必须先为该区域设置 CIDR 块并创建一个初始的 TiDB Cloud Dedicated 集群。一旦第一个集群创建完成，TiDB Cloud 会自动创建相关联的 VPC，从而使你能够与应用的 VPC 建立对等连接。
...
4. 如果一切看起来正常，输入 `yes` 继续：
    ...
    tidbcloud_dedicated_network_container.example: 创建完成，耗时4秒
    ```
...
5. 使用 `terraform show` 或 `terraform state show tidbcloud_dedicated_network_container.${资源名}` 命令检查资源状态。前者显示所有资源和数据源的状态。
    ...
    resource "tidbcloud_dedicated_network_container" "example" {
        ...
        project_id           = "1372813089454000000"
        region_display_name  = "首尔 (ap-northeast-2)"
        region_id            = "aws-ap-northeast-2"
        state                = "INACTIVE"
        vpc_id               = null
    }
...
```shell
  ...
  Terraform 将执行以下操作：
    ...
    - resource "tidbcloud_dedicated_network_container" "example" {
        ...
        - region_display_name  = "首尔 (ap-northeast-2)" -> null
