...
1. 在你 [获取 TiDB Cloud Terraform Provider](/tidb-cloud/terraform-get-tidbcloud-provider.md) 时创建的 `main.tf` 文件中，添加如下的 `data` 和 `output` 块：
    ...
    - 使用 `output` 块定义要在输出中显示的数据源信息，并将信息暴露给其他 Terraform 配置使用。
        ...
        `output` 块的作用类似于编程语言中的返回值。更多详情可参考 [Terraform 文档](https://www.terraform.io/language/values/outputs)。
...
2. 创建一个 `cluster.tf` 文件：
    ...
    - 若要获取 TiDB Cloud Serverless 集群的规格信息，请参见 [tidbcloud_serverless_cluster (Resource)](https://registry.terraform.io/providers/tidbcloud/tidbcloud/latest/docs/resources/serverless_cluster)。
...
按如下方式导入未由 Terraform 创建的 TiDB Cloud Serverless 集群：
