...
1. 在你 [获取 TiDB Cloud Terraform Provider](/tidb-cloud/terraform-get-tidbcloud-provider.md) 时创建的 `main.tf` 文件中，添加 `data` 和 `output` 块如下：
    ...
    - 使用 `data` 块定义 TiDB Cloud 的数据源，包括数据源类型和数据源名称。
        ...
        - 对于 `tidbcloud_projects` 数据源，可以使用 `page` 和 `page_size` 属性限制最多检索的项目数量。
...
2. 创建 `cluster.tf` 文件：
    ...
    - 使用 `resource` 块定义 TiDB Cloud 的资源，包括资源类型、资源名称和资源详情。
        ...
        - 资源详情可以根据项目 ID 和 TiDB Cloud Serverless 集群的规格信息进行配置。
...
4. 如果一切看起来都正常，输入 `yes` 继续：
    ...
    tidbcloud_serverless_cluster.example：仍在创建中...[10秒已过]

    应用已完成！资源：1 个已添加，0 个已更改，0 个已删除。
    ```
...
导入未由 Terraform 创建的 TiDB Cloud Serverless 集群如下：

1. 在你的 `.tf` 文件中添加导入块，替换 `example` 为你想要的资源名，`${id}` 替换为集群 ID：
...
2. 生成新的配置文件。
    ...
    不要在上述命令中指定已有的 `.tf` 文件，否则 Terraform 会返回错误。
...
3. 审查并应用生成的配置。
    ...
    应用已完成！资源：导入 1 个，添加 0 个，更改 0 个，删除 0 个。
    ```
