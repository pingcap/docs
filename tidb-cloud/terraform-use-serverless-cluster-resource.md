...
summary: 学习如何使用 TiDB Cloud Serverless 集群资源创建和修改 TiDB Cloud Serverless 集群。
---
...
每个 TiDB 集群都属于一个项目。在创建 TiDB Cloud Serverless 集群之前，你需要获取你想要创建集群的项目ID。如果没有指定 `project_id`，则会使用默认项目。
...
2. 运行 `terraform apply` 命令应用配置。确认提示时输入 `yes`。
    ...
    若要跳过提示，使用 `terraform apply --auto-approve`：
    ...
    你可以应用此计划，将这些新的输出值保存到 Terraform 状态中，而不改变任何实际基础设施。

    计划完成！资源：0 添加，0 更改，0 删除。
...
2. 创建 `cluster.tf` 文件：
    ...
    - 使用 `resource` 块定义 TiDB Cloud 资源，包括资源类型、名称和详细参数。

        - 若要使用 TiDB Cloud Serverless 集群资源，设置资源类型为 `tidbcloud_serverless_cluster`。
        - 资源名称可根据需要定义，例如 `example`。
        - 资源详细参数应根据项目ID和 TiDB Cloud Serverless 集群规格信息配置。
        - 获取集群规格信息请参见 [tidbcloud_serverless_cluster (Resource)](https://registry.terraform.io/providers/tidbcloud/tidbcloud/latest/docs/resources/serverless_cluster)。
...
3. 运行 `terraform apply` 命令。建议不要使用 `terraform apply --auto-approve` 直接应用资源。
    ...
    Terraform 将执行以下操作：
        ...
        + resource "tidbcloud_serverless_cluster" "example" {
            + annotations             = (创建后可知)
            + automated_backup_policy = (创建后可知)
            + cluster_id              = (创建后可知)
            + create_time             = (创建后可知)
            + created_by              = (创建后可知)
            + display_name            = "test-tf"
            + encryption_config       = (创建后可知)
            + endpoints               = (创建后可知)
            + labels                  = (创建后可知)
            + project_id              = "1372813089454000000"
            + region                  = {
                + cloud_provider = (创建后可知)
                + display_name   = (创建后可知)
                + name           = "regions/aws-us-east-1"
                + region_id      = (创建后可知)
            }
...
4. 如果一切看起来正常，输入 `yes` 继续：
    ...
    tidbcloud_serverless_cluster.example: 正在创建...
    tidbcloud_serverless_cluster.example: 创建中... [已用时10秒]

    计划完成！资源：1 添加，0 更改，0 删除。
    ```

5. 使用 `terraform show` 或 `terraform state show tidbcloud_serverless_cluster.${资源名}` 命令检查资源状态。前者显示所有资源和数据源的状态。
...
对于 TiDB Cloud Serverless 集群，你可以用 Terraform 管理资源。可修改的参数包括：

- `display_name`：集群的显示名称
- `spending_limit`：集群的消费限制
- `endpoints.public.disabled`：是否禁用公共端点
- `automated_backup_policy.start_time`：自动备份开始的UTC时间（`HH:mm` 格式）

要修改 TiDB Cloud Serverless 集群，可以修改 `tidbcloud_serverless_cluster` 资源的配置，然后运行 `terraform apply` 应用更改。例如，可以如下修改 `display_name` 和 `spending_limit`：
...
tidbcloud_serverless_cluster.example: 正在刷新状态...
...
Terraform 将执行以下操作：
  ...
  ~ resource "tidbcloud_serverless_cluster" "example" {
      ...
      ~ state                   = "ACTIVE" -> (创建后可知)
      ~ update_time             = "2025-06-16T07:04:57Z" -> (创建后可知)
      ~ version                 = "v7.5.2" -> (创建后可知)
        # 其他未变更属性隐藏
    }
...
tidbcloud_serverless_cluster.example: 正在修改...
tidbcloud_serverless_cluster.example: 修改完成，耗时8秒

计划完成！资源：0 添加，1 更改，0 删除。
```

然后，你可以用 `terraform show` 或 `terraform state show tidbcloud_serverless_cluster.${资源名}` 查看资源状态。前者显示所有资源和数据源的状态。
...
对于未由 Terraform 管理的 TiDB Cloud Serverless 集群，你可以通过导入方式让 Terraform 管理。

导入未由 Terraform 创建的 TiDB Cloud Serverless 集群步骤如下：

1. 在你的 `.tf` 文件中添加导入块。

    添加如下导入块，替换 `example` 为你想要的资源名，`${id}` 替换为集群ID：
...
要删除 TiDB Cloud Serverless 集群，可以删除 `tidbcloud_serverless_cluster` 资源的配置，然后运行 `terraform apply` 来销毁资源：
...
tidbcloud_serverless_cluster.example: 正在销毁...
tidbcloud_serverless_cluster.example: 销毁完成，耗时1秒

计划完成！资源：0 添加，0 更改，1 删除。
```

现在，如果你运行 `terraform show`，将不会显示任何托管的资源，因为资源已被清除：
