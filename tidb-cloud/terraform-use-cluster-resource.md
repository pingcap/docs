...
1. 在你 [获取 TiDB Cloud Terraform Provider](/tidb-cloud/terraform-get-tidbcloud-provider.md) 时创建的 `main.tf` 文件中，添加如下 `data` 和 `output` 块：
   ...
   - 使用 `output` 块定义要在输出中显示的数据源信息，并将信息暴露给其他 Terraform 配置使用。
      ...
      `output` 块的作用类似于编程语言中的返回值。更多细节请参见 [Terraform 文档](https://www.terraform.io/language/values/outputs)。
...
例如，你可以导入一个不是通过 Terraform 创建的集群，或导入一个 [通过 restore 资源创建的集群](/tidb-cloud/terraform-use-restore-resource.md#create-a-restore-task)。
