...
> **提示：**
>
> TiDB Cloud 为 {{{ .starter }}} 实例提供了 Chat2Query API。启用后，TiDB Cloud 会自动创建一个名为 **Chat2Query** 的系统 Data App，并在 Data Service 中创建一个 Chat2Data 端点。你可以调用该端点，通过提供指令让 AI 生成并 execute SQL statement。
>
...
在创建 Data App 之前，请确保你已经创建了一个托管在 AWS 上的 [{{{ .starter }}}](/tidb-cloud/select-cluster-tier.md) 实例。如果还没有，请按照 [创建 {{{ .starter }}} 或 Essential 实例](/tidb-cloud/create-tidb-cluster-serverless.md) 的步骤进行创建。

> **注意：**
>
> Data Service 仅适用于托管在 AWS 上的 [{{{ .starter }}}](/tidb-cloud/select-cluster-tier.md) 实例。如需在 TiDB Cloud Dedicated 集群中使用 Data Service，请联系 [TiDB Cloud Support](/tidb-cloud/tidb-cloud-support.md)。
...
3. 在对话框中，如有需要可修改 App 名称，选择你希望 Data App access 的 {{{ .starter }}} 实例，然后点击 **Create**。

    创建过程只需几秒钟。

    > **注意：**
    >
    > 如果当前项目下没有 {{{ .starter }}} 实例，可以在 **Link Data Sources** 下拉列表中点击 **Create New Resource** 先创建一个资源。
...
3. 在 **Create Data App** 对话框中，输入名称、描述，并选择你希望 Data App access 的 {{{ .starter }}} 实例。
...
- **Endpoint URL**：（只读）URL 会根据对应 {{{ .starter }}} 实例所在 region、Data App 的 service URL 以及端点 path 自动生成。例如，如果端点 path 为 `/my_endpoint/get_id`，则端点 URL 为 `https://<region>.data.tidbcloud.com/api/v1beta/app/<App ID>/endpoint/my_endpoint/get_id`。
...
1. 选择 {{{ .starter }}} 实例。
    ...
    > **注意：**
    >
    > 只有已关联到 Data App 的 {{{ .starter }}} 实例会显示在下拉列表中。要管理已关联的 {{{ .starter }}} 实例，参见 [管理已关联数据源](/tidb-cloud/data-service-manage-data-app.md#manage-linked-data-sources)。

    在 SQL editor 上方，从下拉列表中选择你希望 execute SQL statement 的 {{{ .starter }}} 实例。然后，你可以在右侧面板的 **Schema** 标签页查看该 {{{ .starter }}} 实例的所有数据库。
...
3. 在 **Create API Key** 对话框中，执行以下操作：
    ...
    2. 选择 API key 的角色。
        ...
        角色用于控制 API key 是否可以对 Data App 关联的 {{{ .starter }}} 实例进行 read 或写操作。你可以选择 `ReadOnly` 或 `ReadAndWrite` 角色：
...
2. 在对话框中，选择你希望用于调用端点的 {{{ .starter }}} 实例和数据库，然后复制代码示例。
    ...
    > **注意：**
    >
    > - 通过请求 region 域名 `<region>.data.tidbcloud.com`，你可以直接 access {{{ .starter }}} 实例所在 region 的端点。
