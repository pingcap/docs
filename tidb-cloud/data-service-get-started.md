...
创建一个示例 Data App 是体验 Data Service 的最佳方式。如果你的项目还没有任何 Data App，可以按照 **Data Service** 页面上的引导创建一个示例 Data App，并用它探索 Data Service 的功能。
...
- **Request Method**：端点的 HTTP 方法。你可以用 `GET` 查询数据，用 `POST` 创建或插入数据，用 `PUT` 更新或修改数据，用 `DELETE` 删除数据。
