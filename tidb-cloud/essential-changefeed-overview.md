...
- Changefeeds は、 `RENAME TABLE`のように、単一の`RENAME TABLE t1 TO t3, t2 TO t4` } ステートメントで複数のテーブルの名前を変更する DDL文をサポートしていません。この文を実行すると、Changefeed のデータレプリケーションが永続的に中断されます。
