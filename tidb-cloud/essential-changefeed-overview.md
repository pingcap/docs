...
- Changefeeds は、 `RENAME TABLE t1 TO t3, t2 TO t4`のように、単一の`RENAME TABLE`ステートメントで複数のテーブルの名前を変更する DDL ステートメントをサポートしていません。このステートメントを実行すると、Changefeed のデータレプリケーションが永続的に中断されます。
