...
| ---------|---------|
| `data_source/cluster.json`     | When updating this file, make sure that you have access to the linked clusters. You can get the cluster ID from the cluster URL. For example, if the cluster URL is `https://{{{.console-url}}}/clusters/1234567891234567890/overview`, the cluster ID is `1234567891234567890`. |
...
2. On the [**Data Service**](https://{{{.console-url}}}/project/data-service) page of your project, [create a new Data App](/tidb-cloud/data-service-manage-data-app.md#create-a-data-app) without connecting to GitHub.
