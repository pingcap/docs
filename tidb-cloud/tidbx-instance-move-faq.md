...
summary: 了解为什么 TiDB Cloud 会提示你移动或转换你的 {{{ .starter }}} 和 Essential 资源、项目迁移期间会发生哪些变化，以及后续可能需要执行哪些操作。
---
...
## 为什么 TiDB Cloud 控制台会提示我移动我的 {{{ .starter }}} 和 Essential 实例？ {#why-does-the-tidb-cloud-console-prompt-me-to-move-my-starter-and-essential-instances}

在 2026 年 4 月 15 日之前，TiDB Cloud 使用单一的 **TiDB dedicated project** 类型来管理所有 TiDB Cloud 资源。此类项目可以同时包含 {{{ .dedicated }}} 集群和 TiDB X 实例。不过，在一个项目中混合不同类型的资源会增加管理复杂性，原因如下：

- TiDB dedicated project 最初是为 {{{ .dedicated }}} 集群设计的。
- TiDB X 实例和 {{{ .dedicated }}} 集群具有不同的行为方式和管理模型。

从 2026 年 4 月 15 日起，TiDB Cloud 引入了独立的项目类型，以便清晰地区分不同的资源类型。现在，每种项目类型仅承载其对应的资源类型：
...
TiDB X project 对 TiDB X 实例来说是轻量且可选的，而 dedicated project 对 {{{ .dedicated }}} 集群来说是必需的。将这些资源分开可以确保更一致的用户体验，并消除关于哪些项目能力适用于哪些资源的混淆。

因此，dedicated project 将不再能够包含 TiDB X 实例。如果你的组织在 dedicated project 中已有 {{{ .starter }}} 或 Essential 资源，TiDB Cloud 会提示你将它们移动到 TiDB X project 中，以符合新的资源模型。
...
- **TiDB X project**：此项目类型仅用于 TiDB X 实例。
    ...
    - 你可以在不同的 TiDB X project 之间移动 TiDB X 实例，或将其移回组织级别。

- **TiDB X virtual project**：此项目是虚拟的，不提供任何管理能力。

    - 它作为不属于任何项目的 TiDB X 实例的逻辑容器，使这些实例能够通过使用项目 ID 的方式经由 TiDB Cloud API 访问。
...
- 如果你的项目仅包含 {{{ .starter }}} 和 Essential 实例，TiDB Cloud 会在 2026 年 4 月 15 日自动将该项目转换为 TiDB X project。无需执行其他操作。
- 如果你的项目同时包含 {{{ .dedicated }}} 集群以及 {{{ .starter }}} 或 Essential 实例，TiDB Cloud 控制台会提示你点击顶部横幅中的 **Move & Unlock**，将 {{{ .starter }}} 和 Essential 实例移动到新的 TiDB X project。
...
迁移完成后，你可以通过 TiDB X project（或在组织级别）管理你的 TiDB X 实例，并继续通过 dedicated project 管理你的 {{{ .dedicated }}} 集群。

## 迁移后需要执行哪些操作？ {#what-actions-are-required-after-migration}

如果你的 {{{ .starter }}} 或 Essential 实例被移动到了新的 TiDB X project，请检查所有依赖原项目 ID 或原项目级配置的内容，例如：
...
项目级 RBAC 权限会复制到新项目中，但迁移后你仍需检查访问配置，以确保用户和工作流仍能按预期运行。

如果你使用 TiDB Cloud API 管理实例，请参见 [Project API Migration Guide for {{{ .starter }}} and Essential](/tidb-cloud/tidbx-starter-essential-project-api-migration-guide.md) 以更新你的 API 调用。

## 我可以在哪里获得帮助？ {#where-can-i-get-help}

如果你不确定你的自动化、集成或 Data Service 设置是否依赖原项目 ID，请在开始迁移之前通过 [support@pingcap.com](mailto:support@pingcap.com) 联系 TiDB Cloud 支持团队。