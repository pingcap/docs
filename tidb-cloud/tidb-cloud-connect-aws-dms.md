...
You are expected to have a TiDB Cloud account and a TiDB Cloud cluster. If not, refer to [Create a TiDB Cloud cluster](/tidb-cloud/create-tidb-cluster-serverless.md).
...
For TiDB Cloud, your clients can connect to clusters via public endpoint or private endpoint.

- To [connect to a TiDB Cloud via public endpoint](/tidb-cloud/connect-via-standard-connection-serverless.md), do one of the following to make sure that the DMS replication instance can access the internet.
...
5. For the **Access to endpoint database** field, select the **Provide access information manually** checkbox and fill in cluster information as follows:
    ...
    - **Server name**: `HOST` of TiDB Cloud cluster.
    - **Port**: `PORT` of TiDB Cloud cluster.
    - **User name**: User of TiDB Cloud cluster for migration. Make sure it meets DMS requirements.
    - **Password**: Password of the TiDB Cloud cluster user.
    ...
    - (Optional) **CA certificate**: Use the [ISRG Root X1 certificate](https://letsencrypt.org/certs/isrgrootx1.pem). For more information, see [TLS Connections to TiDB Cloud](/tidb-cloud/secure-connections-to-serverless-clusters.md).
