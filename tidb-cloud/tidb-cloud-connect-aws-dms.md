...
- To connect to a {{{ .starter }}} or {{{ .essential }}} cluster via private endpoint, refer to the following [Connect to {{{ .starter }}} via AWS PrivateLink](/tidb-cloud/set-up-private-endpoint-connections-serverless.md) to set up a private endpoint first and deploy the replication instance in private subnets.
