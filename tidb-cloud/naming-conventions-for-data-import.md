...
インポートウィザードの**Destination Mapping**ステップで、 **TiDB ファイル命名規則を使用して自動マッピングを行う**の選択を解除し、 **Source** 、 **Target Database** 、 **Target Table**の各フィールドに入力します。**Source**フィールドには`*`および`?`ワイルドカードをサポートするファイル名パターンを指定できます。
