...
TiDB Cloud 支持通过 [AWS PrivateLink](https://aws.amazon.com/privatelink/?privatelink-blogs.sort-by=item.additionalFields.createdDate&privatelink-blogs.sort-order=desc) 实现对托管在 AWS VPC 中的 TiDB Cloud 服务的高度安全且单向的访问，就像该服务部署在你自己的 VPC 中一样。你的 VPC 中会暴露一个私有终端节点，你可以通过该终端节点并具备相应权限后连接到 TiDB Cloud 服务。
...
请确保在 AWS VPC 设置中已启用 DNS 主机名和 DNS 解析。在 [AWS Management Console](https://console.aws.amazon.com/) 中创建 VPC 时，这些选项默认是关闭的。
...
如需使用 AWS Management Console 创建 VPC 接口终端节点，请执行以下步骤：
...
1. 要获取 **VPC ID** 和 **Subnet ID**，请前往 AWS Management Console，在相关区域查找。确保你填写了在 [step 1](#step-1-choose-a-tidb-cluster) 中获取的 **Availability Zone ID**。
...
你可能需要在 AWS Management Console 中为 VPC 终端节点正确设置安全组。进入 **VPC** > **Endpoints**，右键点击你的 VPC 终端节点，选择合适的 **Manage security groups**。在你的 VPC 内选择允许 EC2 实例在 4000 端口或自定义端口入站访问的安全组。
