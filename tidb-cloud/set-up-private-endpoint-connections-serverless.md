...
> **Note:**
>
> 現在、エンドポイント占有モデルは、一部のAWSリージョンにおいて、2026年7月1日以降に作成された {{{ .essential }}} インスタンスでのみ利用できます。お使いのインスタンスで利用できない場合は、代わりに [endpoint shared model](#set-up-a-private-endpoint-with-aws-endpoint-shared-model) を使用できます。
