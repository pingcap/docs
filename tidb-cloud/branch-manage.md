...
> **Note:**
>
> You can only create branches for {{{ .starter }}} instances that are created after July 5, 2023. See [Limitations and quotas](/tidb-cloud/branch-overview.md#limitations-and-quotas) for more limitations.
...
4. In the **Create Branch** dialog, configure the following options:
    ...
    - **Parent branch**: select the original {{{ .starter }}} instance or an existing branch. `main` represents the current instance.
...
To view branches for your {{{ .starter }}} instance, perform the following steps:

1. In the [TiDB Cloud console](https://tidbcloud.com/), navigate to the [**My TiDB**](https://tidbcloud.com/tidbs) page, and then click the name of your target {{{ .starter }}} instance to go to its overview page.
2. Click **Branches** in the left navigation pane.

    The branch list of the {{{ .starter }}} instance is displayed in the right pane.
...
Alternatively, you can get the connection string from the overview page of your {{{ .starter }}} instance:
