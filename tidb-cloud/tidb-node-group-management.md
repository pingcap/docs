...
如果你创建了 TiDB 节点组，但仍然使用默认组的端点连接集群，则该 TiDB 节点组中的节点不会承担任何负载，造成资源浪费。你需要为新 TiDB 节点组的节点创建新的连接。参见 [连接到 TiDB 节点组](#connect-to-a-tidb-node-group)。
...
6. 点击 **Create Private Endpoint Connection**，为该节点组创建新的私有端点连接。
     ...
     - 如果集群部署在 AWS，请参考 [通过 AWS PrivateLink 连接 TiDB Cloud Dedicated 集群](/tidb-cloud/set-up-private-endpoint-connections.md)。
     - 如果集群部署在 Google Cloud，请参考 [通过 Google Cloud Private Service Connect 连接 TiDB Cloud Dedicated 集群](/tidb-cloud/set-up-private-endpoint-connections-on-google-cloud.md)。
...
1. 按照 [通过 VPC Peering 连接 TiDB Cloud Dedicated](/tidb-cloud/set-up-vpc-peering-connections.md) 的说明为该集群创建 VPC Peering。
...
> **Note**:
>
> 当你删除 TiDB 节点组时，其节点及网络配置也会被一并移除，包括私有端点连接和公网访问的 IP 列表。
