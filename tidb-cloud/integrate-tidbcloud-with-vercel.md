...
- [创建一个 starter 或 essential 集群](/tidb-cloud/create-tidb-cluster-serverless.md)
    ...
    > **注意：**
    >
    > TiDB Cloud Vercel 集成支持创建 starter 和 essential 集群。你也可以在集成过程中创建。
...
> **注意：**
>
> 此方法仅适用于 starter 和 essential 集群。如果你想连接 TiDB Cloud 专属集群，请使用[手动方法](#connect-via-manually-setting-environment-variables)。
...
5. 在集成页面，按以下步骤操作：
    ...
    3. 选择 **Cluster** 作为连接类型。
    4. 选择目标 TiDB Cloud 集群。如果 **Cluster** 下拉列表为空，或你想选择新的 starter 或 essential 集群，可点击列表中的 **+ Create Cluster** 创建。
...
> **注意：**
>
> 在 TiDB Cloud 的每个组织中，默认最多可为 starter 集群创建 5 个分支。为避免超出限制，你可以删除不再需要的集群分支。更多信息请参见 [管理 TiDB Cloud 分支](/tidb-cloud/branch-manage.md)。
...
这里以 Prisma 应用和 starter 集群为例，以下是 starter 集群在 Prisma schema 文件中的 datasource 设置：
