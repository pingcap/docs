...
パーミッションエラーを解決するには、[**デフォルトの暗号化**]ページの右上隅にある[<strong>編集</strong>]をクリックし、AWSKMSキーを[AWSKMSキーから選択]または[AWSKMSキーARNを入力]に変更するか、サーバー側の暗号化を変更します「AWSS3マネージドキー（SSE-S3）」へのメソッド。このメソッドに加えて、新しいバケットを作成して、カスタムマネージドキーまたはSSE-S3暗号化メソッドを使用することもできます。
