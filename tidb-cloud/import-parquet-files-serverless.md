...
> - TiDB Cloud 仅支持将 Parquet 文件导入到空表中。若需将数据导入到已存在且包含数据的表中，你可以按照本文档将数据导入到一个临时空表，然后使用 `INSERT SELECT` 语句将数据复制到目标已存在的表中。
...
2. 按如下方式命名 Parquet 文件：
    ...
    - 如果一个表的数据被拆分到多个 Parquet 文件中，应为这些 Parquet 文件添加数字后缀。例如，`${db_name}.${table_name}.000001.parquet` 和 `${db_name}.${table_name}.000002.parquet`。数字后缀可以不连续，但必须为正序。你还需要在数字前补零，以保证所有后缀长度一致。

    > **注意：**
    >
    > 如果在某些情况下你无法按照上述规则修改 Parquet 文件名（例如，这些 Parquet 文件链接也被你的其他程序使用），你可以保持文件名不变，并在 [第 4 步](#step-4-import-parquet-files) 的 **Mapping Settings** 中将源数据导入到单一目标表。
