...
このドキュメントでは、TiDB Cloud のデータ移行機能を使用して、Amazon Aurora MySQL、Amazon RDS、Azure Database for MySQL - Flexible Server、Google Cloud SQL for MySQL、またはSelf-Managed MySQL インスタンスから<CustomContent plan="dedicated">TiDB Cloud Dedicated</CustomContent> 、 <CustomContent plan="essential">TiDB Cloud Essential</CustomContent> <CustomContent plan="premium">TiDB Cloud Premium</CustomContent>[TiDB Cloudコンソール](https://tidbcloud.com/)Cloud Premium へ MySQL データベースを移行する手順を説明します。
