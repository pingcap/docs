...
> When you use physical mode and the migration job has started, you cannot enable PITR (Point-in-time Recovery) or have any changefeed on the cluster. Otherwise the migration job will be stuck. If you need to enable PITR or have any changefeed, use logical mode instead to migrate data.
