...
出于测试目的，你可以使用 <CustomContent plan="dedicated">{{{ .dedicated }}} 集群</CustomContent><CustomContent plan="essential">{{{ .essential }}} 实例</CustomContent><CustomContent plan="premium">{{{ .premium }}} 实例</CustomContent>的 `root` 账户。

对于生产工作负载，建议在目标 <CustomContent plan="dedicated">{{{ .dedicated }}} 集群</CustomContent><CustomContent plan="essential">{{{ .essential }}} 实例</CustomContent><CustomContent plan="premium">{{{ .premium }}} 实例</CustomContent>中为复制创建一个专用用户，并且仅授予必要的权限：
...
例如，你可以在目标 <CustomContent plan="dedicated">{{{ .dedicated }}} 集群</CustomContent><CustomContent plan="essential">{{{ .essential }}} 实例</CustomContent><CustomContent plan="premium">{{{ .premium }}} 实例</CustomContent>中执行以下 `GRANT` 语句来授予相应权限：
...
3. 填写目标连接信息。
    ...
    - **User Name**：输入 TiDB Cloud 目标 <CustomContent plan="dedicated">TiDB Cloud Dedicated 集群</CustomContent><CustomContent plan="essential">TiDB Cloud Essential 实例</CustomContent><CustomContent plan="premium">{{{ .premium }}} 实例</CustomContent>的用户名。
