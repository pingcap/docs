...
进入目标 TiDB 集群的集群总览页面。在左侧导航栏点击 **Data** > **Changefeed**，点击 **Create Changefeed**，并选择 **Amazon S3** 或 **GCS** 作为目标端。配置流程会根据你选择的目标端有所不同。
...
5. 填写以下信息以授予 bucket 访问权限，然后点击 **Save**。
    ...
    > **注意：**
    >
    > 若要移除 TiDB Cloud 的访问权限，只需移除你已授予的访问权限即可。
...
4. 在 **Data Format** 区域，选择 **CSV** 或 **Canal-JSON** 格式。
    ...
    Canal-JSON 是一种纯文本 JSON 格式。配置时填写以下字段：
