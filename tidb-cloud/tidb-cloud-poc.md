...
另外，你也可以[创建 TiDB Cloud Serverless](/tidb-cloud/tidb-cloud-quickstart.md#step-1-create-a-tidb-cluster) 来快速体验 TiDB Cloud，进行初步评估。请注意，TiDB Cloud Serverless 有一些[特殊条款和条件](/tidb-cloud/serverless-limitations.md)。
...
由于 PoC 额度有限，为了最大化额度价值，建议你创建 [TiDB Cloud Serverless 集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 进行兼容性测试和初步分析。
...
现在你已经创建了环境，适配了数据库结构，并导入了数据。接下来可以测试你的业务负载。
...
- 备份
    ...
    为避免厂商锁定，你可以使用每日全量备份将数据迁移到新集群，并使用 [Dumpling](https://docs.pingcap.com/tidb/stable/dumpling-overview) 导出数据。更多信息参见 [备份与恢复 TiDB Cloud Dedicated 数据](/tidb-cloud/backup-and-restore.md#turn-on-auto-backup) 及 [备份与恢复 TiDB Cloud Serverless 或 Essential 数据](/tidb-cloud/backup-and-restore-serverless.md)。
...
PoC 申请通过后，你的账户会收到额度。一般来说，这些额度足以支持 14 天的 PoC。额度按节点类型和节点数量按小时计费。更多信息参见 [TiDB Cloud 计费](/tidb-cloud/tidb-cloud-billing.md#credits)。
...
你也可以在 TiDB Cloud 控制台左上角通过下拉框切换到目标组织，点击左侧导航栏的 **Billing**，然后点击 **Credits** 标签页查看额度信息。

为节省额度，请移除不再使用的集群。目前无法停止集群。你需要确保在移除集群前备份数据，以便后续恢复 PoC。

如果 PoC 结束后仍有未用完的额度，只要额度未过期，你可以继续用来支付 TiDB 集群费用。
