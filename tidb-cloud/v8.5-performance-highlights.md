...
- **Leader write optimization**: allows the leader to early apply not persisted but committed raft logs, reducing IO jitter's effect on write latency for the leader peer.
...
Observed from the preceding test setup, failovers are now improved in multiple IO delay scenarios, and P99/999 latency during impacts is reduced by up to 98%.
