...
The following test results show the performance with and without IO latency jitter improvement:
...
Due to the inherent probability issue of physical disk damage, the problem of cloud disk jitter seems unavoidable. To mitigate the impact of jitter, TiKV has introduced a [slow node detection mechanism](https://docs.pingcap.com/tidb/v8.5/pd-scheduling-best-practices#troubleshoot-tikv-node) to reduce the effects of cloud disk jitter, utilizing the `[evict-slow-store-scheduler](https://docs.pingcap.com/tidb/v8.5/pd-control#scheduler-show--add--remove--pause--resume--config--describe)` to detect and manage slow nodes.

The severity of disk jitter might also be highly related to the user's workload profile. For latency-sensitive scenarios, designing applications in conjunction with TiDB's features can further mitigate the impact of IO jitter on applications. For example, in read-heavy and latency-sensitive scenarios, setting [tikv_client_read_timeout](/system-variables.md#tikv_client_read_timeout-new-in-v740) according to latency requirements and leveraging \`stale/follower reads\` can enable faster failover retries to other replica peers for KV requests raised by TiDB. This helps reduce the impact of IO jitter on a single TiKV node on query latency. The effectiveness of the feature in this context is influenced by the user load profile, which should be evaluated prior to determining its usage.

Additionally, cloud users can reduce the probability of jitter by opting for cloud disks with advanced performance.
