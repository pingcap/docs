...
1.  監査ログを有効にする TiDB クラスターのTiDB Cloudアカウント ID と外部 ID を取得します。
    ...
    2.  ターゲット クラスターの名前をクリックして概要ページに移動し、左側のナビゲーションペインで**[設定]** &gt; **DB Audit Logging**をクリックします。
...
TiDB Cloudコンソールで、 Google Cloud サービス アカウント ID を取得した**[データベース監査ログストレージ設定]**ダイアログボックスに戻り、次の手順を実行します。
