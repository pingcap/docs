...
監査ログ機能は**disabled by default**になっています。クラスターを監査するには、まず監査ログを有効にし、次に監査フィルタルールを指定する必要があります。
...
3.  **「IAM」** &gt; **「アクセス管理」** &gt; **「ロール」**に移動し、前に記録したTiDB Cloudアカウント ID と外部 ID に対応する信頼エンティティを持つロールがすでに存在するかどうかを確認します。
    ...
    -   そうでない場合は、 **「ロールの作成」**をクリックし、信頼エンティティタイプとして**Another AWS account**を選択し、 **「アカウント**ID」フィールドにTiDB CloudのアカウントIDを入力します。次に、 **Require External ID**オプションを選択し、 **「外部ID」**フィールドにTiDB Cloudの外部IDを入力します。
...
3.  ターゲット コンテナーの SAS トークンを生成します。
    ...
    6.  **[SAS トークンと URL の生成] を**クリックし、表示される**Blob SAS token**を後で使用するためにコピーします。
...
1.  **DB Audit Logging**ページで、 **Audit Filters**セクションの**「フィルタ ルールの追加」**をクリックして、監査フィルタ ルールを追加します。
