...
4.  **データベース監査ログ storage 設定**ダイアログで、 [ステップ2. Azure BLOBアクセスを構成する](#step-2-configure-azure-blob-storage-access)から取得した BLOB URL と SAS トークンを指定します。
...
| `SQL_TEXT`       | 実行された SQL 文。監査ログのリダクションが有効な場合、リダクトされた SQL 文が記録されます。     |
| `EXECUTE_PARAMS` | `EXECUTE` 文のパラメーター。イベントクラスに `EXECUTE` が含まれ、かつリダクションが無効な場合にのみ記録されます。 |
