...
summary: 了解如何对 TiDB Cloud 中的集群进行审计。
---
...
> **Note:**
>
> 目前，数据库审计日志功能仅在请求后提供。若要请求此功能，请点击 [TiDB Cloud 控制台](https://tidbcloud.com) 右下角的 **?**，然后点击 **Request Support**。接着，在 **Description** 字段中填写“Apply for database audit logging”，并点击 **Submit**。
...
> **Note:**
>
> 对于部署在 AWS 上的 TiDB 集群，你可以选择在启用数据库审计日志时将审计日志文件存储在 TiDB Cloud 中。此功能目前仅在请求后提供。若要请求此功能，请点击 [TiDB Cloud 控制台](https://tidbcloud.com) 右下角的 **?**，然后点击 **Request Support**。接着，在 **Description** 字段中填写“Apply to store audit log files in TiDB Cloud”，并点击 **Submit**。
...
指定你公司拥有的 AWS 账户中的一个 Amazon S3 存储桶，作为 TiDB Cloud 写入审计日志的目标。

> 注意：
>
> 不要在 S3 存储桶上启用对象锁定。启用对象锁定会阻止 TiDB Cloud 将审计日志文件推送到 S3。

更多信息请参见 [Creating a bucket](https://docs.aws.amazon.com/AmazonS3/latest/userguide/create-bucket-overview.html) in the AWS User Guide。
...
2. 在 AWS 管理控制台中，进入 **IAM** > **Access Management** > **Policies**，检查是否存在具有 `s3:PutObject` 写权限的存储桶策略。
    ...
    - 如果不存在，进入 **IAM** > **Access Management** > **Policies** > **Create Policy**，根据以下策略模板定义存储桶策略。
        ...
        其中，`<Your S3 bucket ARN>` 是你要写入审计日志文件的 S3 存储桶的 Amazon 资源名称（ARN）。你可以在存储桶的 **Properties** 标签页中的 **Bucket Overview** 区域获取 ARN 值。在 `"Resource"` 字段中，需要在 ARN 后添加 `/*`。例如，如果 ARN 是 `arn:aws:s3:::tidb-cloud-test`，则配置 `"Resource"` 字段为 `"arn:aws:s3:::tidb-cloud-test/*"`。

3. 进入 **IAM** > **Access Management** > **Roles**，检查是否已存在信任实体对应之前记录的 TiDB Cloud 账户 ID 和 External ID 的角色。

    - 如果存在，记录该角色以备后用。
    - 如果不存在，点击 **Create role**，选择 **Another AWS account** 作为信任实体类型，然后在 **Account ID** 字段中输入 TiDB Cloud 账户 ID。勾选 **Require External ID**，并在 **External ID** 字段中填写 TiDB Cloud External ID。

4. 在 **IAM** > **Access Management** > **Roles** 中，点击上一步中创建或找到的角色名，进入 **Summary** 页面，执行以下操作：

    1. 在 **Permissions** 标签页，检查是否已附加具有 `s3:PutObject` 写权限的策略。如果没有，选择 **Attach Policies**，搜索所需策略，然后点击 **Attach Policy**。
...
3. 在 **Role ARN** 字段中，填写在 [步骤 2. 配置 Amazon S3 访问](#step-2-configure-amazon-s3-access) 中复制的 Role ARN。
...
> **Note:**
>
> - 启用审计日志后，如果你对存储桶 URI、位置或 ARN 进行了任何新更改，必须再次点击 **Test Connection** 来验证 TiDB Cloud 是否能连接到存储桶。然后，点击 **Enable** 以应用更改。
...
指定你公司拥有的 Google Cloud 账户中的一个 Google Cloud Storage（GCS）存储桶，作为 TiDB Cloud 写入审计日志的目标。

更多信息请参见 [Creating storage buckets](https://cloud.google.com/storage/docs/creating-buckets) in the Google Cloud Storage documentation。
...
1. 获取你想启用审计日志的 TiDB 集群的 Google Cloud Service Account ID。
    ...
    2. 点击目标集群的名称，进入其概览页面，然后在左侧导航栏点击 **Settings** > **DB Audit Logging**。
    3. 在 **DB Audit Logging** 页面，点击右上角的 **Enable**。
    4. 在 **Enable Database Audit Logging** 对话框中，找到 **Google Cloud Server Account ID** 部分，记录 **Service Account ID** 以备后续使用。

2. 在 Google Cloud 控制台中，进入 **IAM & Admin** > **Roles**，检查是否存在具有以下写权限的角色：
...
1. 在 **Bucket URI** 字段中，输入你的 GCS 存储桶的完整名称。
...
> **Note:**
>
> - 启用审计日志后，如果你对存储桶 URI 或位置进行了任何新更改，必须再次点击 **Test Connection** 来验证 TiDB Cloud 是否能连接到存储桶。然后，点击 **Enable** 以应用更改。
...
更多信息请参见 [Create an Azure storage account](https://learn.microsoft.com/en-us/azure/storage/common/storage-account-create?tabs=azure-portal) in Azure documentation。
...
1. 在 [Azure 门户](https://portal.azure.com/) 中，创建用于存储数据库审计日志的容器。
    ...
    1. 在 Azure 门户左侧导航栏中，点击 **Storage Accounts**，然后点击用于存储数据库审计日志的存储账户。
        ...
        > **Tip:**
        >
        > 如果左侧导航栏隐藏，点击左上角的菜单按钮切换其显示。

    2. 在所选存储账户的导航栏中，点击 **Data storage > Containers**，然后点击 **+ Container** 打开 **New container** 面板。

    3. 在 **New container** 面板中，为你的新容器输入名称，设置匿名访问级别（建议选择 **Private**，即无匿名访问），然后点击 **Create**。几秒钟后，新容器会被创建并显示在容器列表中。
...
3. 生成目标容器的 SAS 令牌。
    ...
    4. 在 **Start** 和 **Expiry** 字段中，指定 SAS 令牌的有效期。
        ...
        > - 确保在 SAS 令牌到期前重新生成并更新，以确保审计日志的持续可用。
...
2. 点击目标集群的名称，进入其概览页面，然后在左侧导航栏点击 **Settings** > **DB Audit Logging**。
3. 在 **DB Audit Logging** 页面，点击右上角的 **Enable**。
4. 在 **Enable Database Audit Logging** 对话框中，提供你在 [步骤 2. 配置 Azure Blob 存储访问](#step-2-configure-azure-blob-storage-access) 中获得的 **Blob URL** 和 **SAS Token**：
...
> **Note:**
>
> - 启用审计日志后，如果你对 **Blob URL** 或 **SAS Token** 字段进行了任何新更改，必须再次点击 **Test Connection** 来验证 TiDB Cloud 是否能连接到容器。然后，点击 **Enable** 以应用更改。
...
> **Note:**
>
> - 过滤规则是正则表达式且区分大小写。如果使用通配符规则 `.*`，则会记录集群中所有用户、数据库或表的事件。
> - 由于审计日志会消耗集群资源，建议谨慎指定过滤规则。为了减少资源消耗，建议尽可能限制审计范围，仅对特定数据库对象、用户和操作设置过滤规则。
...
> **Note:**
>
> 如果你已请求并选择将审计日志文件存储在 TiDB Cloud 中，可以在 **Database Audit Logging** 页面上的 **Audit Log Access** 部分下载。

TiDB Cloud 的审计日志是可读的文本文件，文件名中包含集群 ID、Pod ID 和日志创建日期。
...
> **Note:**
>
> 每当日志文件大小达到 10 MiB 时，日志文件会被推送到云存储桶。因此，禁用审计日志后，大小小于 10 MiB 的日志文件不会自动推送到云存储桶。若要获取此类日志文件，请联系 [PingCAP support](/tidb-cloud/tidb-cloud-support.md)。
...
> **Note:**
>
> 在下表中，字段的最大长度为空，表示该字段的数据类型具有明确的常量长度（例如，INTEGER 为 4 字节）。
...
| 16 | ROWS | INTEGER |  | 影响的行数（`0` 表示没有影响任何行）      |
...
- 如果 EVENT_CLASS 值为 `CONNECTION`，数据库事件记录还包含以下字段：
    ...
    | 21 | SERVER_PORT | INTEGER |  | TiDB 服务器监听客户端的端口（MySQL 协议） |
