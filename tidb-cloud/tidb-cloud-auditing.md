...
summary: 了解如何在 TiDB Cloud 中对集群进行审计
---
...
> **Note:**
>
> 对于在 AWS 上部署的 TiDB 集群，你可以选择在启用数据库审计日志时将审计日志文件存储在 TiDB Cloud 中。此功能目前仅在请求后提供。若要请求此功能，请点击 [TiDB Cloud 控制台](https://tidbcloud.com) 右下角的 **?**，然后点击 **Request Support**。接着，在 **Description** 字段中填写“Apply to store audit log files in TiDB Cloud”，并点击 **Submit**。
...
2. 在 AWS 管理控制台中，进入 **IAM** > **Access Management** > **Policies**，检查是否存在具有 `s3:PutObject` 写入权限的存储桶策略。
    ...
    - 如果不存在，进入 **IAM** > **Access Management** > **Policies** > **Create Policy**，根据以下策略模板定义存储桶策略。
        ...
        其中，`<Your S3 bucket ARN>` 是你要写入审计日志文件的 S3 存储桶的 Amazon 资源名称（ARN）。你可以在存储桶的 **Properties** 标签页的 **Bucket Overview** 区域获取 ARN 值。在 `"Resource"` 字段中，需要在 ARN 后添加 `/*`。例如，如果 ARN 是 `arn:aws:s3:::tidb-cloud-test`，则配置 `"Resource"` 的值为 `"arn:aws:s3:::tidb-cloud-test/*"`。
...
4. 点击 **Enable**，启用该集群的审计日志。
    ...
    TiDB Cloud 现已准备好将指定集群的审计日志写入你的 GCS 存储桶。
...
> **Note:**
>
> - 启用审计日志后，如果你对 **Blob URL** 或 **SAS Token** 字段进行了任何新更改，必须再次点击 **Test Connection** 以验证 TiDB Cloud 是否能连接到容器。然后，点击 **Enable** 以应用更改。
...
1. 在 **DB Audit Logging** 页面，点击 **Add Filter Rule**，添加审计过滤规则。
    ...
    你可以一次添加一条审计规则。每条规则指定用户表达式、数据库表达式、表表达式和访问类型。可以添加多条审计规则以满足你的审计需求。

2. 在 **Log Filter Rules** 部分，点击 **>** 展开并查看已添加的审计规则列表。

> **Note:**
>
> - 过滤规则是正则表达式，区分大小写。如果使用通配符规则 `.*`，则会记录集群中的所有用户、数据库或表事件。
> - 由于审计日志会消耗集群资源，建议在指定过滤规则时谨慎。为了减少资源消耗，建议尽可能限制审计范围，仅对特定数据库对象、用户和操作设置过滤规则。
...
> **Note:**
>
> 每当日志文件大小达到 10 MiB 时，日志文件会被推送到云存储存储桶。因此，禁用审计日志后，大小小于 10 MiB 的日志文件不会自动推送到存储桶。若要获取此类日志文件，请联系 [PingCAP support](/tidb-cloud/tidb-cloud-support.md)。
...
| 15 | SQL_TEXT | VARCHAR | 64 KB | 脸罩 SQL 语句   |
...
- 如果 EVENT_CLASS 值为 `CONNECTION`，数据库事件记录还包含以下字段：
    ...
    | 21 | SERVER_PORT | INTEGER |  | TiDB 服务器监听客户端的端口（通过 MySQL 协议） |
