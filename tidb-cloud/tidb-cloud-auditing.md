...
> **Note:**
>
> Database audit logging is in public preview for eligible TiDB Cloud Dedicated clusters that meet the following requirements:
>
> - For clusters hosted on AWS and Google Cloud: the TiDB version must be v7.5.6 or later, or v8.5.2 or later.
> - For clusters hosted on Azure: the TiDB version must be v7.5.6 or later, or v8.5.2 or later, and the clusters must have been created after April 15, 2026.
>
> For all other TiDB versions or cluster configurations, database audit logging is available upon request. To request access for an ineligible cluster, click **?** in the lower-right corner of the [TiDB Cloud console](https://tidbcloud.com), and then click **Support Tickets** to go to the [Help Center](https://tidb.support.pingcap.com/servicedesk/customer/portals). Create a ticket, fill in "Apply for database audit logging" in the **Description** field, and then click **Submit**.
>
> This document applies only to the public preview version of the audit logging feature. If you are using an earlier version of database audit logging, see [TiDB Cloud Database Audit Logging (Legacy)](/tidb-cloud/tidb-cloud-auditing-legacy.md).
...
1. Get the TiDB Cloud Account ID and the External ID of the TiDB cluster for which you want to enable audit logging.
...
1. Get the Google Cloud Service Account ID of the TiDB cluster for which you want to enable audit logging.
...
3. Generate a SAS token for the target container.
    ...
    4. In the **Start** and **Expiry** fields, specify a validity period for the SAS token.
        ...
        > - Make sure to regenerate and update the SAS token before it expires to ensure continuous availability of audit logs.
...
| Field         | Description                                                                                   |
|---------------|-----------------------------------------------------------------------------------------------|
| `ID`            | The unique identifier for the audit record of an operation.                        |
...
| `CONNECTION_TYPE` | The connection type, including Socket, UnixSocket, and SSL/TLS.                                 |
...
{{{ .dedicated }}} does not guarantee that audit logs are written in chronological order, which means that you might have to review all log files to find the most recent events. To sort the logs chronologically, you can use the `TIME` field in the audit logs.
