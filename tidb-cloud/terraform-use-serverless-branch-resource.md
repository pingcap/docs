...
本文档描述了如何使用 `tidbcloud_serverless_branch` 资源管理 [TiDB Cloud 无服务器分支](/tidb-cloud/branch-manage.md)。
...
> **注意：**
>
> TiDB Cloud 无服务器分支资源无法修改。如果你想更改无服务器分支资源的配置，需要删除现有的资源并创建一个新的。
...
以下示例展示了如何创建一个 TiDB Cloud 无服务器分支。
...
2. 创建 `branch.tf` 文件：
    ...
    - 若要使用无服务器分支资源，将资源类型设置为 `tidbcloud_serverless_branch`
...
导入未由 Terraform 创建的 TiDB Cloud 无服务器分支，步骤如下：
...
要删除 TiDB Cloud 无服务器分支，你可以删除 `tidbcloud_serverless_branch` 资源的配置，然后运行 `terraform apply` 来销毁资源：
