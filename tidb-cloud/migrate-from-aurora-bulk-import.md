...
3.  ソースデータベースをS3にバックアップします。
    ...
    Dumplingを使用して、 Auroraからデータをエクスポートします。ご使用の環境に応じて、山括弧（&gt;）内のコンテンツを置き換えてから、次のコマンドを実行します。データをエクスポートするときにフィルタールールを使用する場合は、 [テーブルフィルター](/table-filter.md#syntax)を参照してください。
