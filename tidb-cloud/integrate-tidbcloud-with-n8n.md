...
3. 在 **Parameters** 对话框中，输入 TiDB Cloud 节点的凭证。凭证即你的 TiDB Cloud API 密钥。
...
如果工作流按预期运行，你将收到 Hacker News 简报邮件。这些新闻内容会被记录到你的 TiDB Cloud Serverless 集群中，因此你无需担心丢失。
...
- **Delete Key**：决定数据库中哪些行被删除的项属性名称。item 是从一个节点发送到另一个节点的数据。节点会对每个输入数据项执行操作。关于 n8n 中的 item 更多信息请参见 [n8n documentation](https://docs.n8n.io/workflows/items/)。
...
- **Database**：数据库名称。
- **Table**：表名。你可以使用 `From list` 模式选择一个表，或使用 `Name` 模式手动输入表名。
- **Columns**：以逗号分隔的输入项属性列表，这些属性将作为新行的列。item 是从一个节点发送到另一个节点的数据。节点会对每个输入数据项执行操作。关于 n8n 中的 item 更多信息请参见 [n8n documentation](https://docs.n8n.io/workflows/items/)。
...
- **Database**：数据库名称。
- **Table**：表名。你可以使用 `From list` 模式选择一个表，或使用 `Name` 模式手动输入表名。
- **Update Key**：决定数据库中哪些行被更新的项属性名称。item 是从一个节点发送到另一个节点的数据。节点会对每个输入数据项执行操作。关于 n8n 中的 item 更多信息请参见 [n8n documentation](https://docs.n8n.io/workflows/items/)。
...
- 对于 **Insert** 和 **Update** 操作，你需要在 **Columns** 字段中指定以逗号分隔的列表，并且字段名必须与输入项的属性名一致。