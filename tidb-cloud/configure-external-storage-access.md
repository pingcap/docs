...
aliases: ['/ja/tidbcloud/serverless-external-ストレージ']
---

# 外部ストレージへのアクセスを構成する {#configure-external-ストレージ-access}
...
TiDB Cloud Dedicatedクラスター用にこれらの外部ストレージを構成する必要がある場合は、 [TiDB Cloud Dedicatedの外部ストレージアクセスを構成する](/tidb-cloud/dedicated-external-ストレージ.md)参照してください。
...
3.  AWS CloudFormationテンプレートを使用してロールARNを作成します。
    ...
    5.  CloudFormationスタックの実行後、 **[出力]**タブをクリックすると、 **[値]**列にロールARNの値が表示されます。
        ...
        ![Role ARN](/media/tidb-cloud/serverless-external-ストレージ/serverless-role-arn.png)
...
1.  Google Cloud サービス[サービスアカウントページ](https://console.cloud.google.com/iam-admin/serviceaccounts)ページで、 **[サービス アカウントの作成]**をクリックしてサービス アカウントを作成します。詳細については、 [サービスアカウントの作成](https://cloud.google.com/iam/docs/creating-managing-service-accounts)参照してください。
    ...
    4.  `Grant this service account access to project`で、必要な権限を持つ[IAMロール](https://cloud.google.com/iam/docs/understanding-roles)を選択します。
        ...
        -   TiDB Cloud StarterまたはEssentialインスタンスからデータをエクスポートするには`ストレージ.objects.create`権限を持つロールが必要です。
        -   TiDB Cloud StarterまたはEssentialインスタンスにデータをインポートするには`ストレージ.buckets.get` 、 `ストレージ.objects.get` 、および`ストレージ.objects.list`権限を持つロールが必要です。
    ...
    ![service-account](/media/tidb-cloud/serverless-external-ストレージ/gcs-service-account.png)

2.  サービスアカウントをクリックし、 `KEYS`ページで**[キーの追加]**をクリックして、サービスアカウントキーを作成します。

    ![service-account-key](/media/tidb-cloud/serverless-external-ストレージ/gcs-service-account-key.png)
...
## Azure Blob Storageへのアクセスを構成する {#configure-azure-blob-ストレージ-access}
...
3.  Azure ARMテンプレートを使用してSASトークンを作成します。
    ...
    3.  **カスタムデプロイメント**ページで、**リソースグループ**と**ストレージアカウント名**を入力してください。コンテナが配置されているストレージアカウントの概要ページから、すべての情報を取得できます。
        ...
        ![azure-ストレージ-account-overview](/media/tidb-cloud/serverless-external-ストレージ/azure-ストレージ-account-overview.png)
...
2.  **ストレージアカウントの**ページで、[**Security]+ [ネットワーク]**をクリックし、 **[共有アクセス署名]**をクリックします。
    ...
    ![sas-position](/media/tidb-cloud/serverless-external-ストレージ/azure-sas-position.png)

3.  **[共有アクセス署名]**ページで、次のように必要なアクセス許可を持つサービス SAS トークンを作成します。詳細については、 [サービスSASトークンを作成します](https://docs.microsoft.com/en-us/azure/ストレージ/common/ストレージ-sas-overview)参照してください。
    ...
    ![sas-create](/media/tidb-cloud/serverless-external-ストレージ/azure-sas-create.png)
...
## Alibaba Cloudオブジェクトストレージサービス（OSS）へのアクセスを設定する {#configure-alibaba-cloud-object-ストレージ-service-oss-access}
