...
summary: 了解如何从传统的项目级 Prometheus 集成迁移到新的集群级 Prometheus 集成。
---

# 迁移 Prometheus 集成

TiDB Cloud 现在在集群级别管理 [Prometheus 集成](/tidb-cloud/monitor-prometheus-and-grafana-integration.md)，提供更细粒度的控制和配置。传统的项目级 Prometheus 集成（Beta）将于 2026 年 1 月 9 日下线。如果你的组织仍在使用这些传统集成，请按照本指南将其迁移到新的集群级 Prometheus 集成，以最大程度减少对统计/指标（信息）相关服务的影响。

## 前置条件

- 要为 TiDB Cloud 设置第三方统计/指标（信息）集成，你必须拥有 TiDB Cloud 中的 `Organization Owner` 或 `Project Owner` 访问权限。
...
删除项目级 Prometheus 集成（Beta）后，项目下所有集群将立即停止向 Prometheus 端点暴露统计/指标（信息）。这会导致下游数据的临时丢失，并中断与集成相关的服务（如监控和告警），直到你配置新的集群级 Prometheus 集成为止。
