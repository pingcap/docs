...
完成本教程前，请确保你在支持的区域拥有一个 TiDB Cloud Serverless 集群。如果还没有，可以参考 [Creating a TiDB Cloud Serverless cluster](/develop/dev-guide-build-cluster-in-cloud.md) 来创建。

## 开始使用

### Step 1. 安装 [pytidb](https://github.com/pingcap/pytidb) Python SDK
...
2. 点击右上角的 **Connect**。会弹出连接对话框，列出连接参数。
   ...
   注意，上述示例仅为演示用途。你需要填写自己的参数，并妥善保管。
...
完整示例可参考 [pytidb hybrid search demo](https://github.com/pingcap/pytidb/tree/main/examples/hybrid_search)。
