...
- SQL queries generated by AI is not 100% accurate and might still need your further tweak.
- Chat2Query is still in beta and has limited support on SQL statements. DDLs such as `CREATE TABLE` or `DROP TABLE` are not supported yet.
...
PingCAP takes the privacy and security of users' data as top priority. The AI capacity of Chat2Query only needs to access database schemas to generate SQL queries, not your data itself. For more information, see [Chat2Query Privacy FAQ](https://www.pingcap.com/privacy-policy/privacy-chat2query).
