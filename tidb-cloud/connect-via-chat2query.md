...
> - The AI capacity of Chat2Query relies on data to work. It collects data to provide the service, some of which is then retained for further product improvements. After you enable the AI capacity, Chat2Query only collects your table schema information and will not collect any specific data. For more information, see [Chat2Query Privacy FAQ](https://www.pingcap.com/privacy-policy/privacy-chat2query).

## Limitation

- It is not guaranteed that SQL queries generated by Chat2Query AI is 100% accurate. You can further edit the generated queries according to you need.
- Chat2Query has limited support on SQL statements. DDLs such as `CREATE TABLE` or `DROP TABLE` are not supported yet.
...
For the first-time you access Chat2Query, you will get a dialog about whether to allow PingCAP and OpenAI to use your code snippets to research and improve the services.

- To enable AI to generate SQL queries, select the checkbox and click **Save and Get Started**.
...
1. Write SQL queries.
    ...
    - If AI is enabled, simply type `--` and your instructions to let AI generate SQL queries automatically or write SQL queries manually.
...
1. In the upper-right corner of Chat2Query, click **...** and select **Chat2Query settings**.
