...
TiDB 兼容 MySQL，使得迁移和对接现有应用变得简单，同时具备无缝扩展性，能够从小型负载到大规模高性能集群灵活应对。它在一个系统中同时支持事务型（OLTP）和分析型（OLAP）负载，简化运维并实现实时洞察。
...
你可以部署多个 TiDB 节点以实现水平扩展，承载更高负载。这些节点通常与负载均衡器（如 TiProxy 或 HAProxy）配合，提供无缝接口。TiDB 节点本身不存储数据——它们会将数据请求转发给 TiKV 节点（行存储）或 TiFlash 节点（列存储）。
...
- **列存储**
    ...
    TiFlash 节点以列式格式存储数据，针对分析型查询进行了优化，显著提升读密集型负载的性能。
