...
TiDB Cloud BYOC（Bring Your Own Cloud）は、マネージドサービスとして TiDB Cloud を利用しながら、データの配置場所、クラウドリソース、ネットワーク境界、コンプライアンス体制をより強く制御する必要がある組織向けに設計されています。BYOC では、TiDB Cloud の control plane は PingCAP によって管理され、TiDB の data plane はお客様自身のクラウドアカウント内で実行されます。
...
お客様のアプリケーションは、アプリケーション環境から {{{ .byoc }}} インスタンスに接続します。BYOC の設定に応じて、接続にはプライベートアクセスまたは制御されたパブリックアクセスのいずれかが使用されます。プライベートアクセスについては、BYOC は VPC peering やその他のプライベート接続オプションをサポートします。
