...
- 在 AWS 上，TiDB Cloud 提供 **TiDB Cloud Starter**，适用于自动扩缩、成本高效的负载，**TiDB Cloud Essential**，适用于具备预配置容量的生产级负载，以及 **TiDB Cloud Dedicated**，适用于企业级应用，具备专属资源和高级能力。
...
- **可预测的定价**：根据存储和计算资源的请求容量单元（RCUs）计费，提供透明、按用量计费的定价模式，随需扩展，按实际使用付费，无隐藏费用。
...
- **高可用性**
    ...
    - TiKV 节点中的所有数据都会被复制（默认 3 副本），以保证数据持久性。
