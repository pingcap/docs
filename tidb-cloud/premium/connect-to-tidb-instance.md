...
本文档介绍如何连接到你的 {{{ .premium }}}<CustomContent plan="byoc"> 或 {{{ .byoc }}}</CustomContent> 实例。
...
在你的 {{{ .premium }}}<CustomContent plan="byoc"> 或 {{{ .byoc }}}</CustomContent> 实例于 TiDB Cloud 上创建完成后，你可以通过直连进行连接。
...
对于 {{{ .premium }}}<CustomContent plan="byoc"> 和 {{{ .byoc }}}</CustomContent>，有两种网络连接类型：
...
- [Public endpoint](/tidb-cloud/premium/connect-to-premium-via-public-connection.md)
    ...
    标准连接会暴露一个公共端点，因此你可以通过笔记本电脑上的 SQL 客户端连接到你的 {{{ .premium }}}<CustomContent plan="byoc"> 或 {{{ .byoc }}}</CustomContent> 实例。
