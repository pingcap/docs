...
# AWS PrivateLink 経由でTiDB Cloud Premium に接続する {#connect-to-tidb-cloud-premium-via-aws-privatelink}
...
`TiDB Private Link Service is ready`というメッセージが表示された場合、対応するエンドポイントサービスは準備完了です。エンドポイントを作成するには、以下の情報を提供してください。
...
4.  **サービス設定**領域に、生成されたコマンド（ `--service-name ${your_endpoint_service_name}` ）のサービス名`${your_endpoint_service_name}`を入力します。
