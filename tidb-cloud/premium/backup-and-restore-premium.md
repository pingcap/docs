...
{{{ .premium }}} supports restoring backups from cloud storage (such as AWS S3 and Alibaba Cloud Object Storage Service (OSS)) to a new instance. This feature is compatible with backups generated from {{{ .dedicated }}} clusters or self-managed (on-premises) clusters.

## Limitations

- Currently, manual backups are not supported for {{{ .premium }}} instances.
- Currently, only backups located in **AWS S3** and **Alibaba Cloud OSS** are supported.
- You can only restore backups to a new instance in the same cloud provider as your storage bucket.
...
Before getting started, ensure that you have the access key and secret key with sufficient permissions to access the backup files.
...
4. If the verification is successful, the **Restore to a New Instance** page appears. Review the backup information displayed at the top of the page. Follow the steps in [Create a {{{ .premium }}} Instance](/tidb-cloud/premium/create-tidb-instance-premium.md) to restore the backup to a new instance.

    If the information is not correct, click **Previous** to return to the previous page, and then enter the correct information.
