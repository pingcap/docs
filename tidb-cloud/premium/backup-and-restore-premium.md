...
Currently, only classic backups from AWS Dedicated Clusters are supported to be restored to a new Premium instance.

To restore a classic backup generated by a dedicated cluster, follow these steps:

1. Log in to the [TiDB Cloud console](https://tidbcloud.com), and then navigate to the [**TiDB Instances**](https://tidbcloud.com/tidbs) page. In the top-right corner, click **...**, then click **Restore from Another Plan**.


2. On the **Select Backup** Page, sequentially choose the **Project** that contains the dedicated cluster，select the dedicated **cluster** and then select the **Backup snapshot** you want to restore, then click **Next** button.

    > **Note:**
    >
    > Ensure that the dedicated cluster containing the backup snapshot is either in an **Active** or **Deleted** status within the selected project.
    > The snapshot should be located in the region that Premium supports. If the region is not supported, please contact the TiDB support team to open a new region for Premium, or select another backup snapshot.
