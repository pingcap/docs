...
Currently, you can only restore classic backups from a {{{ .dedicated }}} cluster hosted on AWS to a new {{{ .premium }}} instance.
...
2. On the **Select Backup** page, select the project that contains the {{{ .dedicated }}} cluster. Select the cluster, and then select the backup snapshot that you want to restore. Click **Next**.
