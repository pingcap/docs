...
現在、AWS上でホストされているTiDB Cloud Dedicatedクラスターから新しいTiDB Cloud Premiumインスタンスへのバックアップ復元のみが可能です。
...
### クラウドストレージからバックアップを復元する {#restore-backups-from-cloud-ストレージ}

TiDB Cloud Premiumは、クラウドストレージ（Amazon S3やAlibaba Cloud Object Storage Service（OSS）など）から新しいインスタンスへのバックアップ復元をサポートしています。この機能は、 TiDB Cloud DedicatedクラスターまたはTiDB Self-Managedクラスターから生成されたバックアップと互換性があります。
