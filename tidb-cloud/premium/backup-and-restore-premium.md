...
{{{ .premium }}} supports restoring backups from a cloud storage (such as AWS S3 and Alibaba Cloud Object Storage Service (OSS)) to a new instance. This feature is compatible with backups generated from {{{ .dedicated }}} clusters or self-managed (on-premises) clusters.
...
- Cloud provider consistency: you can only restore backups to a new instance in the same cloud provider as your storage bucket.
