...
本文档介绍如何通过 [AWS Endpoint Service private link 连接](/tidb-cloud/serverless-private-link-connection.md)，将 TiDB Cloud Essential 实例连接到 [Confluent Cloud Dedicated 集群](https://docs.confluent.io/cloud/current/clusters/cluster-types.html)（部署在 AWS 上）。
...
1. 在 [TiDB Cloud 控制台](https://tidbcloud.com)中，进入你的 TiDB Cloud Essential 实例的概览页面，然后点击左侧导航栏的 **Settings** > **Networking**。
...
- Region 匹配：网络必须与 TiDB Cloud Essential 实例位于同一 AWS Region。
- 可用区（AZ）可用性：网络的可用区必须与 TiDB Cloud Essential 实例的可用区有重叠。
