...
本文档介绍如何将 TiDB Cloud Starter 集群连接到支持 Model Context Protocol (MCP) 的 AI 驱动开发工具，如 Cursor、Claude Code、Visual Studio Code (VS Code) 和 Windsurf。
...
3. 确认你已满足所有 **Prerequisites**（前置条件）。如未满足，请根据屏幕提示安装所需依赖项。
4. 配置密码：
    ...
    - 如果你已设置过密码，在 **Enter the password for easy setup** 字段中输入你的密码。
    - 如果你忘记了密码，在 **Prerequisites** 区域点击 **Reset password** 生成新密码。
...
6. 按照所选工具的设置步骤完成配置。
    ...
    更多信息，参见 [针对工具的专属设置](#tool-specific-setup)。
...
- **方法 1**：从 [TiDB Cloud 控制台](https://tidbcloud.com/) 的 **Access `your_cluster_name` with AI tools** 对话框复制设置命令，并在终端中运行：
