...
- [私有 endpoint](/tidb-cloud/set-up-private-endpoint-connections-serverless.md)（推荐）
    ...
    私有 endpoint 连接为你的 VPC 中的 SQL 客户端提供一个私有 endpoint，通过 AWS PrivateLink 安全访问服务，具备高度安全性和单向访问数据库服务的能力，并简化网络管理。

- [公网 endpoint](/tidb-cloud/connect-via-standard-connection-serverless.md)

  标准连接会暴露一个公网 endpoint，因此你可以通过 SQL 客户端从你的笔记本电脑连接到 TiDB 集群。
