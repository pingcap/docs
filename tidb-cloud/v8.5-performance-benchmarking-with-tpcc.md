...
summary: TiDB バージョン v8.5.0 を使用したTiDB Cloud Dedicated クラスターの TPC-C パフォーマンス テストの結果を紹介します。
---

# TiDB v8.5.0 のTiDB Cloud TPC-C パフォーマンス テスト レポート {#tidb-cloud-tpc-c-performance-test-report-for-tidb-v8-5-0}
...
このテストの目的は、オンライントランザクション処理 (OLTP) シナリオにおける TiDB v8.5.0 の TPC-C パフォーマンスを示すことです。
...
TiKVパラメータ[`prefill-for-recycle`](https://docs.pingcap.com/tidb/stable/tikv-configuration-file#prefill-for-recycle-new-in-v700)は、初期化直後にログのリサイクルを有効にすることができます。このドキュメントでは、 `prefill-for-recycle`パラメータを有効にした状態でテストを実施しています。
...
4. TiDB オプティマイザが最適な実行計画を生成できるようにするには、TPC-C テストを実行する前に次の SQL文を実行して統計を収集します。
    ...
    統計の収集を高速化するには、収集前に次の SQL文を実行します。
...
5. TiDB Cloud Dedicated クラスタでストレステストを実行するには、以下のコマンドを`go-tpc tpcc`を実行します。同時実行ごとにテストは 2時間かかります。
...
v8.5.0 の[テスト環境](#test-environment)における TPC-C パフォーマンスは次のとおりです。
