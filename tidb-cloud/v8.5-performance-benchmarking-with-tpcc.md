...
このドキュメントでは、TiDBバージョン8.5.0を搭載したTiDB Cloud DedicatedクラスターのTPC-Cパフォーマンステストの手順と結果について説明します。このレポートは、TiDB Self-Managedバージョン8.5.0クラスターのパフォーマンスに関する参考資料としてもご利用いただけます。
...
-   クラスタータイプ: [TiDB Cloud専用](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated)
-   クラスターバージョン: v8.5.0
...
TiKVパラメータ[`prefill-for-recycle`](https://docs.pingcap.com/tidb/stable/tikv-configuration-file#prefill-for-recycle-new-in-v700) 、初期化直後にログのリサイクルを有効にすることができます。このドキュメントでは、 `prefill-for-recycle`パラメータを有効にした状態でテストを実施しています。
...
ベンチマーク・エグゼキュータはTiDBクラスターにSQLクエリを送信します。このテストでは、ハードウェア構成は次のとおりです。
...
5.  TiDB Cloud Dedicated クラスターでストレステストを実行するには、以下のコマンドを`go-tpc tpcc`実行します。同時実行ごとにテストは 2 時間かかります。
