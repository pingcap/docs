...
2. Configure the source data information.
    ...
    - **Data format**: select **SQL File**. TiDB Cloud supports importing the following compressed file formats: `.gz`, `.lz4`, `.zstd`, `.xz`, `.lzo` and `.snappy`. If you want to import compressed SQL files, name the files in the `${db_name}.${table_name}.sql.${compress}` format. For example, if you want to import the `trips.sql.gz` file to the `bikeshare.trips` table, you can rename the file as `bikeshare.trips.sql.gz`.
