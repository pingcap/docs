...
データ移行ジョブは、増分データ移行のパフォーマンスを行数/秒で測定します。この単位は、1秒あたりにターゲットデータベースへ移行される行数を示します。たとえば、上流データベースが10,000行の`INSERT`、`UPDATE`、または`DELETE`文を約1秒で実行する場合、対応する仕様のデータ移行ジョブは、その10,000行を約1秒で下流データベースに複製できます。
