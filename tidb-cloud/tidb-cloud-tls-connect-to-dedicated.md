...
为确保数据安全，你的 TiDB Cloud Dedicated 集群的 CA 证书托管在 [AWS Private Certificate Authority](https://aws.amazon.com/private-ca/) 上。CA 证书的私钥存储在 AWS 管理的硬件安全模块（HSM）中，这些模块符合 [FIPS 140-2 Level 3](https://csrc.nist.gov/projects/cryptographic-module-validation-program/Certificate/3139) 安全标准。
...
3. 在连接对话框中，从 **Connection Type** 下拉列表选择 **Public**。
    ...
    如果你还未配置 IP 访问列表，请点击 **Configure IP Access List** 进行首次连接前的配置。详细信息参见 [配置 IP 访问列表](/tidb-cloud/configure-ip-access-list.md)。
...
MySQL CLI 客户端默认尝试建立 TLS 连接。连接 TiDB Cloud Dedicated 集群时，你需要设置 `ssl-mode` 和 `ssl-ca`。
...
[MyCLI](https://www.mycli.net/) 在使用 TLS 相关参数时会自动启用 TLS。连接 TiDB Cloud Dedicated 集群时，你需要设置 `ssl-ca` 和 `ssl-verify-server-cert`。
...
TiDB Cloud Dedicated 使用来自 [AWS Private Certificate Authority](https://aws.amazon.com/private-ca/) 的证书作为客户端与 TiDB Cloud Dedicated 集群之间 TLS 连接的证书颁发机构（CA）。通常，CA 证书的私钥安全地存储在 AWS 管理的硬件安全模块（HSM）中，这些模块符合 [FIPS 140-2 Level 3](https://csrc.nist.gov/projects/cryptographic-module-validation-program/Certificate/3139) 安全标准。

## 常见问题

### 连接 TiDB Cloud Dedicated 集群支持哪些 TLS 版本？

出于安全考虑，TiDB Cloud Dedicated 仅支持 TLS 1.2 和 TLS 1.3，不支持 TLS 1.0 和 TLS 1.1 版本。详情参见 IETF [Deprecating TLS 1.0 and TLS 1.1](https://datatracker.ietf.org/doc/rfc8996/)。

### 我的客户端与 TiDB Cloud Dedicated 支持双向 TLS 认证吗？

不支持。

TiDB Cloud Dedicated 目前仅支持单向 TLS 认证，不支持双向 TLS 认证。如果你需要双向 TLS 认证，请联系 [TiDB Cloud Support](/tidb-cloud/tidb-cloud-support.md)。