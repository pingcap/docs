...
-   TiDB Cloudをサポートします。
    ...
    TiDB Cloud APIはRESTfulベースのインターフェースであり、 TiDB Cloudサービスと対話できます。このAPIを使用すると、クラスターなどのリソースを自動的かつ効率的に管理できます。

    TiDB Cloudコンソールでは、[**組織の設定]**ページの[ <strong>APIキー</strong>]タブを見つけることができます。このタブを使用すると、 TiDB Cloudの使用に必要なAPIキーを作成および管理できます。

    詳細については、 [TiDB Cloudリファレンス](https://docs.pingcap.com/tidbcloud/api/v1)を参照してください。

-   新しい開発者層クラスターの自動休止状態と再開をサポートします。

    開発者層クラスタは、7日間使用されなかった後も削除されないため、1年間の無料試用期間が終了するまでいつでも使用できます。非アクティブな状態が数日続くと、開発者層クラスタは自動的に休止状態になります。クラスターを再開するには、クラスタに新しい接続を送信するか、 TiDB Cloudコンソールの[**再開**]ボタンをクリックしクラスタ。
...
-   コードの可読性を向上させるために、[ **TiDBに接続**]ダイアログで提供されるサンプルコードで構文の強調表示をサポートします。サンプルコードで、置き換える必要のあるパラメータを簡単に特定できます。
