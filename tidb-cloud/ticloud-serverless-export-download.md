...
エクスポートされたデータを非対話型モードでダウンロードします。
...
| --output-path            | ダウンロードしたデータの保存先パスを指定します。指定しない場合は、データは現在のディレクトリにダウンロードされます。 | いいえ | 非対話型モードでのみ動作します。         |
| --concurrency int          | ダウンロードの同時実行数を指定します。デフォルト値は`3`です。                           | いいえ | 非対話型モードと対話型モードの両方で動作します。 |
