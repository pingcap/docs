...
TiDB Cloud は、 TiDB がサポートするほぼすべてのワークロードで動作しますが、TiDB Self-Hosted と TiDB Dedicated/Serverless には機能上の違いがいくつかあります。このドキュメントでは、 TiDB Cloudの SQL 機能の制限について説明します。TiDB Self-Hosted と TiDB Dedicated/Serverless 間の機能ギャップは、常に埋められています。ギャップを埋めるためにこれらの機能や機能が必要な場合は、機能リクエスト[お問い合わせ](/tidb-cloud/tidb-cloud-support.md)送信してください。
...
| `information_schema` | `TIFLASH_SEGMENTS`                   | サポート            | サポートされていません[^1] |
| `information_schema` | `TIFLASH_TABLES`                     | サポート            | サポートされていません[^1] |
...
| `mysql`              | `tidb_ttl_job_history`               | サポート            | サポートされていません[^9] |
| `mysql`              | `tidb_ttl_table_status`              | サポート            | サポートされていません[^9] |
| `mysql`              | `tidb_ttl_task`                      | サポート            | サポートされていません[^9] |
...
| `require_secure_transport`                | サポートされていません[^12] | 読み取り専用[^11]                 |
...
| `validate_password.policy`                | 制限なし             | `MEDIUM`または`STRONG` [^10]のみ |
...
[^9]: [存続時間 (TTL)](/time-to-live.md)は現在 TiDB Serverless では利用できません。

[^10]: TiDB Serverless は強力なパスワード ポリシーを適用します。

[^11]: 変数は TiDB Serverless では読み取り専用です。
