...
TiDB Cloud Starter 是一款全托管的多租户 TiDB 服务。它提供了一个即开即用、自动弹性扩缩的 MySQL 兼容数据库，并在超出免费额度后，按用量计费。
...
- 连接到你的集群：
    ...
    > **注意：**
    >
    > TiDB Cloud Starter 和 TiDB Cloud Essential 需要使用 TLS 连接。如何在你的系统中查找 CA 根证书路径，请参见 [根证书默认路径](/tidb-cloud/secure-connections-to-serverless-clusters.md#root-certificate-default-path)。
