...
| tidbcloud_node_storage_used_bytes | gauge | cluster_name: `<cluster name>`<br/>instance: `tikv-0\|tikv-1…\|tiflash-0\|tiflash-1…`<br/>component: `tikv\|tiflash` | The disk usage in bytes for TiKV or TiFlash nodes. This metric primarily reflects logical data size in the storage engine and excludes filesystem overhead, WAL, and temporary files. To calculate the actual disk usage rate, use `(capacity - available) / capacity` instead. |
