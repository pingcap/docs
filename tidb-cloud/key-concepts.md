...
- For <CustomContent plan="starter,essential">{{{ .starter }}} and {{{ .essential }}}</CustomContent></CustomContent><CustomContent plan="premium">{{{ .starter }}}, {{{ .essential }}}, and {{{ .premium }}}</CustomContent>, see [High Availability in TiDB Cloud](/tidb-cloud/serverless-high-availability.md).
