...
- TiDB Cloud 资源可以是 TiDB X instance（基于 [TiDB X architecture](/tidb-cloud/tidb-x-architecture.md) 构建的面向服务的 TiDB Cloud 产品形态），也可以是 TiDB Cloud Dedicated cluster。
...
- **[Project-Level roles](/tidb-cloud/manage-user-access.md#project-roles)**：分配管理特定项目（包括项目范围内的资源和配置）的权限。

- **[Instance-Level roles](/tidb-cloud/manage-user-access.md#instance-roles)**：授予对特定 TiDB X 实例的细粒度访问权限。
