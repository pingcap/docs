...
-   セットアップガイド:
    ...
    -   [{{{ .starter }}} または Essential への TLS 接続](/tidb-cloud/secure-connections-to-serverless-clusters.md)
...
-   CMEK のないTiDB Cloud Dedicated クラスターの場合、 TiDB Cloud はエスクロー キーを使用します。{{{ .starter }}} および {{{ .essential }}} クラスターはエスクロー キーのみに依存します。
