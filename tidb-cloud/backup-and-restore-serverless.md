...
-   **バックアップ保持**期間は、バックアップが保持される期間です。期限切れのバックアップは復元できません。
    ...
    -   無料のTiDB Cloud Starter クラスターの場合、バックアップの保持期間は 1 日です。
    -   TiDB Cloud Starter（支出上限が0より大きい）またはTiDB Cloud Essentialクラスターの場合、バックアップの保持期間は1日から30日の間で任意の値に設定できます。デフォルトの保持期間は14日です。
...
-   **ポイントインタイム リストア (ベータ)** : クラスターを特定の時点に復元します。
    ...
    -   TiDB Cloud Essential クラスター: バックアップ保持期間内の任意の時点に復元しますが、クラスターの作成時刻より前、または現在の時刻の 1 分前より後の時点には復元しません。
