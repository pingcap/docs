---
title: 将本地文件导入 TiDB Cloud Starter
summary: 了解如何将本地文件导入 TiDB Cloud Starter。
---

# 将本地文件导入 {{{ .starter }}}

你可以直接将本地文件导入 {{{ .starter }}}。只需几步简单操作即可完成任务配置，你的本地 CSV 数据就会被快速导入到你的 {{{ .starter }}} 实例中。使用此方法，无需提供云存储和凭证，整个导入过程快速且流畅。
...
- 本地文件导入仅支持 {{{ .starter }}} 实例，不支持 {{{ .essential }}} 实例或 TiDB Cloud Dedicated 集群。
- 你不能同时运行多个导入任务。

## 导入本地文件

1. 打开目标 {{{ .starter }}} 实例的 **Import** 页面。
    ...
    2. 点击目标 {{{ .starter }}} 实例的名称进入其概览页面，然后在左侧导航栏点击 **Data** > **Import**。
...
如果只想导入指定列，可以使用 MySQL 客户端连接到 {{{ .starter }}} 实例，然后通过 [`LOAD DATA`](https://docs.pingcap.com/tidb/stable/sql-statement-load-data) 指定要导入的列。例如：
