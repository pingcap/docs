...
1. 在你的 TiDB Cloud Serverless 集群中创建一张表并插入一些数据。
    ...
    你可以使用 [TiDB Cloud 控制台中的 SQL Editor](/tidb-cloud/explore-data-with-chat2query.md) 执行 SQL 语句。以下是示例：
...
2. 在项目根目录下，创建一个名为 `hello-world.ts` 的文件，并添加以下代码：
...
本节以 Vercel Edge Function 中的 TiDB Cloud Kysely 方言为例。
...
4. 访问 `http://localhost:3000/api/edge-function-example` 获取你的路由响应。
...
2. 访问 `${Your-URL}/api/edge-function-example` 页面获取你的路由响应。
