...
| Command Per Second | Query, StmtExecute, StmtPrepare, and so on. | The number of commands processed by all TiDB instances per second based on command types. |
...
| Metric name  | Labels | Description                                   |
| :------------| :------| :-------------------------------------------- |
| Avg TiDB KV Request Duration | Get, Prewirite, Commit, PessimisticLock, and so on. | The average time consumed in executing KV requests in all TiDB instances based on request types, including `Get`, `Prewrite`, and `Commit`. |
| Avg TiKV GRPC Duration | kv_get, kv_prewirite, kv_commit, kv_pessimisticLock, and so on. | The average time consumed in executing gRPC requests in all TiKV instances based request types, including `kv_get`, `kv_prewrite`, and `kv_commit`. |
