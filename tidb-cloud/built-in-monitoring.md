...
| 指标名称  | 标签 | 描述                                   |
| :------------| :------| :-------------------------------------------- |
| Request Units | RU per second | Request Unit（RU）是用于衡量查询或事务资源消耗的单位。除了你运行的查询外，后台活动也会消耗 Request Units，因此即使 QPS 为 0，每秒的 Request Units 也可能不为 0。|
...
| Average Query Duration | All, {SQL type} | 从客户端请求到 {{{ .starter }}} 或 {{{ .essential }}} 集群，直到集群执行请求并返回结果给客户端的持续时间。|
