...
| 使用済みストレージサイズ | 行ベースstorage、行ベース標準storage、列ベースstorage  | 行ベースstorage、行ベース標準storage、列ベースstorageのサイズ。このメトリックは、各storageタイプのサイズが50 MiB以上の場合にのみ表示されます。現在、**行ベース標準storage**は**行ベースstorage**と同じ意味です。                                                                                             |
