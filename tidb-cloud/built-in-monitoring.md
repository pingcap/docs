...
| 指标名称  | 标签 | 描述                                   |
| :------------| :------| :-------------------------------------------- |
| Database Time by SQL types | database time, {SQL type} | database time：每秒的总数据库时间。<br/> {SQL type}：每秒被不同 SQL 类型（如 `SELECT`、`INSERT`、`UPDATE`）消耗的数据库时间。|
| Query Per Second | {SQL type} | 所有 TiDB 实例每秒执行的 SQL 语句数量，按 SQL 类型（如 `SELECT`、`INSERT`、`UPDATE`）统计。|
| Query Duration | avg-{SQL type}, 99-{SQL type} | 从客户端发送请求到 TiDB，直到 TiDB 执行请求并返回结果给客户端的持续时间。通常，客户端请求以 SQL 语句形式发送；但该持续时间也可能包含如 `COM_PING`、`COM_SLEEP`、`COM_STMT_FETCH`、`COM_SEND_LONG_DATA` 等命令的执行时间。TiDB 支持 Multi-Query，即客户端可以一次发送多条 SQL 语句，如 `select 1; select 1; select 1;`。此时，该查询的总执行时间包含所有 SQL 语句的执行时间。|
...
| Average TiDB KV Request Duration | {Request Type} | 所有 TiDB 实例按请求类型（如 `Get`、`Prewrite`、`Commit`）执行 KV 请求所消耗的平均时间。|
| Average TiKV gRPC Duration | {Request Type} | 所有 TiKV 实例按请求类型（如 `kv_get`、`kv_prewrite`、`kv_commit`）执行 gRPC 请求所消耗的平均时间。|
...
- 对应集群的负载没有触发该指标。例如，如果没有失败的查询，失败查询指标面板会一直为空。
