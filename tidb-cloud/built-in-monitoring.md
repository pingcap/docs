...
| 使用済みストレージサイズ | 行ベースストレージ、行ベースStandardストレージ、列ベースストレージ  | 行ベースストレージ、行ベースStandardストレージ、列ベースストレージのサイズ。TiDB Cloud は、各ストレージタイプのサイズが50 MiB以上の場合にのみこのメトリックを表示します。**Row-based standard storage**は**Row-based storage**と同じ意味です。|
