...
-   TiDB Serverless クラスタの場合、メトリクス データは 3 日間保存されます。
...
## TiDB Serverlessクラスタのメトリクス {#metrics-for-tidb-serverless-clusters}

[メトリック] ページには、TiDB Serverless クラスタのメトリック用の 2 つのタブがあります。
...
| 平均クエリ継続時間      | すべて、{SQL タイプ}            | クライアントから TiDB Serverless クラスタへのリクエストを受信して​​から、クラスターがリクエストを実行して結果をクライアントに返すまでの期間。                                                                                      |
| 失敗したクエリ        | 全て                       | 1 秒あたりの SQL ステートメント実行エラーの数。                                                                                                                                        |
| 1秒あたりのトランザクション | 全て                       | 1 秒あたりに実行されるトランザクションの数。                                                                                                                                            |
| 平均トランザクション期間   | 全て                       | トランザクションの平均実行期間。                                                                                                                                                   |
| 総接続数           | 全て                       | TiDB Serverless クラスタへの接続の数。                                                                                                                                           |
