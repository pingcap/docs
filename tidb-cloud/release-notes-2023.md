...
- 支持在 TiDB Cloud 控制台直接升级到企业支持计划，无需联系销售。
...
- [TiDB Cloud 专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 的 IP 访问列表支持最多添加 100 个 IP 地址。
...
- [TiDB Cloud Serverless](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 集群新增 **Events** 页面，记录集群主要变更。
    ...
    你可以在该页面查看近 7 天的事件历史，追踪触发时间、操作用户等重要信息。
...
- 支持在 [Postman](https://www.postman.com/) 运行 Data App。
    ...
    Postman 集成支持将 Data App 的接口导入为集合到你的工作区，便于协作和无缝 API 测试，支持 Postman Web 和桌面应用。
...
- TiDB Cloud [数据服务](https://tidbcloud.com/project/data-service) 推出自动生成接口功能。
    ...
    开发者可通过极少的点击和配置，轻松创建 HTTP 接口。无需重复编写样板代码，简化并加速接口创建，减少潜在错误。
...
- 优化 TiDB Cloud 用户访问 PingCAP 支持的入口，提升支持体验。包括：
...
- [TiDB Cloud Serverless](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 集群支持 [分支功能（beta）](/tidb-cloud/branch-overview.md)。
    ...
    你可通过 [TiDB Cloud 控制台](/tidb-cloud/branch-manage.md) 或 [TiDB Cloud CLI](/tidb-cloud/ticloud-branch-create.md) 为 2023 年 7 月 5 日后创建的 Serverless 集群创建分支。

    若你使用 GitHub 进行应用开发，可将分支集成到 GitHub CI/CD 流水线，实现 PR 自动测试且不影响生产库。详情参见 [将 TiDB Cloud Serverless 分支（Beta）集成到 GitHub](/tidb-cloud/branch-github-integration.md)。
...
- [TiDB Cloud 专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 引入 Index Insight（beta），为慢查询提供索引推荐，优化查询性能。
    ...
    使用 Index Insight，你可通过以下方式提升数据库操作效率：
...
- 专属集群 **Diagnosis** 页的以下两个标签页发布全新原生 Web 架构：
    ...
    新架构下，浏览和获取信息更直观高效，用户体验更佳，SQL 诊断过程更友好。
...
- [Serverless 集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 的 [**SQL Diagnosis**](/tidb-cloud/tune-performance.md) 页面新增 **SQL Statement** 标签页。
    ...
    **SQL Statement** 标签页提供：
    ...
    - 每条 SQL 语句的详细信息，如查询时间、执行计划、数据库响应，助力性能优化。
...
- 支持直接访问 TiDB [Serverless 集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 所在区域的 [数据服务接口](/tidb-cloud/tidb-cloud-glossary.md#endpoint)。

    新建 Serverless 集群的接口 URL 现包含集群区域信息。通过 `<region>.data.tidbcloud.com` 可直接访问对应区域接口。

    也可通过 `data.tidbcloud.com` 全局域名访问，系统会自动重定向，但可能增加延迟。此方式下，调用接口时需为 curl 命令添加 `--location-trusted` 选项。
...
- 组织下前五个 [Serverless 集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 均享有免费额度：
  ...
  2023 年 5 月 31 日前，Serverless 集群仍免费（100% 折扣）。之后超出免费额度部分将计费。
    ...
    你可在集群 **Overview** 页的 **Usage This Month** 区域 [监控用量或提升额度](/tidb-cloud/manage-serverless-spend-limit.md)。免费额度用尽后，读写操作将被限流，直至提升额度或新月重置。

    各资源（读、写、SQL CPU、网络出口）RU 消耗、定价及限流详情参见 [TiDB Cloud Serverless 计费详情](https://www.pingcap.com/tidb-cloud-serverless-pricing-details)。
...
- 优化 AWS 上新建的 [专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 的 TiDB 负载均衡，缩放 TiDB 节点时减少连接中断。
...
- [专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 的 [Monitoring](/tidb-cloud/built-in-monitoring.md#view-the-metrics-page) 页面发布全新原生 Web 架构。
    ...
    新架构下，浏览 [Monitoring](/tidb-cloud/built-in-monitoring.md#view-the-metrics-page) 页面更直观高效，用户体验更佳，监控过程更友好。
...
- 全新 UI 设计，提升 [集群创建](https://tidbcloud.com/clusters/create-cluster) 体验，几步即可完成集群创建和配置。
...
- [专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 支持 [New Relic](https://newrelic.com/) 集成。
    ...
    你可将 TiDB 集群的监控数据发送到 [New Relic](https://newrelic.com/)，实现应用与数据库的统一监控和分析，便于快速定位和排查问题，缩短故障处理时间。
...
- [数据服务（beta）](/tidb-cloud/data-service-overview.md) 支持 Data App 更细粒度的访问控制。
    ...
    在 Data App 详情页，你可关联集群并为每个 API key 指定角色（`ReadOnly` 或 `ReadAndWrite`），控制其对集群的读写权限，实现集群级和权限级访问控制，灵活满足业务需求。
...
- [Serverless 集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 的 [Slow Query](/tidb-cloud/tune-performance.md#slow-query) 页面发布全新原生 Web 架构。
    ...
    新架构下，浏览 [Slow Query](/tidb-cloud/tune-performance.md#slow-query) 页面更直观高效，用户体验更佳，SQL 诊断过程更友好。

## 2023 年 3 月 21 日

**通用变更**

- [Serverless 集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 推出 [数据服务（beta）](https://tidbcloud.com/project/data-service)，支持通过自定义 API 接口以 HTTPS 请求访问数据。
...
- 支持为 2022 年 12 月 31 日后在 AWS 上创建的 [专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 缩小 TiDB、TiKV、TiFlash 节点规格，实现集群缩容。
...
- [专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 新增 **Events** 页面，记录集群主要变更。
    ...
    可查看近 7 天事件历史，追踪触发时间、操作用户等。例如，可查看集群暂停、规格变更等事件。
...
- [Serverless 集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 新增 [SQL 诊断](/tidb-cloud/tune-performance.md) 功能。

    通过 SQL 诊断可深入了解 SQL 运行状态，提升 SQL 性能调优效率。目前 Serverless 集群仅提供慢查询数据。
...
- [Serverless 集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 的 Monitoring 页面发布全新原生 Web 架构。
    ...
    新架构下，浏览 Monitoring 页面更直观高效，用户体验更佳，监控过程更友好。
...
- [TiDB Cloud CLI](/tidb-cloud/cli-reference.md) 新增 [`ticloud connect`](/tidb-cloud/ticloud-serverless-shell.md) 命令。
    ...
    `ticloud connect` 支持本地连接 TiDB Cloud 集群，无需安装 SQL 客户端。连接后可在 CLI 执行 SQL 语句。

## 2023 年 2 月 14 日

**通用变更**

- 支持缩减 TiKV 和 TiFlash 节点数量，实现 TiDB [专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 缩容。
...
- 每个集群新增 **Get Support** 选项，简化针对特定集群的支持请求流程。
    ...
    - 在项目 [**Clusters**](https://tidbcloud.com/project/clusters) 页面，点击集群行的 **...**，选择 **Get Support**。
...
- 支持为 2022 年 12 月 31 日后在 AWS 上创建的 TiDB Cloud 专属集群扩容 TiDB、TiKV、TiFlash 节点（提升 **Node Size(vCPU + RAM)**）。
...
- [**Monitoring**](/tidb-cloud/built-in-monitoring.md) 页的指标保留期延长至两天。
    ...
    该提升无需额外费用，可在集群 [**Monitoring**](/tidb-cloud/built-in-monitoring.md) 页的 **Diagnosis** 标签页访问，有助于更高效地定位和排查性能问题，监控集群健康。

- 支持为 Prometheus 集成自定义 Grafana dashboard JSON。

    已集成 Prometheus 的集群可导入预置 Grafana dashboard 并自定义，便于快速监控集群并及时发现性能问题。
