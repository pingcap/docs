...
- 对 2023 年 8 月 15 日后在 AWS 上创建的 [TiDB Cloud 专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 禁用负载均衡改进，包括：
  ...
  此变更避免混合部署时的资源争用，不影响已启用该功能的现有集群。如需为新集群启用负载均衡改进，请联系 [TiDB Cloud 支持](/tidb-cloud/tidb-cloud-support.md)。
...
- 支持在 [Postman](https://www.postman.com/) 中运行 Data App。
    ...
    Postman 集成支持将 Data App 的接口作为集合导入到你的工作区，便于协作和无缝 API 测试，支持 Postman Web 和桌面应用。
...
- 推出 TiDB Bot（Beta），基于 OpenAI 的智能聊天机器人，支持多语言、7x24 实时响应和文档集成。
...
- [TiDB Cloud Serverless](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 集群支持 [分支功能（Beta）](/tidb-cloud/branch-overview.md)。
    ...
    如果你使用 GitHub 进行应用开发，可将分支集成到 GitHub CI/CD 流水线，实现 PR 自动测试而不影响生产库。详细信息参见 [与 GitHub 集成 TiDB Cloud Serverless 分支（Beta）](/tidb-cloud/branch-github-integration.md)。
...
- [TiDB Cloud 专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 的 [**Key Visualizer**](/tidb-cloud/tune-performance.md#key-visualizer) 页面发布全新原生 Web 架构。
    ...
    新架构下，你可更便捷地浏览 **Key Visualizer** 页面，获取所需信息，提升 SQL 诊断体验和用户友好性。

## 2023 年 6 月 6 日

**通用变更**

- [TiDB Cloud 专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 引入 [Index Insight（Beta）](/tidb-cloud/index-insight.md)，为慢查询提供索引优化建议，提升查询性能。
  ...
  使用方法：进入专属集群 **Diagnosis** 页面，点击 **Index Insight BETA** 标签。
    ...
    详细信息参见 [使用 Index Insight（Beta）](/tidb-cloud/index-insight.md)。
...
- 支持直接访问 TiDB [Serverless 集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 所在区域的 [Data Service endpoint](/tidb-cloud/tidb-cloud-glossary.md#endpoint)。
    ...
    也可通过 `data.tidbcloud.com` 全局域名访问，系统会自动重定向，但可能增加延迟。若采用此方式，调用接口时需在 curl 命令中添加 `--location-trusted`。
...
- 组织下前五个 [Serverless 集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 均享有免费额度：
    ...
    - 行存储：5 GiB
    - [请求单位（RU）](/tidb-cloud/tidb-cloud-glossary.md#request-unit)：每月 5000 万 RU
...
- 提供维护窗口功能，便于你为 [专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 计划和管理维护任务。
    ...
    维护窗口为自动执行操作系统更新、安全补丁、基础设施升级等计划性维护的时间段，保障 TiDB Cloud 服务的可靠性、安全性和性能。

    维护期间可能出现短暂连接中断或 QPS 波动，但集群可用，SQL 操作、数据导入、备份、恢复、迁移和同步任务均可正常运行。维护期间允许和禁止的操作见 [操作列表](/tidb-cloud/configure-maintenance-window.md#allowed-and-disallowed-operations-during-a-maintenance-window)。

    我们将尽量减少维护频率。若有维护计划，默认开始时间为目标周三 03:00（以组织时区为准）。请关注维护计划，合理安排操作。

    - 每次维护窗口，TiDB Cloud 会发送三封邮件通知：维护前、开始时和结束后。
...
- [专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 的 [Monitoring](/tidb-cloud/built-in-monitoring.md#view-the-metrics-page) 页面发布全新原生 Web 架构。
    ...
    新架构下，浏览和获取监控信息更直观高效，用户体验大幅提升。

## 2023 年 4 月 18 日

**通用变更**

- [专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 支持扩/缩容 [数据迁移任务规格](/tidb-cloud/tidb-cloud-billing-dm.md#specifications-for-data-migration)。
...
- 专属集群 Prometheus 集成新增以下 [changefeed](/tidb-cloud/changefeed-overview.md) 指标：
...
- 将 TiDB Cloud 的帮助信息和操作整合到统一入口。

    你可在 [TiDB Cloud 控制台](https://tidbcloud.com/) 右下角点击 **?** 获取所有帮助信息及联系支持。

- 推出 [Getting Started](https://tidbcloud.com/getting-started) 页面，帮助你快速了解 TiDB Cloud。

    **Getting Started** 页面提供交互式教程、基础指南和常用链接。通过交互式教程，你可体验 TiDB Cloud 功能和 HTAP 能力，内置行业数据集（Steam 游戏、S&P 500）。
...
- 支持为 2023 年 3 月 28 日后创建的 [changefeed](/tidb-cloud/changefeed-overview.md) 扩/缩容规格。
    ...
    详细信息参见 [扩/缩容 changefeed](/tidb-cloud/changefeed-overview.md#scale-a-changefeed)。
...
- [专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 的 [数据迁移](/tidb-cloud/migrate-from-mysql-using-data-migration.md) 新增 GCP 区域：`Singapore (asia-southeast1)` 和 `Oregon (us-west1)`。

    新区域为数据迁移提供更多选择，靠近数据源时可获得更快、更可靠的迁移体验。
...
- [Serverless 集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 的 [Slow Query](/tidb-cloud/tune-performance.md#slow-query) 页面发布全新原生 Web 架构。
    ...
    新架构下，浏览和获取慢查询信息更直观高效，SQL 诊断体验更友好。
...
- 支持为 2022 年 12 月 31 日后在 AWS 上创建的 [专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 缩容 TiDB、TiKV、TiFlash 节点规格。
...
- [专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 的 [数据迁移](/tidb-cloud/migrate-from-mysql-using-data-migration.md) 新增 GCP 区域：`Tokyo (asia-northeast1)`。
...
- [专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 新增 **Events** 页面，记录集群主要变更。
    ...
    你可查看最近 7 天的事件历史，追踪触发时间和操作用户。例如，查看集群暂停时间或修改集群规格的操作者。
...
- [Serverless 集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 新增 [SQL Diagnosis](/tidb-cloud/tune-performance.md) 功能。
    ...
    你可深入了解 SQL 运行状态，提升 SQL 性能调优效率。目前 Serverless 集群仅提供慢查询数据。
...
- [TiDB Cloud 控制台](https://tidbcloud.com/) 支持 [控制台审计日志](/tidb-cloud/tidb-cloud-console-auditing.md)，可追踪组织成员的各类操作。
...
- [专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 支持缩容 TiKV 和 TiFlash 节点数量。
...
- [专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 新增 AWS 区域：`Seoul (ap-northeast-2)`。
...
- [Serverless 集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 的 SQL Editor（Beta）更名为 Chat2Query（Beta），并支持 AI 生成 SQL 查询。
  ...
  在 Chat2Query 中，你可让 AI 自动生成 SQL 查询，也可手动编写 SQL，无需终端即可对数据库执行查询。
...
- 支持为 2022 年 12 月 31 日后在 AWS 上创建的 TiDB Cloud 专属集群扩容 TiDB、TiKV、TiFlash 节点规格（vCPU + RAM）。

    可通过 [TiDB Cloud 控制台](/tidb-cloud/scale-tidb-cluster.md#change-vcpu-and-ram) 或 [TiDB Cloud API (beta)](https://docs.pingcap.com/tidbcloud/api/v1beta#tag/Cluster/operation/UpdateCluster) 操作。

- [**Monitoring**](/tidb-cloud/built-in-monitoring.md) 页的指标保留期延长至两天。

    你可访问近两天的指标数据，提升集群性能和趋势分析能力。
