...
- [TiDB Cloud Dedicated](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 支持从备份中恢复 SQL 绑定。
    ...
    TiDB Cloud Dedicated 现在在从备份恢复时，默认恢复用户账号和 SQL 绑定。该增强适用于 v6.2.0 及以上版本的集群，简化了数据恢复流程。SQL 绑定的恢复确保了查询相关配置和优化的顺利回归，为你提供更全面、高效的恢复体验。
...
- [TiDB Cloud Serverless](/tidb-cloud/select-cluster-tier.md#starter) 支持监控 SQL 语句的 RU 消耗。
    ...
    TiDB Cloud Serverless 现在可以详细展示每条 SQL 语句的 [Request Units (RUs)](/tidb-cloud/tidb-cloud-glossary.md#request-unit-ru) 消耗。你可以查看每条 SQL 语句的 **Total RU** 和 **Mean RU**。该功能有助于你识别和分析 RU 消耗，为运营成本优化提供参考。
...
- [数据迁移](/tidb-cloud/migrate-from-mysql-using-data-migration.md) 支持 Google Cloud 上部署的 TiDB 集群的高速物理模式。
    ...
    现在你可以在 AWS 和 Google Cloud 上部署的 TiDB 集群使用物理模式。物理模式的迁移速度可达 110 MiB/s，是逻辑模式的 2.4 倍。该性能提升适用于大数据集的快速迁移到 TiDB Cloud。
...
- [TiDB Cloud Dedicated](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 支持 Google Cloud 上的双区域备份（beta）。
    ...
    托管于 Google Cloud 的 TiDB Cloud Dedicated 集群可无缝对接 Google Cloud Storage。类似于 Google Cloud Storage 的 [Dual-regions](https://cloud.google.com/storage/docs/locations#location-dr) 功能，TiDB Cloud Dedicated 的双区域需选择同一多区域下的两个区域。例如，东京和大阪同属 `ASIA` 多区域，可共同用于双区域存储。
...
- [将数据变更日志流式写入 Apache Kafka](/tidb-cloud/changefeed-sink-to-apache-kafka.md) 功能现已正式 GA。
    ...
    经过 10 个月的 beta 试用，该功能现已正式发布。将 TiDB Cloud 的数据变更日志流式写入 Apache Kafka 是数据集成场景的常见需求。你可以通过 Kafka sink 集成其他数据处理系统（如 Snowflake）或支持业务消费。
...
- 支持 [TiDB Cloud Dedicated](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群在 AWS 上的 [双区域备份（beta）](/tidb-cloud/backup-and-restore.md#turn-on-dual-region-backup)。
    ...
    你现在可以在云服务商的不同地理区域间复制备份。该功能为数据保护和灾备能力提供了额外保障。
...
- 对 2023 年 8 月 15 日后在 AWS 上新建的 [TiDB Cloud Dedicated](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群，禁用负载均衡改进，包括：
    ...
    - 扩容 TiDB 节点时，禁用自动迁移现有连接到新节点。
    - 缩容 TiDB 节点时，禁用自动迁移现有连接到可用节点。

  该变更避免了混合部署场景下的资源争用，不影响已启用该功能的现有集群。如需为新集群启用负载均衡改进，请联系 [TiDB Cloud 支持](/tidb-cloud/tidb-cloud-support.md)。
...
- 支持在 [Postman](https://www.postman.com/) 中运行 Data App。
    ...
    Postman 集成支持你将 Data App 的接口作为集合导入到工作区，便于协作和无缝 API 测试，支持 Postman Web 和桌面应用。
...
- [TiDB Cloud Dedicated](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群新增 **Pausing** 状态，暂停期间无需付费。
    ...
    当你点击 **Pause** 时，集群会先进入 **Pausing** 状态，暂停完成后状态变为 **Paused**。
...
- TiDB Cloud [Data Service](https://tidbcloud.com/project/data-service) 推出自动生成接口功能。
    ...
    开发者现在可以通过极少的点击和配置，快速创建 HTTP 接口。无需重复编写样板代码，简化并加速接口创建，减少潜在错误。

    详情参见 [自动生成接口](/tidb-cloud/data-service-manage-endpoint.md#generate-an-endpoint-automatically)。

- TiDB Cloud [Data Service](https://tidbcloud.com/project/data-service) 支持 `PUT` 和 `DELETE` 请求方法。
...
- TiDB Cloud [Data Service](https://tidbcloud.com/project/data-service) 支持 `POST`、`PUT`、`DELETE` 方法的 **批量操作**。
...
- 优化 TiDB Cloud 用户访问 PingCAP 支持的入口，包括：
    ...
    - 优化 [TiDB Cloud 控制台](https://tidbcloud.com/) 右下角 **?** 菜单，使其更直观。
...
- 推出 TiDB Bot（beta），基于 OpenAI 的多语言智能助手，支持 7x24 实时响应和文档集成。
...
- [TiDB Cloud Serverless](/tidb-cloud/select-cluster-tier.md#starter) 集群支持 [分支功能（beta）](/tidb-cloud/branch-overview.md)。
    ...
    TiDB Cloud 支持为 Serverless 集群创建分支。分支是原集群数据的独立副本，提供隔离环境，便于自由实验而不影响原集群。
    ...
    如果你使用 GitHub 进行应用开发，可将分支集成到 CI/CD 流程，实现自动化测试而不影响生产库。详情参见 [将 TiDB Cloud Serverless 分支（Beta）集成到 GitHub](/tidb-cloud/branch-github-integration.md)。
...
- TiDB Cloud 控制台现支持暗黑模式，提供更舒适、护眼的体验。你可以在左侧导航栏底部切换明暗模式。
...
- [TiDB Cloud Dedicated](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群引入 Index Insight（beta），为慢查询提供索引优化建议，提升查询性能。
    ...
    使用 Index Insight，你可以通过以下方式提升数据库操作效率：
...
- 推出 [TiDB Playground](https://play.tidbcloud.com/?utm_source=docs&utm_medium=tidb_cloud_release_notes)，无需注册或安装即可体验 TiDB 全功能的交互式平台。
    ...
    立即体验，请访问 [**TiDB Playground**](https://play.tidbcloud.com/?utm_source=docs&utm_medium=tidb_cloud_release_notes) 页面，选择你想探索的功能，开始探索之旅。
...
- 为 Dedicated 集群 **Diagnosis** 页的以下两个标签页发布全新原生 Web 架构：
...
- 支持导入压缩文件。你可以导入 `.gzip`、`.gz`、`.zstd`、`.zst`、`.snappy` 格式的 CSV 和 SQL 文件，更高效、经济地导入数据，降低数据传输成本。
...
- [**Event**](/tidb-cloud/tidb-cloud-events.md) 页面新增事件类型，记录 [Dedicated Tier](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群的备份、恢复和 changefeed 操作。
    ...
    事件类型详情参见 [已记录事件](/tidb-cloud/tidb-cloud-events.md#logged-events)。
...
- 组织下前五个 [Serverless Tier](/tidb-cloud/select-cluster-tier.md#starter) 集群可享免费额度：
  ...
  2023 年 5 月 31 日前，Serverless Tier 集群全免费。之后超出免费额度部分将计费。

    你可以在集群 **Overview** 页的 **Usage This Month** 区域 [监控用量或提升额度](/tidb-cloud/manage-serverless-spend-limit.md)。免费额度用尽后，集群读写将被限流，直至提升额度或新月重置。
...
- [Dedicated Tier](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群支持维护窗口功能，便于你安排和管理计划性维护。
    ...
    维护期间可能出现短暂连接中断或 QPS 波动，但集群可用，SQL 操作、数据导入、备份、恢复、迁移和同步任务均可正常运行。详见 [维护期间允许与禁止的操作](/tidb-cloud/configure-maintenance-window.md#allowed-and-disallowed-operations-during-a-maintenance-window)。
...
- 优化 AWS 托管、2023 年 4 月 25 日后创建的 [Dedicated Tier](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群的 TiDB 负载均衡，缩容/扩容时减少连接中断。
...
- 全新 UI 优化 [集群创建](https://tidbcloud.com/clusters/create-cluster) 流程，更加友好，几步即可完成集群创建和配置。
...
- 优化 AWS 托管 [Dedicated Tier](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群的 TiDB 负载均衡，缩容/扩容时减少连接中断。
...
- [Dedicated Tier](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群支持 [New Relic](https://newrelic.com/) 集成。
    ...
    你可以将 TiDB 集群的监控数据发送到 [New Relic](https://newrelic.com/)，实现应用与数据库的统一监控和分析，快速定位和解决问题。
...
- 为 Dedicated Tier 集群的 Prometheus 集成新增以下 [changefeed](/tidb-cloud/changefeed-overview.md) 指标：
    ...
    已 [集成 Prometheus](/tidb-cloud/monitor-prometheus-and-grafana-integration.md) 的用户可实时监控 changefeed 性能和健康，并可基于这些指标创建告警。
...
- 优化 [Billing](/tidb-cloud/tidb-cloud-billing.md#billing-details) 页面，重组 **Summary by Project** 和 **Summary by Service**，账单信息更清晰。
...
- 支持将 AWS 区域 [Dedicated Tier](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群的增量数据实时同步到同项目同区域的 [Serverless Tier](/tidb-cloud/select-cluster-tier.md#starter) 集群。
...
- 支持为 2022 年 12 月 31 日后在 AWS 托管的 [Dedicated Tier](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群缩小 TiDB、TiKV、TiFlash 节点规格。
...
- [Dedicated Tier](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群新增 **Events** 页面，记录集群主要变更。
    ...
    你可以查看最近 7 天的事件历史，追踪触发时间、操作用户等重要信息。例如，查看集群暂停、规格变更等事件。
...
- 上传带表头的本地 CSV 文件到 [Serverless Tier](/tidb-cloud/select-cluster-tier.md#starter) 集群时，支持修改 TiDB Cloud 自动创建目标表的列名。
    ...
    如果表头列名不符合 TiDB Cloud 命名规范，将在对应列名旁显示警告图标。你可悬停查看提示并修改列名。
...
- 支持使用 [控制台审计日志](/tidb-cloud/tidb-cloud-console-auditing.md) 跟踪组织成员在 [TiDB Cloud 控制台](https://tidbcloud.com/) 的各类操作。
    ...
    仅 `Owner` 或 `Audit Admin` 角色可见，默认关闭。开启方法：右上角 <MDSvgIcon name="icon-top-organization" /> **Organization** > **Console Audit Logging**。
...
- [TiDB Cloud CLI](/tidb-cloud/cli-reference.md) 新增 `ticloud cluster connect-info` 命令。
    ...
    该命令可获取集群连接字符串。需将 `ticloud` 升级至 v0.3.2 或更高版本，详见 [升级 ticloud](/tidb-cloud/ticloud-upgrade.md)。

## 2023 年 2 月 21 日

**通用变更**

- 支持使用 IAM 用户的 AWS access key 访问 Amazon S3 bucket 导入数据。

    该方式比 Role ARN 更简单。详情参见 [配置 Amazon S3 访问](/tidb-cloud/dedicated-external-storage.md#configure-amazon-s3-access)。

- 监控指标保留期由 2 天延长：

    - Dedicated Tier 集群可查看近 7 天数据
    - Serverless Tier 集群可查看近 3 天数据
...
- [TiDB Cloud CLI](/tidb-cloud/cli-reference.md) 新增 [`ticloud connect`](/tidb-cloud/ticloud-serverless-shell.md) 命令。
    ...
    该命令支持你无需安装 SQL 客户端，直接从本地连接 TiDB Cloud 集群并执行 SQL。
...
- 优化 [Serverless Tier](/tidb-cloud/select-cluster-tier.md#starter) 集群本地 CSV 文件导入体验。
...
- [Serverless Tier](/tidb-cloud/select-cluster-tier.md#starter) 集群将 SQL Editor (beta) 重命名为 Chat2Query (beta)，并支持 AI 生成 SQL 查询。
  ...
  在 Chat2Query 中，你可以让 AI 自动生成 SQL 查询，也可手动编写 SQL，无需终端即可对数据库执行查询。
...
- 支持为 2022 年 12 月 31 日后在 AWS 托管的 TiDB Cloud Dedicated 集群扩容 TiDB、TiKV、TiFlash 节点（提升 **Node Size(vCPU + RAM)**）。

    你可以通过 [TiDB Cloud 控制台](/tidb-cloud/scale-tidb-cluster.md#change-vcpu-and-ram) 或 [TiDB Cloud API (beta)](https://docs.pingcap.com/tidbcloud/api/v1beta#tag/Cluster/operation/UpdateCluster) 扩容节点。

- [**Monitoring**](/tidb-cloud/built-in-monitoring.md) 页的指标保留期延长至两天。

    你现在可以访问近两天的指标数据，更灵活地分析集群性能和趋势。
...
- 所有 [Serverless Tier](/tidb-cloud/select-cluster-tier.md#starter) 集群的默认 TiDB 版本由 [v6.3.0](https://docs.pingcap.com/tidb/v6.3/release-6.3.0) 升级至 [v6.4.0](https://docs.pingcap.com/tidb/v6.4/release-6.4.0)。升级后已解决 Serverless Tier 集群的冷启动问题。
