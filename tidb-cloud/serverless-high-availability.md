...
summary: {{{ .starter }}} と Essential の高可用性アーキテクチャについて学びましょう。ゾーンおよびリージョンの高可用性オプション、自動バックアップ、フェイルオーバープロセス、そして TiDB がデータの耐久性とビジネス継続性を確保する仕組みについてご紹介します。
---

# {{{ .starter }}} と Essential の高可用性 {#high-availability-in-starter-and-essential}
...
-   **ゾーン高可用性（デフォルト）** ：このオプションでは、すべてのノードを単一のアベイラビリティゾーン内に配置し、ネットワークレイテンシーを削減します。ゾーンをまたがるアプリケーションレベルの冗長性を必要とせずに高可用性を確保するため、単一ゾーン内での低レイテンシーを優先するアプリケーションに適しています。ゾーン高可用性は、{{{ .starter }}} または {{{ .essential }}} をサポートするすべてのクラウドリージョンでデフォルトで利用できます。詳細については、 [ゾーン高可用性アーキテクチャ](#zonal-high-availability-architecture)参照してください。
...
> **注記：**
>
> ゾーン高可用性はデフォルトのオプションであり、{{{ .starter }}} または {{{ .essential }}} をサポートするすべてのクラウド リージョンで利用できます。
...
>
> -   地域的な高可用性は現在ベータ版です。
> -   {{{{ .essential }}} クラスターを作成するときに、リージョンの高可用性を有効にできます。
