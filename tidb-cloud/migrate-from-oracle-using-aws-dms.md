...
本文档描述了如何使用 AWS Database Migration Service (AWS DMS) 将数据从 Amazon RDS for Oracle 迁移到 [{{{ .starter }}}](/tidb-cloud/select-cluster-tier.md#starter) 的分步示例。

> **Tip:**
>
> 除了 {{{ .starter }}} 实例外，本文档中的步骤同样适用于 {{{ .essential }}} 实例。
...
## 步骤 4. 创建 {{{ .starter }}} 实例 {#step-4-create-a-starter-instance}

1. 登录 [TiDB Cloud 控制台](https://tidbcloud.com/)。

2. [创建 {{{ .starter }}} 实例](/tidb-cloud/tidb-cloud-quickstart.md)。

3. 在 [**My TiDB**](https://tidbcloud.com/tidbs) 页面，点击目标 {{{ .starter }}} 实例名称进入其概览页面。
...
> **Note:**
>
> 关于创建可用于 {{{ .starter }}} 的 AWS DMS 复制实例的详细步骤，请参阅 [连接 AWS DMS 到 TiDB Cloud](/tidb-cloud/tidb-cloud-connect-aws-dms.md)。
...
> **Note:**
>
> 关于创建 {{{ .starter }}} DMS 端点的详细步骤，请参阅 [连接 AWS DMS 到 TiDB Cloud](/tidb-cloud/tidb-cloud-connect-aws-dms.md)。
...
连接到 [{{{ .starter }}} 实例](https://tidbcloud.com/tidbs)，检查 `admin.github_event` 表中的数据。如下面截图所示，DMS 已成功迁移表 `github_events` 及 10000 行数据。
