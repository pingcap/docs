...
This document describes a step-by-step example of how to migrate data from Amazon RDS for Oracle to [{{{ .starter }}}](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) using AWS Database Migration Service (AWS DMS).
...
2. Set up the target [{{{ .starter }}}](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) cluster.
...
3. On the [**TiDBs**](https://tidbcloud.com/tidbs) page, click the target cluster name to go to its overview page.
...
Connect to the [{{{ .starter }}}](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) cluster and check the `admin.github_event` table data. As shown in the following screenshot, DMS successfully migrated table `github_events` and 10000 rows of data.
