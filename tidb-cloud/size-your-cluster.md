...
在规划集群规模时，你可以根据你的负载类型、整体期望性能（QPS）以及对应负载类型下单个 TiDB 节点的性能，使用以下公式估算 TiDB 节点数：
...
例如，你的整体期望性能为 110,000 QPS，负载类型为混合，P95 延迟约为 100 ms，并且你希望使用 8 vCPU, 16 GiB 的 TiDB 节点。你可以从上表中获得 8 vCPU, 16 GiB TiDB 节点的预估性能（即 `15,500`），并按如下方式计算大致的 TiDB 节点数：
...
在规划集群规模时，你可以根据你的负载类型、整体期望性能（QPS）以及对应负载类型下单个 TiKV 节点的性能，使用以下公式估算 TiKV 节点数：

`node count = ceil(overall expected performance ÷ performance per node * (1 - performance deviation coefficient))`

在该公式中，你需要先计算 `node count = ceil(overall expected performance ÷ performance per node)` 得到一个大致的节点数，然后再结合对应的性能偏差系数，得到最终的节点数结果。

例如，你的整体期望性能为 110,000 QPS，负载类型为混合，P95 延迟约为 100 ms，并且你希望使用 8 vCPU, 32 GiB 的 TiKV 节点。你可以从上表中获得 8 vCPU, 32 GiB TiKV 节点的预估性能（即 `17,800`），并按如下方式计算大致的 TiKV 节点数：
