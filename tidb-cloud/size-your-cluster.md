...
通常情况下，TiDB 的性能会随着 TiDB 节点数量的增加而线性提升。但当 TiDB 节点数量超过 8 时，性能提升会略低于线性增长。每增加 8 个节点，性能偏差系数约为 5%。
...
如果 TiDB 节点数量小于 8，性能偏差系数几乎为 0%，因此 16 vCPU, 32 GiB TiDB 节点的性能大致是 8 vCPU, 16 GiB TiDB 节点的两倍。如果 TiDB 节点数量超过 8，建议选择 16 vCPU, 32 GiB TiDB 节点，这样所需节点更少，性能偏差系数也更小。
...
例如，你的整体期望性能为 110,000 QPS，负载为混合型，P95 延迟约为 100 ms，并且希望使用 8 vCPU, 16 GiB TiDB 节点。你可以从上表中获得 8 vCPU, 16 GiB TiDB 节点的预估性能（即 `15,500`），然后计算大致的 TiDB 节点数量如下：
...
与 TiDB 性能类似，TiKV 的性能会随着 TiKV 节点数量的增加而线性提升。但当 TiKV 节点数量超过 8 时，性能提升会略低于线性增长。每增加 8 个节点，性能偏差系数约为 5%。
...
如果 TiKV 节点数量小于 8，性能偏差系数几乎为 0%，因此 16 vCPU, 64 GiB TiKV 节点的性能大致是 8 vCPU, 32 GiB TiKV 节点的两倍。如果 TiKV 节点数量超过 8，建议选择 16 vCPU, 64 GiB TiKV 节点，这样所需节点更少，性能偏差系数也更小。
...
例如，你的整体期望性能为 110,000 QPS，负载为混合型，P95 延迟约为 100 ms，并且希望使用 8 vCPU, 32 GiB TiKV 节点。你可以从上表中获得 8 vCPU, 32 GiB TiKV 节点的预估性能（即 `17,800`），然后计算大致的 TiKV 节点数量如下：
...
接下来，你可以将根据数据量和期望性能分别计算出的 TiKV 节点数量进行比较，取较大的值作为推荐的 TiKV 节点数量。
...
标准存储类型会自动应用于托管在 AWS 上、使用 TiDB v7.5.5、v8.1.2、v8.5.0 或更高版本创建的新集群。

#### 高性能和 Plus 存储

高性能和 Plus 存储提供更高的性能和稳定性，价格也会相应提升。目前，这两种存储类型仅可按需为部署在 AWS 上的集群申请。如需申请高性能或 Plus 存储，请在 [TiDB Cloud 控制台](https://tidbcloud.com) 右下角点击 **?**，然后点击 **Request Support**。在 **Description** 字段填写 "Apply for TiKV storage type"，并点击 **Submit**。
...
Plus 存储提供更高的性能和稳定性，价格也会相应提升。目前，该存储类型仅可按需为部署在 AWS 上的集群申请。如需申请，请在 [TiDB Cloud 控制台](https://tidbcloud.com) 右下角点击 **?**，然后点击 **Request Support**。在 **Description** 字段填写 "Apply for TiFlash storage type"，并点击 **Submit**。