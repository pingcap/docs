...
4. 按如下方式配置备份计划：
    ...
    - 在 **Backup Cycle** 中，点击 **Daily Backup** 或 **Weekly Backup** 标签。若选择 **Weekly Backup**，需指定每周的备份日期。
        ...
        > - 若将备份周期从每周切换为每日，Point-in-time Restore 功能会保持原有设置。如有需要可手动关闭。
    ...
    - 在 **Backup Retention** 中，配置最小备份数据保留周期。默认周期为 7 天。为尽量减少对业务的影响，建议在业务低峰期安排自动备份。
        ...
        > **注意**
        >
        > - 除最新备份外，所有超出保留周期的自动备份将被删除。最新的自动备份不会被删除，除非你手动删除它。这样可以确保在意外删除时仍可恢复集群数据。
...
> - 托管在 Google Cloud 上的 TiDB Cloud 专属集群可无缝对接 Google Cloud Storage。与 Google Cloud Storage 类似，**TiDB Cloud 专属集群仅支持在与 Google 双区域存储相同的多区域代码内进行双区域配对**。例如，在亚洲，目前必须将东京和大阪配对实现双区域存储。更多信息请参见 [Dual-regions](https://cloud.google.com/storage/docs/locations#location-dr)。

TiDB Cloud 专属集群支持通过将备份从集群所在区域复制到另一个不同区域，实现双区域备份。开启该功能后，所有备份会自动复制到指定区域。这为数据提供了跨区域保护和灾备能力。预计约 99% 的数据可在一小时内复制到次级区域。
...
> **注意**
>
> - 手动备份会被无限期保留，除非你手动删除或账户被关闭。
> - 删除 TiDB Cloud 专属集群后，现有的手动备份会被移入回收站，直至被手动删除或账户关闭。
...
6. 在 **Summary** 部分检查恢复信息，然后点击 **Restore**。
