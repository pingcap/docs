...
> **提示**
>
> 如需了解如何在 {{{ .starter }}} 或 {{{ .essential }}} 集群上备份和恢复数据，请参见 [在 {{{ .starter }}} 或 Essential 集群上备份与恢复数据](/tidb-cloud/backup-and-restore-serverless.md)。

## 限制

- 对于 v6.2.0 及以上版本的集群，TiDB Cloud 专属集群默认支持从备份中恢复用户账户和 SQL 绑定。
- TiDB Cloud 专属集群不支持恢复存储在 `mysql` schema 中的系统变量。
- 建议你先导入数据，再进行**手动**快照备份，最后开启 Point-in-time Restore。因为通过 TiDB Cloud 控制台导入的数据**不会**生成变更日志，无法被自动检测和备份。更多信息请参见 [从云存储导入 CSV 文件到 TiDB Cloud 专属集群](/tidb-cloud/import-csv-files.md)。
- 如果你多次开启和关闭 Point-in-time Restore，只能选择最近一次开启后可恢复范围内的时间点，早期的可恢复范围将无法访问。
...
> - 托管在 Google Cloud 上的 TiDB Cloud 专属集群可无缝对接 Google Cloud Storage。与 Google Cloud Storage 类似，**TiDB Cloud 专属集群仅支持在与 Google 双区域存储相同的多区域代码内进行双区域配对**。例如，在亚洲，目前必须将东京和大阪配对用于双区域存储。更多信息请参见 [Dual-regions](https://cloud.google.com/storage/docs/locations#location-dr)。

TiDB Cloud 专属集群支持通过将备份从集群所在区域复制到另一个不同区域，实现双区域备份。开启该功能后，所有备份会自动复制到指定区域，从而提供跨区域数据保护和灾备能力。预计约 99% 的数据可在一小时内复制到次级区域。
...
> **注意**
>
> 关闭自动备份会默认同时关闭 point-in-time restore。
...
> - 删除 TiDB Cloud 专属集群后，现有的手动备份会被移至回收站，直至手动删除或账户关闭。
