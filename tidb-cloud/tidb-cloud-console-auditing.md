...
控制台审计日志功能默认处于禁用状态。启用后，TiDB Cloud 控制台中所有受支持的事件类型都会被审计，并且你无法将其配置为仅审计特定事件类型。要启用该功能，请按照以下步骤操作：
...
## 控制台审计日志完整性和访问控制 {#console-audit-log-integrity-and-access-control}

为帮助你满足监管合规要求，TiDB Cloud 为控制台审计日志提供以下控制：

- **完整性**：为你的组织启用控制台审计日志后，审计系统会记录触发[受支持事件类型](#console-audit-event-types)的用户操作。日志条目通常会在事件发生后的 60 分钟内可供查看、下载以及通过 API 进行程序化访问。

    > **注意：**
    >
    > 控制台审计日志会记录启用后发生的所有受支持事件。在启用审计日志之前执行的操作不会被记录。TiDB Cloud 会持续扩展受审计事件类型的覆盖范围。

- **访问控制**：只有你组织中具有 `Organization Owner` 或 `Organization Console Audit Manager` 角色的用户才能管理你组织的控制台审计日志。其他组织成员无法启用或禁用审计日志，也无法修改日志设置。
