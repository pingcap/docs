...
- 每个 TiDB Cloud Essential instance 最多可创建 10 个 changefeed。
...
确保你的 TiDB Cloud Essential instance 能够连接到 MySQL service。你可以选择以下任一连接方式：
...
**Sink to MySQL** 连接器只能将 TiDB Cloud Essential instance 中某一时间点之后的增量数据同步到 MySQL。如果你的 TiDB Cloud Essential instance 中已经存在数据，可以在启用 **Sink to MySQL** 之前，将已有数据导出并加载到 MySQL。
...
2. 使用 [Export](/tidb-cloud/serverless-export.md) 功能从 TiDB Cloud Essential instance 导出数据，然后使用社区工具如 [mydumper/myloader](https://centminmod.com/mydumper.html) 将数据加载到 MySQL service。
...
1. 进入目标 TiDB Cloud Essential instance 的概览页面，在左侧导航栏点击 **Data** > **Changefeed**。
...
8. 在 **Start Replication Position** 中，配置 MySQL sink 的起始位置。
    ...
    - 如果上游 TiDB Cloud Essential instance 没有任何数据，选择 **Start replication from now on**。
