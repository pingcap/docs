...
本文档介绍如何使用 [阿里云 Endpoint Service 私有链路连接](/tidb-cloud/serverless-private-link-connection.md)，将 TiDB Cloud Essential 实例连接到阿里云自建 Kafka 集群。
...
如需部署新的 Kafka 集群，请参考 [部署新 Kafka 集群](#deploy-a-new-kafka-cluster)。

如需暴露已有的 Kafka 集群，请参考 [重配置运行中的 Kafka 集群](#reconfigure-a-running-kafka-cluster)。
...
确保你的 Kafka 集群部署在与 TiDB Cloud Essential 实例相同的 Region 和 AZ。如果有 broker 在不同 AZ，请迁移到正确的 AZ。
