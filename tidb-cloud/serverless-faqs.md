...
{{{ .starter }}} 中的列存储作为行存储的额外副本，确保强一致性。与按行存储数据的传统行存储不同，列存储按列组织数据，从而针对数据分析任务进行了优化。

列存储是实现 TiDB 混合事务和分析处理（HTAP）能力的关键特性，它能够无缝融合事务型和分析型工作负载。

为了高效管理列存储数据，{{{ .starter }}} 使用独立的弹性 TiFlash 引擎。在查询执行期间，优化器会引导 {{{ .starter }}} 实例自动决定是从行存储还是列存储中检索数据。
...
### 在 TiDB Cloud 中我最多可以创建多少个 {{{ .starter }}} 实例？

对于 TiDB Cloud 中的每个组织，默认最多可以创建五个 [{{{ .starter }}}](/tidb-cloud/select-cluster-tier.md#starter) 实例。要创建更多 {{{ .starter }}} 实例，你需要添加信用卡并为使用量设置[消费上限](/tidb-cloud/manage-serverless-spend-limit.md)。
...
### 在 {{{ .starter }}} 可用之前，我创建了一个 Developer Tier 集群。我还能继续使用我的 {{{ .starter }}} 实例吗？

可以，你的 Developer Tier 集群已自动迁移为 {{{ .starter }}} 实例，在不影响你之前使用的情况下提供了更好的用户体验。
...
当你的 {{{ .starter }}} 实例超过最大并发连接数限制时，会出现该错误。详情请参见 [连接限制](/tidb-cloud/serverless-limitations.md#connection)。

### 为什么在导入数据时我会观察到 RU 使用量激增？

在 {{{ .starter }}} 实例的数据导入过程中，只有在数据成功导入时才会产生 RU 消耗，因此会导致 RU 使用量激增。

### 什么是 Request Units？

{{{ .starter }}} 采用按需付费模式，即你只需为存储空间和 {{{ .starter }}} 实例使用量付费。在该模式下，所有 {{{ .starter }}} 实例活动（如 SQL 查询、大批量操作和后台作业）都以 [Request Units (RU)](/tidb-cloud/tidb-cloud-glossary.md#request-unit-ru) 进行量化。RU 是对在 {{{ .starter }}} 实例上发起的请求规模和复杂度的抽象度量。详情请参见 [{{{ .starter }}} 价格详情](https://www.pingcap.com/tidb-cloud-starter-pricing-details/)。
...
如果为某个 TiDB Cloud Starter 实例设置了每月消费上限，超出免费额度的部分将会计费。对于免费的 TiDB Cloud Starter 实例，达到免费额度后，该实例的读写操作将被限流，直到你设置每月消费上限或新月开始时用量重置。

详情请参见 [TiDB Cloud Starter 使用额度](/tidb-cloud/select-cluster-tier.md#usage-quota)。

### 免费计划有哪些限制？

在免费计划下，{{{ .starter }}} 实例性能受限于不可扩展的资源。每个查询的内存分配被限制为 256 MiB，并且每秒的 RU 可能出现瓶颈。为最大化 {{{ .starter }}} 实例性能并避免这些限制，你可以为 {{{ .starter }}} 实例[设置每月消费上限](/tidb-cloud/manage-serverless-spend-limit.md)。
...
要查看 {{{ .starter }}} 实例的 RU 和存储用量，请进入 {{{ .starter }}} 实例的概览页面，然后查看 **Usage this month** 面板。结合历史资源用量和该面板的实时用量数据，你可以跟踪 {{{ .starter }}} 实例的资源消耗并合理预估消费上限。如果免费额度无法满足需求，可以编辑消费上限以获取更多资源。详情请参见 [{{{ .starter }}} 使用额度](/tidb-cloud/select-cluster-tier.md#usage-quota)。

### 如何优化我的负载以减少 RU 消耗？

请确保你的查询已根据 [优化 SQL 性能](/develop/dev-guide-optimize-sql-overview.md) 指南进行优化，以获得最佳性能。要识别消耗 RU 最多的 SQL 语句，可进入 {{{ .starter }}} 实例的 [**诊断**](/tidb-cloud/tune-performance.md#view-the-diagnosis-page) 页面，查看 **SQL Statements** 标签页，在此可观察 SQL 执行情况，并按 **Total RU** 或 **Mean RU** 排序查看高消耗语句。详情请参见 [语句分析](/tidb-cloud/tune-performance.md#statement-analysis)。此外，减少出口流量同样有助于降低 RU 消耗。建议在查询中仅返回必要的列和行，从而减少网络出口流量。通过精确选择和过滤返回的列与行，可以优化网络利用率。
...
### 当我的 {{{ .starter }}} 实例用尽免费额度或超过消费上限时，会发生什么？

一旦 {{{ .starter }}} 实例达到其免费额度或消费上限，该实例会立即拒绝任何新的连接尝试，直到额度提高或在新月开始时用量重置。达到额度之前已建立的现有连接将保持活动状态，但会受到限流。详情请参见 [{{{ .starter }}} 限制与配额](/tidb-cloud/serverless-limitations.md#usage-quota)。
...
- TiDB Cloud Starter 上所有持久化数据均采用 {{{ .starter }}} 实例所在云服务商的工具进行静态加密。

## 运维常见问题

### 我可以升级 {{{ .starter }}} 实例运行的 TiDB 版本吗？

不可以。随着我们在 TiDB Cloud 上推出新的 TiDB 版本，{{{ .starter }}} 实例会自动升级。你可以在 [TiDB Cloud 控制台](https://tidbcloud.com/tidbs) 或最新的[发布说明](https://docs.pingcap.com/tidbcloud/tidb-cloud-release-notes)中查看你的 {{{ .starter }}} 实例正在运行的 TiDB 版本。或者，你也可以连接到 {{{ .starter }}} 实例，并使用 `SELECT version()` 或 `SELECT tidb_version()` 来检查 TiDB 版本。
