...
- [TiDB Cloud Serverless](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 针对大数据写入场景，最多可降低 80% 的成本，适用于以下场景：
...
- 新增指标：为 Prometheus 集成添加 `tidbcloud_changefeed_checkpoint_ts`。
    ...
    该指标用于跟踪 changefeed 的检查点时间戳，表示已成功写入下游的最大 TSO（Timestamp Oracle）。更多可用指标信息，参见 [将 TiDB Cloud 集成到 Prometheus 和 Grafana（Beta）](/tidb-cloud/monitor-prometheus-and-grafana-integration.md#metrics-available-to-prometheus)。
