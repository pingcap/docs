...
- 新增指标：为 Prometheus 集成添加 `tidbcloud_changefeed_checkpoint_ts`。
    ...
    该指标用于跟踪 changefeed 的检查点时间戳，表示已成功写入下游的最大 TSO（时间戳 Oracle）。更多可用指标信息，参见 [将 TiDB Cloud 集成到 Prometheus 和 Grafana（测试版）](/tidb-cloud/monitor-prometheus-and-grafana-integration.md#metrics-available-to-prometheus)。
...
- [TiDB Cloud Dedicated](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 在 AWS 上的负载均衡计费变更。
    ...
    自 2024 年 8 月 1 日起，TiDB Cloud Dedicated 账单将包含 AWS 公共 IPv4 地址的新费用，与 [AWS 自 2024 年 2 月 1 日起的定价变更](https://aws.amazon.com/blogs/aws/new-aws-public-ipv4-address-charge-public-ip-insights/) 保持一致。每个公共 IPv4 地址的费用为 $0.005/小时，每个托管在 AWS 上的 TiDB Cloud Dedicated 集群每月约增加 $10。
...
- 将 Chat2Query（测试版）重命名为 SQL Editor（测试版）。
    ...
    原 Chat2Query 界面现更名为 SQL Editor。此变更明确了手动 SQL 编辑与 AI 辅助查询生成的区别，提升了可用性和整体体验。
