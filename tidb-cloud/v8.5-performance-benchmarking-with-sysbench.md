...
このドキュメントでは、TiDBバージョン8.5.0を搭載したTiDB Cloud DedicatedクラスターのSysbenchパフォーマンステストの手順と結果について説明します。このレポートは、TiDB Self-Managedバージョン8.5.0クラスターのパフォーマンスの参考資料としてもご利用いただけます。
