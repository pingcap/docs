...
> **注記：**
>
> 現在、エンドポイント占有モデルは、特定のリージョンで 2026 年 7 月 1 日以降に作成された {{{ .essential }}} インスタンスでのみ使用できます。お使いのインスタンスで利用できない場合は、代わりに[endpoint shared model](#connect-via-a-public-endpoint-endpoint-shared-model)を使用できます。
