...
二级索引是 TiDB 中的一个逻辑对象。你可以简单地将其视为 TiDB 用来提高查询性能的一种数据排序方式。在 TiDB 中，创建二级索引是一个在线操作，不会阻塞表上的任何数据读写操作。对于每个索引，TiDB 为表中的每一行创建引用，并按选定的列对引用进行排序，而不是直接对数据进行排序。
...
向量索引依赖 TiFlash 副本进行数据存储和搜索功能。如果你使用的是 TiDB Cloud Dedicated 集群，请在创建或使用向量索引之前确保集群中有可用的 TiFlash 节点。更多信息，请参见[向量搜索索引](/ai/reference/vector-search-index.md)。
