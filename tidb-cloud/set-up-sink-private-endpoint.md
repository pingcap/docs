...
> **注意：**
>
> - 私有端点是项目级别的资源，不绑定到特定集群。在同一项目和同一区域内创建的私有端点可以由多个连接到同一下游服务的 TiDB Cloud Dedicated 集群共享，因此你无需为每个集群单独创建端点。
> - 删除集群不会删除其私有端点。如果某个私有端点连续 30 天未被任何集群使用，即使之前使用它的集群仍然可用，该私有端点也会被自动删除。你也可以在不再需要时手动删除私有端点。但是，当它仍被任何集群使用时，你无法删除它。
