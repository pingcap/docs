...
> **Note:**
>
> - プライベートエンドポイントはプロジェクトレベルのリソースであり、特定のクラスターには関連付けられていません。同じプロジェクト内かつ同じリージョン内で作成されたプライベートエンドポイントは、同じダウンストリームサービスに接続する複数の TiDB Cloud Dedicated クラスターで共有できるため、クラスターごとに個別のエンドポイントを作成する必要はありません。
> - クラスターを削除しても、そのプライベートエンドポイントは削除されません。プライベートエンドポイントは、以前に使用していたクラスターが引き続き利用可能であっても、30 日間どのクラスターからも使用されていない場合は自動的に削除されます。不要になったプライベートエンドポイントは手動で削除することもできます。ただし、いずれかのクラスターで使用中の場合は削除できません。
...
7. **TiDB Managed**ドメインまたは**カスタム**ドメインのいずれかを使用して、Kafka のアドバタイズされたリスナーを構成します。
    ...
    - アドバタイズされたリスナーに独自の**Custom**ドメインを使用するには、ドメイン タイプを**Custom**に切り替え、 **Custom Domain**フィールドにルート ドメインを入力し、**Check**をクリックして、各アベイラビリティゾーンのブローカー サブドメインを指定します。
