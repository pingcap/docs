...
summary: このドキュメントは、TiDBのデータベーススキーマ設計の基本について説明します。TiDBのオブジェクト、アクセス制御、データベーススキーマの変更、オブジェクトの制限などについて説明します。データベースの設計や操作方法についての例も提供されています。データベースはテーブルやインデックスなどのオブジェクトのコレクションであり、テーブルは関連するデータをまとめたものです。インデックスを使用するとクエリのパフォーマンスが向上し、TiDBはユーザーベースとロールベースの両方のアクセス制御をサポートします。データベーススキーマの変更を実行する際には、MySQLクライアントまたはGUIクライアントを使用することが推奨されています。詳細についてはTiDBの制限事項を参照してください。
---
