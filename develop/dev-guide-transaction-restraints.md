...
- 示例 1：**事务 A** 首先根据查询获取了 `n` 行数据，然后 **事务 B** 修改了这 `n` 行之外的 `m` 行，或者新增了 `m` 行满足 **事务 A** 查询条件的数据。当 **事务 A** 再次执行该查询时，发现有 `n+m` 行数据满足条件。就像出现了“幻影”一样，因此称为 **幻读**。

- 示例 2：**管理员 A** 将数据库中所有学生的成绩从具体分数改为 ABCDE 等级，但此时 **管理员 B** 插入了一条带有具体分数的记录。当 **管理员 A** 修改完后，发现还有一条记录（即 **管理员 B** 插入的那条）没有被修改。这也是 **幻读**。

## SI 无法避免写偏差

TiDB 的 SI 隔离级别无法避免 **写偏差**（write skew）异常。你可以使用 `SELECT FOR UPDATE` 语法来避免 **写偏差** 异常。

**写偏差** 异常发生在两个并发事务分别读取了不同但相关的记录，然后每个事务都更新了自己读取到的数据并最终提交事务。如果这些相关记录之间存在约束，要求不能被多个事务同时修改，那么最终结果就会违反该约束。
...
在两个事务中，应用程序首先检查是否有两名或以上医生值班，如果是，则认为可以安全地让一名医生请假。由于数据库使用的是快照隔离，两个检查都返回 `2`，因此两个事务都进入下一阶段。`Alice` 将自己的记录更新为不值班，`Bob` 也做了同样的操作。两个事务都成功提交。现在没有医生值班，违反了至少有一名医生值班的要求。下图（引自 **_Designing Data-Intensive Applications_**）展示了实际发生的情况。
...
**Spring** 支持的 `PROPAGATION_NESTED` 传播行为会触发一个嵌套事务，即独立于当前事务启动的子事务。嵌套事务开始时会记录一个 `savepoint`。如果嵌套事务失败，事务会回滚到 `savepoint` 状态。嵌套事务属于外部事务的一部分，并会与外部事务一起提交。
...
基本原则是限制事务的大小。在 KV 层面，TiDB 对单个事务的大小有限制。在 SQL 层面，一行数据映射为一个 KV entry，每增加一个索引会多一个 KV entry。SQL 层面的限制如下：

- 单行记录的最大大小为 120 MiB。

    - 你可以通过 [`performance.txn-entry-size-limit`](https://docs.pingcap.com/tidb/stable/tidb-configuration-file#txn-entry-size-limit-new-in-v4010-and-v500) 配置参数调整（适用于 TiDB v4.0.10 及后续 v4.0.x 版本、TiDB v5.0.0 及更高版本）。v4.0.10 之前的版本该值为 `6 MB`。
...
需要注意的是，无论是大小限制还是行数限制，在事务执行过程中还要考虑编码和事务附加 key 的开销。为了获得最佳性能，建议每 100 ~ 500 行写入一个事务。
...
这是与 MySQL 的已知兼容性差异。你可以通过显式使用 `BEGIN;COMMIT;` 语句来解决该问题。
