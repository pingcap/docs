...
The following explains **phantom reads**: **Transaction A** first gets `n` rows according to the query, and then **Transaction B** changes `m` rows other than these `n` rows or adds `m` rows that match the query of **Transaction A**. When **Transaction A** starts the request again, it finds that there are `n+m` rows that match the condition. That is a **phantom read**.

For example, **Admin A** changes the grades of all students in the database from specific scores to ABCDE grades, but **Admin B** inserts a record with a specific score at this time. When **Admin A** finishes changing and finds that there is still a record (the one inserted by **Admin B**) that has not been changed yet. It is like a phantom, so it is called a **phantom read**.
...
Now there is a situation where doctors `Alice` and `Bob` are on call. Both of them are feeling sick, so they decide to take time off. They happen to click the button at the same time. Let's simulate this process with the following program:
...
This is a known incompatibility with MySQL. You can solve this issue by using the explicit `BEGIN;COMMIT;`.
