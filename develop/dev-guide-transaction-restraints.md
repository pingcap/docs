...
-   1行あたりのレコードの最大サイズは120MiBです。
    ...
    -   TiKV も単一の書き込みリクエストのデータサイズを制限していることに注意してください。単一の書き込みリクエストのデータサイズがデフォルトで `8 MiB` の [`raftstore.raft-entry-max-size`](/tikv-configuration-file.md#raft-entry-max-size) を超えると、TiKV はそのリクエストを拒否します。単一行が大きい場合は、TiDB の `tidb_txn_entry_size_limit` と TiKV の `raftstore.raft-entry-max-size` の両方を調整する必要があります。
