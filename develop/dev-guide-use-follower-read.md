...
-   TiDB セルフマネージド: [TiDB Dashboardのキービジュアライザー](/dashboard/dashboard-key-visualizer.md)に移動し、「メトリック選択ボックス」を`Read (bytes)`または`Read (keys)`に選択して、読み取りホットスポットが発生するかどうかを確認します。
...
-   [TiDBセルフマネージド向けTiDB Dashboardのキービジュアライザー](/dashboard/dashboard-key-visualizer.md)
