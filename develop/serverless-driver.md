...
| `decoders`   | 物体   | `{}`      | キーと値のペアの集合で、さまざまな列タイプに合わせてデコード処理をカスタマイズできます。各ペアでは、キーとして列タイプを指定し、値として対応する関数を指定できます。この関数は、TiDB Cloud Serverless Driverから受け取った生の文字列値を引数として受け取り、デコードされた値を返します。 |
