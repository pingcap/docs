...
In the [Create Database](/develop/dev-create-table.md#using-htap-capabilities) chapter, we have introduced how to enable the HTAP capability of TiDB. Below we'll take a closer look at how to use HTAP capabilities to analyze data faster.

## Data preparation

Before starting, you can import much larger sample data [via the `tiup demo` command](/develop/dev-bookshop-schema-design.md#method-1-through-tiup-demo-command-line), for example:
...
For example, in the previous [Pagination Query](/develop/dev-paginate-results.md) chapter, we have introduced how to use the `row_number()` function to achieve efficient pagination batch processing.

## Hybrid workload

### Enable column replica

TiDB's default storage engine, TiKV, is row-stored. Before proceeding to the next steps, you can read the section [Enable HTAP Capability](/develop/dev-create-table.md#using-htap-capabilities) and add a TiFlash column-stored replica of the `books` and `orders` tables using the following SQL.
...
> 2. Also, setting read_from_storage Hint for [common table expression](/develop/dev-use-common-table-expression.md) does not work.
