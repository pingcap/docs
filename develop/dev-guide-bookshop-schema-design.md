...
- 20 万行用户信息（通过 `--users` 参数）
- 50 万行图书信息（通过 `--books` 参数）
- 10 万行作者信息（通过 `--authors` 参数）
- 100 万行评分记录（通过 `--ratings` 参数）
- 100 万行订单记录（通过 `--orders` 参数）
...
3. 在 **Import Data from Amazon S3** 页面，配置以下源数据信息：
    ...
    - **Import File Count**：对于 {{{ .starter }}}，选择 **Multiple files**。在 TiDB Cloud Dedicated 中此项不可用。
...
| 字段名     | 类型         | 描述                         |
|------------|--------------|------------------------------|
| id         | bigint(20)   | 作者的唯一 ID                |
| name       | varchar(100) | 作者姓名                     |
| gender     | tinyint(1)   | 性别（0：女，1：男，NULL：未知） |
| birth_year | smallint(6)  | 出生年份                     |
| death_year | smallint(6)  | 去世年份                     |
...
作者可能会写多本书，一本书也可能由多位作者合作。此表存储图书与作者的对应关系。

| 字段名   | 类型       | 描述                                              |
|----------|------------|--------------------------------------------------|
| book_id  | bigint(20) | 图书的唯一 ID（关联 [books](#books-table)）   |
| author_id| bigint(20) | 作者的唯一 ID（关联 [authors](#authors-table)） |
...
| 字段名   | 类型       | 描述                                              |
|----------|------------|--------------------------------------------------|
| id       | bigint(20) | 订单的唯一 ID                                    |
| book_id  | bigint(20) | 图书的唯一 ID（关联 [books](#books-table)）   |
| user_id  | bigint(20) | 用户的唯一标识（关联 [users](#users-table)）  |
...
可以在 [Discord](https://discord.gg/DQZ2dy3cuc?utm_source=doc) 或 [Slack](https://slack.tidb.io/invite?team=tidb-community&channel=everyone&ref=pingcap-docs) 上向社区提问，或 [提交支持工单](/support.md)。

</CustomContent>

<CustomContent platform="tidb-cloud">

可以在 [Discord](https://discord.gg/DQZ2dy3cuc?utm_source=doc) 或 [Slack](https://slack.tidb.io/invite?team=tidb-community&channel=everyone&ref=pingcap-docs) 上向社区提问，或 [提交支持工单](https://tidb.support.pingcap.com/)。
