...
summary: 学习如何使用 Spring Boot 连接 TiDB。本教程提供了适用于 TiDB 的 Java 示例代码片段，结合 Spring Boot 使用。
---
...
配置完成后，设置环境变量 `TIDB_JDBC_URL`、`TIDB_USER` 和 `TIDB_PASSWORD` 为你的 TiDB 集群的实际值。配置文件为这些环境变量提供了默认值。如果未配置环境变量，默认值如下：
