...
TiDB 是一个与 MySQL 兼容的数据库，[Hibernate](https://hibernate.org/orm/) 是一个流行的开源 Java ORM 框架。从版本 `6.0.0.Beta2` 开始，Hibernate 支持 TiDB 方言，能够很好地适应 TiDB 的特性。
...
完整示例代码及运行方式，请查阅 [tidb-samples/tidb-java-hibernate-quickstart](https://github.com/tidb-samples/tidb-java-hibernate-quickstart) 仓库。
...
使用此方法时，需要将 `${your_entity_class}` 替换为你自己的数据实体类。若有多个实体类，则需为每个添加 `.addAnnotatedClass(${your_entity_class})` 语句。上述方法只是配置 Hibernate 的一种方式。如果在配置中遇到问题或想了解更多信息，可以参考 [Hibernate 官方文档](https://hibernate.org/orm/documentation)。
...
- 学习 TiDB 应用开发的最佳实践，参考 [开发者指南](https://github.com/pingcap/tidb-dev-guide)，如 [Insert data](/develop/dev-guide-insert-data.md)、[Update data](/develop/dev-guide-update-data.md)、[Delete data](/develop/dev-guide-delete-data.md)、[单表读取](/develop/dev-guide-get-data-from-single-table.md)、[事务](/develop/dev-guide-transaction-overview.md)、[SQL 性能优化](/develop/dev-guide-optimize-sql-overview.md)。
