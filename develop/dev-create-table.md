...
This page provides a best practice guide for creating tables and an example of a [bookshop](/develop/dev-bookshop-schema-design.md) database based on TiDB.
...
[Table](/develop/dev-schema-design-overview.md#table) is a logical object. It would stores data sent from the persistence layer of an application or from other SQL. Tables saved data records in the form of rows and columns.
...
- Please use meaningful column names. we recommend that you follow your company or organization's table naming convention. If your company or organization does not have a corresponding naming convention, refer to the [column naming convention](/develop/dev-object-naming-guidelines.md#field-naming-convention).
...
> **Note:**
>
> If your cluster, does not contain **TiFlash** nodes, this SQL statement will report an error: `1105 - the tiflash replica count: 1 should be less than the total tiflash server count: 0` You can use [Build a TiDB Cluster in TiDB Cloud (DevTier)](/develop/dev-build-cluster-in-cloud.md#step-1-create-a-free-cluster) to create a free cluster include **TiFlash**.
...
- Following the SQL development [specification for table build and delete](/develop/dev-sql-development-specification.md#specification-for-table-build-and-delete), it is recommended to wrap the build and delete statements inside the business application to add judgment logic.

#### Execute the `CREATE TABLE` statement example

After creating all the tables as above rules, our `dbinit.sql` file should look similar to the [database initialization](/develop/dev-bookshop-schema-design.md#database-initialization-script-dbinitsql) shown. If you need to see the table information in detail, please refer to [Details about Table](/develop/dev-bookshop-schema-design.md#details-about-table).
...
Please note that all the tables we have created so far do not contain secondary indexes. For a guide to adding secondary indexes, please refer to [Creating Secondary Indexes](/develop/dev-create-secondary-indexes.md).
