...
如果你还没有 TiDB 集群，可以 [创建一个 {{{ .starter }}} 集群](https://docs.pingcap.com/tidbcloud/create-tidb-cluster-serverless)，该集群免费且大约在 30 秒内即可创建完成。
...
- 一个符合以下要求的 AWS [身份与访问管理（IAM）用户](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_users.html)：
...
2. （可选）如果你还没有配置 AWS 访问密钥 ID 和密钥，可以执行：
...
3. 将 JAR 包作为 Lambda 上传：
    ...
    > - 该引导流程假设使用 AWS 作为 {{{ .starter }}} 的云提供商。若要使用 Amazon S3 作为源或目标，你需要将 AWS Lambda 的区域设置为与 Amazon S3 相同。
    > - 如果你之前已经运行过 `sam deploy --guided`，可以直接运行 `sam deploy`，SAM CLI 会使用 `samconfig.toml` 配置文件简化操作。

    如果你看到类似如下的输出，说明 Lambda 已成功部署。
...
6. 在 **Connect to TiDB-Connector** 对话框中，输入 TiDB 集群的连接属性。如果你使用 {{{ .starter }}} 集群，需要将 **TLS** 选项设置为 `Yes`，这样 TiDB 连接器会使用 TLS 连接。然后点击 **Connect**。
...
7. 现在可以获取你指定连接的数据库中的所有表。从下拉列表中选择 **sf_account** 表。
    ...
    下图显示了将 Salesforce **Account** 对象中的数据传输到 TiDB 中的 `sf_account` 表的配置示例：
