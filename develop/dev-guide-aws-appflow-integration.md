...
4. 在点击 **Connect** 之前，你需要在 TiDB 中为 Salesforce 的 **Account** 对象创建一个 `sf_account` 表。注意，该表结构与 [Tutorial of Amazon AppFlow](https://docs.aws.amazon.com/appflow/latest/userguide/flow-tutorial-set-up-source.html) 中的示例数据不同。
...
6. 在 **Connect to TiDB-Connector** 对话框中，输入 TiDB 集群的连接属性。如果你使用的是 TiDB Cloud Starter 集群，需要将 **TLS** 选项设置为 `Yes`，以便 TiDB 连接器使用 TLS 连接。然后点击 **Connect**。
