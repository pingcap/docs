...
### `UPDATE`のベストプラクティス {#update-best-practices}
...
[authors](/develop/dev-guide-bookshop-schema-design.md#authors-table)テーブルの著者が名前を**Helen Haruki**に変更したとします。テーブルを変更する必要があります。彼女の固有の`id`が**1で**あると仮定すると、フィルターは`id = 1`になります。
...
## `INSERT ON DUPLICATE KEY UPDATE`を使用する {#use-insert-on-duplicate-key-update}
...
### `INSERT ON DUPLICATE KEY UPDATE` SQL構文 {#insert-on-duplicate-key-update-sql-syntax}
