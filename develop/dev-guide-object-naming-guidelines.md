...
summary: データベースオブジェクトの命名規則についてのルールを紹介します。意味のある英単語をアンダースコアで区切り、文字、数字、アンダースコアのみを使用し、予約語を避けることをお勧めします。データベース名はビジネスや製品によって区別し、20文字以内を使用することをお勧めします。テーブル名は同じプレフィックスを使用し、32文字以内で、理解を深めるために注釈を付けることをお勧めします。列名は実際の意味または略称で、同じ意味を持つテーブル間では同じ列名を使用し、30文字を超えないようにすることをお勧めします。インデックスの命名規則も紹介されています。
---
