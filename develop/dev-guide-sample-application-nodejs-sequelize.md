...
次のクエリは、1 つのレコード`Players`を作成し、 `Players`オブジェクトを返します。
...
次のクエリは、コインが`300`より大きい`Players`レコードを 1 つ返します。
...
次のクエリは、セクション[データを挿入する](#insert-data)で作成された ID `6`の`Player`のレコードを削除します。
