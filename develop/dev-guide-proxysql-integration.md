...
在本集成方案中，你将使用 [ProxySQL Docker 镜像](https://hub.docker.com/r/proxysql/proxysql) 和 {{{ .starter }}} 实例。以下步骤会将 ProxySQL 设置在 `16033` 端口，请确保该端口可用。
...
3. 通过以下命令集成 ProxySQL，在 **ProxySQL Admin Interface** 内执行 `proxysql-prepare.sql`：
    ...
    >
    > 1. 使用你的 {{{ .starter }}} 实例用户名和密码添加用户。
    > 2. 将用户分配给监控账户。
    > 3. 将你的 {{{ .starter }}} 实例添加到主机列表。
    > 4. 启用 ProxySQL 与 {{{ .starter }}} 实例之间的安全连接。
    >
    ...
    以下为示例输出。你会看到输出中显示了你的 {{{ .starter }}} 实例主机名，说明 ProxySQL 与 {{{ .starter }}} 实例的连通性已建立。
