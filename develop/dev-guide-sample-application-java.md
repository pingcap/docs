...
`PlayerMapper.java` is a mapping interface file generated using Mybatis Generator. It's only an interface. The implementation classes will automatically generated at runtime by Mybatis through XML or annotations.
...
`PlayerMapper.xml` is a mapping XML file generated using Mybatis Generator. Mybatis will use it to automatically generate the implementation class for the `PlayerMapper` interface.
...
Because Mybatis Generator needs to generate the source code, the table needs to be create in the database first. You can generated by using `dbinit.sql`.
...
Split the interface `PlayerMapperEx` additionally to extends from `PlayerMapper` and write a matching `PlayerMapperEx.xml` file. Avoid changing `PlayerMapper.java` and `PlayerMapper.xml` directly. This is to avoid overwrite by Mybatis Generator.
