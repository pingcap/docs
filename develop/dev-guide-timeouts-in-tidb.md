...
- **wait_timeout**, controls the non-interactive idle timeout for the connection to Java applications. The default value is `28800`  (The unit is second, so means 8 hours) at `5.4` version and above. And the rest version, hte default value is `0`, which allows the connection to be idle indefinitely.
