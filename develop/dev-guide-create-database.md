...
本文档介绍如何使用 SQL 和各种编程语言创建数据库，并列出了数据库创建的规则。在本文档中，以 [Bookshop](/develop/dev-guide-bookshop-schema-design.md) 应用程序为例，指导你完成数据库创建的步骤。
...
要创建数据库，你可以使用 `CREATE DATABASE` 语句。
...
- TiDB 自带一个名为 `test` 的默认数据库。但是，如果不是必须，建议不要在生产环境中使用它。你可以使用 `CREATE DATABASE` 语句创建自己的数据库，并在 SQL 会话中使用 [`USE {databasename};`](/sql-statements/sql-statement-use.md) 语句更改当前数据库。
- 使用 `root` 用户创建数据库、角色和用户等对象。仅向角色和用户授予必要的权限。
- 作为最佳实践，建议使用 **MySQL 命令行客户端**或 **MySQL GUI 客户端**而不是驱动程序或 ORM 来执行数据库架构更改。

## 下一步

创建数据库后，你可以向其添加**表**。更多信息，请参见[创建表](/develop/dev-guide-create-table.md)。
