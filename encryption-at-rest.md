...
TiFlash は保存時の暗号化をサポートしています。データキーは TiFlash によって生成されます。 TiFlash (TiFlash Proxy を含む) に書き込まれるすべてのファイル (データ ファイル、スキーマ ファイル、および一時ファイルを含む) は、現在のデータ キーを使用して暗号化されます。 TiFlash は現在、 CTRモードで AES を使用した透過的なデータ暗号化のみをサポートしており、SM4 暗号化アルゴリズムはサポートしていません。関連する暗号化構成項目 ( [`tiflash-learner.toml`](/tiflash/tiflash-configuration.md#configure-the-tiflash-learnertoml-file)ファイル内)、および監視メトリックの意味は、TiKV のものと一致しています。
