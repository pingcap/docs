...
* TiFlash supports encryption at rest starting from v4.0.5. For details, refer to [Encryption at Rest for TiFlash](/encryption-at-rest.md#encryption-at-rest-for-tiflash-new-in-v405). When deploying TiKV with TiFlash before v4.0.5, data stored in TiFlash is not encrypted.
...
TiFlash supports encryption at rest starting from v4.0.5. All files written by TiFlash (including TiFlash Proxy), including data files, schema files, temporary files, etc., are encrypted by the current data key.

Data keys are generated by TiFlash. All files written by TiFlash (including TiFlash Proxy), including data files, schema files, temporary files, etc., are encrypted by the current data key. The encryption algorithms, encryption configuration (configuration items in the `tiflash-learner.toml` file) of TiFlash, and the meanings of monitoring items are consistent with those of TiKV.

If you deploy TiFlash with Grafana, you can look at **TiFlash-Proxy-Details** -> **Encryption**.
