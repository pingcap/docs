...
Data keys are generated by TiFlash. All files written by TiFlash (including TiFlash Proxy), including data files, Schema files, temporary files, etc., are encrypted by the current data key. The encryption algorithms, encryption configuration (configuration items in the `tiflash-learner.toml` file) and the meaning of TiFlash monitoring items are consistent with TiKV.
