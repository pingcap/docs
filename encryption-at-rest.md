...
SM4 encryption is only supported in v6.3.0 and later versions of TiKV. TiKV versions earlier than v6.3.0 only support AES encryption. SM4 encryption impacts performance: it might cause a ​50% to 80% throughput degradation in the worst-case scenarios; increasing the block cache size can significantly reduce this impact, lowering throughput degradation to approximately 10%. It is recommended to adjust the block cache configuration based on actual application requirements to balance security and performance.
