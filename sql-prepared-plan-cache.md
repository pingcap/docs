...
The `Statements Summary` table contains two fields, `plan_cache_unqualified` and `last_plan_cache_unqualified_reason`, which indicate the number of times and the reason why the corresponding query is unable to use the plan cache. You can use these two fields for diagnostic purposes:
