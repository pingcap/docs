...
- When the value of `tidb_replica_read` is set to `prefer-leader`, TiDB selects the leader replica as priority to perform read operations. And if the leader replica is obviously slow on processing such as by Disk / Net performance jitter, TiDB will select other available follower replicas to perform.
