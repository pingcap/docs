...
> - When the value of `tidb_replica_read` is set to `closest-adaptive`, to prevent the unbalanced distribution of the read traffic in various Regions, TiDB dynamically detects whether the Region distribution of all online TiDB and TiKV nodes is balanced. If a Region contains only TiDB or TiKV nodes, TiDB forces to select the leader replica to perform read operations forcefully. For example, if all TiDB nodes in a Region are down, then other online TiDB nodes are downgraded to read using leader replicas. After at least one TiDB node in this Region is back online, all TiDB nodes switch back to prefer to select the replica in the same Region to perform read operations.
