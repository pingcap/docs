...
The starting point information needed for [Step 3. (Optional) Use DM to perform incremental replication to TiDB](data-migration\migrate-sharding-mysql-tidb-less-tb.md#step-3. (Optional)-use-DM-to-perform-incremental-replication) is in the `${data-path}` directory, and in the `metadata` files of `my_db1` and `my_db2`. They are meta-information files automatically generated by Dumpling. To perform incremental replication, you need to note down the binlog location information.
