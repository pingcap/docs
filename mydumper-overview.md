...
In this situation, you need to add a `--skip-tz-utc` option. Otherwise, Mydumper will pre-configure the UTC time zone and convert the time zone when the `tidb-snapshot` is configured, which causes this error.
