...
> シャードビットの選択（ `S` ）：
>
> -   `_tidb_rowid`の合計ビット数は64であるため、シャードビットの数は自動インクリメントビットの数に影響します。シャードビットの数が増えると自動インクリメントビットの数は減り、その逆もまた然りです。したがって、自動インクリメント値のランダム性と利用可能な自動インクリメントリージョンのバランスを取る必要があります。
