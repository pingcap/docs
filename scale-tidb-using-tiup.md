...
>
> 1. Confirm that the current TiDB version supports using TiFlash, otherwise upgrade your TiDB cluster to v5.0.0-rc or later versions.
> 2. Execute the `tiup ctl pd -u <pd-host>:<pd-port> config set enable-placement-rules true` command to enable the PD's Placement Rules. Or execute the corresponding command through [pd-ctl](/pd-control.md).
