...
## TiDB/PD/TiKV クラスターをスケールアウトする {#scale-out-a-tidbpdtikv-cluster}
...
2. スケールアウト コマンドを実行します。
    ...
    - `scale-out.yml`はスケールアウト設定ファイルです。
...
このセクションでは、ホスト`10.0.1.3`と`10.0.1.4`に 2つの TiCDC ノードを追加する方法を例で説明します。
...
## TiDB/PD/TiKV クラスターのスケールイン {#scale-in-a-tidbpdtikv-cluster}
