...
このテストは、ワーキング セットがメモリ内に収まる OLTP における TiDB 2.1 と TiDB 2.0 のパフォーマンスを比較することを目的としています。
...
Sysbenchを使用して、**各テーブルに10,000,000行が含まれる16個のテーブル**をインポートします。HAProxyを使用すると、リクエストは同時実行数に応じて増加しながらクラスターに送信されます。1回の同時実行テストは5分間続きます。

### TiDBのバージョン情報 {#tidb-version-information}

### v2.1.0-rc.2 {#v210-rc-2}
...
### `Update Non-Index` {#update-non-index-test}
