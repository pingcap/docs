...
This test aims to compare the TPC-C performance of TiDB v5.3.0 and TiDB v5.2.2 in the online transactional processing (OLTP) scenario. The results show that compared with v5.2.2, the TPC-C performance of v5.3.0 is reduced by 2.99%.
...
4. Execute the `tiup bench tpcc run -U root --db tpcc --host 127.0.0.1 --port 4000 --time 1800s --warehouses 5000 --threads {{thread}}` command to perform stress tests on TiDB via HAProxy. Testing on each concurrency takes 30 minutes.
