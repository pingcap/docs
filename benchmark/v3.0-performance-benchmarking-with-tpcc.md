...
BenchmarkSQL を使用して、 **1000 の倉庫**のデータを TiDB クラスターにロードします。 HAProxy を使用して、同時リクエストを増分番号でクラスターに送信します。 1 つの同時テストは 10 分間続きます。
...
テスト統計によると、TiDB 3.0 のパフォーマンスは TiDB 2.1 のパフォーマンスよりも**450% 向上しています**。
