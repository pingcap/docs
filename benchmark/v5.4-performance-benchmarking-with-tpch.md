...
v5.0 では、TiDB は[<a href="/tiflash/tiflash-overview.md">TiFlash</a>](/tiflash/tiflash-overview.md)に MPP モードを導入し、TiDB のハイブリッド トランザクションおよび分析処理 (HTAP) 機能を大幅に強化します。このレポートのテスト対象は次のとおりです。
