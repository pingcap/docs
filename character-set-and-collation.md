...
By default, TiDB provides the same 3-byte limit on `utf8` to ensure that data created in TiDB can still safely be restored in MySQL. This can be disabled by changing the value of system variable `tidb_check_mb4_value_in_utf8` to `OFF`.
