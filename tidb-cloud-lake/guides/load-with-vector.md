...
Vector natively supports delivering data to [{{{ .lake }}} as a sink](https://vector.dev/docs/reference/configuration/sinks/databend/), this means that Vector can send data to {{{ .lake }}} for storage or further processing. {{{ .lake }}} acts as the destination for the data collected and processed by Vector. By configuring Vector to use {{{ .lake }}} as a sink, you can seamlessly transfer data from Vector to {{{ .lake }}}, enabling efficient data analysis, storage, and retrieval.

## Integrating with Vector

To integrate {{{ .lake }}} with Vector, start by creating an SQL account in {{{ .lake }}} and assigning appropriate permissions. This account will be used for communication and data transfer between Vector and {{{ .lake }}}. Then, in the Vector configuration, set up {{{ .lake }}} as a Sink.

### Step 1: Creating an SQL User in {{{ .lake }}}

For instructions on how to create a SQL user in {{{ .lake }}} and grant appropriate privileges, see [Create User](/tidb-cloud-lake/sql/create-user.md). Here's an example of creating a user named *user1* with the password *abc123*:
...
### Step 2: Configure {{{ .lake }}} as a Sink in Vector

In this step, configure {{{ .lake }}} as a sink in Vector by specifying the necessary settings such as the input source, compression, database, endpoint, table, and authentication credentials (username and password) for {{{ .lake }}} integration. The following is a simple example of configuring {{{ .lake }}} as a sink. For a comprehensive list of configuration parameters, refer to the Vector documentation at <https://vector.dev/docs/reference/configuration/sinks/databend/>
...
### Step 4. Analyze Nginx Log in {{{ .lake }}}
...
#### 4.2 Analyze Nginx Access Logs in {{{ .lake }}}
