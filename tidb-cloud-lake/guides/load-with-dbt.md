...
[tidbcloudlake-dbt](https://github.com/tidbcloud/lake-dbt) 是由 {{{ .lake }}} 开发的一个插件，主要目标是实现 dbt 与 {{{ .lake }}} 的平滑集成。借助该插件，你可以使用 dbt 无缝执行数据建模、转换和清洗任务，并方便地将输出导入到 {{{ .lake }}} 中。下表展示了 tidbcloudlake-dbt 插件对 dbt 常用功能的支持级别：
