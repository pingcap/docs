...
The metadata service is a multi-tenant service that stores the metadata of each tenant in {{{ .lake }}} in a highly available Raft cluster. This metadata includes:
...
The architecture of complete separation of storage and compute gives {{{ .lake }}} a unique computational elasticity.
...
The storage layer of {{{ .lake }}} is based on FuseEngine, which is designed and optimized for inexpensive object storage. FuseEngine efficiently organizes data based on the properties of object storage, allowing for high-throughput data ingestion and retrieval.
