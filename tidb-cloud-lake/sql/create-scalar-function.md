...
Python runtime requires {{{ .lake }}} Enterprise. You can install PyPI packages via `PACKAGES` and import stage files via `IMPORTS`.

### Data type mappings (Python)

| {{{ .lake }}} Type | Python Type |
|--------------|-------------|
| NULL | None |
| BOOLEAN | bool |
| INT | int |
