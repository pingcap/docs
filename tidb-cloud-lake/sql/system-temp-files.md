...
summary: 包含由 {{{ .lake }}} 创建的临时文件信息，例如 spill 文件。要删除临时文件，请使用 VACUUM TEMPORARY FILES 命令。
---

# system.temp_files

包含由 {{{ .lake }}} 创建的临时文件信息，例如 spill 文件。要删除临时文件，请使用 [VACUUM TEMPORARY FILES](/tidb-cloud-lake/sql/vacuum-temporary-files.md) 命令。
