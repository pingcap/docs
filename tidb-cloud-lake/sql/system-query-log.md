...
summary: 一个只读的内存表，用于存储所有查询日志。有关查询日志中可用的具体字段，请参阅“示例”部分。
---

# system.query_log

一个只读的内存表，用于存储所有查询日志。有关查询日志中可用的具体字段，请参阅[示例](#examples)部分。

## 设置会话标签 {#setting-a-session-tag}

你可以选择为当前会话分配一个标签，这样就能更方便地根据分配的会话标签在日志表中过滤日志。例如，以下语句将标签 `eric` 分配给当前会话：

```sql
set session query_tag='eric';
```

然后，你可以执行一个查询，并使用已分配的标签从日志表中检索对应的日志：
...
`system.query_log` 表存储已执行查询的详细日志。以下是一个日志条目的示例：
