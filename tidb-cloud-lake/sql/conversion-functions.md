...
summary: This page provides a comprehensive overview of Conversion functions in {{{ .lake-short }}}, organized by functionality for easy reference.
---

# Conversion Functions

This page provides a comprehensive overview of Conversion functions in {{{ .lake-short }}}, organized by functionality for easy reference.
...
- When converting from floating-point, decimal numbers, or strings to integers or decimal numbers with fractional parts, {{{ .lake-short }}} rounds the values to the nearest integer. This is determined by the setting `numeric_cast_option` (defaults to 'rounding') which controls the behavior of numeric casting operations. When `numeric_cast_option` is explicitly set to 'truncating', {{{ .lake-short }}} will truncate the decimal part, discarding any fractional values.
...
- {{{ .lake-short }}} also offers a variety of functions for converting expressions into different date and time formats. For more information, see [Date & Time Functions](/tidb-cloud-lake/sql/date-time-functions.md).
