...
- When `error_on_nondeterministic_update` = `true` (default): {{{ .lake }}} returns an error if a target row matches multiple source rows and there is no clear rule for selecting which value to use.
