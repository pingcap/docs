...
{{{ .lake-short }}} accepts a variety of file formats both as a source and as a target for data loading or unloading. This page explains the supported file formats and their available options.
...
{{{ .lake-short }}} determines the file format used by a COPY or Select statement in the following order of priority:
...
3. If no file format was defined for the stage during its creation, {{{ .lake-short }}} defaults to using the PARQUET format.

> **Note:**
>
> - {{{ .lake-short }}} currently supports ORC and AVRO as a source ONLY. Unloading data into an ORC or AVRO file is not supported yet.
> - For managing custom file formats in {{{ .lake-short }}}, see [File Format](/tidb-cloud-lake/sql/file-format.md).
...
{{{ .lake-short }}} CSV is compliant with [RFC 4180](https://www.rfc-editor.org/rfc/rfc4180) and is subject to the following conditions:
...
{{{ .lake-short }}} TSV is subject to the following conditions:
...
> **Note:**
>
> 1. In {{{ .lake-short }}}, the main difference between TSV and CSV is NOT using a tab instead of a comma as a field delemiter (which can be changed by options), but using escaping instead of quoting for 
