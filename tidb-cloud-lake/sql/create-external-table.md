...
| ENDPOINT_URL                | The bucket endpoint URL starting with `https://`. To use a URL starting with `http://`, set `allow_insecure` to `true` in the [storage] block of the file `lake-query-node.toml`.                                    | Optional   |
