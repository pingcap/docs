...
summary: A subquery is a query nested within another one. {{{ .lake }}} supports the following subquery types.
---

# Subquery Operators

A subquery is a query nested within another one. {{{ .lake }}} supports the following subquery types:
...
- If a scalar subquery returns 0 rows, {{{ .lake }}} will use NULL as the subquery output.
- If a scalar subquery returns more than one row, {{{ .lake }}} will throw an error.
