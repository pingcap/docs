---
title: Table Engines
summary: "{{{ .lake }}} provides several table engines so that you can balance performance and interoperability needs without moving data. Each engine is optimized for a specific scenario—ranging from {{{ .lake }}}’s native Fuse storage to external data lake formats."
---

# Table Engines

{{{ .lake }}} provides several table engines so that you can balance performance and interoperability needs without moving data. Each engine is optimized for a specific scenario—ranging from {{{ .lake }}}’s native Fuse storage to external data lake formats.
...
- Use **Fuse** when you manage data directly inside {{{ .lake }}} and want the best storage and query performance.
- Choose **Iceberg** when you already manage datasets through Iceberg catalogs and need tight lakehouse integration.
- Configure **Hive** when you rely on an existing Hive Metastore but want {{{ .lake }}}’s query engine.
