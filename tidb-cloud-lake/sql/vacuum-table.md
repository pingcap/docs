...
{{{ .lake }}} 提供了两个用于删除表中历史数据文件的命令：`VACUUM TABLE` 和 [OPTIMIZE TABLE](/tidb-cloud-lake/sql/optimize-table.md)（带 `PURGE` 选项）。虽然这两个命令都可以永久删除数据文件，但它们在处理孤儿文件方面有所不同：`OPTIMIZE TABLE` 能够删除孤儿快照，以及相应的 segments 和 blocks。但是，也可能存在没有任何关联快照的孤儿 segments 和 blocks。在这种场景下，只有 `VACUUM TABLE` 能帮助清理它们。
