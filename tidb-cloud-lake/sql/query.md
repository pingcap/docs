...
> **Note:**
>
> {{{ .lake }}}'s QUERY function is inspired by Elasticsearch's [QUERY](https://www.elastic.co/guide/en/elasticsearch/reference/current/sql-functions-search.html#sql-functions-search-query).
...
Use dot notation to address inner fields inside a `VARIANT` column. {{{ .lake }}} evaluates the path across objects and arrays.
