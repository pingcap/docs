...
summary: This tutorial walks you through how to back up and restore data using BendSave. We'll use a local MinIO instance as both the S3-compatible storage backend for {{{ .lake }}} and the destination for storing backups.
---

# Backup & Restore with BendSave

This tutorial walks you through how to back up and restore data using BendSave. We'll use a local MinIO instance as both the S3-compatible storage backend for {{{ .lake }}} and the destination for storing backups.
...
1. Download the latest {{{ .lake }}} release and extract it to get the necessary binaries:
...
1. Run the following command to back up your {{{ .lake }}} data to the **backupbucket** in MinIO:
