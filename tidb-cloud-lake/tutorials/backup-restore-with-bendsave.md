...
summary: This tutorial walks you through how to back up and restore data using BendSave. We'll use a local MinIO instance as both the S3-compatible storage backend for {{{ .lake-short }}} and the destination for storing backups.
---

# Backup & Restore with BendSave

This tutorial walks you through how to back up and restore data using BendSave. We'll use a local MinIO instance as both the S3-compatible storage backend for {{{ .lake-short }}} and the destination for storing backups.

## Before You Start

Before you start, ensure you have the following prerequisites in place:

- A Linux machine (x86_64 or aarch64 architecture): In this tutorial, we'll deploy {{{ .lake-short }}} on a Linux machine. You can use a local machine, a virtual machine, or a cloud instance such as AWS EC2.
    ...
    - If you are on AWS EC2, make sure your security group allows inbound traffic on port `8000`, as this is required for BendSQL to connect to {{{ .lake-short }}}.
...
- The {{{ .lake-short }}} release package: Download the release from the [{{{ .lake-short }}} GitHub Releases page](https://github.com/databendlabs/databend/releases). The package contains the `databend-bendsave` binary in the `bin` directory, which is the tool we'll use for backup and restore operations in this tutorial.
...
2. Set your MinIO credentials as environment variables, then use the AWS CLI to create two buckets: one for storing backups (**backupbucket**) and another for {{{ .lake-short }}} data (**databend**):
...
## Step 2: Set up {{{ .lake-short }}}

1. Download the latest {{{ .lake-short }}} release and extract it to get the necessary binaries:
...
4. Connect to your {{{ .lake-short }}} instance from your local machine with BendSQL, then apply your {{{ .lake-short }}} Enterprise license, create a table, and insert some sample data.
...
5. Back on your Linux machine, verify that the table data has been stored in your {{{ .lake-short }}} bucket:
...
1. Run the following command to back up your {{{ .lake-short }}} data to the **backupbucket** in MinIO:
...
2. After the removal, you can verify using BendSQL that querying the table in {{{ .lake-short }}} fails:
...
3. Run the following command to restore your {{{ .lake-short }}} data to the **databend** bucket in MinIO:
