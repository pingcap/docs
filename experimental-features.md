...
+ Improve the stability of the optimizer's choice of indexes (Introduced in v5.0)
    ...
    + Refactor the statistics module, including deleting the `TopN` value from `CMSKetch` and the histogram, and adding NDV information for histogram buckets of each table index. For details, see [Statistics - `tidb_analyze_version = 2`](/statistics.md).
