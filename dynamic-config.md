...
| `raftstore.store-io-pool-size` | The allowable number of threads that process Raft I/O tasks, which is also the size of the StoreWriter thread pool (**DO NOT** modify this value from `> 0` to `== 0` or from `== 0` to `> 0`) |
