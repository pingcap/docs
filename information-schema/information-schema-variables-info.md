...
* `VARIABLE_SCOPE`: the scope of the system variable. `SESSION` means the system variable is only valid in the current session. `INSTANCE` means the system variable is valid in the TiDB instance. `GLOBAL` means the system variable is valid in the TiDB cluster.
