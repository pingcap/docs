...
> * Due to some implementation limitations, `SQL_DIGEST` and `SQL_DIGEST_TEXT` is `null` for optimistic transaction. To get all the SQL statements of the optimistic transaction, you can join the table with [`CLUSTER_TIDB_TRX`](/information-schema/information-schema-tidb-trx.md).
