---
title: CLUSTER_LOG
summary: CLUSTER_LOGテーブルを使用すると、TiDBクラスターのログをクエリできます。このテーブルはTiDBセルフホスト型にのみ適用され、TiDB Cloudでは使用できません。クラスターログテーブルを使用する際には、オーバーヘッドを軽減するために検索条件を指定する必要があります。例えば、DDLステートメントの実行プロセスをクエリすることができます。
---
