...
`CLUSTER_LOG`テーブルでクラスターログをクエリできます。クエリ条件を各インスタンスにプッシュダウンすることで、クエリがクラスターのパフォーマンスに与える影響は、コマンド`grep`よりも小さくなります。
...
2. `127.0.0.1:4000` TiDB インスタンスがこの DDL リクエストを処理します。これは、 `127.0.0.1:4000`のインスタンスがその時点で DDL 所有者であることを示します。
