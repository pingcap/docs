...
`CLUSTER_CONFIG`クラスター構成テーブルを使用すると、クラスター内のすべてのサーバーコンポーネントの現在の構成を取得できます。これにより、以前のリリースの TiDB では同様の情報を取得するために各インスタンスの HTTP API エンドポイントにアクセスする必要がありましたが、より使いやすくなります。
