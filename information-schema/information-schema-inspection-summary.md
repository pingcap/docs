...
summary: 特定のリンクやモジュールの監視に注意が必要。TiDBは`inspection_summary`表を提供し、監視メトリクスの要約を可能にする。要約ルールや監視対象のインスタンス、メトリクス名、平均値、最小値、最大値などが含まれる。期間のデータを比較し、差異が大きい監視項目を素早く見つけることができる。オーバーヘッドを軽減するため、特定の`rule`を表示することが推奨される。
---
