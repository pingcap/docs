...
| [`MEMORY_USAGE`](/information-schema/information-schema-memory-usage.md)                           | 現在の TiDB インスタンスのメモリ使用量。この表はTiDB Cloudには適用されません。                             |
| [`MEMORY_USAGE_OPS_HISTORY`](/information-schema/information-schema-memory-usage-ops-history.md)   | メモリ関連の操作の履歴と、現在の TiDB インスタンスの実行ベース。この表はTiDB Cloudには適用されません。                 |
