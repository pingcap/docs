...
> **Note:**
>
> 以下部分表格中有些只在 TiDB Self-Managed 上支持，不支持在 TiDB Cloud 上。要获取在 TiDB Cloud 上不支持的完整表格列表，请参见 [System tables](https://docs.pingcap.com/tidbcloud/limited-sql-features#system-tables)。
...
| `CLUSTER_MEMORY_USAGE`                                                                  | 提供集群级别的 `MEMORY_USAGE` 表视图。                            |
| `CLUSTER_MEMORY_USAGE_OPS_HISTORY`                                                      | 提供集群级别的 `MEMORY_USAGE_OPS_HISTORY` 表视图。                |
| `CLUSTER_PROCESSLIST`                                                                   | 提供集群级别的 `PROCESSLIST` 表视图。                            |
| `CLUSTER_SLOW_QUERY`                                                                    | 提供集群级别的 `SLOW_QUERY` 表视图。此表在 [{{{ .starter }}}](https://docs.pingcap.com/tidbcloud/select-cluster-tier#tidb-cloud-serverless) 集群上不可用。 |
| `CLUSTER_STATEMENTS_SUMMARY`                                                            | 提供集群级别的 `STATEMENTS_SUMMARY` 表视图。此表在 [{{{ .starter }}}](https://docs.pingcap.com/tidbcloud/select-cluster-tier#tidb-cloud-serverless) 集群上不可用。 |
| `CLUSTER_STATEMENTS_SUMMARY_HISTORY`                                                    | 提供集群级别的 `STATEMENTS_SUMMARY_HISTORY` 表视图。此表在 [{{{ .starter }}}](https://docs.pingcap.com/tidbcloud/select-cluster-tier#tidb-cloud-serverless) 集群上不可用。 |
| `CLUSTER_TIDB_TRX` | 提供集群级别的 `TIDB_TRX` 表视图。                                |
...
| [`TABLE_STORAGE_STATS`](/information-schema/information-schema-table-storage-stats.md)  | 提供存储中的表大小详情。                                         |
