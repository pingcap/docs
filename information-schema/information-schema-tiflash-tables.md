...
- `SEGMENT_COUNT`：表中的 Segment 数量。Segment 是 TiFlash 的数据管理单元。
- `TOTAL_ROWS`：表中的总行数。
- `TOTAL_SIZE`：表的总大小（字节数）。
- `TOTAL_DELETE_RANGES`：表中的 Delete Range 总数。
- `DELTA_RATE_ROWS`：表在 Delta 层的总行数占该表总行数的比例。
- `DELTA_RATE_SEGMENTS`：表中包含非空 Delta 层的 Segment 占比。
- `DELTA_PLACED_RATE`：表在 Delta 层已完成索引构建的行数占比。
- `DELTA_CACHE_SIZE`：表在 Delta 层的缓存大小（字节数）。
- `DELTA_CACHE_RATE`：表在 Delta 层的缓存数据占比。
- `DELTA_CACHE_WASTED_RATE`：表在 Delta 层的无效缓存数据占比。
- `DELTA_INDEX_SIZE`：Delta 层索引占用的内存大小（字节数）。
- `AVG_SEGMENT_ROWS`：表所有 Segment 的平均行数。
- `AVG_SEGMENT_SIZE`：表所有 Segment 的平均大小（字节数）。
- `DELTA_COUNT`：表中包含非空 Delta 层的 Segment 数量。
...
- `AVG_DELTA_DELETE_RANGES`：所有 Delta 层的平均 Delete Range 操作数。
- `STABLE_COUNT`：表中包含非空 Stable 层的 Segment 数量。
...
- `TOTAL_PACK_COUNT_IN_DELTA`：所有 Delta 层中的 Column File 总数。
- `MAX_PACK_COUNT_IN_DELTA`：单个 Delta 层中的最大 Column File 数量。
- `AVG_PACK_COUNT_IN_DELTA`：所有 Delta 层中的平均 Column File 数量。
- `AVG_PACK_ROWS_IN_DELTA`：所有 Delta 层中所有 Column File 的平均行数。
- `AVG_PACK_SIZE_IN_DELTA`：所有 Delta 层中所有 Column File 的平均数据大小（字节数）。
- `TOTAL_PACK_COUNT_IN_STABLE`：所有 Stable 层中的 Pack 总数。
- `AVG_PACK_COUNT_IN_STABLE`：所有 Stable 层中的平均 Pack 数量。
...
- `STORAGE_STABLE_OLDEST_SNAPSHOT_TRACING_ID`：Stable 层最早快照的 tracing ID。
...
- `STORAGE_DELTA_OLDEST_SNAPSHOT_TRACING_ID`：Delta 层最早快照的 tracing ID。
...
- `STORAGE_META_OLDEST_SNAPSHOT_TRACING_ID`：元信息中最早快照的 tracing ID。
