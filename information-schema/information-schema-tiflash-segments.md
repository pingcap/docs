...
- `DELTA_MEMTABLE_ROWS`：Delta 层缓存中的总行数。
- `DELTA_MEMTABLE_SIZE`：Delta 层缓存数据的总大小（字节数）。
- `DELTA_MEMTABLE_COLUMN_FILES`：Delta 层缓存的列文件（Column File）数量。
- `DELTA_MEMTABLE_DELETE_RANGES`：Delta 层缓存的删除区间（Delete Range）数量。
- `DELTA_PERSISTED_PAGE_ID`：Delta 层落盘数据的存储页 ID。
- `DELTA_PERSISTED_ROWS`：Delta 层已持久化数据的总行数。
- `DELTA_PERSISTED_SIZE`：Delta 层已持久化数据的总大小（字节数）。
- `DELTA_PERSISTED_COLUMN_FILES`：Delta 层已持久化的列文件数量。
- `DELTA_PERSISTED_DELETE_RANGES`：Delta 层已持久化的删除区间数量。
- `DELTA_CACHE_SIZE`：Delta 层缓存的大小（字节数）。
- `DELTA_INDEX_SIZE`：Delta 层索引的大小（字节数）。
- `STABLE_PAGE_ID`：Stable 层数据的磁盘存储页 ID。
- `STABLE_ROWS`：Stable 层中的总行数。
- `STABLE_SIZE`：Stable 层数据的总大小（字节数）。
- `STABLE_DMFILES`：Stable 层中的 DMFile 文件数量。
- `STABLE_DMFILES_ID_0`：Stable 层第一个 DMFile 的磁盘存储 ID。
- `STABLE_DMFILES_ROWS`：Stable 层 DMFile 文件中的总行数。
- `STABLE_DMFILES_SIZE`：Stable 层 DMFile 文件中的数据总大小（字节数）。
- `STABLE_DMFILES_SIZE_ON_DISK`：Stable 层 DMFile 文件实际占用的磁盘空间（字节数）。
- `STABLE_DMFILES_PACKS`：Stable 层 DMFile 文件中的 Pack 数量。
