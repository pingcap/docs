...
| `--tidb-mem-quota-query`     | Dumplingコマンドの1行でエクスポートするSQLステートメントのメモリ制限値で、単位はバイトです。v4.0.10以降のバージョンでは、このパラメータを設定しない場合、TiDBはデフォルトで`mem-quota-query`設定項目の値をメモリ制限値として使用します。v4.0.10より前のバージョンでは、パラメータ値のデフォルト値は32GBです。                                                                                                                                                                                                    | 34359738368                                                                                                                                                         |
