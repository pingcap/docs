...
`ANALYZE`時間がかかる可能性があることを考慮して、TiDB は最初の Reorg の実行時間に基づいてタイムアウトしきい値を設定します。`ANALYZE`タイムアウトすると、 `ADD INDEX` `ANALYZE`完了を同期的に待つのをやめ、後続のプロセスを続行します。これにより、インデックスがユーザーにとってより早く表示されます。つまり、インデックス統計は`ANALYZE`非同期的に完了した後に更新されます。
