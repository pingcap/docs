...
3. Start the cluster in the current session:
    ...
    - To specify the TiDB version and the number of instances of each component, run a command like this:
        ...
        ```shell
        tiup playground v7.1.4 --db 2 --pd 3 --kv 3
        ```

        The command downloads a version cluster to the local machine and starts it, such as v7.1.4. To view the latest version, run `tiup list tidb`.
...
7. Execute the cluster deployment command:
    ...
    - `<version>`: Set the TiDB cluster version, such as `v7.1.4`. You can see all the supported TiDB versions by running the `tiup list tidb` command
