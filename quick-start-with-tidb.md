...
3.  現在のセッションでクラスターを起動します。
    ...
    -   TiDB のバージョンと各コンポーネントのインスタンスの数を指定するには、次のようなコマンドを実行します。
        ...
        このコマンドは、バージョン クラスター (v7.5.3 など) をローカル マシンにダウンロードして起動します。最新バージョンを表示するには、 `tiup list tidb`実行します。
...
7.  クラスター デプロイメント コマンドを実行します。
    ...
    -   `<version>` : TiDBクラスタバージョンを設定します（例： `v7.5.3` `tiup list tidb`コマンドを実行すると、サポートされているすべてのTiDBバージョンを確認できます。
