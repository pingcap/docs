...
1. Partition pruning uses the query conditions on the partitioned table, so if the query conditions can not be pushed down to the partitioned table according to the planner's optimize rules, partition pruning does not apply for this query.
...
In this query, the left out join is converted to the inner join, then `t1.x > 5` is derived from `t1.x = t2.x` and `t2.x > 5`, it could be used in partition pruning and only the partition `p1` remains.

```sql
explain select * from t1 left join t2 on t1.x = t2.x and t2.x > 5;
```

In this query, `t2.x > 5` can not be pushed down to the partitioned table `t1`, so partition pruning would not take effect on this query.

2. Since partition pruning is done during the plan optimizing phase, it does not apply for those cases that filter conditions are unknown until the execute phase.
...
This query reads a row from the `t2` and use the result for the subquery on the `t1`. Theoretically, partition pruning could benefit from `t1.x > val` expression in the subquery, but it does not take effect there as that happens in the execution phase.

3. As a result of the limitation from current implementation, if a query condition can not be pushed down to the TiKV, it can not be used by the partition pruning.

For an expression `fn(col)`, if the TiKV coprocessor supports this `fn` function, `fn(col)` may be pushed down to the partitioned table according to the predicate push down rule during the plan optimizing phase, and partition pruning can use it.
...
4. For hash partition, the only supported query by partition pruning is the equal condition.

5. For range partition, for partition pruning to take effect, the partition expression must be in those forms: `col` or `fn(col)`, and the query condition must be one of `>` `<` `=` `>=` `<=`. If the partition expression is something like `fn(col)`, the function `fn` must be monotonous.

If a function `fn` is monotonous, for any `x` `y`, if `x > y`, then `fn(x) > fn(y)`. We call this function `fn` strictly monotonous. For a function `fn`, for any `x` `y`, if `x > y` then `fn(x) >= fn(y)`, `fn` could also be called 'monotonous'. In theory, all monotonous function are supported by partition pruning.
...
An exception is `floor(unix_timestamp())` as the partition expression. TiDB does some case by case optimize for that, so it is supported by partition pruning.
