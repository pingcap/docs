...
- Scope: GLOBAL
- Default value: `2`
- Controls the format version of the newly stored data in the table. In TiDB v4.0, the [new storage row format](https://github.com/pingcap/tidb/blob/master/docs/design/2018-07-19-row-format.md) version `2` is used by default to store new data.
...
- Note that modifying this variable does not affect the old data that has been stored, but applies the corresponding version format only to the newly written data after modifying this variable.
