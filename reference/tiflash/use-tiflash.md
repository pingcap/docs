...
* `AVAILABLE` indicates whether the TiFlash replicas of this table is available or not. `1` means available and `0` means unavailable. Once the replicas become available, this status does not change. If you have used DDL statements to modify the number of replicas, the replication status is recalculated.
...
Note that if a table has only a single TiFlash replica and the node cannot provide service, queries in the smart mode will repeatedly retry. In this situation, you need to specify the engine or use the manual Hint to read data from TiKV.
