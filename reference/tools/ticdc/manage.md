...
## Use `cdc cli` to manage cluster status and data replication

The following sections introduce how to use `cdc cli` to manage the TiCDC cluster status and the data replication.

### Manage TiCDC service processes (`capture`)
...
## Use HTTP interface to manage cluster status and data replication

Currently, the HTTP interface provides some basic query and operation features. In the following examples, suppose that the interface IP address for querying the TiCDC server is `127.0.0.1`, and the port address is `8300` (you can specify the IP and port in `--status-addr=ip:port` when starting the TiCDC server). In later TiDB versions, these features will be integrated to `cdc cli`.

### Get the interface of the TiCDC server status

Use the following command to get the interface of the TiCDC server status:
...
The above command takes effect only for evicting own nodes.
...
For nodes other than own nodes, executing the above command will have the following error returned.
...
- `admin-job=3` means to delete the replication task. After the interface receives the request, all `processor`s are stopped and the configuration information of the task is cleared. The replication status is reserved. No service is available except for the query.
- `cf-id=xxx` is the ID of `changefeed` that needs operation.
...
- If the error occurs because the downstream is incompatible with SQL, resuming the task will fail. In this situation, you can configure the `ignore-txn-commit-ts` parameter in the replication configuration to skip the transaction at `commit-ts` and resume the task.
