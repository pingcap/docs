...
-   [TiUP を使用してTiUP TiDB Lightningをインストールする](/migration-tools.md)
...
`/data/my_datasource/`や`s3://my-bucket/sql-backup`のように、すべての SQL ファイルを同じディレクトリに配置します。 TiDB Lightningは、このディレクトリとそのサブディレクトリ内の`.sql`のファイルすべてを再帰的に検索します。
