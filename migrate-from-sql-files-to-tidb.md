...
Put all the SQL files in the same directory, like `/data/my_datasource/` or `s3://my-bucket/sql-backup?region=us-west-2`. TiDB Lightning recursively searches for all `.sql` files in this directory and its subdirectories.
