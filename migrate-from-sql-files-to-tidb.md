...
ターゲット TiDB クラスターと同じバージョンの TiDB Lightning を使用することをお勧めします。`<version>`はターゲット TiDB クラスターのバージョンに置き換えてください。
...
- ログ内のキーワード`progress`を`grep`することで、インポートの進行状況を確認できます。このログはデフォルトで 5分ごとに更新されます。
