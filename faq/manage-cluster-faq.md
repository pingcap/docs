...
By default, there is no limit on the maximum number of connections per TiDB server. If needed, you can limit the maximum number by setting `instance.max_connections` in `config.toml`, or changing the value of system variable `max_connections`. If too large concurrency leads to an increase of response time, it is recommended to increase the capacity by adding TiDB nodes.
