...
-   モニタリングのために、 [TiDB監視フレームワーク](/tidb-monitoring-framework.md)[プロメテウス](https://prometheus.io/)を使用してモニタリングとパフォーマンスのメトリクスを保存し、[Grafana](https://grafana.com/grafana/)を使用してこれらのメトリクスを視覚化します。数百のメトリクスを備えた数十の組み込みパネルが利用可能です。
