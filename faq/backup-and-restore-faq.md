...
In most cases, flashback is a better recovery solution than PITR for data errors caused by human mistakes, as it has a much shorter RPO (close to zero) and RTO. However, when a cluster is completely unavailable, flashback cannot run either, and PITR is the only solution to recover the cluster in this case. Therefore, PITR is always the foundation of data security that must be considered when developing a database disaster recovery strategy., even though it has a longer RPO (up to 5 minutes) and RTO than flashback.
