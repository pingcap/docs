...
- Have a file system token and region code for an existing file system. See [Access an Existing File System](/tidb-cloud-filesystem/access-filesystem.md).
...
> **Warning:**
>
> If unmounting fails, do not stop or remove the container. Pending writes might still exist only inside the container. Keep the container running, resolve the error, and verify that the required files have reached the file system first.
