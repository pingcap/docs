...
summary: 学习如何将 TiDB 向量检索与 Django ORM 集成，用于存储嵌入向量并执行语义检索。
---

# 将 TiDB 向量检索集成到 Django ORM

本教程将指导你如何使用 [Django](https://www.djangoproject.com/) ORM 与 [TiDB 向量检索](/vector-search/vector-search-overview.md) 进行交互，存储嵌入向量，并执行向量检索查询。
...
你可以按照以下步骤快速了解如何将 TiDB 向量检索与 Django ORM 集成。
...
如果你在本地运行 TiDB，`TIDB_HOST` 默认为 `127.0.0.1`。初始的 `TIDB_PASSWORD` 为空，因此如果是首次启动集群，可以省略该字段。
...
在浏览器中访问 `http://127.0.0.1:8000` 体验示例应用。以下是可用的 API 路径：
