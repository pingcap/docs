...
> **Note:**
>
> 此功能为 Beta 版本，可能会在没有提前通知的情况下发生变更。如果你发现了 bug，可以在 GitHub 上提交 [issue](https://github.com/pingcap/tidb/issues)。
...
当你使用接受多种数据类型的操作符或函数时，需要在将字符串传递给该操作符或函数前，显式将字符串类型转换为向量类型，因为此时 TiDB 不会进行隐式类型转换。例如，在进行比较操作前，需要显式将字符串转换为向量，否则 TiDB 会按字符串值进行比较，而不是按向量数值进行比较：
...
目前不支持 Vector 与其他数据类型（如 `JSON`）之间的直接类型转换。你可以在 SQL 语句中通过 String 作为中间类型进行转换来规避此限制。
