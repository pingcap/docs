---
title: 将 TiDB 向量检索集成到 SQLAlchemy
summary: 学习如何将 TiDB 向量检索与 SQLAlchemy 集成，用于存储嵌入向量并执行语义检索。
---

# 将 TiDB 向量检索集成到 SQLAlchemy

本教程将指导你如何使用 [SQLAlchemy](https://www.sqlalchemy.org/) 与 [TiDB 向量检索](/vector-search/vector-search-overview.md) 进行交互，存储嵌入向量，并执行向量检索查询。
