---
title: 集成 TiDB Vector Search 与 SQLAlchemy
summary: 学习如何将 TiDB Vector Search 与 SQLAlchemy 集成，以存储嵌入向量并执行语义搜索。
---

# 集成 TiDB Vector Search 与 SQLAlchemy

本教程将指导你如何使用 [SQLAlchemy](https://www.sqlalchemy.org/) 与 [TiDB Vector Search](/vector-search/vector-search-overview.md) 进行交互，存储嵌入向量，并执行向量搜索查询。

<CustomContent platform="tidb">

> **Warning:**
>
> 向量搜索功能为实验性功能。不建议在生产环境中使用。该功能可能会在没有提前通知的情况下发生变更。如果你发现了 bug，可以在 GitHub 上提交 [issue](https://github.com/pingcap/tidb/issues)。
...
对于 TiDB Cloud Starter 集群，请按照以下步骤获取集群连接字符串并配置环境变量：
