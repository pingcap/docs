...
| 函数名                                               | 描述                                                      | [向量索引](/vector-search/vector-search-index.md#vector-search-index)支持 |
| ---------------------------------------------------- | --------------------------------------------------------- | --------------------------- |
...
关于如何使用 `CAST()`，参见 [向量数据类型 | 类型转换](/vector-search/vector-search-data-types.md#cast)。
...
对于来自 OpenAI 的嵌入，建议你[使用此函数](https://help.openai.com/en/articles/6824809-embeddings-faq)。
...
将字符串转换为向量。在许多场景下，此转换会自动进行，例如向 `VECTOR` 数据类型的列插入数据时。然而，在某些表达式中（如向量的算术运算），如果不支持隐式转换，则需要显式调用此函数。
