...
> **Warning:**
>
> Use `*` with caution because it might introduce security risks by allowing a client of any IP address to report its IP address. In addition, even if `*` is set to allow proxy connections from all IP addresses, internal components that connect directly to TiDB (such as TiDB Dashboard) will not be able to connect to the TiDB server. Consider setting `--proxy-protocol-fallbackable` to `true`.
