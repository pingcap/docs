...
1.  設定ファイルを変更します。
    ...
    3.  設定ファイルの[`security`](/tidb-configuration-file.md#security)セクションに設定項目[`skip-grant-table`](/tidb-configuration-file.md)追加します。`security`がない場合は、 `tidb.toml`設定ファイルの末尾に次の2行を追加します。
