...
Quick access: [Quick start](https://docs.pingcap.com/tidb/v7.1/quick-start-with-tidb) | [Production deployment](https://docs.pingcap.com/tidb/v7.1/production-deployment-using-tiup) | [Installation packages](https://www.pingcap.com/download/?version=v7.1.5#version-list)
...
+ TiDB <!--tw@hfxsd 5 条-->
    ...
    - Fix the issue that TiDB keeps sending live requests to a TiFlash node that has been offline [#46602](https://github.com/pingcap/tidb/issues/46602) @[zyguan](https://github.com/zyguan)
    - Fix the issue that AutoID Leader change might cause the value of the auto-incrementing column to decrease in the case of `AUTO_ID_CACHE=1` [#52600](https://github.com/pingcap/tidb/issues/52600) @[tiancaiamao](https://github.com/tiancaiamao)
    ...
    - Fix the issue that executing `ANALYZE TABLE` blocks DDL statements [#47475](https://github.com/pingcap/tidb/issues/47475) @[wjhuang2016](https://github.com/wjhuang2016)
...
+ TiKV <!--tw@hfxsd 4 条-->
    ...
    - Fix the issue that resolve-ts is blocked when an outdated Region peer ignores the GC message [#16504](https://github.com/tikv/tikv/issues/16504) @[crazycs520](https://github.com/crazycs520)
    - Fix the issue that an error occurs when RocksDB synchronizes some data to an inactive Write Ahead Log (WAL, Write Ahead Log), which can lead to data corruption after TiKV restarts [#16705](https://github.com/tikv/tikv/issues/16705) @[Connor1996](https://github.com/Connor1996)
    - Fix the issue that the monitoring item `tikv_unified_read_pool_thread_count` sometimes has no data [#16629](https://github.com/tikv/tikv/issues/16629) @[YuJuncen](https://github.com/YuJuncen)
    - Fix the issue that `CheckTxnStatus` does not set the rollback record on the primary key of an optimistic transaction to be protected, potentially destroying the atomicity of the data [#16620](https://github.com/tikv/tikv/issues/16620) @[MyonKeminta](https://github.com/MyonKeminta)
