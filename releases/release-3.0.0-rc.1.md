...
-   SQLオプティマイザー
    -   列間の順序相関を使用してコスト見積もりの​​精度を向上させます。ヒューリスティック パラメーター`tidb_opt_correlation_exp_factor`を導入して、相関関係を推定に直接使用できない場合のシナリオでのインデックス スキャンの優先順位を制御します。 [#9839](https://github.com/pingcap/tidb/pull/9839)
