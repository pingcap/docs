...
-   DDL
    ...
    -   無効な DDL メタデータの有効期間を最適化して、TiDBクラスタのアップグレード後に DDL 操作の通常の実行を回復する速度を上げます
-   取引
...
-   その他
    ...
    -   ツールによる収集と分析を容易にするために、ログシステムを再構築した統一ログフォーマット仕様を開発する
    -   スケジューリング パラメーター、クラスタラベル情報、PD が TSO 要求を処理するために消費する時間、ストア ID およびアドレス情報などを含む監視メトリックを追加します。
...
-   TiKV サマリー監視ダッシュボードを追加して、クラスタのステータスを便利に表示します
...
-   TiDB 監視を最適化し、SQL カテゴリ別にレイテンシ関連の監視項目を追加
