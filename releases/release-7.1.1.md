...
summary: TiDB 7.1.1は2023年7月24日にリリースされました。このバージョンでは、新しいシステム変数`tidb_lock_unchanged_keys`が導入され、プラン キャッシュ、ディスクのパフォーマンス最適化、PDの改善などが行われました。さらに、バグの修正も含まれています。TiDB 7.1.1のリリースノートには、多くの改善点やバグの修正が含まれています。
---
