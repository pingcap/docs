...
-   TiDB
    ...
    -   `ENUM`のタイプの列で`JOIN`を実行するときに発生する可能性がある誤った結果を修正します[＃27831](https://github.com/pingcap/tidb/issues/27831)
...
-   TiKV
    ...
    -   悲観的なトランザクションでプリライトリクエストを再試行するときのまれなデータの不整合の問題を修正します[＃11187](https://github.com/tikv/tikv/issues/11187)
    ...
    -   書き込みフローが少ない場合に誤った「GCが機能しない」アラートを報告する問題を修正[＃9910](https://github.com/tikv/tikv/issues/9910)
...
-   TiFlash
    ...
    -   `DECIMAL`のデータ型のデータを比較するときにオーバーフローのバグと`Can't compare`のエラーを報告する問題を修正しました
    -   `3rd arguments of function substringUTF8 must be constants.`の予期しないエラーを修正します
    -   `nsl`のライブラリがないプラットフォームでTiFlashが起動しない問題を修正します
    ...
    -   TiFlashレプリカの数を0に設定した後、古いデータを再利用できない問題を修正します
...
-   ツール
    ...
    -   TiCDC
        ...
        -   [＃3929](https://github.com/pingcap/tiflow/issues/3929)のデフォルト値のパディング例外によって引き起こされるデータの不整合を修正[＃3918](https://github.com/pingcap/tiflow/issues/3918)
    ...
    -   TiDB Binlog
        ...
        -   CSVファイルのサイズが約256MBで`strict-format`が35の場合、CSVのインポートが`true`で失敗する問題を修正し[＃27763](https://github.com/pingcap/tidb/issues/27763) 。
