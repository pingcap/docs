...
| TiKV | [`raft-max-size-per-msg`](/tikv-configuration-file.md#raft-max-size-per-msg) | Modified | <ul><li>Change the minimum value from `0` to larger than `0`.</li><li>Add a maximum value of `3 GB`.</li><li>Change the unit from `MB` to `KB\|MB\|GB`.</li></ul> |
