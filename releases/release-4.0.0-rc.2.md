...
+ TiDB
    ...
    - Remove the 100 MB size limit for a single transaction when TiDB Binlog is enabled. Now the size limit for a transaction is 10 GB except that if TiDB Binlog is enabled and the downstream is Kafka, configure the `txn-total-size-limit` parameter according to the message size limit of 1 GB in kafka [#16941](https://github.com/pingcap/tidb/pull/16941)
