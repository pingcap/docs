...
summary: TiDB 4.0 RC.2は2020年5月15日にリリースされました。このバージョンでは、TiDB Binlogの有効化により、単一トランザクションのサイズ制限が削除され、トランザクションのサイズ制限は10 GBになりました。また、TiDB Lightningの互換性が向上し、データをインポートする際のデフォルトのSQLモードが変更されました。さらに、TiDBには新しいバックアップおよび復元コマンドのサポートが追加され、TiKVには暗号化関連の構成が移動されました。PDにはピア削除の速度制限が排除されました。TiFlashにはシステム負荷が低いときのファイル記述子の最適化が追加されました。
---
