...
| [構成の永続性を分析する](/statistics.md#persist-analyze-configurations)機能を有効にするかどうかを制御します。デフォルト値は`ON`です。                                                                                                                                                                                       |
...
-   v5.3 以前のバージョンでは、 [DMドキュメント](/dm/dm-overview.md) DM ドキュメント サイトにアクセスせずに直接読むことができます。
...
-   **カラムナ型storageエンジンTiFlashとコンピューティング エンジン MPP の安定性とパフォーマンスの継続的な向上**
    ...
    ユーザードキュメント: [tflash.toml ファイルを構成する](/tiflash/tiflash-configuration.md#configure-the-tiflashtoml-file)
...
-   ツール
    ...
    -   TiDBBinlog
        ...
        -   `CREATE PLACEMENT POLICY`ステートメント[#1118](https://github.com/pingcap/tidb-binlog/issues/1118)と互換性がないためにDrainerが失敗する問題を修正
