...
-   MySQL互換の多値インデックスが一般提供（GA）される[＃39592](https://github.com/pingcap/tidb/issues/39592) @ [xiongjiwei](https://github.com/xiongjiwei) @ [qw4990](https://github.com/qw4990) @ [YangKeao](https://github.com/YangKeao)
    ...
    JSON列内の配列の値をフィルタリングすることは一般的な操作ですが、通常のインデックスではこのような操作を高速化できません。配列に多値インデックスを作成すると、フィルタリングのパフォーマンスが大幅に向上します。JSON列の配列に多値インデックスがある場合、 `MEMBER OF()` `JSON_CONTAINS()` `JSON_OVERLAPS()`で検索条件をフィルタリングするために多値インデックスを使用できます。これにより、 I/O消費量が削減され、操作速度が向上します。
