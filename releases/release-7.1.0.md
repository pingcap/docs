...
* Improve the stability of TiDB Lightning when importing TiB-level data [#43510](https://github.com/pingcap/tidb/issues/43510) [#43657](https://github.com/pingcap/tidb/issues/43657) @[D3Hunter](https://github.com/D3Hunter) @[lance6716](https://github.com/lance6716) **tw:hfxsd**
    ...
    - `tikv-importer.pause-pd-scheduler-scope` controls the scope of pausing PD scheduling during data import. Available values are `"table"` and `"global"`. The default value is `"table"`. For TiDB versions earlier than v6.1.0, you can only configure the `"global"` option, which pauses global scheduling during data import. Starting from v6.1.0, the `"table"` option is supported, which means that scheduling is only paused for the Region that stores the target table data. It is recommended to set this parameter to `"global"` in scenarios with large data volumes to improve stability.
...
* Refactor the database auditing feature (Enterprise Edition) **tw:hfxsd**

    In v7.1.0, the database auditing capabilities have been refactored for TiDB Enterprise Edition, which significantly increases the scope of capabilities and improves the experience to meet the needs of enterprises for database security compliance:
    
    - Introduce the concepts of "Filter" and "Rule" to provide more granular audit event definitions and support more fine-grained audit settings.
    - Support defining rules in JSON format, providing a more friendly way for configuration.
    - Add automatic log rotation and space management functions. Support configuring log rotation in two dimensions: retention time and log size.
