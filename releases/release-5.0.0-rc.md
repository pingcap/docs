...
+ Reduced jitters. This is done by improving the optimizer stability and by limiting system tasks' usages of I/O, network, CPU, memory. For example, in a 72-hour performance test, the standard deviation of Sysbench TPS jitter was reduced from 11.09% to 3.36%.
+ Raft Join Consensus algorithm, which ensures the system availability during the membership change of Region.
Optimized `EXPLAIN` features and invisible index, which helps Database Administrators (DBAs) users debug SQL statements with higher efficiency.
+ Guaranteed reliability for enterprise data. You can back up data from TiDB to AWS S3 storage and Google Cloud GCS or can restore data from these cloud storage platforms.
+ Improved performance of importing data from or exporting data to AWS S3 storage or TiDB/MySQL helps enterprises quickly build applications in the cloud. For example, in a TPC-C test, the performance of importing 1 TiB data improved by 40%, from 254 GiB/h to 366 GiB/h.
...
However, when async commit is enabled, the external consistency of transactions can only be guaranteed when `tidb_guarantee_external_consistency = ON` is set. With async commit enabled, the performance significantly drops.
...
+ Reduce the scheduling issues caused by fluctuations of node capacity (always near the waterline) and by PD's `store-limit` configuration value set too large. This is done by introducing a new set of scheduling calculation formulas enabled via the `region-score-formula-version = v2` configuration item. [#3269](https://github.com/tikv/pd/pull/3269)
...
+ TiDB Lightning supports importing Aurora snapshot data from AWS S3 storages to TiDB. (Related issue: [#266](https://github.com/pingcap/tidb-lightning/issues/266))
+ In a TPC-C test of importing 1 TiB of data into DBaaS T1.standard, the performance improves by 40%, from 254 GiB/h to 366 GiB/h.
