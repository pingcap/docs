...
-   データベースパフォーマンスの可観測性を向上させるための継続的なプロファイリング実験的機能をサポートする
-   システムのパフォーマンスと安定性を向上させるために、ストレージエンジンとコンピューティングエンジンの最適化を継続します
-   I / O操作をRaftstoreスレッドプールから分離することにより、TiKVの書き込みレイテンシを短縮します（デフォルトでは無効）
...
| TiKV                           | [`storage.reserve-space`](/tikv-configuration-file.md#reserve-space)                               | 変更         | TiKVの起動時にディスク保護用に予約されているスペースを制御します。 v5.3.0以降、予約されたスペースの80％は、ディスクスペースが不足している場合の運用と保守に必要な追加のディスクスペースとして使用され、残りの20％は一時ファイルの保存に使用されます。                                                              |
