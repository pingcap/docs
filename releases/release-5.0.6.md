...
-   PD
    ...
    -   スケジューラが空のリージョンをスケジュールできるようにし、スケジューラ[＃4497](https://github.com/tikv/pd/issues/4497)の構成を固定することで、 `scatter-range-scheduler`スケジューラのスケジュール結果をより均等にします。
...
-   ツール
    ...
    -   TiCDC
        ...
        -   Kafkaシンク[＃3352](https://github.com/pingcap/tiflow/issues/3352)の`config.Metadata.Timeout`デフォルト設定を追加する
...
-   ティドブ
    ...
    -   符号なし`BIGINT`引数[＃30101](https://github.com/pingcap/tidb/issues/30101)を渡したときに`GREATEST`と`LEAST`の誤った結果が返される問題を修正
