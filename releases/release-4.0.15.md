...
summary: TiDB 4.0.15は2021年9月27日にリリースされました。互換性の変更により、新しいセッションで`SHOW VARIABLES`を実行すると遅くなる問題が修正されました。機能強化として、TiKVではTiCDC構成の動的変更がサポートされました。さらに、改善点として読み取りレイテンシーの短縮やPD間のリージョン情報の同期パフォーマンスの向上が行われました。バグの修正として、範囲を構築するときのバイナリリテラルの照合順序の修正やTiKVのトゥームストーンストアの問題の修正が行われました。
---
