...
summary: TiDB 3.0.8は2019年12月31日にリリースされました。SQLオプティマイザ、SQL実行エンジン、DDL、サーバー、トランザクション、モニター、TiKV、PD、TiDB Ansibleに関する様々な修正と改善が含まれています。主な変更点としては、SQLバインディングプランの修正、エラーメッセージの最適化、証明書ベースの認証のサポートなどが挙げられます。tidb_txn_mode`変数のデフォルト値が`"悲観的"`に更新されました。PDではパフォーマンスの最適化とバグ修正も行われました。TiDB Ansibleでは、様々なロジックの最適化とアップグレードが行われました。
---
...
- トランザクション
    ...
    - 悲観的トランザクションモードで未変更データがロック解除されるため、誤ったデータが読み取られる可能性がある問題を修正しました。 [#14050](https://github.com/pingcap/tidb/pull/14050)
    - mocktikv で事前書き込みを実行するときにトランザクション タイプが区別されないため、挿入値の制限チェックが繰り返される問題を修正しました。 [#14175](https://github.com/pingcap/tidb/pull/14175)
    - `session.TxnState`が`Invalid` ときにトランザクションが正しく処理されないためpanicを修正 [#13988](https://github.com/pingcap/tidb/pull/13988)
    - mocktikvの`ErrConfclit`構造に`ConflictCommitTS` が含まれていない問題を修正しました [#14080](https://github.com/pingcap/tidb/pull/14080)
    - TiDBがロックを解決した後にロックタイムアウトを正しくチェックしないため、トランザクションがブロックされる問題を修正しました。 [#14083](https://github.com/pingcap/tidb/pull/14083)
- モニター
    - `LockKeys` に`pessimistic_lock_keys_duration`監視項目を追加 [#14194](https://github.com/pingcap/tidb/pull/14194)

## TiKV {#tikv}

- コプロセッサー
    - コプロセッサーでエラーが発生した場合の出力ログのレベルを`error`から`warn`に変更します [#6051](https://github.com/tikv/tikv/pull/6051)
...
- TiDB Binlog
    - PumpがコミットされたDDLbinlogを受信しないため、binlogが無視される問題を修正しました。 [#853](https://github.com/pingcap/tidb-binlog/pull/853)
...
- Grafana のTiKV-Detailsページで監視項目`Thread Voluntary Context Switches`と`Thread Nonvoluntary Context Switches`を最適化 [#1071](https://github.com/pingcap/tidb-ansible/pull/1071)
