...
-   ティドブ
    ...
    -   グローバルソートを使用して一意のインデックスを追加する際のパフォーマンスを改善し、重複する一意のインデックスを追加する際のエラーメッセージを改良しました[＃61689](https://github.com/pingcap/tidb/issues/61689) @ [Cbcウェストウルフ](https://github.com/CbcWestwolf)
    -   `IMPORT INTO`でグローバルソート[＃60361](https://github.com/pingcap/tidb/issues/60361) @ [D3ハンター](https://github.com/D3Hunter)を有効にすると、TiKV のインポートモードへの切り替えが無効になります。
    -   インデックス追加[＃60925](https://github.com/pingcap/tidb/issues/60925) @ [Cbcウェストウルフ](https://github.com/CbcWestwolf)中の TiKV への書き込み速度を観察するための監視メトリックを追加します。
    -   `merge sort`サブタスクのスケジュールロジックを最適化して、ソートパフォーマンス[＃60375](https://github.com/pingcap/tidb/issues/60375) @ [接線](https://github.com/tangenta)を向上させる
...
-   ティクブ
    ...
    -   Raftstoreの`CompactedEvent`処理を`split-check`ワーカーに移動して最適化し、メインのRaftstoreスレッド[＃18532](https://github.com/tikv/tikv/issues/18532) @ [リクックスサシネーター](https://github.com/LykxSassinator)のブロッキングを削減します。
    -   SST の取り込みが遅すぎる場合は`SST ingest is experiencing slowdowns`ログに記録し、パフォーマンスのジッターを回避するために`get_sst_key_ranges`呼び出しをスキップします[＃18549](https://github.com/tikv/tikv/issues/18549) @ [LykxSassinator](https://github.com/LykxSassinator)
    -   KvDB と RaftDB が別々のマウント パス[＃18463](https://github.com/tikv/tikv/issues/18463) @ [リクックスサシネーター](https://github.com/LykxSassinator)を使用する場合の KvDB ディスク上のI/Oジッターの検出メカニズムを最適化します。
    -   Raft Engineの`fetch_entries_to`のパフォーマンスを最適化して競合を減らし、混合ワークロード[＃18605](https://github.com/tikv/tikv/issues/18605) @ [リクックスサシネーター](https://github.com/LykxSassinator)でのパフォーマンスを向上します。
    -   残留データのクリーンアップメカニズムを最適化して、リクエストのレイテンシー[＃18107](https://github.com/tikv/tikv/issues/18107) @ [リクックスサシネーター](https://github.com/LykxSassinator)への影響を軽減します。

-   PD

    -   Prometheus [＃8931](https://github.com/tikv/pd/issues/8931) @ [バッファフライ](https://github.com/bufferflies)に GO ランタイム関連の監視メトリクスを追加する
...
-   TiFlash
    ...
    -   storageスナップショットを取得する際の最大再試行回数を増やして、大規模なテーブルのクエリの安定性を向上させます[＃10300](https://github.com/pingcap/tiflash/issues/10300) @ [ジェイソン・ファン](https://github.com/JaySon-Huang)
...
-   ツール
    ...
    -   バックアップと復元 (BR)
        ...
        -   インデックスを同時に[＃59158](https://github.com/pingcap/tidb/issues/59158) @ [リーヴルス](https://github.com/Leavrth)修復することで、PITR 中のインデックス復元速度が向上します。
        -   TiKVのダウンロードAPIは、バックアップファイルをダウンロードする際に特定の時間範囲内のデータをフィルタリングすることをサポートしており、復元時に古いデータバージョンや将来のデータバージョンがインポートされることを回避します[＃18399](https://github.com/tikv/tikv/issues/18399) @ [3ポイントシュート](https://github.com/3pointer)
...
-   ティドブ
    ...
    -   `IMPORT INTO ... FROM SELECT`を使用してTiFlash [＃58443](https://github.com/pingcap/tidb/issues/58443) @ [D3ハンター](https://github.com/D3Hunter)にデータをインポートするときにエラーが発生する可能性がある問題を修正しました
    ...
    -   `CREATE INDEX IF NOT EXISTS`ステートメント[＃61265](https://github.com/pingcap/tidb/issues/61265) @ [Cbcウェストウルフ](https://github.com/CbcWestwolf)を実行すると空のインデックス名が生成される問題を修正しました
    -   メタデータ ロック (MDL) を無効にした後、スキーマ バージョン[＃61210](https://github.com/pingcap/tidb/issues/61210) @ [wjhuang2016](https://github.com/wjhuang2016)の更新に失敗して DDL 操作が停止する問題を修正しました。
    ...
    -   HashAgg 演算子[＃61749](https://github.com/pingcap/tidb/issues/61749) @ [xzhangxian1008](https://github.com/xzhangxian1008)で、 `basePartialResult4GroupConcat`のバッファが`nil`の場合にディスクへの書き込み時にpanicが発生する問題を修正しました。
    ...
    -   `IndexMerge`と`IndexLookUp`演算子で共有された KV リクエストが、クエリ[＃60175](https://github.com/pingcap/tidb/issues/60175)を[あなた06](https://github.com/you06)にプッシュダウンするときにデータ競合を引き起こす問題を修正しました。
    -   `_charset(xxx), _charset(xxx2), ...`を含む SQL 文が[xhebox](https://github.com/xhebox)に対して[＃58447](https://github.com/pingcap/tidb/issues/58447)の異なるダイジェストを生成する問題を修正しました
    -   無効な UTF-8 文字[＃47521](https://github.com/pingcap/tidb/issues/47521) @ [定義2014](https://github.com/Defined2014)を処理するときに TiDB がpanic可能性がある問題を修正しました
    ...
    -   ネットワークプロトコルでTiDBが返す列の長さが`0`になる場合がある問題を修正しました。3 `0`場合、TiDBは各フィールドタイプのデフォルトの長さである[＃60503](https://github.com/pingcap/tidb/issues/60503) @ [xhebox](https://github.com/xhebox)を返します。
    -   ネットワークプロトコルで返される型`blob`がMySQL [＃60195](https://github.com/pingcap/tidb/issues/60195) @ [ドヴェーデン](https://github.com/dveeden)と一致しない問題を修正しました
...
-   ティクブ
    ...
    -   TiKV が正常なシャットダウン中に進行中の手動圧縮タスクを終了できない問題を修正[＃18396](https://github.com/tikv/tikv/issues/18396) @ [リクックスサシネーター](https://github.com/LykxSassinator)
    ...
    -   Titan を無効にした後に BLOB インデックスがスナップショットの適用エラーを引き起こす可能性がある問題を修正[＃18434](https://github.com/tikv/tikv/issues/18434) @ [v01dスター](https://github.com/v01dstar)
...
-   PD
    ...
    -   クラスタのアップグレード[＃9416](https://github.com/tikv/pd/issues/9416) @ [rleungx](https://github.com/rleungx)後に Evict Leaderスケジューラが誤って一時停止される可能性がある問題を修正しました
    -   TiDBダッシュボードTCP接続を不適切に閉じるとPD goroutineリークが発生する可能性がある問題を修正[＃9402](https://github.com/tikv/pd/issues/9402) @ [バウリン](https://github.com/baurine)
    -   新しく追加された TiKV ノードが[＃9145](https://github.com/tikv/pd/issues/9145) @ [バッファフライ](https://github.com/bufferflies)でスケジュールされない可能性がある問題を修正しました
...
-   ツール
    ...
    -   バックアップと復元 (BR)
        ...
        -   HTTP/2 GOAWAY エラーが発生したときに外部storageからのデータインポートが自動的に再試行されない問題を修正[＃60143](https://github.com/pingcap/tidb/issues/60143) @ [ジョーチェン](https://github.com/joechenrh)
        -   インポートモードの切り替え[＃18541](https://github.com/tikv/tikv/issues/18541) @ [リーヴルス](https://github.com/Leavrth)により復元中に発生する`keepalive watchdog timedout`エラーを修正
    ...
    -   TiCDC
        ...
        -   Debeziumプロトコルで`column-selector`設定するとpanicが発生する可能性がある問題を修正しました[＃12208](https://github.com/pingcap/tiflow/issues/12208) @ [wk989898](https://github.com/wk989898)

    -   TiDB Lightning

        -   TiKVへのRPCリクエストが[＃60143](https://github.com/pingcap/tidb/issues/60143) @ [ジョーチェン](https://github.com/joechenrh)でタイムアウトすると、 TiDB Lightningが`context deadline exceeded`エラーを返す問題を修正しました。
