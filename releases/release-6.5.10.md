...
- ツール
    ...
    - TiDB Lightning
        ...
        - TiDB Lightning を使用して`SHARD_ROW_ID_BITS`と`AUTO_ID_CACHE=1`の両方が設定されたテーブルをインポートした後、TiDB がAUTO_INCREMENT ID を生成できず、エラー`Failed to read auto-increment value from storage engine`を報告する問題を修正しました。 [#52654](https://github.com/pingcap/tidb/issues/52654) @[D3Hunter](https://github.com/D3Hunter)
