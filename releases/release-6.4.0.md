...
| TiCDC | [`transaction-atomicity`](/ticdc/ticdc-sink-to-mysql.md#configure-sink-uri-for-mysql-or-tidb) | Modified | Changes the default value from `table` to `none`. This change helps reduce replication latency and reduce OOM risks. In addition, TiCDC now only splits a few transactions, instead of all transactions. |
