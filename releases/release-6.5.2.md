...
- To fix the potential issue of data loss in partitioned table scenarios, the default value of the TiCDC [`sink.enable-partition-separator`](/ticdc/ticdc-changefeed-config.md#changefeed-configuration-parameters) configuration item is changed from `false` to `true`. This means that partitions in a table are stored in separate directories by default when TiCDC replicates the table to storage services. It is recommended that you keep the value as `true` to avoid this issue. [#8581](https://github.com/pingcap/tiflow/issues/8581) @[CharlesCheung96](https://github.com/CharlesCheung96)
