...
summary: TiDB 5.1は2021年6月24日にリリースされました。新機能と改善点には、MySQL 8.0のCommon Table Expression (CTE)機能のサポート、動的特権機能のサポート、統計タイプの導入などが含まれます。さらに、安定性とパフォーマンスの向上が行われました。互換性の変更には、システム変数とコンフィグレーションファイルのパラメータの変更が含まれます。その他、バグの修正も行われました。
---
