...
| [`tidb_tso_client_rpc_mode`](/system-variables.md#tidb_tso_client_rpc_mode-new-in-v840) | Newly added | The original TSO requests were in synchronous mode. An asynchronous batch mode for TSO requests is now introduced with different concurrency capabilities. The asynchronous mode reduces the latency of getting TSOs, but might increase the load on the PD.  |
