...
2017年3月1日、TiDB RC2がリリースされました！このリリースでは、MySQLとの互換性、SQLクエリオプティマイザ、システムの安定性とパフォーマンスの向上に重点が置かれています。さらに、新しい権限管理メカニズムが追加され、ユーザーはMySQLの権限管理システムと同様にデータアクセスを制御できるようになりました。
...
- 基本的な権限管理をサポート
- MySQLの組み込み関数を多数追加
- Alter Table文を改良し、テーブル名、デフォルト値、コメントの変更をサポートします。
- Create Table Like文をサポートする
- 警告の表示ステートメントをサポートする
- Rename Table文をサポートする
- 大規模なトランザクションのクラスタブロックを回避するために、単一のトランザクションのサイズを制限します。
- データのロードプロセスでデータを自動的に分割する
- AddIndex および Delete文のパフォーマンスを最適化します
...
- リージョン数に基づいて迅速なスケジュールを実施
- pd-ctl はより多くの機能をサポートします
    - PDを追加または削除する
    - キーでリージョン情報を取得する
    - スケジューラとオペレーターの追加または削除
    - クラスタラベル情報を取得する
