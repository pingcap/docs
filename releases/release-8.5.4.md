...
- Modify the default value of the TiKV configuration item [`max-manifest-file-size`](https://docs.pingcap.com/tidb/v8.5/tikv-configuration-file#max-manifest-file-size) from `128MiB` to `256MiB` to avoid frequent manifest file compactions that could impact overall performance when a single TiKV node contains a large number of SST files. [#18996](https://github.com/tikv/tikv/issues/18996) @[glorv](https://github.com/glorv)
