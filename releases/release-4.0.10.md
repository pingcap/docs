...
summary: TiDB 4.0.10は2021年1月15日にリリースされました。新機能には、PDのログからユーザーデータを編集するための構成項目の追加があります。改善点では、トランザクション内のキーと値のエントリのサイズ制限が構成可能になりました。バグの修正では、同時実行性のバグや重複バインディングの問題が修正されています。TiFlashやツールにもさまざまな修正が加えられています。
---
