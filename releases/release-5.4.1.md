...
summary: このリリースには、TiDB、TiKV、PD、 TiFlash、およびさまざまなツールの互換性の変更、改善、バグ修正が含まれています。改善には、PointGet プランの使用のサポート、ログとメトリックの追加、Grafana ダッシュボードでの複数の Kubernetes クラスターの表示が含まれます。バグ修正では、date_format の不適切な処理、データの書き込みミス、クエリ結果の誤り、さまざまなパニックやエラーなどの問題に対処しています。TiKV、PD、 TiFlash、およびツールの修正も含まれています。」
---
...
TiDB v5.4.1 では、製品設計に互換性の変更は導入されていません。ただし、このリリースでのバグ修正により互換性も変更される可能性があることに注意してください。詳細については、 [バグの修正](#bug-fixes)を参照してください。
...
-   ティビ
    ...
    -   統計[＃34256](https://github.com/pingcap/tidb/issues/34256)を収集するために使用される Analyze バージョン 2 の`TopN`プルーニング ロジックを改善します。
...
-   ティビ
    ...
    -   `tidb_enable_vectorized_expression`が有効になっている`compress()`式を持つ SQL ステートメントを実行すると失敗する問題を修正しました[＃33397](https://github.com/pingcap/tidb/issues/33397)
    ...
    -   `sql_mode`が`NO_ZERO_DATE` [＃34099](https://github.com/pingcap/tidb/issues/34099)に設定されている場合に`'0000-00-00 00:00:00'` `datetime`列に挿入されるバグを修正
