...
- **Lock ビューの一般提供開始（GA）**
    ...
    - `TIDB_DECODE_SQL_DIGESTS`関数を追加して、クラスタ内の一連の SQL ダイジェストに対応する正規化された SQL文 (フォーマットや引数のない形式) を照会します。これにより、トランザクションによって過去に実行された文の照会操作が簡素化されます。
...
- TiFlash
    ...
    - 新しい照合順序が有効になっている場合、MPP モードでシャッフルハッシュ結合計算とシャッフルハッシュ集計計算をサポートします。
...
- TiFlash
    ...
    - 書き込み負荷が高い場合に発生するTiFlash panic問題を修正します。
