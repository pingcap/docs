...
summary: TiDB 5.2が2021年8月27日にリリースされました。このバージョンでは、式インデックスでの複数の関数の使用がサポートされ、オプティマイザーのカーディナリティ推定の精度が向上しました。また、トランザクション ロック イベントを監視するロックビュー機能の一般提供が発表され、TiFlashの読み取りおよび書き込みの安定性が向上しました。さらに、データ移行 (DM) の運用と保守が簡素化され、TiCDC タスクを管理するための HTTP プロトコル OpenAPI がサポートされました。MySQL 8.0 からの移行を容易にするためにcaching_sha2_password認証方法がサポートされています。
---
