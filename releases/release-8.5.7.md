...
| 変数名 | 変更の種類 | 説明 |
|--------|------------------------------|------|
| [`tidb_enable_telemetry`](https://docs.pingcap.com/tidb/v8.5/system-variables#tidb_enable_telemetry-new-in-v402) | 非推奨 | v8.5.7 以降、TiDB はこのシステム変数と テレメトリ機能を非推奨とします。この変数は互換性のためにのみ保持されており、今後の使用は推奨されません。|
...
| [`tidb_enable_batch_query_region`](https://docs.pingcap.com/tidb/v8.5/system-variables#tidb_enable_batch_query_region-new-in-v857) | 新しく追加された | TiDB が `QueryRegion` gRPC stream を介して Region 情報のポイントクエリを PD に対してバッチ送信するかどうかを制御します。デフォルト値は `OFF` です。有効にすると、一部のシナリオで PD に送信されるリクエスト数を減らし、PD leader の CPU オーバーヘッドを低減できます。|
...
| -------- | -------- | -------- | -------- |
| TiDB | [`enable-telemetry`](https://docs.pingcap.com/tidb/v8.5/tidb-configuration-file#enable-telemetry-new-in-v402) | 非推奨 | v8.5.7 以降、TiDB はこの設定項目と テレメトリ機能を非推奨とします。この項目は互換性のためにのみ保持されており、今後の使用は推奨されません。|
