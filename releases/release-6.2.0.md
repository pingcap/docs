...
* TiDB Dashboard adds a Monitoring page
    ...
    [User document](), ([#1381](https://github.com/pingcap/tidb-dashboard/issues/1381), @[YiniXu9506](https://github.com/YiniXu9506))
...
* TiDB Dashboard supports visual execution plans
    ...
    [User document](), ([#1224](https://github.com/pingcap/tidb-dashboard/issues/1224), @[time-and-fate](https://github.com/time-and-fate))
...
* Improve the `LEADING` optimizer hint to support outer join ordering
    ...
    [User document](), ([#29932](https://github.com/pingcap/tidb/issues/29932), @[Reminiscent](https://github.com/Reminiscent))
...
* Add a new optimizer `SEMI_JOIN_REWRITE` to improve the performance of `EXISTS` queries
    ...
    [User document](), ([#35323](https://github.com/pingcap/tidb/issues/35323), @[winoros](https://github.com/winoros))
...
* Add a new optimizer hint `MERGE` to improve the performance of analytical queries
    ...
    [User document](), ([#36122](https://github.com/pingcap/tidb/issues/36122), @[dayicklp](https://github.com/dayicklp))
...
* Optimize the performance of aggregation operations in some analytical scenarios
    ...
    [User document](), ([#36169](https://github.com/pingcap/tidb/issues/36169), @[fixdb](https://github.com/fixdb))
...
* Window functions pushed down to TiFlash can be executed in multiple threads
    ...
    [User document](/system-variables.md#tiflash_fine_grained_shuffle_batch_size-new-in-v620),  [User document](/system-variables.md#tiflash_fine_grained_shuffle_stream_count-new-in-v620),  ([4631](https://github.com/pingcap/tiflash/issues/4631), @[guo-shaoge](https://github.com/guo-shaoge))
...
* TiKV supports automatically tuning the CPU usage (experimental)
    ...
    Database usually has background processes to perform internal operations. By collecting statistical information, it can help users identify performance problems, generate better execution plans, and improve the stability and performance of the database. However, how to more efficiently collect information, and how to balance the resource overhead of background operations and foreground operations without affecting the daily use have always been one of the headaches in the database industry. Starting from v6.2.0, TiDB supports setting the CPU usage rate of background requests using the TiKV configuration file, thereby limiting the CPU usage ratio of background operations such as automatically collecting statistics in TiKV, and avoiding the resource preemption of user operations by background operations in extreme cases. This ensures that the operations of the database are stable and efficient. At the same time, TiDB also supports automatically adjusting CPU usage. Then, TiKV will adaptively adjust the CPU resources occupied by background requests according to the CPU usage of the instance. This feature is disabled by default.

    [User document](), ([#12503](https://github.com/tikv/tikv/issues/12503), @[BornChanger](https://github.com/BornChanger))
...
* Support setting savepoints in transactions
    ...
    [User document](), ([#6840](https://github.com/pingcap/tidb/issues/6840), @[crazycs520](https://github.com/crazycs520))
...
* Support configuring disk quota for TiDB Lightning (experimental)
    ...
    [User document](), ([#446](https://github.com/pingcap/tidb-lightning/issues/446), @[buchuitoudegou](https://github.com/buchuitoudegou))
...
* Automatically identify the region of Amazon S3 buckets
    ...
    [User document](), ([#34275](https://github.com/pingcap/tidb/issues/34275), @[WangLe1321](https://github.com/WangLe1321))
...
* TiDB Lightning supports importing data to production clusters in physical import mode
    ...
    [User document](), ([#35148](https://github.com/pingcap/tidb/issues/35148), @[gozssky](https://github.com/gozssky))
...
* BR supports restoring user and privilege data
    ...
    [User document](), ([#35395](https://github.com/pingcap/tidb/issues/35395), @[D3Hunter](https://github.com/D3Hunter))
...
* Support point-in-time recovery (PITR) based on backup and restoration of log and snapshot
    ...
    [User document](), ([#29501](https://github.com/pingcap/tidb/issues/29501), @[joccau](https://github.com/joccau))
...
* Support cross-cluster RawKV replication (experimental)
    ...
    [User document](), ([#11965](https://github.com/tikv/tikv/issues/11965), @[pingyu](https://github.com/pingyu))
...
- In v6.2.0, it is **strongly not recommended** that you set `dt_enable_logical_split` to `true` because of a know issue [#5576](https://github.com/pingcap/tiflash/issues/5576).
...
- Slow query logs, `information_schema.statements_summary`, and `information_schema.slow_query`can export `binary_plan`, execution plans encoded in the binary format.
- Two columns are added to the `SHOW TABLE xx REGIONS` statement: `SCHEDULING_CONSTRAINTS` and `SCHEDULING_STATE`, which respectively indicate Region scheduling constraints in Placement in SQL and the current scheduling state.
...
+ TiDB
    ...
    - Optimize the accuracy of memory tracking for some operators (HashJoin, HashAgg, Update, Delete) ([#35634](https://github.com/pingcap/tidb/issues/35634), @[wshwsh12](https://github.com/wshwsh12)), ([#35631](https://github.com/pingcap/tidb/issues/35631), @[wshwsh12](https://github.com/wshwsh12)), ([#35635](https://github.com/pingcap/tidb/issues/35635), @[wshwsh12](https://github.com/wshwsh12)), ([#34096](https://github.com/pingcap/tidb/issues/34096), @[ekexium](https://github.com/ekexium))
...
+ Tools
    ...
    - TiDB Lightning
        ...
        - Add more retryable errors, including EOF, Read index not ready, and Coprocessor timeout ([#36674](https://github.com/pingcap/tidb/issues/36674), @[D3Hunter](https://github.com/D3Hunter)), ([#36566](https://github.com/pingcap/tidb/issues/36566), @[D3Hunter](https://github.com/D3Hunter))
...
+ TiKV
    ...
    - (dup) Avoid reporting `WriteConflict` errors in pessimistic transactions ([#11612](https://github.com/tikv/tikv/issues/11612), @[sticnarf](https://github.com/sticnarf))
    - (dup) Fix the possible duplicate commit records in pessimistic transactions when async commit is enabled ([#12615](https://github.com/tikv/tikv/issues/12615), @[sticnarf](https://github.com/sticnarf))
    ...
    - Fix the issue that TiKV keeps reconnecting PD clients ([#12506](https://github.com/tikv/tikv/issues/12506), @[Connor1996](https://github.com/Connor1996)) ([#12827](https://github.com/tikv/tikv/issues/12827), @[Connor1996](https://github.com/Connor1996))
    - (dup) Fix the issue that TiKV panics when performing type conversion for an empty string ([#12673](https://github.com/tikv/tikv/issues/12673), @[wshwsh12](https://github.com/wshwsh12))
    - (dup) Fix the issue of time parsing error that occurs when the `DATETIME` values contain a fraction and `Z` ([#12739](https://github.com/tikv/tikv/issues/12739), @[gengliqi](https://github.com/gengliqi))
    ...
    - (dup) Fix the panic issue that might occur when a peer is being split and destroyed at the same time ([#12825](https://github.com/tikv/tikv/issues/12825), @[BusyJay](https://github.com/BusyJay))
    - (dup) Fix the panic issue that might occur when the source peer catches up logs by snapshot in the Region merge process ([#12663](https://github.com/tikv/tikv/issues/12663), @[BusyJay](https://github.com/BusyJay))
    - (dup) Fix the panic issue caused by analyzing statistics when `max_sample_size` is set to `0` ([#11192](https://github.com/tikv/tikv/issues/11192), @[LykxSassinator](https://github.com/LykxSassinator))
...
- [u5surf](https://github.com/u5surf) in [#36345](https://github.com/pingcap/tidb/pull/36345)
- [e1ijah1](https://github.com/e1ijah1) in [#36067](https://github.com/pingcap/tidb/pull/36067), [#35008](https://github.com/pingcap/tidb/pull/35008), and [#36065](https://github.com/pingcap/tidb/pull/36065)
- [PrajwalBorkar](https://github.com/PrajwalBorkar) in [#36048](https://github.com/pingcap/tidb/pull/36048)
- [likzn](https://github.com/likzn) in [#36024](https://github.com/pingcap/tidb/pull/36024)
- [rahulk789](https://github.com/rahulk789) in [#36021](https://github.com/pingcap/tidb/pull/36021)
- [virusdefender](https://github.com/virusdefender) in [#35999](https://github.com/pingcap/tidb/pull/35999)
- [joycse06](https://github.com/joycse06) in [#35995](https://github.com/pingcap/tidb/pull/35995)
- [morgo](https://github.com/morgo) in [#35992](https://github.com/pingcap/tidb/pull/35992) and others
- [ixuh12](https://github.com/ixuh12) in [#35760](https://github.com/pingcap/tidb/pull/35760)
- [blacktear23](https://github.com/blacktear23) in [#35560](https://github.com/pingcap/tidb/pull/35560)
- [johnhaxx7](https://github.com/johnhaxx7) in [#35202](https://github.com/pingcap/tidb/pull/35202)
- [GoGim1](https://github.com/GoGim1) in [#35143](https://github.com/pingcap/tidb/pull/35143)
- [renbaoshuo](https://github.com/renbaoshuo) in [#35124](https://github.com/pingcap/tidb/pull/35124)
- [Zheaoli](https://github.com/Zheaoli) in [#35089](https://github.com/pingcap/tidb/pull/35089)
- [fanrenhoo](https://github.com/fanrenhoo) in [#34751](https://github.com/pingcap/tidb/pull/34751) and [#34047](https://github.com/pingcap/tidb/pull/34047)
- [wirybeaver](https://github.com/wirybeaver) in [#5269](https://github.com/pingcap/tiflash/pull/5269)
- [hey-kong](https://github.com/hey-kong) in [#5235](https://github.com/pingcap/tiflash/pull/5235)
- [fatelei](https://github.com/fatelei) in [#6314](https://github.com/pingcap/tiflow/pull/6314) and [#5631](https://github.com/pingcap/tiflow/pull/5631)
- [eastfisher](https://github.com/eastfisher) made their first contribution in [#5831](https://github.com/pingcap/tiflow/pull/5831)
- [Juneezee](https://github.com/Juneezee) made their first-time contributor in in [#5660](https://github.com/pingcap/tiflow/pull/5660)
