...
- TiFlashは、より新しいバージョンのストレージフォーマットをサポートしています。
    ...
    TiDB v6.2.0では、データはデフォルトで新しいストレージ形式で保存されます。TiFlashを以前のバージョンからv6.2.0にアップグレードする場合、以前のTiFlashバージョンでは新しいストレージ形式を認識できないため、 TiFlash上でインプレースダウングレードを実行することはできませんのでご注意ください。
