...
- When this variable is disabled, if there is a write conflict among multiple pessimistic transactions, the pessimistic lock may be forced to roll back when the other pessimistic transactions commit, thus generating a `Pessimistic lock not found` error. When this error occurs, it means that the application scenario is not suitable for deferring the unique constraint check of the pessimistic transaction, and you should consider adjusting the application logic to avoid the conflict or retrying the transaction after the error occurs.
