...
Starting from v7.3, TiFlash introduces a new DMFile version: DMFile V3. This new DMFile version can merging multiple small files into a single larger file to reduce the total number of files. In v7.3, the default DMFile version is still V2. To use V3, you can set the [TiFlash configuration parameter](/tiflash/tiflash-configuration.md) `storage.format_version = 5`. After the setting, TiFlash can still read V2 DMFiles and will gradually rewrite existing V2 DMFiles to V3 DMFiles during subsequent data compaction.

After upgrading TiFlash to v7.3 and configuring TiFlash to use V3 DMFiles, if you need to revert TiFlash to an earlier version, you can use the DTTool offline to rewrite V3 DMFiles back to V2 DMFiles. For more information, see [DTTool Migration Tool](/tiflash/tiflash-command-line-flags.md#dttool-migrate).
