---
title: FAQs and Solutions after Upgrading
summary: List some FAQs and their solutions after upgrading.
category: deployment
---

# FAQs and Solutions after Upgrading
...
In v2.1.0 and the versions before (including all versions of v2.0), The character set of TiDB uses UTF-8 by default. But starting from v2.1.1, the default character set has been changed into UTF8MB4. 

If you explicitly specify the charset of a newly created table as UTF8 in v2.1.0 or the versions before, then you might fail to execute DDL operations after upgrading TiDB to v2.1.1.
...
Before upgrading, the following operations are executed in v2.1.0 and the versions before.
...
Solution: you can explicitly specify the column charset as the same with the original charset.
...
According to point #2, you can obtain the metadata of the table through HTTP API, and find the column charset by searching the column name and the keyword "Charset".
    ...
    ▶ curl "http://$IP:10080/schema/test/t" | python -m json.tool  # A python tool is used here to format JSON, which is not required and only for the convenience to add comments.
    {
        ...
        "collate": "",
        "cols": [			 # The relevant information about the column.
            {
                ...
                "id": 1,
                "name": {
...
You can also specify the column charset as done in issue #1, namely, make it stay consistent with the original column charset (UTF8MB4).
...
In TiDB v2.1.1 and the versions before, if the charset is UTF8, there is no UTF8 Unicode encoding check on the inserted 4-byte data. But in v2.1.2 and the later versions, this check is added.

Before upgrading, the following operations are executed in v2.1.1 and the versions before.
...
- In v2.1.2: this version does not support modifying the column charset, so you have to skip the UTF8 check.
...
- In v2.1.3 and the later versions: it is recommended to modify the column charset into UTF8MB4. Or you can set `tidb_skip_utf8_check` to skip the UTF8 check. But if you skip the check, you might fail to replicate data from TiDB to MySQL because MySQL executes the check. 
...
Specifically, you can use the parameter `tidb_skip_utf8_check` to skip the legal UTF8 and UTF8MB4 check on the data. But if you skip the check, you might fail to replicate the data from TiDB to MySQL because MySQL executes the check. 
