...
TiKV uses two RocksDB instance: the default RocksDB instance stores KV data, the Raft RocksDB instance (RaftDB) stores Raft logs.

TiKV implements `Column Families` (CF) from RocksDB.

Default RocksDB stores KV data in the `default`, `write` and `lock` CFs.
...
Raft RocksDB (RaftDB) stores Raft logs.
