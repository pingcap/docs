...
Repeatable Read分離レベルでは、トランザクション開始前にコミットされたデータのみが参照され、コミットされていないデータや、トランザクション実行中に同時実行トランザクションによってコミットされた変更は参照されません。ただし、トランザクション内の文は、同じトランザクション内の以前の文によって行われた変更を参照できます。これらの変更は、まだコミットされていない場合でも参照されます。
...
- 読み取りプロセス中に更新されたデータバージョンが検出されると、TiDB は新しいタイムスタンプを取得してこのステートメントを再試行します。
...
## トランザクション分離レベルの表示と変更 {#view-and-modify-transaction-isolation-levels}
