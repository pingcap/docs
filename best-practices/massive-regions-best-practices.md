...
`Region Merge`を有効にすることで、Regionの数を減らすこともできます。`Region Split`は異なり、 `Region Merge`スケジュール設定によって隣接する小さなRegionを結合するプロセスです。データを削除した後、または`Drop Table`もしくは`Truncate Table`文を実行した後、小さなRegion、あるいは空のRegionを結合することで、リソース消費を削減できます。
