...
To check the source IP address using `SHOW PROCESSLIST`, you need to configure the [PROXY protocol](https://www.haproxy.org/download/1.8/doc/proxy-protocol.txt) to connect TiDB.
...
> **Note:**
>
> Before using the PROXY protocol, you need to add target parameters in the `proxy-protocol.networks` configuration file of the TiDB server.
