...
2.  演算子を生成する
    ...
    さまざまなスケジューラーが、以下の考慮事項を考慮して、独自のロジックと要件に基づいてオペレーターを生成します。
...
ノードによってパフォーマンスが異なる可能性があるため、ストアごとに負荷分散の重みを設定することもできます。 `leader-weight`と`region-weight`は、それぞれリーダーの重みと領域の重みを制御するために使用されます（両方のデフォルトで「1」）。たとえば、ストアの`leader-weight`が「2」に設定されている場合、スケジューリングが安定した後、ノード上のリーダーの数は他のノードの数の約2倍になります。同様に、ストアの`leader-weight`を「0.5」に設定すると、ノード上のリーダーの数は他のノードの約半分になります。
...
スケールダウンとは、ストアをオフラインにし、コマンドを使用して「オフライン」としてマークするプロセスを指します。 PDは、スケジューリングによってオフラインノード上の領域を他のノードに複製します。障害回復は、ストアに障害が発生して回復できない場合に適用されます。この場合、対応するストアにピアが分散されているリージョンはレプリカを失う可能性があり、PDが他のノードに補充する必要があります。

スケールダウンと障害回復のプロセスは基本的に同じです。 `replicaChecker`は、異常状態のリージョンピアを検出し、正常なストアで異常ピアを新しいピアと交換するための演算子を生成します。
...
次のコマンドでpd-ctlを使用して、オペレーターにクエリを実行できます。

-   `operator show` ：現在のスケジューリングタスクで生成されたすべてのオペレーターを照会します
...
-   `region topread [limit]` ：読み取りトラフィックが多い地域を照会します
-   `region topwrite [limit]` ：書き込みトラフィックが多い地域を照会します
...
-   ハードウェアとソフトウェアの環境は店舗によって異なります。それに応じて`leader-weight`と`region-weight`の値を調整して、リーダー/リージョンの分布を制御できます。
-   その他の不明な理由。それでも、 `leader-weight`と`region-weight`の値を調整して、リーダー/リージョンの分布を制御できます。
...
-   PDは、対応するバランシングスケジューラの生成に失敗します。考えられる理由は次のとおりです。
    ...
    -   スケジューラーがアクティブ化されていません。たとえば、対応するスケジューラーが削除されるか、その制限が「0」に設定されます。
...
このシナリオでは、関連するメトリックを介してオペレーターの生成と実行を調べる必要があります。
...
-   オペレーターが停止するか、 `replica-schedule-limit`が「0」に設定されます。
-   リージョン移行用の適切なノードがありません。たとえば、（同じラベルの）置換ノードの使用可能な容量サイズが20％未満の場合、PDは、そのノードのストレージが不足しないようにスケジューリングを停止します。このような場合、スペースを解放するには、ノードを追加するか、一部のデータを削除する必要があります。
...
v3.0.4およびv2.1.16以前の場合、特定の状況では`approximate_keys`の領域が不正確になり（ほとんどの場合、テーブルを削除した後に発生します）、キーの数が`max-merge-region-keys`の制約を破ります。この問題を回避するには、 `max-merge-region-keys`をより大きな値に調整します。
...
実際には、ノードの障害が回復不能と見なされた場合は、すぐにオフラインにすることができます。これにより、PDは別のノードですぐにレプリカを補充し、データ損失のリスクを軽減します。対照的に、ノードが回復可能であると見なされても、回復が30分で実行できない場合は、タイムアウト後にレプリカとリソースの無駄が不要に補充されないように、一時的に`max-store-down-time`をより大きな値に調整できます。
