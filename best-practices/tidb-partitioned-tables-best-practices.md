...
| 設定                | 平均クエリ時間 | タスクの収集（インデックススキャン） | 警官タスク（テーブル検索） | 警官の合計タスク |
| ------------------------- | ------- | ------------------ | ------------- | -------- |
...
TiDB では、読み取りまたは書き込みトラフィックが[地域](/tidb-ストレージ.md#region)に不均等に分散されている場合にホットスポットが発生します。ホットスポットは、次のような場合によく発生します。
