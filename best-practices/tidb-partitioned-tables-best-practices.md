...
| 設定                | 平均クエリ時間 | COP タスク（インデックススキャン） | COP タスク（テーブル検索） | COP タスクの合計 |
| ------------------------- | ------- | ------------------ | ------------- | -------- |
| 非パーティションテーブル              | 12.6ミリ秒 | 72                 | 79            | 151      |
...
<details><summary><b>非パーティションテーブル</b></summary>
...
- 大規模または時間ベースのデータクリーンアップには、 `DROP PARTITION`を使用したパーティション分割テーブルを使用してください。このアプローチにより、パフォーマンスが向上し、システムへの影響が軽減され、操作が簡素化されます。
...
パーティション化されていないテーブルでは、新しい行は常に最大のキー値を持ち、同じ「最後の」リージョンに書き込まれます。この単一のリージョンは、1つのTiKVノードによって処理されるため、書き込みのボトルネックになる可能性があります。
