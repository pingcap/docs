...
summary: TiDB パーティション テーブルを使用してパフォーマンスを向上させ、データ管理を簡素化し、大規模なデータセットを効率的に処理するためのベストプラクティスを学習します。
...
### セカンダリ インデックスのクエリパフォーマンス: 非パーティション テーブルとローカル インデックスとグローバル インデックスの比較 {#query-performance-on-secondary-indexes-non-partitioned-tables-vs-local-indexes-vs-global-indexes}
...
-   **グローバル インデックスを持つパーティション テーブル**: インデックススキャンの効率は向上しますが、多くの行が一致する場合、テーブル検索のコストは依然として高くなります。
