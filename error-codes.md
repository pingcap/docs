...
* Error Number: 8249
    ...
    The resource group does not exist. This error is returned when you modify or bind a resource group that does not exist.

* Error Number: 8250

    The complete error message is `ERROR 9001 (HY000) : Resource control feature is disabled. Run "SET GLOBAL tidb_enable_resource_control='on'" to enable the feature`

    This error is returned when you try to use the `Resource Control` feature but the resource control feature is not enabled.
