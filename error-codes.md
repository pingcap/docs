...
TiDB は MySQL のエラー コードと互換性があり、ほとんどの場合、MySQL と同じエラー コードを返します。 MySQL のエラー コードのリストについては、 [MySQL 8.0 エラー メッセージ リファレンス](https://dev.mysql.com/doc/mysql-errors/8.0/en/)を参照してください。さらに、TiDB には次のような固有のエラー コードがあります。
...
-   エラー番号: 8024
    ...
    無効なトランザクションです。 TiDB が、実行中のトランザクションのトランザクション ID (開始タイムスタンプ) が取得されていないことを検出した場合、つまりこのトランザクションが無効であることを意味し、このエラーが返されます。
