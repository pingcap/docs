...
現在、TiDBは、リージョンマージの動作を制御するために、テーブルまたはパーティションに属性`merge_option`を追加することのみをサポートしています。属性`merge_option`は、ホットスポットに対処する方法の一部にすぎません。
...
テーブル属性は`key=value`の形式です。複数の属性はカンマで区切られます。以下の例では、 `t`は変更するテーブル名、 `p`は変更するパーティション名です。 `[]`の項目はオプションです。
