...
summary: TiDB DashboardのSQL文ページには、クラスター内のすべてのSQL文の実行ステータスが表示されます。このページでは、実行時間の長いSQL文を分析でき、アクセス、フィルタリング、列の追加表示、並べ替え、設定の変更などのオプションが提供されます。また、保存するSQL文の数を制限する機能も備えています。詳細については、TiDB Dashboardのドキュメントをご覧ください。
---

# TiDB DashboardのSQL文ページ {#sql-statements-page-of-tidb-dashboard}
...
SQL文の概要ページにアクセスするには、次の2つの方法のいずれかを使用できます。
...
> **Note:**
>
> SQL文の概要ページの**Mean Latency**列では、青いバーが平均実行時間を示します。SQL文の青いバーに黄色の線が表示されている場合、黄色の線の左側と右側はそれぞれ、最新のデータ収集サイクルにおけるその SQL文の最小実行時間と最大実行時間を表します。
...
リスト ページで、右上にある**Settings**ボタンをクリックして、SQL 文機能の設定を変更します。
...
> **Note:**
>
> - ステートメント システムテーブルはメモリ内にのみ保存されるため、SQL文機能が無効にされると、システムテーブル内のデータはクリアされます。
>
