...
- `tidb_kv_request.Get` : TiDB が`Get`種類のキーバリュー要求を送信する期間。
- `tidb_kv_request.Cop` : TiDB が`Cop`種類のキーバリュー要求を送信する期間。
...
TiDB がキーバリュー要求を送信する合計時間は 14745.07秒で、そのうち、タイプ`Get`と`Cop`キーバリュー要求にはそれぞれ 9798.02秒と 4946.46秒かかります。
