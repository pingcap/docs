...
-   `19306.46s` 、メトリック`tidb_execute`の合計実行時間が 19306.46 秒であることを示します。`89.40%` 、19306.46 秒がすべての SQL クエリ（ユーザー SQL クエリと TiDB 内部 SQL クエリを含む）の合計実行時間の 89.40% を占めていることを示します。クエリの合計実行時間は、 `tidb_query`の合計実行時間です。
-   `9070.18s` 、 `tidb_execute`ノード自体の合計実行時間が 9070.18 秒であり、残りがその子ノードによって消費された時間であることを表します。`42.00%` 、9070.18 秒がすべてのクエリの合計クエリ時間の 42.00% を占めることを表します。
