...
Continuous Profiling is only enabled by default in TiDB v6.1.0 and has been disabled by default since TiDB v6.1.1. After enabling it, you can have performance data continuously collected in the background without keeping the web pages always active. Data collected can be kept for a certain period of time and expired data is automatically cleared.
