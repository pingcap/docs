...
* `Optimize_time`: The optimization time for the query plan.
...
* `Stats`: The health state of tables. `pseudo` indicates that the statistics is unhealthy.
...
* `KV_total`: The time consumed for all the RPC requests on TiKV/TiFlash.
...
* `IsExplicitTxn`: Whether this statement is in an explicit transaction. If the value is `false`, the transaction is `autocommit=1` and the statement is committed after execution.
...
* `Num_cop_tasks`: The number of Coprocessor tasks that a statement sends.
