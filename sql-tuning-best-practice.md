...
列`id`は主キーであるため、暗黙的にインデックス`test_idx`に含まれます。ただし、 `IndexRangeScan_20`は順序を保証しません。これは、列`test_idx`はインデックスプレフィックス列`snapshot_id`の後に 2 つの追加列（ `user_id`と`status` ）が含まれているためです。その結果、列`id`の順序は保持されません。
