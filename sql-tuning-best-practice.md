...
SQL ステートメントは、TiDB オプティマイザーで主に 3つの最適化段階を経ます。
...
実行計画を読むときは、次の 2つの重要な概念を考慮してください。
...
列`id`は主キーであるため、暗黙的にインデックス`test_idx`に含まれます。ただし、 `IndexRangeScan_20`は順序を保証しません。これは、`test_idx`はインデックスプレフィックス列`snapshot_id`の後に 2つの追加列（ `user_id`と`status` ）が含まれているためです。その結果、列`id`の順序は保持されません。
...
-   TiKV プラン: TiKV ではクエリに 2分 38.6秒かかります。データが 5,121 のリージョンに分散されているため、 `TableRangeScan`によって、5,121 の cop タスクが送信されます。
