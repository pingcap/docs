...
* Currently, the threshold for operator-level spilling is calculated for each operator separately. For a query containing two Hash Aggregation operators, if the query-level spilling is not configured and the threshold of the operator is set to 10 GiB, each of the two Hash Aggregation operators will only spill data when their respective memory usage exceeds 10 GiB.
