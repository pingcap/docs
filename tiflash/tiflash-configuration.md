...
- `max_threads` indicates the internal thread concurrency when TiFlash executes an MPP task. When it is set to `0`, TiFlash uses the number of CPU logical cores as the execution concurrency.
...
- Default value: `0`, which means twice the number of CPU logical cores
