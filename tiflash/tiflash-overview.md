...
To deploy TiFlash under the Linux AMD64 architecture, the CPU must support AVX2 instruction sets. Ensure `cat /proc/cpuinfo | grep avx2` has output. To deploy TiFlash under the Linux ARM64 architecture, the CPU must support ARMv8 instruction sets. Ensure that `cat /proc/cpuinfo | grep 'crc32' | grep 'asimd'` has output. By using such CPU instruction sets, TiFlash's vectorization engine can deliver better performance.
