...
TiFlashは、TiKVでの書き込みをブロックしない低コストで、TiKVノードのデータのリアルタイムレプリケーションを実行します。一方、TiKVと同じ読み取り整合性を提供し、最新のデータが確実に読み取られるようにします。 TiFlashのリージョンレプリカはTiKVのリージョンレプリカと論理的に同一であり、同時に分割され、TiKVのリーダーレプリカとともにマージされます。
