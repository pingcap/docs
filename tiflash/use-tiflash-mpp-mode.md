...
- [`tidb_prefer_broadcast_join_by_exchange_data_size`](/system-variables.md#tidb_prefer_broadcast_join_by_exchange_data_size-new-in-v71): Used to control whether to minimum the overhead of network transmission. If this variable is enabled, TiDB will estimate the approximate size of the data to be exchanged by network when using `Broadcast Hash Join` and `Shuffled Hash Join`, then choose the one with the smaller size. [`tidb_broadcast_join_threshold_count`](/system-variables.md#tidb_broadcast_join_threshold_count-new-in-v50) and [`tidb_broadcast_join_threshold_size`](/system-variables.md#tidb_broadcast_join_threshold_count-new-in-v50) will be invalid after this variable enabled.
