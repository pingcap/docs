...
TiFlash のバージョンを確認するには、次の2つの方法があります。
...
- TiFlashのバイナリファイル名が`tiflash`の場合、 `./tiflash version`コマンドを実行することでバージョンを確認できます。
    ...
    たとえば、 `tiflash`と`libtiflash_proxy.so`が同じディレクトリにある場合は、まずこのディレクトリに切り替えて、次のコマンドを使用してTiFlash のバージョンを確認できます。
...
| LOCATION_LABELS    | PDのヒント。リージョン内の複数のレプリカが分散されているかどうかに基づいている。 |
| AVAILABLE     | 利用可能かどうか（0/1）                             |
| PROGRESS       | レプリケーションの進行状況 [0.0~1.0]                   |
