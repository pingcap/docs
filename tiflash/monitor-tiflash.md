...
- Write Stall Duration: The stall duration of write and delete range.
...
- Write Command OPS By Instance: The total count of different kinds of commands received by instance. `write block` means some data sync through Raft. `delete_range` means some Regions are removed from or move to this instance. `ingest` means some Region snapshot are applied to this instance.
