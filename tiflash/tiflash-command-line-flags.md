...
- Parameters:
    ...
    - `--version`: The target version of DTFile. The value options are `1` and `2` (default). `1` is the old version, `2` is the version corresponding to the new checksum, and `3` is the version that supports merging small files.
