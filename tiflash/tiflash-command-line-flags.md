...
- Parameters:
    ...
    - `--level`: The target compression level. If not specified, the recommended compression level is used by default according to the compression algorithm. If `compression` is set to `LZ4`, the default level is 1. If `compression` is set to `zstd`, the default level is 1. If `compression` is set to `LZ4HC`, the default level is 9. 
