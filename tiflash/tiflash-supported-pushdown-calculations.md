...
* [Window functions](/functions-and-operators/window-functions.md): Currently, TiFlash supports `ROW_NUMBER()`, `RANK()`, `DENSE_RANK()`, `LEAD()`, `LAG()`, `FIRST_VALUE()`, and `LAST_VALUE()`. Moreover, aggregation functions including `min`,`max`,`avg`,`count` and `sum` can also be pushed down.
