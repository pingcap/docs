...
* HashJoin：使用 [Hash Join](/explain-joins.md#hash-join) 算法进行 join 计算，但需满足以下条件：
    ...
    * 上述 join 类型均支持等值 join 和非等值 join（笛卡尔 join 或 Null-aware Semi Join）。在计算笛卡尔 join 或 Null-aware Semi Join 时，采用 Broadcast 算法，而非 Shuffle Hash Join 算法。
...
| [逻辑函数](/functions-and-operators/control-flow-functions.md) 及 [运算符](/functions-and-operators/operators.md) | `AND`、`OR`、`NOT`、`CASE WHEN`、`IF()`、`IFNULL()`、`ISNULL()`、`IN`、`LIKE`、`ILIKE`、`COALESCE`、`IS` |
