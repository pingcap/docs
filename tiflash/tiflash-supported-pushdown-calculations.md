...
* [Window functions](/functions-and-operators/window-functions.md): Currently, TiFlash supports window functions `ROW_NUMBER()`, `RANK()`, `DENSE_RANK()`, `LEAD()`, `LAG()`, `FIRST_VALUE()`, `LAST_VALUE()` and aggregation functions `MIN()`, `MAX()`, `AVG()`, `COUNT()`, `SUM()`.
