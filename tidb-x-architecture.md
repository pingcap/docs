...
The underlying storage engine (RocksDB) in classic TiDB uses a single LSM-tree protected by a global mutex. This creates a scalability ceiling where the system struggles to handle large datasets (e.g., 6TB+ data per TiKV node or 200k+ SST files), preventing it from utilizing the full capacity of the hardware.
