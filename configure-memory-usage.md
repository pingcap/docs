...
In the default configuration, a tidb-server instance prints an alarm log and records associated status files when the machine memory usage reaches 80% of its total memory. You can set the memory usage ratio threshold by changing `tidb_memory_usage_alarm_ratio`. For detailed alarm rules, refer to the description of [`tidb_memory_usage_alarm_ratio`](/system-variables.md#tidb_memory_usage_alarm_ratio).
...
1. Set `tidb_memory_usage_alarm_ratio` to `0.8`:
