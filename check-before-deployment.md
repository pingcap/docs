...
1.  次のコマンドを実行します。`running`返された場合、NTPサービスは実行中です。
...
NTPサービスの同期をできるだけ早く開始するには、次のコマンドを実行してください。`pool.ntp.org` NTPサーバーに置き換えてください。
