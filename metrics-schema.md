...
- `internal` : 統計情報を更新し、グローバル変数を取得するために使用される TiDB の内部 SQL文。
...
上記の結果から、実行計画には`PromQL` 、 `start_time` 、 `end_time` 、 `step`が含まれていることがわかります。実行プロセス中、TiDBはPrometheusの`query_range` HTTP APIを呼び出して監視データを照会します。
...
3. 実行計画を表示します。結果から、実行計画の`PromQL`と`step`値が30秒に変更されていることも確認できます。
