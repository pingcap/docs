...
[sync-diff-inspector](https://github.com/pingcap/tiflow/tree/master/sync_diff_inspector)は、MySQL互換データベース（MySQL および TiDB を含む）に保存されているデータを比較するためのツールです。例えば、MySQLのデータとTiDBのデータ、MySQLのデータとMySQLのデータ、またはTiDBのデータとTiDBのデータを比較できます。さらに、少量のデータに不整合がある場合、このツールを使用してデータを修復することもできます。
...
- テーブルスキーマとデータを比較する
- データ不整合が存在する場合に、データ修復に使用するSQL文を生成します。
...
- バイナリパッケージ: TiDB Toolkitに含まれています (従来の[`tidb-tools`](https://github.com/pingcap/tidb-tools)リポジトリから)。ツールキットをダウンロードするには、 [TiDBツールをダウンロード](/download-ecosystem-tools.md)を参照してください。
...
- データ型サポートに関する注意事項:
    ...
    - **FLOAT/DOUBLE**: 浮動小数点型は TiDB と MySQL で実装が異なります。 `FLOAT`と`DOUBLE`は、それぞれチェックサムの計算に 6 桁と 15 桁の有効数字を使用します。この機能を使用しない場合は、 `ignore-columns`を設定して、これらの列のチェックをスキップしてください。
    - **JSON**: 比較がサポートされています。JSON 文字列値については、アップストリームとダウンストリーム間の照合順序および文字セットの違いにより、誤検知が発生する可能性があることに注意してください。
    - **BLOB/VARBINARY**: バイナリデータとしてサポートされ、バイト単位で比較されます。
    - **BIT**: MySQL から TiDB への比較でサポートされています。幅 1、8、16、64 で検証されています。スキーマで標準外の幅やアプリケーションレベルの変換を使用している場合は、対象を絞った検証テストを実施してください。
...
## 設定ファイルの説明 {#configuration-file-description}

sync-diff-inspectorの設定は、以下の部分から構成されます。

- `Global config` : 一般的な設定。チェックするスレッド数、不整合なテーブルを修正するために SQL文をエクスポートするかどうか、データを比較するかどうか、上流または下流に存在しないテーブルのチェックをスキップするかどうかなど。
...
このコマンドは`config.toml`の`output-dir`にチェック レポート`summary.txt`とログ`sync_diff.log`を出力します。また、 `output-dir`には、 `config. toml`ファイルのハッシュ値で命名されたフォルダも生成されます。このフォルダには、ブレークポイントのチェックポイント ノード情報と、データに不整合が生じた場合に生成される SQL ファイルが含まれます。
...
データチェック処理中に異なる行が存在する場合、それらを修正するための SQL文が生成されます。データの不整合がチャンク内に存在する場合、 `chunk.Index`という名前の SQL ファイルが生成されます。この SQL ファイルは`${output}/fix-on-${instance}`にあり、 `${instance}`は`config.toml`ファイル内の`task.target-instance`の値です。
...
- MySQL と TiDB のデータを比較する前に、テーブルの文字セットと`collation`構成を確認してください。これは、UTF-8 データを含む `varchar`、`text`、または `JSON` カラムを持つテーブル、特にこれらのカラムが主キーまたは一意キーの一部である場合に重要です。MySQL 8.0 のデフォルトは `utf8mb4_0900_ai_ci`（大文字小文字を区別せず、アクセントも区別しない）である一方、TiDB では `utf8mb4_bin`（バイナリ/大文字小文字を区別する）がよく使用されます。この不一致により、sync-diff-inspector は同一の UTF-8 文字列や JSON 文字列値に対して誤った差分を報告する可能性があります。誤検知を避けるには、アップストリームとダウンストリームの両方のテーブルで照合順序（たとえば `utf8mb4_bin`）を揃えるか、`ignore-columns` を使用して影響を受ける UTF-8 テキストカラムおよび JSON カラムを除外してください。
- sync-diff-inspector 設定ファイルで`collation`を設定し、チャンクベースの比較時にアップストリームとダウンストリームの両方で同じ照合順序を明示的に使用する場合、インデックス列の順序はテーブルの照合順序設定に依存することに注意してください。照合順序が異なると、一方の側でインデックスを使用できなくなる可能性があります。さらに、アップストリームとダウンストリームで文字セットが異なる場合 (たとえば、MySQL が `utf8` を使用し、TiDB が `utf8mb4` を使用する場合)、照合順序設定を統一することはできません。
- データが論理的に同一であっても、次のようなシナリオでは誤った差分が生じる可能性があります:
    - アップストリームとダウンストリームで照合順序が異なる VARCHAR および TEXT カラム（たとえば、`utf8mb4_0900_ai_ci` と `utf8mb4_bin`）では、同一の文字列値に対して差分が報告されます。
    - 文字列値を含む JSON カラムは、同じ照合順序ベースの比較の問題の影響を受けます。
    - 自動入力される TIMESTAMP カラム（`DEFAULT CURRENT_TIMESTAMP` や `ON UPDATE CURRENT_TIMESTAMP` など）は、スキーマが異なる時刻にロードされた場合や、わずかなタイミング差のあるデータを比較する場合にノイズを生じさせる可能性があります。
