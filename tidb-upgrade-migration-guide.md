...
> **Note:**
>
> `tidb_gc_life_time`増やすと、 [MVCC](/glossary.md#multi-version-concurrency-control-mvcc)データのストレージ使用量が増加し、クエリのパフォーマンスに影響する可能性があります。詳細については、 [GCの概要](/garbage-collection-overview.md)を参照してください。ストレージとパフォーマンスへの影響を考慮しながら、推定操作時間に基づいてGC期間を調整してください。
...
- **設定の整合性**：古いクラスタと新しいクラスタの構成が[`new_collations_enabled_on_first_bootstrap`](/tidb-configuration-file.md#new_collations_enabled_on_first_bootstrap)であることを確認してください。同一でない場合、 BRの復元は失敗します。
...
- レイテンシー メトリック: `Changefeed checkpoint lag`が 5分以内などの許容範囲内に留まることを確認します。
...
- (オプション) TiCDC 設定項目[`gc-ttl`](/ticdc/ticdc-server-config.md#gc-ttl)を調整します (デフォルトは 24時間)。
    ...
    `gc-ttl`の値を増やすと、 `tidb_gc_life_time`値を増やした場合と同様に、より多くのMVCCデータが蓄積されます。適度に長く、かつ適切な値に設定することをお勧めします。

### 2. データの整合性を検証する {#2-verify-data-consistency}
...
2. TiCDC が追いつくようにする:
    ...
    - 古いクラスターを読み取り専用モードに設定した後、現在の`up-tso`を取得します。
...
- 1週間など、指定された期間にわたってシステムを監視します。問題が発生した場合は、古いクラスターに戻してください。
