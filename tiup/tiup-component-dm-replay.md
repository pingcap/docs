...
summary: tiup dm replay` コマンドを使用すると、失敗したクラスター操作を再試行し、正常に実行された手順をスキップできます。再試行するコマンドの `audit-id` を使用してください。このIDは `tiup dm audit` コマンドで確認できます。これにより、大規模クラスターで操作を再実行する際の時間節約に役立ちます。
---
