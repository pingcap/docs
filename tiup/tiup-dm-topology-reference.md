...
summary: TiDB データ マイグレーション (DM) クラスターをデプロイまたはスケーリングするには、クラスター トポロジーを記述するトポロジー ファイルを提供する必要があります。トポロジー ファイルを変更するには、クラスターのデプロイ後は、トポロジー ファイル内のフィールドの一部のみを変更できます。トポロジ ファイルの各セクションと各セクションの各フィールドを紹介します。ファイル構造には、グローバル、サーバー構成、マスターサーバー、ワーカーサーバー、監視サーバー、グラファナサーバー、アラートマネージャー_サーバーのセクションが含まれます。
---
