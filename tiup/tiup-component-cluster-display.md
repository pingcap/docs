...
> **Note:**
>
> If the `-R, --role` option is specified at the same time, only the service status that matches both the specifications of `-N, --node` and `-R, --role` are queried.
...
> **Note:**
>
> If the `-N, --node` option is specified at the same time, only the service status that matches both the specifications of `-N, --node` and `-R, --role` are queried.
