...
- バイナリの更新後、 `<binary> --config-check <config-file>`を使用して TiDB、TiKV、PD コンポーネントの構成チェックが実行されます。`<binary>`は、新しくデプロイされたバイナリへのパスであり、 `<config-file>`は、ユーザー設定に基づいて生成された設定ファイルです。このチェックをスキップするには、 `--ignore-config-check`オプションを使用します。
- データ型: `BOOLEAN`
- デフォルト: false
