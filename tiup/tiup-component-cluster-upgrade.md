...
-   バイナリの更新後、 `<binary> --config-check <config-file>`使用して TiDB、TiKV、PD コンポーネントの構成チェックが実行されます。`<binary>`新しくデプロイされたバイナリへのパス、 `<config-file>`ユーザー設定に基づいて生成された構成ファイルです。このチェックをスキップするには、 `--ignore-config-check`オプションを使用します。
-   データ型: `BOOLEAN`
-   デフォルト: false
