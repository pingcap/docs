...
summary: TiUPクラスターチェックは、本番環境のクラスターが最高のパフォーマンスを発揮するためのチェックを提供します。チェック項目には、オペレーティングシステムのバージョン、CPUのサポート、システム時刻の同期などが含まれます。チェック結果はPass、Warn、またはFailとなります。CLIコマンドを使用して、指定したクラスターのターゲットマシンのハードウェアおよびソフトウェア環境が正常に動作するかどうかを確認できます。
---
