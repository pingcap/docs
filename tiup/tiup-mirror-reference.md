...
+ The running client performs the following tasks based on the existing `root.json`:
    ...
    5. Obtain the version number `N` of the `index.json` file from `snapshot.json` and request `{N}.index.json`. Then use the public key recorded in `root.json` to verify whether the file is valid.
    6. For components such as `tidb.json` and `tikv.json`, the client obtains the version numbers `N` of the components from `snapshot.json` and requests `{N}.{component}.json`. Then the client uses the public key recorded in `index.json` to verify whether the file is valid.
