...
TiUPミラーでは、ルート証明書は他のメタデータファイルの公開鍵を格納するために使用されます。メタデータファイル（ `*.json` ）が取得されるたびに、TiUPクライアントは、メタデータファイルの種類（ルート、インデックス、スナップショット、タイムスタンプ）に基づいて、インストールされている`root.json`で対応する公開鍵を見つける必要があります。次に、TiUPクライアントは公開鍵を使用して、署名が有効かどうかを確認します。
