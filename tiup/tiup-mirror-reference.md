...
summary: TiUPミラーは、コンポーネントとそのメタデータを保存するウェアハウスであり、ローカルディスク上のディレクトリとリモートディスクディレクトリに基づいて開始されたHTTPミラーの2つの形式があります。ミラーはtiup mirror initまたはtiup mirror cloneを使用して作成でき、ルート証明書やインデックス、コンポーネントのメタデータなどが含まれます。クライアントは、ミラーからダウンロードされたファイルが安全であることを確認するために、root.jsonやtimestamp.jsonなどのファイルを使用します。
---
