...
このコマンドでは、クラスター名、TiDB クラスター バージョン ( `v7.5.3`など)、およびクラスターのトポロジ ファイルを指定する必要があります。
...
ファイルを`/tmp/topology.yaml`として保存します。TiDB v7.5.3 を使用し、クラスター名が`prod-cluster`場合は、次のコマンドを実行します。
...
PDコンポーネントの場合、 `|L`または`|UI` `Up`または`Down`に追加されることがあります。 `|L` PD ノードがLeaderであることを示し、 `|UI` [TiDBダッシュボード](/dashboard/dashboard-intro.md)が PD ノード上で実行されていることを示します。
...
たとえば、次のコマンドはクラスターを v7.5.3 にアップグレードします。
...
-   クラスターをデプロイ: `tiup cluster deploy <cluster-name> <version> <topo> --ssh=system` `<cluster-name>`にクラスターの名前、 `<version>`にデプロイする TiDB バージョン ( `v7.5.3`など)、 `<topo>`にトポロジ ファイルを入力します。
