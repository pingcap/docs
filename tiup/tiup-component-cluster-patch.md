...
summary: tiup cluster patchコマンドは、クラスターの実行中にサービスのバイナリを動的に置換するために使用されます。コマンドを実行する前に、必要なバイナリパッケージを準備する必要があります。オプションを使用して特定のコンポーネントにパッチを適用した後の動作をカスタマイズすることができます。また、置換するノードや役割を指定することも可能です。TiUPは実行ログを出力します。
---
