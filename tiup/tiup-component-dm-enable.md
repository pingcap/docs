...
summary: tiup dm enableコマンドは、マシンの再起動後のクラスターサービスの自動有効化を設定するために使用されます。指定したノードでサービスの自動有効化を有効にします。<cluster-name>は、サービスの自動有効化が有効になるクラスターです。-N, --nodeオプションを指定すると、指定したノードの自動有効化が有効になります。-R, --roleオプションを指定すると、指定した役割の自動有効化が有効になります。
---
