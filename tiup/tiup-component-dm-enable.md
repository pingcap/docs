...
summary: `tiup dm enable`コマンドは、マシンの再起動後のクラスターサービスの自動有効化を設定するために使用されます。指定したノードでサービスの自動有効化を有効にします。`<cluster-name>`は、サービスの自動有効化が有効になるクラスターです。`-N, --node`オプションを指定すると、指定したノードの自動有効化が有効になります。`-R, --role`オプションを指定すると、指定した役割の自動有効化が有効になります。
---
