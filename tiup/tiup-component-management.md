...
- link: Links the binary of a component to the executable file directory (`$TIUP_HOME/bin/`)
...
TiUP v1.13.0 adds the experimental `link` and `unlink` commands to link the component binary to the executable directory (`$TIUP_HOME/bin/`) and delete the link. This feature allows users to call components without going through TiUP every time, while retaining the ability to switch between multiple versions. However, this method lacks the process of automatic update checking and setting some environment variables. Some components (such as ctl) might not be available. Use it only when necessary.
...
Example 2: Switch the cluster component to v1.13.0

```shell
tiup link cluster：v1.13.0
```

The `tiup link cluster` command outputs the following content:

```shell
package cluster provides these executables: tiup-cluster
```

The binary file name of the cluster component is `tiup-cluster`. After the link command is executed, you can directly enter `tiup-cluster` on the command line to use the cluster component.

Example 3: Unlink the cluster component
...
In versions earlier than v1.13.0, TiUP is installed under `~/.tiup/bin/` and cannot coexist with multiple versions. Starting from v1.13.0, TiUP can be installed and linked like all other components.
