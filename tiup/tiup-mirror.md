...
-   パッケージの特定のバージョンをクローンするかどうかを決定します
    ...
    -   `tiup mirror clone <target-dir> --tidb v7.5.3`コマンドを実行して、TiDBコンポーネントの v7.5.3 バージョンをクローンします。
    -   `tiup mirror clone <target-dir> --tidb v7.5.3 --tikv all`コマンドを実行して、TiDBコンポーネントの v7.5.3 バージョンと TiKVコンポーネントのすべてのバージョンを複製します。
    -   `tiup mirror clone <target-dir> v7.5.3`コマンドを実行して、クラスター内のすべてのコンポーネントの v7.5.3 バージョンを複製します。
...
`TIUP_MIRRORS`設定では、ミラー構成を永続的に変更できます (例: `tiup mirror set` 。詳細については、 [ティアップ号 #651](https://github.com/pingcap/tiup/issues/651)を参照してください。
