...
- Because this command does not specify the version of the TiDB, PD, and TiKV component, TiUP playground uses the latest version of each component by default. Assume that the latest version is v5.1.2, then this command works the same as `tiup playground:v1.5.0 v5.1.2`.
...
In the command above, `nightly` is the version number of the cluster. Similarly, you can replace `nightly` with `v5.1.2`, and the command is `tiup playground v5.1.2`.
