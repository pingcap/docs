...
-   このコマンドはTiDB、PD、およびTiKVコンポーネントのバージョンを指定しないため、 TiUPプレイグラウンドはデフォルトで各コンポーネントの最新バージョンを使用します。最新バージョンがv8.5.2であると仮定すると、このコマンドは`tiup playground:v1.12.3 v8.5.2`と同じように動作します。
