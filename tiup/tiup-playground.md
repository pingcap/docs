...
summary: TiDB クラスターを迅速にデプロイするために、TiUPのプレイグラウンドコンポーネントを使用します。このコンポーネントを使うと、ローカルにTiDB、TiKV、PDコンポーネントをインストールし、1つのTiDBクラスターを開始することができます。さらに、プレイグラウンドによって開始されたクラスターにすばやく接続するためのclientコンポーネントも提供されています。また、クラスターの情報を表示したり、クラスターをスケールアウトしたり、スケールインしたりすることも可能です。
---
...
-   このコマンドは TiDB、PD、および TiKVコンポーネントのバージョンを指定しないため、 TiUPプレイグラウンドはデフォルトで各コンポーネントの最新バージョンを使用します。最新バージョンが v7.5.1 であると仮定すると、このコマンドは`tiup playground:v1.12.3 v7.5.1`と同じように機能します。
