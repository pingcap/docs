---
title: tiup dm list
summary: tiup dm listコマンドは、同じ制御マシンを使用した複数のクラスターの展開をサポートします。現在ログインしているユーザーが制御マシンを使用してデプロイされているクラスターを確認できます。デフォルトでは、デプロイされたクラスターのデータは~/.tiup/storage/dm/clusters/ディレクトリに保存されます。現在ログインしているユーザーは、同じ制御マシン上に他のユーザーがデプロイしたクラスターを表示できません。tiup dm listコマンドのオプションには-h、--helpがあります。出力には、クラスター名、ユーザー、バージョン、パス、秘密キーのパスが含まれます。
---
