...
- ログが保存される最大日数
- 設定項目が設定されていない場合、またはその値がデフォルト値 0 に設定されている場合、PD はログファイルを消去しません。
- デフォルト値: `0`

### `max-backups` {#max-backups}

- 保存するログファイルの最大数
- 設定項目が設定されていない場合、またはその値がデフォルト値 0 に設定されている場合、PD はすべてのログファイルを保持します。
- デフォルト値: `0`
...
このセクションでは、 [リソース管理](/tidb-resource-control-ru-groups.md)のためにPDに組み込まれている設定項目について説明します。
