...
- 例如，Docker 的内网 IP 地址为 `172.17.0.1`，而主机的 IP 地址为 `192.168.100.113`，端口映射设置为 `-p 2380:2380`。此时，可以将 `advertise-client-urls` 设置为 `"http://192.168.100.113:2380"`。客户端可以通过 `"http://192.168.100.113:2380"` 找到此服务。
...
- 例如，Docker 的内网 IP 地址为 `172.17.0.1`，主机 IP 为 `192.168.100.113`，端口映射为 `-p 2380:2380`。此时，可以将 `advertise-peer-urls` 设置为 `"http://192.168.100.113:2380"`。其他 PD 节点可以通过 `"http://192.168.100.113:2380"` 访问此服务。
...
- 如果需要启动三个 PD 服务器，`initial-cluster` 可能为：
...
+ 在引导阶段识别不同的集群
+ 默认值：`"pd-cluster"`
+ 如果连续部署多个配置相同的集群节点，必须指定不同的 token 以隔离不同的集群。
...
+ 默认值：`3000ms`
+ 该值必须至少是 [`tick-interval`](#tick-interval) 的五倍。例如，如果 `tick-interval` 为 `500ms`，则 `election-interval` 必须大于等于 `2500ms`。

### `enable-prevote`

+ 等同于 etcd 的 `pre-vote` 配置项。控制 PD 节点中嵌入式 etcd 是否启用 Raft 预投票。当启用时，etcd 会进行额外的选举阶段，以确认是否有足够的票数赢得选举，从而最小化服务中断。
+ 默认值：`true`
...
+ PD 更新 TSO 物理时间的间隔
+ 在 TSO 物理时间的默认更新间隔中，PD 最多提供 262144 个 TSOs。若需获取更多 TSOs，可以减小此配置项的值。最小值为 `1ms`。
+ 减小此配置可能会增加 PD 的 CPU 使用率。根据测试，与 `50ms` 的间隔相比，`1ms` 时 PD 的 CPU 使用率大约增加 10%。
+ 默认值：`50ms`
+ 最小值：`1ms`
...
+ 默认值：3
+ PD 对流量数字的最低位进行四舍五入，减少因 Region 流量信息变化引起的统计数据更新。此配置项用于指定对 Region 流量信息进行最低位数的四舍五入。例如，流量 `100512` 将被四舍五入为 `101000`，因为默认值为 `3`。此配置取代了 `trace-region-flow`。
...
+ 默认值：在 v6.3.0 之前为 `"0s"`，从 v6.3.0 起为 `"1s"`，为最小的正值。
+ 最小值：`0`
+ 单位：秒
...
+ 控制是否在 PD 日志中启用日志内容脱敏
+ 设置为 `true` 时，用户数据在 PD 日志中会被脱敏。
+ 默认值：`false`
...
+ 单个日志文件的最大大小。超过此值，系统会自动将日志拆分成多个文件。
+ 默认值：`300`
+ 单位：MiB
+ 最小值：`1`
...
+ 日志保留的最大天数
+ 如果未设置此配置项，或其值为默认值 0，PD 不会清理日志文件。
+ 默认值：`0`

### `max-backups`

+ 要保留的最大日志文件数
+ 如果未设置此配置项，或其值为默认值 0，PD 会保留所有日志文件。
+ 默认值：`0`
...
> - 现有集群，请使用命令行工具 [PD Control](/pd-control.md) 进行修改。直接在配置文件中修改这些与 `schedule` 相关的 PD 配置项，不会影响已存在的集群。

### `max-merge-region-size`

+ 控制 `Region 合并` 的大小限制。当 Region 大小超过指定值时，PD 不会将其与相邻的 Region 合并。
+ 默认值：`20`
+ 单位：MiB

### `max-merge-region-keys`

+ 指定 `Region 合并` 的最大 key 数。当 Region 的 key 数大于此值时，PD 不会将其与相邻的 Region 合并。
+ 默认值：`200000`
...
+ 控制存储节点上线的最大等待时间。在存储节点上线阶段，PD 可以查询存储节点的上线进度。当超过此时间，PD 认为存储节点已上线，不再查询上线进度，但不会阻止 Region 转移到新上线的存储节点。大多数场景下无需调整此参数。
+ 默认值：`48h`
...
+ 设置识别热点 Region 所需的分钟数阈值。Region 在热点状态超过此时间后，PD 才会参与热点调度。
+ 默认值：`3`
...
+ 同时进行的 `Region 合并` 调度任务数。将此参数设置为 `0` 可禁用 `Region 合并`。
+ 默认值：`8`
...
> **Note:**
>
> 如果你将集群从 TiDB 4.0 版本升级到当前版本，`region-score-formula-version` 会默认自动禁用，以确保升级前后 PD 行为一致。如果你想切换公式版本，需要通过 `pd-ctl` 手动切换。详情请参考 [PD Control](/pd-control.md#config-show--set-option-value--placement-rules)。
...
+ 默认值：`v1`
+ 取值选项：
    + `v1`：在 v1 模式下，可以手动修改 `store limit` 以限制单个 TiKV 的调度速度。
    + `v2`：（实验性功能）在 v2 模式下，无需手动设置 `store limit`，PD 会根据 TiKV 快照的能力动态调整。详细信息请参考 [Principles of store limit v2](/configure-store-limit.md#principles-of-store-limit-v2)。
...
> **Note:**
>
> 热点 Region 信息每三分钟更新一次。如果间隔设置小于三分钟，期间的更新可能没有意义。
...
+ 控制是否启用 Region 心跳的细分指标。这些指标衡量 Region 心跳处理的每个阶段耗时，便于监控分析。
+ 默认值：`true`

### `enable-heartbeat-concurrent-runner` <span class="version-mark">New in v8.0.0</span>

+ 控制是否启用 Region 心跳的异步并发处理。启用后，独立的执行器会异步并发处理 Region 心跳请求，有助于提升处理吞吐量和降低延迟。
+ 默认值：`true`
...
+ 副本数，即领导者和跟随者的总数。默认值 `3` 表示 1 个领导者和 2 个跟随者。动态修改此配置后，PD 会在后台调度 Region，使副本数匹配此配置。
+ 默认值：`3`
...
+ 根 CA 证书文件路径。连接 TiDB 的 SQL 服务时使用 TLS 时可以配置此路径。
+ 默认值：`""`

### `tidb-cert-path`

+ SSL 证书文件路径。连接 TiDB 的 SQL 服务时使用 TLS 时可以配置此路径。
+ 默认值：`""`

### `tidb-key-path`

+ SSL 私钥文件路径。连接 TiDB 的 SQL 服务时使用 TLS 时可以配置此路径。
+ 默认值：`""`
...
以下是关于[请求单元（RU）](/tidb-resource-control.md#what-is-request-unit-ru)的配置项。
