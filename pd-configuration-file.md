...
The PD configuration file supports more options than command-line parameters. You can find the default configuration file [here](https://github.com/pingcap/pd/blob/release-7.5/conf/config.toml).
...
> **Note:**
>
> To modify these PD configuration items related to scheduling, choose one of the following methods based on your cluster status:
>
> - For clusters to be newly deployed, you can modify the PD configuration file directly.
> - For existing clusters, use the command-line tool [PD Control](/pd-control.md) to make changes instead. Direct modifications to these PD configuration items related to scheduling in the configuration file does not take effect on existing clusters.
