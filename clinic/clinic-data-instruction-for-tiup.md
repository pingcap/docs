...
summary: PingCAPクリニック診断サービスは、TiUPを使用して展開されたTiDBおよびDMクラスターから収集された診断データを提供します。収集されたデータは、クラウドに展開された診断サービスであるClinic Serverに保存されます。データの種類に応じて必要なパラメータをコマンドに追加することができます。診断データはクラスターの問題のトラブルシューティングにのみ使用されます。
---
