...
2. Get and set an access token (token) to upload data.
    ...
    > **Note:**
    >
    > - When accessing Clinic Server for the first time, before getting a token, you need to refer [Quick Start with PingCAP Clinic](/clinic/quick-start-with-clinic.md) to do the pre-steps.
