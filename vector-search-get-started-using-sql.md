...
TiDB は MySQL 構文を拡張して[ベクトル検索](/vector-search-overview.md)サポートし、新しい[ベクトルデータ型](/vector-search-data-types.md)といくつかの[ベクトル関数](/vector-search-functions-and-operators.md)を導入します。
...
テーブルを作成するときに、 `VECTOR`データ型を指定して、列を[ベクター](/vector-search-overview.md#vector-embedding)列として定義できます。
...
[ベクトル埋め込み](/vector-search-overview.md#vector-embedding)を含む 3 つのドキュメントを`embedded_documents`つのテーブルに挿入します。
...
>
> 実際のアプリケーションでは、数[埋め込みモデル](/vector-search-overview.md#embedding-model)または数千の次元を持つベクトル埋め込みが生成されることがよくあります。
