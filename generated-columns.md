...
- The following functions and expressions are illegal in generated column definitions, and an error occurs if they are used:
    - Non-deterministic functions and expressions (such as `RAND`, `UUID`, and `CURRENT_TIMESTAMP`).
    - Functions dependent on session-specific or global state (such as `CONNECTION_ID` and `CURRENT_USER`).
