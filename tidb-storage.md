...
TiKV chooses the second solution that divides the whole Key-Value space into a series of consecutive Key segments. Each segment is called a Region. When the size of a Region exceeds a threshold (144 MiB by default), TiKV splits it to two or more Regions. There is a size limit for each Region to store data (the default value is 96 MiB and the size can be configured). Each Region can be described by `[StartKey, EndKey)`, a left-closed and right-open interval.
