...
-   説明：
    ...
    PD が 20 秒以内に TiKV/TiFlash ハートビートを受信しません。通常、TiKV/TiFlash のハートビートは 10 秒ごとに送信されます。
...
-   解決：
    ...
    -   スペースを解放できない場合は、積極的にノードをオフラインにすることを検討してください。これにより、ダウンタイムにつながるディスク領域の不足を防ぎます。
...
-   解決：
    ...
    -   リージョン health パネルを見て、 `pending_peer_region_count`が継続的に減少しているかどうかを確認します。
...
-   解決：
    ...
    -   構成されている場合は、ラベル構成に従って、追加のレプリカ用のストアがあるかどうかを確認します。
...
-   解決：
    ...
    1.  Raft Propose モニターを見て、アラートが発生した TiKV ノードのRaft提案が他の TiKV ノードよりもはるかに高いかどうかを確認します。その場合、この TiKV に 1 つ以上のホット スポットがあることを意味します。ホット スポット スケジューリングが適切に機能するかどうかを確認する必要があります。
...
-   解決：
    ...
    2.  TiKV へのプレッシャーが少ない場合は、PD のスケジュールが頻繁すぎないかどうかを検討してください。 PD ページの Operator Create パネルを表示し、PD スケジューリングのタイプと数を確認できます。
