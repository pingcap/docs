...
TiDB supports MySQL 5.7's privilege management system, including the syntax and privilege types. The following features from MySQL 8.x are also supported:
...
* `CREATE MASKING POLICY` allows creating column-level masking policies. For details, see [Column-Level Masking Policy](/column-level-masking-policy.md).
* `ALTER MASKING POLICY` allows modifying column-level masking policies (for example, enabling or disabling policies, and changing expressions). For details, see [Column-Level Masking Policy](/column-level-masking-policy.md).
* `DROP MASKING POLICY` allows deleting column-level masking policies. For details, see [Column-Level Masking Policy](/column-level-masking-policy.md).
