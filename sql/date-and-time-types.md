...
When dealing with date and time value types, please note: 
...
+ If the year-portion of a date is specified as 2 digits, TiDB converts it based on the following rules:
...
+ If a numberic value is needed in the context, TiDB automatically converts the date or time value into a numeric type. For example,
...
Please pay attention to the abbreviated form of `TIME`. For example, '11:12' means '11:12:00' instead of '00:11:12'. However, '1112' means '00:11:12'. These differences are caused by the presence or absence of the colon:, because the 2 situations are handled differently. 
...
The two-digit year-portion may not be properly calculated in some functions such  `MIN()` and  `MAX()`. For these functions, the four-digit format suites better.