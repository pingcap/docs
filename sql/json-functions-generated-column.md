...
In order to functionally keep pace with the latest version of MySQL (version 5.7 and above) and provide better support for document storage business as well, TiDB supports JSON in the latest version. You can use JSON type fields in TiDB tables, and index the fields in the JSON document in the form of generated column. Based on this, you can flexibly deal with those business with uncertain schema, no longer limited by the bad read performance and lack of support for transaction in traditional document databases.
...
Put a legal JSON string at the location corresponding to the JSON field, and then you can insert JSON into the table. TiDB will parse the text and saves it in a more compact and easy-to-access form.

You can also convert other types of data into JSON using CAST:
...
Now, if you want to query all the users living in Beijing, what should you do? Is it necessary to pull back all the data and then filter in the business layer? Not really. The same with traditional document databases like MongoDB, TiDB supports complex combinations of query conditions in the user side.
...
You can get the general use of these functions directly from the function name. These functions in TiDB behave the same as in MySQL 5.7. If you want to query the specific use, see the [JSON Functions document of MySQL 5.7](https://dev.mysql.com/doc/refman/5.7/en/json-functions.html). If you are a user of MySQL 5.7, you can migrate seamlessly.
...
The full table scan is executed when you query a JSON field. When you run `EXPLAIN` statements in TiDB, the results show that it is full table scan. Then, can you index the JSON field?
