...
In TiDB, the `UniqueID` field returned from `SHOW MASTER STATUS` is actually current `TSO` obtained from `PD`, the `TSO` is used during binlog increment synchronization.
