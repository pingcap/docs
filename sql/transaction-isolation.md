...
TiDB implements the Repeatable Read isolation in the Snapshot Isolation level. However, the TiDB Repeatable Read isolation level differs from the [ANSI Repeatable Read isolation level](/sql/transaction-isolation.md#difference-between-tidb-and-ansi-repeatable-read) and the [MySQL Repeatable Read level](/sql/transaction-isolation.md#difference-between-tidb-and-mysql-repeatable-reads).
