...
この例のソース コードは[`avro-checksum-verification`](https://github.com/pingcap/tiflow/tree/master/examples/golang/avro-checksum-verification)ディレクトリにあります。

このドキュメントの例では、 [カフカ号](https://github.com/segmentio/kafka-go)を使用して単純な Kafka コンシューマー プログラムを作成します。このプログラムは、指定されたトピックからデータを継続的に読み取り、チェックサムを計算し、その値を検証します。
