...
| データベース   | string   | 行が配置されているデータベースの名前                                                                                    |
| テーブル     | string   | 行が配置されているテーブルの名前                                                                                      |
...
| sqlタイプ   | object  | isDdlが`false`場合、各列のデータ型がJavaでどのように表現されるかを記録します。                                                       |
| mysqlタイプ | object  | isDdlが`false`場合、各列のデータ型がMySQLでどのように表現されるかを記録します。                                                      |
...
`update`ステートメントでは、TiCDCは以下に示すように、 `type`を`UPDATE`としてイベントメッセージを出力します。7 `update`ステートメントは、列number`c_int`と`c_tinyint`のみを変更します。出力イベントメッセージの`old`フィールドには、すべての列データが含まれます。
