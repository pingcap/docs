...
v6.5.6 および v7.1.3 以降、 TiCDC Canal-JSON は公式 Canal のデータ形式との互換性をサポートします。チェンジフィードを作成するときに、 `content-compatible=true` / `sink-uri`を設定してこの機能を有効にすることができます。このモードでは、TiCDC は公式の Canal と互換性のある Canal-JSON 形式のデータを出力します。具体的な変更点は以下の通りです。

-   `mysqlType`フィールドには、各タイプのタイプ パラメータの完全な情報が含まれます。
