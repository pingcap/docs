...
Since v6.2, TiCDC supports splitting a single-table transaction into multiple transactions. This can greatly reduce the latency and memory consumption of replicating large transactions. Therefore, if your application does not have a high requirement on transaction atomicity, it is recommended to enable the splitting of large transctions to solve the possible replication latency and OOM issues. To enable the splitting, set the value of the sink uri parameter [`transaction-atomocity`](/ticdc/manage-ticdc.md#configure-sink-uri-with-mysqltidb) to `none`.
