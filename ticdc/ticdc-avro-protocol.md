...
TiCDC Avroプロトコルは、 [コンフルエントプラットフォーム](https://docs.confluent.io/platform/current/platform.html)で定義された[Confluent Avro](https://docs.confluent.io/platform/current/schema-registry/fundamentals/serdes-develop/serdes-avro.html)データ交換プロトコルのサードパーティ実装です。Avroは[Apache Avro](https://avro.apache.org/)で定義されたデータ形式です。
