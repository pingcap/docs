...
summary: TiCDC コマンドラインツール`cdc cli`を使用して、TiCDC 変更フィードを作成および管理する方法について説明します。HTTP インターフェイスを介して変更フィードを管理することもできます。レプリケーションタスクを作成、リスト、一時停止、再開、削除する方法についても説明します。また、特定のレプリケーションタスクやプロセッサの状態をクエリする方法についても説明します。統合ソーターの有効化や有効なインデックスのないテーブルの複製についても説明します。
---
