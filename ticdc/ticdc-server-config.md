...
このドキュメントでは、TiCDC で使用される CLI および設定ファイルのパラメータについて説明します。
...
- `data-dir` : TiCDC がファイルを保存するためにディスクを使用する必要があるときに使用するディレクトリを指定します。TiCDC が使用するソートエンジンと REDO ログは、このディレクトリに一時ファイルを保存します。このディレクトリの空きディスク容量は 500 GiB 以上確保することをお勧めします。TiUPを使用している場合は、セクション[`cdc_servers`](/tiup/tiup-cluster-topology-reference.md#cdc_servers)で`data_dir`を設定するか、 `global`でデフォルトのパス`data_dir`を直接使用できます。
