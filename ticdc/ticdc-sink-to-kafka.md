...
| `debezium-include-start-ts` | Optional. `false` by default. Requires the new TiCDC architecture and `protocol=debezium`. When enabled, adds `source.start_ts` (the source transaction TSO) to Debezium JSON DML messages. An explicit URI value overrides `[sink.debezium] include-start-ts`, including `false` overriding `true`. Enabling it with another protocol, including `debezium-avro`, is rejected. See [TiCDC Debezium Protocol](/ticdc/ticdc-debezium.md#include-the-transaction-start-tso). |
| `simple-include-start-ts` | Optional. `false` by default. Requires the new TiCDC architecture, `protocol=simple`, and JSON encoding. When enabled, adds the top-level integer `startTs` (the source transaction TSO) to DML messages. An explicit URI value overrides `[sink.simple] include-start-ts`, including `false` overriding `true`. Enabling it with another protocol or `encoding-format=avro` is rejected. See [TiCDC Simple Protocol](/ticdc/ticdc-simple-protocol.md#include-the-transaction-start-tso). |
