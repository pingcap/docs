...
| `required-acks` | An configuration item configured in the `Produce` request, which notifies the broker the number of replicas to be confirmed of reception before returning a response. Value options are `0` (`NoResponse`: No response. Only `TCP ACK` is provided), `1` (`WaitForLocal`: Respond only after local commits are submitted successfully), and `-1` (`WaitForAll`: Respond after all replicated replicas are committed successfully. You can configure the minimum number of replicated replicas using the [`min.insync.replicas`](https://kafka.apache.org/33/documentation.html#brokerconfigs_min.insync.replicas) configuration item of the broker). (Optional, the default value is `-1`).    |
