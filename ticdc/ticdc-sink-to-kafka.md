...
| `max-message-bytes`  | The maximum size of data that is sent to Kafka broker each time (optional, `10MB` by default, the maximum value is `100MB`). From v5.0.6 and v4.0.6, the default value has changed from `64MB` and `256MB` to `10MB`. |
