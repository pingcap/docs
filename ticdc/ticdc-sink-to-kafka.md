...
> **注記：**
>
> `protocol`が`open-protocol`場合、TiCDC は複数のイベントを 1 つの Kafka メッセージにエンコードし、 `max-message-bytes`で指定された長さを超えるメッセージの生成を回避します。単一行変更イベントのエンコードされた結果が`max-message-bytes`の値を超えると、変更フィードはエラーを報告し、ログを出力。
