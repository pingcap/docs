...
TiCDC は通常 DDL ステートメントを順番にレプリケートしますが、ダウンストリームがTiDBの場合、インデックスの作成および追加のための DDL 操作を非同期的に実行します。つまり、 `ADD INDEX`と`CREATE INDEX` DDLをダウンストリームTiDBクラスターにレプリケーションして実行した後、TiCDCはDDL実行の完了を待たずに直ちに戻ります。これにより、変更フィードレプリケーションのレイテンシーへの影響を最小限に抑え、後続のDML実行がブロックされることを回避できます。
