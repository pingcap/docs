...
Currently, TiCDC uses an allow list to replicate DDL statements. Only the DDL statements in the allow list are replicated to the downstream. The DDL statements not in the allow list are not replicated.
...
Due to the lack of some context during the replication process, TiCDC has some constraints on the replication of DDLs of the `RENAME TABLE` type.
...
| `RENAME TABLE test.n1 TO test.t1` | Report an error and exit the replication | `test.n1` does not match the filter rule, but `test.t1` matches the filter rule. This operation is illegal. Please refer to the error message for handling. |
...
If a DDL statement renames multiple tables, TiCDC only replicates the DDL statement when the old database name, old table name, and the new database name all match the filter rule.
...
| `RENAME TABLE test.t1 TO test.ignore1, test.t3 TO test.ignore2` | Replicate | The old database name, the old table name, and the new database name match the filter rule. |
