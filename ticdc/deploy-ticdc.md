...
- Since v6.3.0, TiCDC supports rolling upgrade. You can directly perform a rolling upgrade between minor versions (for example, v8.5.0 -> v8.5.3 is a minor version upgrade, while v8.1.x -> v8.5.x is a major version upgrade). It is not recommended to run Changefeed during a cross-major-version upgrade for the old TiCDC architecture. You are advised to pause Changefeed before upgrading the old TiCDC architecture. The new TiCDC architecture supports running Changefeed during the rolling upgrade process. For more information, see [Compatibility notes for rolling upgrades from earlier TiCDC versions](/ticdc/ticdc-compatibility.md#compatibility-notes-for-upgrades-from-earlier-versions). Rolling upgrade takes effect automatically if the following conditions are met:
