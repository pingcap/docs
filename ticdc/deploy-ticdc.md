...
## TiCDC `cdc server` command line parameter description
...
- `sort-dir`: Specifies the temporary file directory of the sorting engine. The default value for this configuration item is `/tmp/cdc_sort`. When Unified Sorter is enabled, if the directory of the server is not writable or the available space is insufficient, you need to manually specify a directory in `sort-dir`. Make sure that TiCDC can read and write data under the `sort-dir` path.