...
`force_replicate` : このパラメータのデフォルトは`false`です。4 `true`を指定すると、TiCDC は一意インデックスを持たないテーブルを強制的に複製しようとします。

`ignore_ineligible_table` : このパラメータのデフォルトは`false`です。4 `true`を指定すると、TiCDC は複製できないテーブルを無視します。
