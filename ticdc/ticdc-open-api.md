...
`protocol`: For the sink of MQ type, you can specify the protocol format of the message. Currently the following protocols are supported: `default`, `canal-json` (experimental), `avro`, `debezium`, and `simple`. The default protocol is the TiCDC Open Protocol.
