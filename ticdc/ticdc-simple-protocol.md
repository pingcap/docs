...
| `type`           | 文字列   | DDL イベント タイプ ( `CREATE` 、 `RENAME` 、 `CINDEX` 、 `DINDEX` 、 `ERASE` 、 `TRUNCATE` 、 `ALTER` 、 `QUERY` 。 |
| `sql`            | 文字列   | DDL ステートメント。                                                                                          |
| `commitTs`       | 番号  | DDL ステートメントの実行がアップストリームで完了したときのコミット タイムスタンプ。                                                          |
| `buildTs`        | 番号  | TiCDC 内でメッセージが正常にエンコードされたときの UNIX タイムスタンプ。                                                            |
| `tableSchema`    | オブジェクト  | テーブルの現在のスキーマ情報。詳細については、 [TableSchemaの定義](#tableschema-definition)参照してください。                            |
| `preTableSchema` | オブジェクト  | DDL文が実行される前のテーブルのスキーマ情報。1 `CREATE`のDDLイベントを除くすべてのDDLイベントにこのフィールドがあります。                                |
...
| `buildTs`       | 番号   | TiCDC 内でメッセージが正常にエンコードされたときの UNIX タイムスタンプ。      |
| `schemaVersion` | 番号   | DML メッセージがエンコードされるときのテーブルのスキーマ バージョン番号。         |
| `data`          | オブジェクト   | 更新後のデータ。フィールド名は列名、フィールド値は列値です。                  |
| `old`           | オブジェクト   | 更新前のデータ。フィールド名は列名、フィールド値は列値です。                  |
...
| `buildTs`       | 番号  | TiCDC 内でメッセージが正常にエンコードされたときの UNIX タイムスタンプ。      |
| `schemaVersion` | 番号  | DML メッセージがエンコードされるときのテーブルのスキーマ バージョン番号。         |
| `old`           | オブジェクト  | 削除されたデータ。フィールド名は列名、フィールド値は列値です。                 |
...
| ------------- | --- | ----------------------------------------------------------------------------- |
| `version`     | 番号  | プロトコルのバージョン番号。現在は`1`です。                                                       |
| `type`        | 文字列   | `BOOTSTRAP`のイベントタイプ。                                                          |
| `commitTs`    | 番号  | `BOOTSTRAP`のうちの`commitTs` `0`です。これはTiCDCによって内部的に生成されるため、 `commitTs`は意味を持ちません。 |
| `buildTs`     | 番号  | TiCDC 内でメッセージが正常にエンコードされたときの UNIX タイムスタンプ。                                    |
| `tableSchema` | オブジェクト  | テーブルのスキーマ情報。詳細については、 [TableSchemaの定義](#tableschema-definition)参照してください。       |
...
| ---------- | ---- | -------------------------------------- |
| `name`     | 文字列    | 列の名前。                                  |
| `dataType` | オブジェクト   | MySQL データ型、文字セット、照合順序、フィールド長などのデータ型情報。 |
| `nullable` | ブール値 | 列が null にできるかどうか。                      |
| `default`  | 文字列    | 列のデフォルト値。                              |
...
| bigint 符号なし        | [9223372036854775808, 18446744073709551615] | uint64        | 文字列           |
| フロート               | /                                           | float32       | フロート        |
| ダブル                | /                                           | フロート64        | ダブル         |
| 小数点                | /                                           | 文字列             | 文字列           |
| varchar            | /                                           | []uint8       | 文字列           |
