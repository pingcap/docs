...
| `transaction-atomicity`  |  The atomicity level of a transaction. This is an optional parameter and the default value of `none`， indicating that TiCDC splits a single-table transaction. When the value is `table`, TiCDC ensures the atomicity of a single-table transaction.  |
