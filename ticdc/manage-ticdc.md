...
| `protocol` | The protocol with which messages are output to Kafka. The value options are `default`, `canal-json`, `avro`, and `maxwell` (`default` by default)    |
...
# Currently the following protocols are supported: default, canal-json, avro, and maxwell. The default protocol is the TiCDC Open Protocol.
protocol = "default"
