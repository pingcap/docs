...
- MySQL sink
    ...
    - TiCDC does not split in-table transactions. This is to **ensure** the transactional consistency within a single table. However, TiCDC does **not ensure** its consistency with the transactional order in the upstream table. If the primary keys between transactions have intersections, TiCDC still **ensures** its consistency with the transactional order in the upstream table.
