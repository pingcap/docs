...
When the downstream is MySQL or TiDB, because TiCDC does not directly obtain the original DML statements executed in the upstream, but generates SQL statements based on the data change information, TiCDC cannot guarantee that the SQL statements written to the downstream are exactly the same as the SQL statements executed in the upstream. However, it ensures the consistency of the final results.
