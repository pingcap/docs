...
- MySQL sink
    ...
    > **Note:**
    >
    > Since v6.2, you can use the sink uri parameter [`transaction-atomicity`](/ticdc-sink-to-mysql.md#configure-sink-uri-with-mysqltidb) to control whether to split single-table transactions. Splitting single-table transactions can greatly reduce the latency and memory consumption of replicating large transactions.
...
- [Use `cdc cli` to manage cluster status and data replication task](/ticdc-manage-changefeed.md)
