...
クラウドストレージサービスをダウンストリームのシンクとして使用する場合、DMLイベントをCSV形式でクラウドストレージサービスに送信できます。
...
| `NULL`                                                                        | ヌル    | `\N`                            | -                              |
| `TIMESTAMP` / `DATETIME`                                                      | 文字列     | `"1973-12-30 15:30:00.123456"`  | 形式: `yyyy-MM-dd HH:mm:ss.%06d` |
| `DATE`                                                                        | 文字列     | `"2000-01-01"`                  | 形式: `yyyy-MM-dd`               |
| `TIME`                                                                        | 文字列     | `"23:59:59"`                    | 形式: `yyyy-MM-dd`               |
| `YEAR`                                                                        | 整数    | `1970`                          | -                              |
| `VARCHAR` / `JSON` / `TINYTEXT` / `MEDIUMTEXT` / `LONGTEXT` / `TEXT` / `CHAR` | 文字列     | `"test"`                        | UTF-8エンコード                     |
| `VARBINARY` / `TINYBLOB` / `MEDIUMBLOB` / `LONGBLOB` / `BLOB` / `BINARY`      | 文字列     | `"6Zi/5pav"`または`"e998bfe696af"` | Base64または16進数エンコード             |
| `BIT`                                                                         | 整数    | `81`                            | -                              |
| `DECIMAL`                                                                     | 文字列     | `"129012.1230000"`              | -                              |
| `ENUM`                                                                        | 文字列     | `"a"`                           | -                              |
| `SET`                                                                         | 文字列     | `"a,b"`                         | -                              |
| `TiDBVectorFloat32`                                                           | 文字列     | `"[1.23, -0.4]"`                | -                              |
