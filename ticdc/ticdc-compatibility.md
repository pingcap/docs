...
- TiCDC v6.2.0 以降、 `cdc cli`は TiCDC Open API を介して TiCDCサーバーと直接やり取りし、PD へのアクセスは不要です。 `--pd`サブコマンドの`cdc cli`パラメータは非推奨となり、TiCDCサーバーアドレスを指定するために`--server`パラメータが追加されました。 `--pd`の代わりに`--server`を使用してください。
