...
summary: TiCDCオープンプロトコルは、行レベルのデータ変更通知プロトコルであり、データ変更イベントをダウンストリームに複製するための基本単位としてイベントを使用します。制限として、特殊な状況では同じバージョンの行変更イベントが複数回送信されることがあります。メッセージフォーマットには、イベントの形式が含まれ、カラムの型コードやDDLタイプコードが定義されています。また、カラムのビットフラグも列の特定の属性を表します。コンシューマ向けのプロトコル解析も提供されています。
---
