...
- In versions earlier than v8.5.7, `dispatchers` takes effect only when the downstream is an MQ sink and is used to configure event dispatchers. Starting from v8.5.7, for the [new TiCDC architecture](/ticdc/ticdc-architecture.md), `dispatchers` can also be used to configure table routing, mapping upstream tables to specified downstream database names or table names. For more information, see [TiCDC table routing](/ticdc/ticdc-table-routing.md).
