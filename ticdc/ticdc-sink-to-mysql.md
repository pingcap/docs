...
| `safe-mode` | Specifies how `INSERT` and `UPDATE` statements are handled during replicating data to the downstream. When you set it to `true`, TiCDC converts all upstream `INSERT` statements into `REPLACE INTO` statements and all `UPDATE` statements into `DELETE` + `REPLACE INTO` statements. Before v6.1.3, the default value of this parameter is `true`. Starting from v6.1.3, the default value changes to `false`. When TiCDC starts, it retrieves a current timestamp `ThresholdTs`. <ul><li>For `INSERT` and `UPDATE` statements with `CommitTs` smaller than `ThresholdTs`, TiCDC converts them into `REPLACE INTO` and `DELETE` + `REPLACE INTO` statements, respectively. </li><li>For `INSERT` and `UPDATE` statements with `CommitTs` greater than or equal to `ThresholdTs`, `INSERT` statements are directly replicated to the downstream. For the behavior of `UPDATE` statements, see [TiCDC Behavior in Splitting UPDATE Events](/ticdc/ticdc-split-update-behavior.md). </li></ul>|
