...
| `multi-stmt-enable` | Controls whether the SQL statements executed downstream support multiple SQL statements separated by semicolons (optional, default value is `true`). If set to `false`, each SQL statement is executed as a separate transaction. When `multi-stmt-enable` is true, `cache-prep-stmts` does not take effect. |
