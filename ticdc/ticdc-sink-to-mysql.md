...
| `safe-mode` | Specifies how `INSERT` and `UPDATE` statements are handled during replicating data downstream. When set to `true`, TiCDC converts all upstream `INSERT` statements into `REPLACE INTO` statements and all `UPDATE` statements into `DELETE` + `REPLACE INTO` statements. Before v6.1.3, the default value of this parameter is `true`. Starting from v6.1.3, the default value has been changed to `false`. When TiCDC starts, it retrieves a current timestamp `ThresholdTs`. <ul><li>For `INSERT` and `UPDATE` statements with `CommitTs` less than `ThresholdTs`, TiCDC converts them into `REPLACE INTO` and `DELETE` + `REPLACE INTO` statements, respectively. </li><li>For `INSERT` and `UPDATE` statements with `CommitTs` greater than or equal to `ThresholdTs`, `INSERT` statements are directly replicated to the downstream, while the behavior of `UPDATE` statements depends on the explanation in [TiCDC Behavior in Splitting UPDATE Events](/ticdc/ticdc-split-update-behavior.md). </li></ul>|
