...
-   上記の更新バージョンでは、システムメモリが不足している場合に、ユニファイドソーターを有効にしてディスク内のデータを並べ替えることができます。この機能を有効にするには、レプリケーションタスクを作成するときに`--sort-engine=unified`から`cdc cli`のコマンドを渡すことができます。例えば：
