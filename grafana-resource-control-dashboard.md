...
- Available RU: the available tokens in the RU token bucket in terms of resource groups. when it is `0``, this resource group consumes tokens at the rate of RU_PER_SEC and can be considered to be in a rate-limited state.
- Query Max Duration：maximum Query Duration in terms of resource groups.
