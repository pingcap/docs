...
- Token Request Count: the number of token requests from the server side for each Resource Controller Client, calculated in real time and on the basis of resource groups. `successful` and `failed` are the sum of the successful and failed token requests for all Resource Controller Clients.
