...
summary: TiDB Binlogクラスターをデプロイする方法を学びます。
---

# TiDBBinlogクラスタの展開 {#tidb-binlog-cluster-deployment}

このドキュメントでは、 [バイナリパッケージを使用してTiDB Binlogをデプロイする](#deploy-tidb-binlog-using-a-binary-package)方法について説明します。
...
TiDB Binlogのバイナリ パッケージはTiDB Toolkitに含まれています。TiDB TiDB Toolkitをダウンロードするには、 [TiDBツールをダウンロード](/download-ecosystem-tools.md)参照してください。

### 使用例 {#the-usage-example}

PD ノードが 3 つ、TiDB ノードが 1 つ、 Pumpノードが 2 つ、 Drainerノードが 1 つあると仮定すると、各ノードの情報は次のようになります。
...
次の部分では、上記のノードに基づいて、PumpとDrainerを使用する方法を示します。

1.  バイナリを使用してPumpをデプロイ。

    -   Pumpのコマンドラインパラメータを表示するには、 `./pump -help`実行します。
    ...
    -   「192.168.0.11」にPumpをデプロイする場合を例にとると、 Pump構成ファイルは次のようになります。
    ...
    -   Pumpの起動例:
        ...
        コマンドラインパラメータが構成ファイルのパラメータと同じ場合は、コマンドラインパラメータの値が使用されます。

2.  バイナリを使用してDrainerをデプロイ。

    -   Drainerのコマンドラインパラメータを表示するには、 `./drainer -help`実行します。
    ...
    -   「192.168.0.13」にDrainerをデプロイする場合を例にとると、 Drainer構成ファイルは次のようになります。
    ...
    -   開始Drainer:
        ...
        コマンドラインパラメータと構成ファイルパラメータが同じ場合は、コマンドラインのパラメータ値が使用されます。
...
> **注記：**
>
> -   TiDB が実行中の場合、少なくとも 1 つのPumpが正常に実行されていることを保証する必要があります。
