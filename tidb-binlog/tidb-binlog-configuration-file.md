...
* `read-timeout`: Specifies the I/O read timeout of database connections downstream. The default value is `1m`. If Drainer keeps failing on some DDLs that take a long time, you can set this configuration to a larger value.
