...
summary: オンラインの安全でないリカバリは、TiKV 上のデータの一部が読み取りおよび書き込み不能になった場合に使用されます。この機能を使用すると、損失を伴うリカバリ操作を実行できます。また、オフライン ストアでは一部のデータが利用できなくなり、自動的に回復することはできません。回復の進行状況を確認し、完了を待ちます。データとインデックスの一貫性を確認し、必要に応じて修正します。回復不可能なストアを削除することもできます。
---
