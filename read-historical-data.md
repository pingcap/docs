...
- For simple cases, use `SELECT` after setting the `tidb_snapshot` variable and copy-paste the output, or use [`SELECT ... INTO OUTFILE`](/sql-statements/sql-statement-select.md) and then use [`LOAD DATA`](/sql-statements/sql-statement-load-data.md) to import the data later on.
