...
履歴バージョンからデータを読み取った後、現在のセッションを終了するか、 `SET`文を使用して`tidb_snapshot`変数の値を "" (空の文字列) に設定することで、最新バージョンからデータを読み取ることができます。
...
以前のバージョンからデータを復元する前に、作業中にガベージコレクション（GC）によって履歴データが消去されないように注意してください。これは、以下の例のように変数`tidb_gc_life_time`を設定することで実現できます。復元後は、この変数を以前の値に戻すことを忘れないでください。
