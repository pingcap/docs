...
* Changing the field type to its superset is unsupported. For example, it does not support changing from `INTEGER` to `VARCHAR`, or from `TIMESTAMP` to `DATETIME`. Otherwise, it might output the error information `Unsupported modify column: type %d not match origin %d`.
