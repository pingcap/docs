...
TiDB は[プロメテウスとグラファナ](/tidb-monitoring-api.md)の組み合わせを利用して、パフォーマンス監視メトリックの保存とクエリを実行します。 TiDB では、ほとんどの [パフォーマンススキーマテーブル](/performance-schema/performance-schema.md)結果を返しません。
