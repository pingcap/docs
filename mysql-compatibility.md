...
+ In TiDB, auto-increment columns are only guaranteed to be unique and incremental in a single TiDB server, but are *not* guaranteed to be incremental among multiple TiDB servers or allocated sequentially. It is recommended that you do not mix default values and custom values. Otherwise, you might encounter the `Duplicated Error` error message.
