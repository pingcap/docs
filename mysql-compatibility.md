...
+ `FULLTEXT` 语法和索引 [#1793](https://github.com/pingcap/tidb/issues/1793)
    ...
    >**Note:**
    >
    > 目前，只有特定 AWS 区域的 {{{ .starter }}} 和 {{{ .essential }}} 集群支持 [`FULLTEXT` 语法和索引](https://docs.pingcap.com/tidbcloud/vector-search-full-text-search-sql)。TiDB 自建版和 TiDB Cloud 专属集群支持解析 `FULLTEXT` 语法，但不支持使用 `FULLTEXT` 索引。
