...
* Adding/Dropping the primary key of the `CLUSTERED` type is unsupported unless [`alter-primary-key`](/tidb-configuration-file.md#alter-primary-key) is enabled. For more details about the primary key of `CLUSTERED`, refer to [clustered index](/clustered-indexes.md).
