...
* It is not supported to alter the same column or index when altering multiple schemas in a single `ALTER TABLE` statement. For example, when you run the `ALTER TABLE t1 MODIFY COLUMN c1 INT, DROP COLUMN c1` command and a `Unsupported operate same column/index` error is output.
