...
* Some type changings through `ALTER TABLE` statement is not supported. For example, TiDB does not support changing the field type from `DECIMAL` to `DATE`. Otherwise, the error information `Unsupported modify column: type %d not match origin %d` might be output. See [`ALTER TABLE`](/sql-statements/sql-statement-alter-table.md) for further details.
