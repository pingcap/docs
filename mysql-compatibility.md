...
+ In TiDB, auto-incremental columns are globally unique, but only guaranteed to be incremental and sequential on a single TiDB server. They are *not* guaranteed to be incremental among multiple TiDB servers or allocated sequentially. It is recommended that you do not mix default values and custom values. Otherwise, you might encounter the `Duplicated Error` error message.
