...
TiDB 構成ファイルは、コマンドライン パラメータよりも多くのオプションをサポートしています。デフォルトの構成ファイル[`config.toml.example`](https://github.com/pingcap/tidb/blob/release-7.5/pkg/config/config.toml.example)をダウンロードして、名前を`config.toml`に変更することができます。このドキュメントでは、 [コマンドラインオプション](/command-line-flags-for-tidb-configuration.md)に関係のないオプションについてのみ説明します。
...
-   `"TLSv1.1"` `"TLSv1.2"` `"TLSv1.3"` `"TLSv1.0"`
...
-   [`tidb_auth_token`](/security-compatibility-with-mysql.md#tidb_auth_token)の認証方法の JSON Web キー セット (JWKS) のローカル ファイル パスを設定します。
-   デフォルト値: `""`
...
-   セッション移行のために[Tiプロキシ](https://docs.pingcap.com/tidb/v7.6/tiproxy-overview)で使用される証明書ファイル パス。
-   デフォルト値: &quot;&quot;
...
-   セッション移行のために[Tiプロキシ](https://docs.pingcap.com/tidb/v7.6/tiproxy-overview)で使用されるキー ファイル パス。
-   デフォルト値: &quot;&quot;
...
> **警告：**
>
> この構成は将来のバージョンでは廃止される可能性があります。この構成の値を変更**しないでください**。
...
-   この構成は、各演算子の実行情報をスロー クエリ ログに記録するかどうかを制御します。
-   デフォルト値: `true`
