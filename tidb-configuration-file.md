...
- In TiDB v6.5.0 and later versions, this configuration is no longer recommended. The memory size of a transaction will be accumulated into the memory usage of the session, and the [`tidb_mem_quota_query`](/system-variables.md#tidb_mem_quota_query) variable will take effect when the session memory threshold is exceeded. To be compatible with previous versions, the behavior of this configuration is as follows when you upgrade from an earlier version to TiDB v6.5.0 or a later version:
    ...
    - If the value of this configuration is not the default value (104857600), the behavior of the configuration to limit the size of a single transaction remains unchanged before and after the upgrade. The configuration still takes effect, and the memory size of the transaction is not controlled by the `tidb_mem_quota_query` variable.
