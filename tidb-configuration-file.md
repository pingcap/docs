...
### <code>tidb-max-reuse-chunk</code> <span class="version-mark">v6.4.0の新機能</span> {#code-tidb-max-reuse-chunk-code-span-class-version-mark-new-in-v6-4-0-span}
...
### <code>tidb-max-reuse-column</code> <span class="version-mark">v6.4.0で追加</span> {#code-tidb-max-reuse-column-code-span-class-version-mark-new-in-v6-4-0-span}
...
### <code>temp-dir</code> <span class="version-mark">v6.3.0 で追加されました。</span> {#code-temp-dir-code-span-class-version-mark-new-in-v6-3-0-span}

-   TiDBが一時データを保存するために使用されるファイルシステム上の場所。機能がTiDBノード内でローカルstorageを必要とする場合、TiDBはこの場所に対応する一時データを保存します。
...
-   単一の SQL ステートメントがシステム変数[`tidb_mem_quota_query`](/system-variables.md#tidb_mem_quota_query)で指定されたメモリクォータを超えた場合に、一部のオペレーターの一時的なstorageパスを指定します。
...
-   デフォルト値: `-1`
-   `tmp-storage-path`の残りの使用可能なstorageが`tmp-storage-quota`で定義された値よりも少ない場合、TiDBサーバーは起動時にエラーを報告して終了します。
...
### <code>table-column-count-limit</code> <span class="version-mark">v5.0の新機能</span> {#code-table-column-count-limit-code-span-class-version-mark-new-in-v5-0-span}

-   単一テーブル内の列数の上限を設定します。
-   デフォルト値: `1017`
-   現在、有効な値の範囲は`[1017, 4096]`です。

### <code>index-limit</code> <span class="version-mark">v5.0で追加</span> {#code-index-limit-code-span-class-version-mark-new-in-v5-0-span}

-   単一テーブル内のインデックス数の上限を設定します。
-   デフォルト値: `64`
-   現在、有効な値の範囲は`[64, 512]`です。

### <code>enable-telemetry</code> <span class="version-mark">v4.0.2の新機能</span> {#code-enable-telemetry-code-span-class-version-mark-new-in-v4-0-2-span}

> **警告：**
>
> -   バージョン8.1.0から8.5.1までは、TiDBはテレメトリ機能を削除しており、この設定項目は無効になります。以前のバージョンとの互換性のためにのみ残されています。
...
### <code>enable-tcp4-only</code> <span class="version-mark">v5.0の新機能</span> {#code-enable-tcp4-only-code-span-class-version-mark-new-in-v5-0-span}
...
### <code>enable-enum-length-limit</code> <span class="version-mark">v5.0で追加</span> {#code-enable-enum-length-limit-code-span-class-version-mark-new-in-v5-0-span}
...
### <code>graceful-wait-before-shutdown</code> <span class="version-mark">（v5.0の新機能）</span> {#code-graceful-wait-before-shutdown-code-span-class-version-mark-new-in-v5-0-span}
...
### <code>enable-global-kill</code> <span class="version-mark">v6.1.0 で追加されました。</span> {#code-enable-global-kill-code-span-class-version-mark-new-in-v6-1-0-span}
...
### <code>enable-32bits-connection-id</code> <span class="version-mark">v7.3.0で追加</span> {#code-enable-32bits-connection-id-code-span-class-version-mark-new-in-v7-3-0-span}
...
### <code>initialize-sql-file</code> <span class="version-mark">v6.6.0で追加</span> {#code-initialize-sql-file-code-span-class-version-mark-new-in-v6-6-0-span}
...
### <code>enable-table-lock</code> <span class="version-mark">v4.0.0で追加</span> {#code-enable-table-lock-code-span-class-version-mark-new-in-v4-0-0-span}
...
### <code>general-log-file</code> <span class="version-mark">v8.0.0で追加</span> {#code-general-log-file-code-span-class-version-mark-new-in-v8-0-0-span}
...
### <code>timeout</code> <span class="version-mark">v7.1.0の新機能</span> {#code-timeout-code-span-class-version-mark-new-in-v7-1-0-span}
...
#### <code>compression</code> <span class="version-mark">（v8.0.0の新機能）</span> {#code-compression-code-span-class-version-mark-new-in-v8-0-0-span}
...
### <code>auth-token-jwks</code> <span class="version-mark">v6.4.0の新機能</span> {#code-auth-token-jwks-code-span-class-version-mark-new-in-v6-4-0-span}

-   [`tidb_auth_token`](/security-compatibility-with-mysql.md#tidb_auth_token)認証方式で使用するJSON Web Key Sets（JWKS）のローカルファイルパスを設定します。
-   デフォルト値: `""`

### <code>auth-token-refresh-interval</code> <span class="version-mark">v6.4.0 で追加されました。</span> {#code-auth-token-refresh-interval-code-span-class-version-mark-new-in-v6-4-0-span}

-   [`tidb_auth_token`](/security-compatibility-with-mysql.md#tidb_auth_token)認証方式のJWKS更新間隔を設定します。
-   デフォルト値: `1h`

### <code>disconnect-on-expired-password</code> <span class="version-mark">（v6.5.0の新機能）</span> {#code-disconnect-on-expired-password-code-span-class-version-mark-new-in-v6-5-0-span}
...
### <code>server-memory-quota</code> <span class="version-mark">v4.0.9の新機能</span> {#code-server-memory-quota-code-span-class-version-mark-new-in-v4-0-9-span}
...
### <code>txn-entry-size-limit</code><span class="version-mark">は v4.0.10 および v5.0.0 で追加されました。</span> {#code-txn-entry-size-limit-code-span-class-version-mark-new-in-v4-0-10-and-v5-0-0-span}
...
### <code>enable-stats-cache-mem-quota</code> <span class="version-mark">v6.1.0で追加</span> {#code-enable-stats-cache-mem-quota-code-span-class-version-mark-new-in-v6-1-0-span}

-   統計キャッシュのメモリ割り当てを有効にするかどうかを制御します。
-   デフォルト値: `true`

### <code>stats-load-concurrency</code> <span class="version-mark">v5.4.0の新機能</span> {#code-stats-load-concurrency-code-span-class-version-mark-new-in-v5-4-0-span}
...
### <code>stats-load-queue-size</code> <span class="version-mark">v5.4.0で追加</span> {#code-stats-load-queue-size-code-span-class-version-mark-new-in-v5-4-0-span}

-   TiDBの同期統計情報読み込み機能がキャッシュできる列リクエストの最大数。
-   デフォルト値: `1000`
-   現在、有効な値の範囲は`[1, 100000]`です。

### <code>concurrently-init-stats</code> <span class="version-mark">v8.1.0 および v7.5.2 で追加されました。</span> {#code-concurrently-init-stats-code-span-class-version-mark-new-in-v8-1-0-and-v7-5-2-span}
...
### <code>lite-init-stats</code> <span class="version-mark">v7.1.0の新機能</span> {#code-lite-init-stats-code-span-class-version-mark-new-in-v7-1-0-span}
...
### <code>force-init-stats</code> <span class="version-mark">v6.5.7 および v7.1.0 で追加されました。</span> {#code-force-init-stats-code-span-class-version-mark-new-in-v6-5-7-and-v7-1-0-span}
...
### <code>enable-async-batch-get</code> <span class="version-mark">v8.5.5で追加</span> {#code-enable-async-batch-get-code-span-class-version-mark-new-in-v8-5-5-span}
...
### <code>pd-server-timeout</code> {#code-pd-server-timeout-code}
...
### <code>batch-policy</code> <span class="version-mark">v8.3.0の新機能</span> {#code-batch-policy-code-span-class-version-mark-new-in-v8-3-0-span}
...
### <code>copr-req-timeout</code> <span class="version-mark">v7.5.0で追加</span> {#code-copr-req-timeout-code-span-class-version-mark-new-in-v7-5-0-span}
...
### <code>enable-replica-selector-v2</code> <span class="version-mark">v8.0.0の新機能</span> {#code-enable-replica-selector-v2-code-span-class-version-mark-new-in-v8-0-0-span}
...
### pessimistic-auto-commit は<span class="version-mark">v6.0.0 で追加されました。</span> {#pessimistic-auto-commit-span-class-version-mark-new-in-v6-0-0-span}
...
### constraint-check-in-place-pessimistic） <span class="version-mark">v6.4.0の新機能</span> {#constraint-check-in-place-pessimistic-span-class-version-mark-new-in-v6-4-0-span}
...
### <code>in-mem-slow-query-topn-num</code> <span class="version-mark">v7.3.0の新機能</span> {#code-in-mem-slow-query-topn-num-code-span-class-version-mark-new-in-v7-3-0-span}

-   この設定は、メモリにキャッシュされる最も遅いクエリの数を制御します。
-   デフォルト値: 30

### <code>in-mem-slow-query-recent-num</code> <span class="version-mark">（v7.3.0で追加）</span> {#code-in-mem-slow-query-recent-num-code-span-class-version-mark-new-in-v7-3-0-span}
...
### <code>tidb_enable_stats_owner</code> <span class="version-mark">v8.4.0 で追加されました。</span> {#code-tidb-enable-stats-owner-code-span-class-version-mark-new-in-v8-4-0-span}
...
### <code>tidb_stmt_summary_enable_persistent</code> <span class="version-mark">v6.6.0で追加</span> {#code-tidb-stmt-summary-enable-persistent-code-span-class-version-mark-new-in-v6-6-0-span}
...
### <code>tidb_stmt_summary_filename</code> <span class="version-mark">v6.6.0で追加</span> {#code-tidb-stmt-summary-filename-code-span-class-version-mark-new-in-v6-6-0-span}
...
### <code>tidb_stmt_summary_file_max_days</code> <span class="version-mark">v6.6.0 で追加されました。</span> {#code-tidb-stmt-summary-file-max-days-code-span-class-version-mark-new-in-v6-6-0-span}
...
### <code>tidb_stmt_summary_file_max_size</code> <span class="version-mark">v6.6.0 で追加されました。</span> {#code-tidb-stmt-summary-file-max-size-code-span-class-version-mark-new-in-v6-6-0-span}
...
### <code>tidb_stmt_summary_file_max_backups</code> <span class="version-mark">v6.6.0で追加</span> {#code-tidb-stmt-summary-file-max-backups-code-span-class-version-mark-new-in-v6-6-0-span}
...
> **警告：**
>
> `*`は、任意の IP アドレスのクライアントが自身の IP アドレスを報告できるようになるため、セキュリティ上のリスクが生じる可能性があるため、慎重に使用してください。また、 `*`を使用すると、TiDB に直接接続する内部コンポーネント(TiDB ダッシュボードなど) が利用できなくなる可能性もあります。

### <code>fallbackable</code> <span class="version-mark">（v6.5.1の新機能）</span> {#code-fallbackable-code-span-class-version-mark-new-in-v6-5-1-span}
...
### <code>allow-expression-index</code> <span class="version-mark">v4.0.0の新機能</span> {#code-allow-expression-index-code-span-class-version-mark-new-in-v4-0-0-span}
