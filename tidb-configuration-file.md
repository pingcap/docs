...
+ Default value: `false`
+ `compatible-kill-query` takes effect only when [`enable-global-kill`](#enable-global-kill-introduced since -v610-) is set to `false`.
...
+ When [`enable-global-kill`](#enable-global-kill-new-in-v610) is `true`, `KILL xxx` and `KILL TIDB xxx` have the same effect, but using <kbd>Ctrl</kbd>+<kbd>C</kbd> to terminate a query is not supported.
