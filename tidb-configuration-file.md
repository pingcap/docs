...
> **Warning:**
>
> Since v6.3.0, this configuration is deprecated. Instead, use [`tidb_enable_tmp_storage_on_oom`](/system-variables.md#tidb_enable_tmp_storage_on_oom) for setting. When the TiDB cluster upgrades to v6.3.0 or newer versions, it will automatically initialize the variable with the value of `oom-use-tmp-storage`. After that, changing the value of `oom-use-tmp-storage` does NOT take effect any more.
...
+ This configuration takes effect only when [`tidb_enable_tmp_storage_on_oom`](/system-variables.md#tidb_enable_tmp_storage_on_oom) is `ON`.
...
- Default value: `false`
- Since v6.1.0, enabling the `utf8mb4` character check is determined by configuration `instance.tidb_check_mb4_value_in_utf8` or system variable `tidb_check_mb4_value_in_utf8`. `check-mb4-value-in-utf8` still takes effect. But if `check-mb4-value-in-utf8` and `instance.tidb_check_mb4_value_in_utf8` are setting at the same time, TiDB will apply the value of `instance.tidb_check_mb4_value_in_utf8`.
...
- Since v6.2.0, the number of concurrent client connections is set by configuration [`instance.max_connections`](/tidb-configuration-file.md#max_connections) or system variable [`max_connections`](/system-variables.md#max_connections). `max-server-connections` still takes effect. But if `max-server-connections` and `instance.max_connections` are setting at the same time, TiDB will apply the value of `instance.max_connections`.
...
- Since v6.1.0, enabing the slow query log is determined by configuration [`instance.tidb_enable_slow_log`](/tidb-configuration-file.md#tidb_enable_slow_log) or system variable [`tidb_enable_slow_log`](/system-variables.md#tidb_enable_slow_log). `enable-slow-log` still takes effect. But if `enable-slow-log` and `instance.tidb_enable_slow_log` are setting at the same time, TiDB will apply the value of `instance.tidb_enable_slow_log`.
...
- Since v6.1.0, the threshold value of consumed time in the slow log is determined by configuration [`instance.tidb_slow_log_threshold`](/tidb-configuration-file.md#tidb_slow_log_threshold) or system variable [`tidb_slow_log_threshold`](/system-variables.md#tidb_slow_log_threshold). `slow-threshold` still takes effect. But if `slow-threshold` and `instance.tidb_slow_log_threshold` are setting at the same time, TiDB will apply the value of `instance.tidb_slow_log_threshold`.

### `record-plan-in-slow-log`

- Determines whether to record execution plans in the slow log.
- Default value: `1`
- Since v6.1.0, the threshold value of consumed time in the slow log is determined by configuration [`instance.tidb_record_plan_in_slow_log`](/tidb-configuration-file.md#tidb_record_plan_in_slow_log) or system variable [`tidb_record_plan_in_slow_log`](/system-variables.md#tidb_record_plan_in_slow_log). `record-plan-in-slow-log` still takes effect. But if `record-plan-in-slow-log` and `instance.tidb_record_plan_in_slow_log` are setting at the same time, TiDB will apply the value of `instance.tidb_record_plan_in_slow_log`.
...
+ Since v6.1.0, the threshold is determined by configuration [`instance.tidb_memory_usage_alarm_ratio`](/tidb-configuration-file.md#tidb_memory_usage_alarm_ratio) or system variable [`tidb_memory_usage_alarm_ratio`](/system-variables.md#tidb_memory_usage_alarm_ratio). `memory-usage-alarm-ratio` still takes effect. But if `memory-usage-alarm-ratio` and `instance.tidb_memory_usage_alarm_ratio` are setting at the same time, TiDB will apply the value of `instance.tidb_memory_usage_alarm_ratio`.
...
- Since v6.1.0, the priority for all statements is determined by configuration [`instance.tidb_force_priority`](/tidb-configuration-file.md#tidb_force_priority) or system variable [`tidb_force_priority`](/system-variables.md#tidb_force_priority). `force-priority` still takes effect. But if `force-priority` and `instance.tidb_force_priority` are setting at the same time, TiDB will apply the value of `instance.tidb_force_priority`.
...
- Default value: `1`
- Value options: `1` (enable, default) or `0` (disable).
...
- The value of this configuration will initialize the value of system variable [`tidb_enable_ddl`](/system-variables.md#tidb_enable_ddl)
