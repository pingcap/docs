...
[自転車シェアリングのサンプルデータベース](/import-example-data.md)から、次の 2つのクエリが同様の方法で実行されていることがわかります。
...
上記の最初の文では、ビュー定義を満たすためにインデックスが使用され、TiDBがテーブル行を読み取る際に`bike_number = 'W00950'`が適用されていることがわかります。2番目の文では、文を満たすインデックスがないため、 `TableFullScan`が使用されています。
