...
If you know in advance that an unsupported SQL statement is going to be replicated, you can also use `dmctl` to manually preset the skip/replace operation, which will be automatically executed when DM replicates the corresponding binlog event into the downstream and thus avoid breaking the replication.

#### Restrictions
- The skip/replace operation is an one-time operation that is only used to skip/replace the SQL statement unsupported by the downstream TiDB. Do not handle other replication errors with this approach.
    - For other replication errors, try using [Black and white table lists](/tools/dm/data-synchronization-features.md#black-and-white-table-lists) or [Binlog event filtering](/tools/dm/data-synchronization-features.md#binlog-event-filtering).
- If it is unacceptable in the actual production environment that the abnormal DDL statement is skipped in the downstream TiDB and it cannot be replaced with other DDL statements, then using this approach is not recommended.
...
- A single skip/replace operation targets at a single binlog event.
...
- After the operator successfully matches with the binlog event (not after the execution succeeds), it will be deleted. If you need to match again (using `--sql-pattern`) later, you have to register a new operator.

### Support Scenarios

- During the replication, if the DDL statement unsupported by TiDB is executed in the upstream and replicated to the downstream, the replication task will get interrupted.
...
- During the replication, if you know in advance that an unsupported SQL statement is going to be replicated, you can handle it beforehand to avoid breaking the replication.
...
5. Execute the skip/replace operation corresponding to the operator and then the replication task continues.
...
4. Execute the skip/replace operation corresponding to the operator and then the replication task continues and will not get interrupted.
...
7. Execute the skip/replace operation corresponding to the operator and then the replication task continues. 
...
+ `task-name`:
    ...
    - If it is not specified, this command queries the errors of all subtasks; if it is specified, this command queries the error of the specified subtask.
...
**Assume that we know in advance that this DDL statement is not supported by TiDB before it is executed in the upstream.** Then we can use `sql-skip` or `sql-replace` to preset a skip/replace operation for this DDL statement. 
