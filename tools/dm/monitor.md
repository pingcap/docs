...
| unsynced tables | The number of tables that have not received the sharding DDL statement in the current subtask | N/A |
| shard lock resolving | Whether the current subtask is waiting for the sharding DDL statement to be replicated; a value greater than 0 indicates that it is waiting for the DDL replication | N/A |
