...
Reparo is a TiDB-Binlog tool, used to recover the incremental data. Drainer of TiDB-Binlog outputs the binlog data in the protobuf format to files and the incremental data is backed up in this way; when the incremental data needs to be recovered, Reparo is used to parse the binlog data in the files and apply the data in TiDB／MySQL.
...
# Correspond to `tso` of `start-datetime` and `stop-datetime` respectively. 
