...
```bash
store limit                         // Show the speed limit of adding learners/peers in all stores (if a specific type is not set, this command shows the speed of adding learners/peers)
store limit region-add              // Show the speed limit of adding learners/peers in all stores 
store limit region-remove           // Show the speed limit of deleting learners/peers in all stores 
```
...
Because the store limit is a mapping in the memory, the above modification is reset after the leader is switched or 
PD is restarted. If you want to persist the modification, run the following command:
