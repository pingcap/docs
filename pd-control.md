...
- `hot-region-cache-hits-threshold` is used to set the threshold of a hot Region. A Region is considered as hot only if the number of its cache hits exceed this threshold.
...
>> schedule pause balance-region-scheduler 10     // Pause the balance-region scheduler for 10 seconds
>> schedule pause all 10                          // Pause all schedulers for 10 seconds
>> schedule resume balance-region-scheduler       // Continue to run the balance-region scheduler
>> schedule resume all                            // Continue to run all schedulers
