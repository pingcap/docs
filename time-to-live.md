...
-   TTL 属性を持つテーブルを作成します。
    ...
    上記の例では、テーブル`t1`を作成し、TTLタイムスタンプ列に`created_at`を指定しています。これはデータの作成時刻を示します。また、テーブル内で行が保持できる最長期間を 3 か月から`INTERVAL 3 MONTH`に設定しています。この値を超えて保持されるデータは、後で削除されます。
