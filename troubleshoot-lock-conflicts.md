...
TiDB Dashboardの`KV Errors`パネルには、トランザクションによって発生した書き込み競合を確認するための2つの監視メトリック`Lock Resolve OPS`と`KV Backoff OPS`あります。9 `Lock Resolve OPS` `resolve`番目の項目と`KV Backoff OPS`の`txnLock`番目の項目に明らかな上昇傾向が見られる場合、「KeyIsLocked」エラーが発生します。15 `resolve`ロックを解除しようとする操作を指し、 `txnLock`書き込み競合を表します。
