...
In a database management system, in addition to the core transactional processing (TP) and analytical processing (AP) workloads, there are other important tasks, such as DDL operations, [`IMPORT INTO`], [TTL](/time-to-live.md), [Analyze](/sql-statements/sql-statement-analyze-table.md), and [Backup/Restore](/br/br-use-overview.md). These tasks need to process a large amount of data in database objects (tables), so they typically have the following characteristics:
