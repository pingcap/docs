...
- 4.1.2 If TiKV is deployed on a virtual machine, when the virtual machine is killed or the physical machine is powered off, the error `entries[X, Y]  is unavailable from storage` is reported.
...
- 4.2.2 Coprocessor receives many large queries and returns a large volume of data. gRPC fails to send the data returned by the coprocessor, which results in OOM.
...
- 4.4.1 Re-election because TiKV is restarted
    ...
    - After TiKV panics, it is pulled up by systemd and runs normally. You can check whether panic has occurred in TiKV by viewing the TiKV log. Since this issue is unexpected, report a bug if it happens.
...
- 6.1.6 Slow replication
    ...
    - The downstream is TiDB/MySQL, and the table to be replicated has no primary key and no unique index, which causes reduced performance in binlog. It is recommended to add primary key or unique index.
