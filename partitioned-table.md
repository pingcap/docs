...
`last_plan_from_cache`変数は、最後のクエリがプランキャッシュにヒットしたかどうかを示すことができます。例1から、 `static`モードでは、パーティション化されたテーブルで同じクエリが複数回実行された場合でも、プランキャッシュがヒットしないことがわかります。
...
例3から、 `TIDB_INLJ`のヒントを使用しても、パーティションテーブルのクエリでIndexJoinを使用して実行プランを選択できないことがわかります。
