...
レンジCOLUMNSパーティショニングは、レンジパーティショニングのバリアントです。1つ以上の列をパーティショニング キーとして使用できます。パーティション列のデータ型は、整数、文字列 ( `CHAR`または`VARCHAR` )、 `DATE` 、および`DATETIME` 。列を使用しないパーティショニングなどの式はサポートされていません。
