...
上記の 3 つの次元では、インデックス`idx_a`パフォーマンスが 3 つの次元すべてでインデックス`idx_b`より悪くなく、1 つの次元で`idx_b`より優れている場合、 `idx_a`が推奨されます。9 `EXPLAIN FORMAT = 'verbose' ...`を実行すると、スカイライン プルーニングによって一部のインデックスが除外されると、TiDB はスカイライン プルーニング除外後の残りのインデックスをリストする NOTE レベルの警告を出力します。
...
[多値インデックス](/sql-statements/sql-statement-create-index.md#multi-valued-indexes)通常のインデックスとは異なります。TiDB は現在、多値インデックスへのアクセスに[インデックスマージ](/explain-index-merge.md)を使用します。したがって、データ アクセスに多値インデックスを使用するには、システム変数[`tidb_enable_index_merge`](/system-variables.md#tidb_enable_index_merge-new-in-v40)の値が`ON`に設定されていることを確認してください。
