...
3. Click **Apply for a PoC Trial** and refer to [Quick Start](\cloud\tidb-cloud-quickstart.md) to create a [Dedicated Tier](\cloud\select-cluster-tier.md#dedicated-tier) cluster for the PoC.
...
If you have any questions, contact [TiDB Cloud Support](\cloud\tidb-cloud-support.md) for consultation.
...
- Go to **Diagnostic > Statements**, where you can observe SQL execution and easily locate performance problems without querying the system tables. See [Statement Analysis](\cloud\tune-performance.md).
- Go to **Diagnostic > Key Visualizer**, where you can view TiDB data access patterns and data hotspots. See [Key Visualizer](\cloud\tune-performance.md#key-visualizer).
...
If your trial points are running out and you want to continue with the PoC, contact the [TiDB Cloud Support](\cloud\tidb-cloud-support.md) for consultation.
...
You can scale out clusters on the console by yourself. If you need to scale in a cluster, you need to contact [TiDB Cloud Support](\cloud\tidb-cloud-support.md) for help. For more information about scaling, see [Scale Your TiDB Cluster](\cloud\scale-tidb-cluster.md). You can keep in touch with the support team to track the exact progress. You must wait for the scaling operation to finish before starting your test because it can impact the performance due to data rebalancing.

### 3. How to make the best use of my PoC trial points?

Once your application for the PoC is approved, you will receive trial points in your account. Generally, the trial points are sufficient for a 14-day PoC. The trial points are charged by the type of nodes and the number of nodes, on an hourly basis. For more information, see [TiDB Cloud Billing](\cloud\tidb-cloud-billing.md#trial-points).
...
If you want to extend the PoC trial period or are running out of trial points, contact [TiDB Cloud Support](\cloud\tidb-cloud-support.md) for help.

### 5. I'm stuck with a technical problem. How do I get help for my PoC?

You can always contact [TiDB Cloud Support](\cloud\tidb-cloud-support.md) for help.