...
> **Note:**
>
> [Developer Tier clusters](/cloud/select-cluster-tier.md#developer-tier) are free to use for up to one year. You will not be charged for the use of your Developer Tier cluster, and your TiDB Cloud bill will not display any Developer Tier charges.
...
TiDB Cloud lets you pay for your database by the hour, which is ideal for development, testing, and other short-lived workloads.

In TiDB Cloud, you can control your custer size easily by specifying the node quantity of TiDB, TiKV, and TiFlash<sup>beta</sup>, as well as the storage size of TiKV and TiFlash<sup>beta</sup>.

The total cost varies according to your specified node quantity and storage size.

- For TiDB, the node cost consists of the CPU cost only.
- For TiKV and TiFlash<sup>beta</sup>, the node cost consists of the CPU cost and the storage cost.

The cost of TiDB, TiKV, and TiFlash<sup>beta</sup> nodes are different. For TiDB, TiKV, and TiFlash<sup>beta</sup> node cost of different cloud service providers and in different regions, refer to [TiDB Cloud pricing](https://en.pingcap.com/tidb-cloud-pricing/).
...
Both AWS and GCP charge for data transfer, calculated per GB, whenever data moves within or leaves its cloud. Examples of when these charges are incurred with TiDB Cloud include the following:
...
TiDB Cloud passes these charges onto customers as they are incurred, calculated using the published [AWS](https://aws.amazon.com/pricing/) and [GCP](https://cloud.google.com/vpc/network-pricing) price tables as applicable, and without any additional fees.

This cost policy applies to all TiDB Cloud customers with [Dedicated Tier](/cloud/select-cluster-tier.md#dedicated-tier) clusters, regardless of how those clusters are licensed.
