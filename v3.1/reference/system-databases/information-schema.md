...
The `CHARACTER_SETS` table provides information about [character sets](/v3.1/reference/sql/character-set.md). Currently, TiDB only supports some of the character sets.
...
The `SLOW_QUERY` table maps slow query logs. Its column names and field names of slow query logs have an one-to-one corresponse relationship. For details, see [Identify Slow Queries](/v3.1/how-to/maintain/identify-slow-queries.md#identify-slow-queries).
