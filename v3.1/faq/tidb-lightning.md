...
or [CSV files](/v3.1/reference/tools/tidb-lightning/csv.md) stored in the local filesystem.
...
It is not recommended to directly use `nohup` in the command line to start `tidb-lightning`. You can [start `tidb-lightning`](/v3.1/reference/tools/tidb-lightning/deployment.md) by executing a script.
