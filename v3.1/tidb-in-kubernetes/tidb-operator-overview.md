...
category: reference
aliases: ['/docs/v3.1/reference/tidb-operator-overview/']
---
...
+ For test environment:
    ...
    - [kind](/v3.1/tidb-in-kubernetes/get-started/deploy-tidb-from-kubernetes-kind.md): Deploy TiDB clusters in local Kubernetes using kind
    - [DinD](/v3.1/tidb-in-kubernetes/get-started/deploy-tidb-from-kubernetes-dind.md): Deploy TiDB clusters in a local DinD environment using TiDB Operator
    - [Minikube](/v3.1/tidb-in-kubernetes/get-started/deploy-tidb-from-kubernetes-minikube.md): Deploy TiDB clusters in a local Minikube environment using TiDB Operator
    - [GKE](/v3.1/tidb-in-kubernetes/get-started/deploy-tidb-from-kubernetes-gke.md): Deploy TiDB clusters on GKE using TiDB Operator

+ For production environment:

    + On public cloud:
        - [Deploy TiDB on AWS EKS](/v3.1/tidb-in-kubernetes/deploy/aws-eks.md)
        - [Deploy TiDB on GCP GKE (beta)](/v3.1/tidb-in-kubernetes/deploy/gcp-gke.md)
        - [Deploy TiDB on Alibaba Cloud ACK](/v3.1/tidb-in-kubernetes/deploy/alibaba-cloud.md)

    - In an existing Kubernetes cluster:

        First install TiDB Operator in a Kubernetes cluster according to [Deploy TiDB Operator in Kubernetes](/v3.1/tidb-in-kubernetes/deploy/tidb-operator.md), then deploy your TiDB clusters according to [Deploy TiDB in General Kubernetes](/v3.1/tidb-in-kubernetes/deploy/general-kubernetes.md).

        You also need to adjust the configuration of the Kubernetes cluster based on [Prerequisites for TiDB in Kubernetes](/v3.1/tidb-in-kubernetes/deploy/prerequisites.md) and configure the local PV for your Kubernetes cluster to achieve low latency of local storage for TiKV according to [Local PV Configuration](/v3.1/tidb-in-kubernetes/reference/configuration/storage-class.md#local-pv-configuration).

Before deploying TiDB on any of the above two environments, you can always refer to [TiDB Cluster Configuration Document](/v3.1/tidb-in-kubernetes/reference/configuration/tidb-cluster.md) to customize TiDB configurations.

After the deployment is complete, see the following documents to use, operate, and maintain TiDB clusters in Kubernetes:

+ [Access the TiDB Cluster](/v3.1/tidb-in-kubernetes/deploy/access-tidb.md)
+ [Scale TiDB Cluster](/v3.1/tidb-in-kubernetes/scale-in-kubernetes.md)
+ [Upgrade TiDB Cluster](/v3.1/tidb-in-kubernetes/upgrade/tidb-cluster.md#upgrade-the-version-of-tidb-cluster)
+ [Change the Configuration of TiDB Cluster](/v3.1/tidb-in-kubernetes/upgrade/tidb-cluster.md#change-the-configuration-of-tidb-cluster)
...
+ [Monitor a TiDB Cluster in Kubernetes](/v3.1/tidb-in-kubernetes/monitor/tidb-in-kubernetes.md)
+ [Collect TiDB Logs in Kubernetes](/v3.1/tidb-in-kubernetes/maintain/log-collecting.md)
+ [Maintain Kubernetes Nodes that Hold the TiDB Cluster](/v3.1/tidb-in-kubernetes/maintain/kubernetes-node.md)

When a problem occurs and the cluster needs diagnosis, you can:

+ See [TiDB FAQs in Kubernetes](/v3.1/tidb-in-kubernetes/faq.md) for any available solution;
+ See [Troubleshoot TiDB in Kubernetes](/v3.1/tidb-in-kubernetes/troubleshoot.md) to shoot troubles.
...
+ Use `tkctl` according to [`tkctl` Guide](/v3.1/tidb-in-kubernetes/reference/tools/tkctl.md );
+ See [Tools in Kubernetes](/v3.1/tidb-in-kubernetes/reference/tools/in-kubernetes.md) to understand how TiDB tools are used in Kubernetes.

Finally, when a new version of TiDB Operator is released, you can refer to [Upgrade TiDB Operator](/v3.1/tidb-in-kubernetes/upgrade/tidb-operator.md) to upgrade to the latest version.
