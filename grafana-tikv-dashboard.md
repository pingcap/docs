...
- Abnormal Checkpoint TS Lag: Lag of the current checkpoint TS to the present time for each task
...
> **Note:**
>
> The following monitoring items all use TiDB as their data source, but they have some impact on the log backup process. For ease of reference, they are placed in TiKV Details. In addition, TiKV actively pushes progress most of the time, so it is normal for some of the following monitoring items to occasionally not have sampled data.
...
- Region Checkpoint Failure Reason: A record of the reason why a Region checkpoint cannot advance within the coordinator
