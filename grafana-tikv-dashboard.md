...
-   スケジューラワーカーの CPU: `scheduler worker`スレッドの CPU 使用率。通常の場合、CPU 使用率は 90% * `ストレージ.scheduler-worker-pool-size`未満である必要があります。
-   Storage ReadPool CPU: `ストレージ read pool`スレッドの CPU 使用率
...
![TiKV Dashboard - Storage metrics](/media/tikv-dashboard-ストレージ.png)
...
TiKVのパフォーマンスに関する問題（QPSジッター、レイテンシージッター、レイテンシー増加傾向など）が発生した場合は、 **TiKV-FastTune**ダッシュボードを確認してください。このダッシュボードには、特にクラスター内の書き込みワークロードが中規模または大規模の場合に、診断に役立つ一連のパネルが含まれています。
