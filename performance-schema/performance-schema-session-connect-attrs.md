...
summary: SESSION_CONNECT_ATTRSは、接続属性に関する情報を提供します。セッション属性は、接続の確立時にクライアントによって送信されるキーと値のペアです。共通の属性には、_client_name、_client_version、_os、_pid、_platform、program_nameがあります。SESSION_CONNECT_ATTRSテーブルの列は、PROCESSLIST_ID、ATTR_NAME、ATTR_VALUE、ORDINAL_POSITIONで構成されています。これにより、セッション属性に関する情報を表示できます。
---
