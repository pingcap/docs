...
You can use `TIDB_KV_READ_TIMEOUT(N)` to set the timeout as `N` milliseconds for TiDB to send a TiKV RPC read request in a query statement. When the TiDB cluster is in an environment with unstable network or serious TiKV I/O latency jitter, and your business is sensitive to the latency of the query SQL, you can set `TIDB_KV_READ_TIMEOUT(N)` to reduce the timeout time of the TiKV RPC read request. In this case, when a TiKV has I/O latency jitter, the TiDB side can time out quickly and re-send the TiKV RPC request to the next TiKV where the TiKV Region Peer is located. If all TiKV Region Peers requests time out, they will retry with the default timeout.
