...
This document introduces how to upgrade TiDB Binlog that is deployed with Ansible and deployed manually to the latest [cluster](/v2.1/reference/tidb-binlog/overview.md) version. There is also a section on how to upgrade TiDB Binlog from an earlier incompatible version (Kafka/Local version) to the latest version.
...
The new TiDB versions (v2.0.8-binlog, v2.1.0-rc.5 or later) are not compatible with the [Kafka version](/v2.1/reference/tidb-binlog/tidb-binlog-kafka.md) or [Local version](/v2.1/reference/tidb-binlog/tidb-binlog-local.md) of TiDB Binlog. If TiDB is upgraded to one of the new versions, it is required to use the cluster version of TiDB Binlog. If the Kafka or local version of TiDB Binlog is used before upgrading, you need to upgrade your TiDB Binlog to the cluster version.
...
> **Note:**
>
> If importing the full data is acceptable, you can abandon the old version and deploy TiDB Binlog following [TiDB Binlog Cluster Deployment](/v2.1/reference/tidb-binlog/deploy.md).
