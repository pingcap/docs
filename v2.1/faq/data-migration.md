...
During the DM processes of relay log pulling and incremental replication processes, this two errors might occur if the binlog file in the upstream exceeds **4 GB**.
...
1. Identify in the upstream that the corresponding binlog file that has exceeded 4GB when the error occurs.
...
1. Identify in the upstream the corresponding binlog file that has exceeded 4GB when error occurs.
