...
为了解决上述场景中的热点问题，TiDB 引入了 pre-split 功能，可以根据指定参数为某个表预先拆分多个 Region，并将它们分散到每个 TiKV 节点。
...
- 均匀拆分的语法：
    ...
    `BETWEEN lower_value AND upper_value REGIONS region_num` 定义了上边界、下边界和 Region 数量。然后当前 Region 会在上下边界之间平均拆分成指定数量（`region_num`）的 Region。
...
>
> - `tidb_wait_split_region_finish`：散布 Region 可能需要一段时间。此时间取决于 PD 调度和 TiKV 负载。此变量用于控制执行 `SPLIT REGION` 时，是否在所有 Region 散布完成之前返回结果给客户端。若其值设为 `1`（默认值），TiDB 只有在散布完成后才返回结果。若设为 `0`，TiDB 无论散布状态如何都立即返回结果。
...
由于 `row_id` 为整数，可以根据指定的 `lower_value`、`upper_value` 和 `region_num` 计算出拆分的 key 值。TiDB 首先计算步长（`step = (upper_value - lower_value)/region_num`），然后在 `lower_value` 和 `upper_value` 之间每个“步长”均匀拆分，生成指定数量的 Region。

例如，如果你想用 `minInt64` 到 `maxInt64` 的范围，将表 t 拆分成 16 个均匀 Region，可以使用以下语句：
...
如果已知数据分布不均，且希望在 `-inf ~ 10000`、`10000 ~ 90000` 和 `90000 ~ +inf` 的范围内拆分 Region，可以通过设置固定点实现，如下所示：
...
同一索引在同一表中的 `table_id` 和 `index_id` 相同。要拆分索引 Region，需要根据 `index_value` 进行拆分。

#### 均匀拆分

均匀拆分索引的方法与数据均匀拆分相同，但计算步长更复杂，因为 `index_value` 可能不是整数。

首先将 `upper` 和 `lower` 转换为字节数组。去除 `lower` 和 `upper` 字节数组的最长公共前缀后，将 `lower` 和 `upper` 的前 8 个字节转换为 uint64 格式。然后计算 `step = (upper - lower)/num`。之后将计算得到的步长编码成字节数组，附加到 `lower` 和 `upper` 的最长公共前缀后，用于索引拆分。示例如下：

如果索引列 `idx` 为整数类型，可以使用以下 SQL 语句拆分索引数据：

```sql
SPLIT TABLE t INDEX idx BETWEEN (-9223372036854775808) AND (9223372036854775807) REGIONS 16;
```

此语句会将表 t 中索引 `idx` 的 Region 从 `minInt64` 拆分成 16 个 Region。

如果索引列 `idx1` 为 varchar 类型，且希望按前缀字母拆分索引数据，可以使用：
...
在上述拆分方法中，`y` 和 `z` 前缀的索引数据都写入第 25 个 Region，因为上界不是 `z`，而是字符 `{`（ASCII 中 `z` 后的字符）。因此，更精确的拆分方法如下：
...
如果索引 `idx2` 的列为时间类型（如 timestamp/datetime），且希望按年份拆分索引 Region，可以使用：

```sql
SPLIT TABLE t INDEX idx2 BETWEEN ("2010-01-01 00:00:00") AND ("2020-01-01 00:00:00") REGIONS 10;
```

此语句会将表 `t` 中索引 `idx2` 的 Region 从 `2010-01-01 00:00:00` 到 `2020-01-01 00:00:00` 拆分成 10 个 Region。每个 Region 的范围如下：

- Region 1：`[minIndexValue, 2011-01-01 00:00:00)`
- Region 2：`[2011-01-01 00:00:00, 2012-01-01 00:00:00)`
- 以此类推。

如果想按天拆分索引 Region，可以参考以下示例：
...
对于联合索引的索引数据 Region 拆分，唯一不同的是可以指定多个列的值。

例如，索引 `idx3 (a, b)` 包含 2 列，列 `a` 为 timestamp 类型，列 `b` 为 int。如果只想根据列 `a` 的时间范围拆分，可以使用单列时间索引的拆分语法，不在 `lower_value` 和 `upper_value` 中指定列 `b` 的值。
...
4. 你可以再次使用 `SHOW TABLE REGIONS` 查看该表的 Region：
