...
The SST files generated by backup use the `zstd` compression algorithm by default. You can specify another compression algorithm according to your needs using the `COMPRESSION_TYPE` parameter. Supported algorithms include `lz4`, `zstd`, and `snappy`. You can also set the compression level using `COMPRESSION_LEVEL`; the higher the number, the higher the compression ratio, but the higher the CPU consumption.
