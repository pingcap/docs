...
-   クラスターのBDRロールを設定するには、 `ADMIN SET BDR ROLE`を使用します。現在、TiDBクラスターには`PRIMARY`と`SECONDARY` BDRロールを設定できます。BDRロールの詳細については、 [TiCDC 双方向レプリケーションにおける DDL 同期](/ticdc/ticdc-bidirectional-replication.md#ddl-replication)を参照してください。
-   クラスターの BDR ロールを表示するには、 `ADMIN SHOW BDR ROLE`を使用します。
-   クラスターの BDR ロールを設定解除するには、 `ADMIN UNSET BDR ROLE`を使用します。
