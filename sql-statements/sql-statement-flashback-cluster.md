...
> **警告：**
>
> `FLASHBACK CLUSTER TO [TIMESTAMP|TSO]`構文は[{{{ .starter }}}](https://docs.pingcap.com/tidbcloud/select-cluster-tier#tidb-cloud-serverless)クラスターには適用できません。予期しない結果を避けるため、このステートメントを {{{ .starter }}} クラスターでは実行しないでください。
