...
- `STATE`: the state of the DDL operation. Common states include the following:
    ...
    - `done`: indicates that the operation has been successfully executed on the owner node, but other TiDB instances have not yet synchronized the changes performed by this DDL job.
