...
`ADMIN SHOW DDL [JOBS|JOB QUERIES]` 语句用于显示正在运行和最近完成的 DDL 语句信息。
...
要查看当前正在运行的 DDL 语句状态，可使用 `ADMIN SHOW DDL`。输出内容包括当前 schema 版本、owner 的 DDL ID 和地址、正在运行的 DDL 语句及 SQL 语句，以及当前 TiDB 实例的 DDL ID。返回结果字段说明如下：
...
- `RUNNING_JOBS`：正在运行的 DDL 语句的详细信息。
...
`ADMIN SHOW DDL JOBS` 语句用于查看当前 DDL 语句队列中的 10 条语句（包括正在运行和等待中的语句，如有），以及已执行 DDL 语句队列中的最近 10 条语句（如有）。返回结果字段说明如下：

<CustomContent platform="tidb">

- `JOB_ID`：每个 DDL 操作对应一个 DDL 语句。`JOB_ID` 在全局唯一。
...
- `STATE`：DDL 操作的状态。常见状态包括：
    - `none`：表示操作尚未开始。
    - `queueing`：表示操作语句已进入 DDL 语句队列，但因等待更早的 DDL 语句完成尚未执行。另一种情况是执行 `DROP` 操作后，`queueing` 状态会变为 `done`，但很快会更新为 `synced`，表示所有 TiDB 实例已同步到该状态。
    - `running`：表示操作正在执行。
    - `synced`：表示操作已成功执行，且所有 TiDB 实例已同步到该状态。
    - `rollback done`：表示操作失败且回滚已完成。
    - `rollingback`：表示操作失败，正在回滚。
    - `cancelling`：表示操作正在被取消。该状态仅在使用 [`ADMIN CANCEL DDL JOBS`](/sql-statements/sql-statement-admin-cancel-ddl.md) 命令取消 DDL 语句时出现。
    - `cancelled`：表示操作已被取消。
    - `pausing`：表示操作正在暂停。
    - `paused`：表示操作已暂停。该状态仅在使用 [`ADMIN PAUSED DDL JOBS`](/sql-statements/sql-statement-admin-pause-ddl.md) 命令暂停 DDL 语句时出现。你可以使用 [`ADMIN RESUME DDL JOBS`](/sql-statements/sql-statement-admin-resume-ddl.md) 命令恢复 DDL 语句。
    - `done`：表示操作已在 TiDB owner 节点成功执行，但其他 TiDB 节点尚未同步该 DDL 语句的变更。
...
- `START_TIME`：DDL 操作的开始时间。
- `STATE`：DDL 操作的状态。常见状态包括：
    - `queueing`：表示操作语句已进入 DDL 语句队列，但因等待更早的 DDL 语句完成尚未执行。另一种情况是执行 `DROP` 操作后，会变为 `none` 状态，但很快会更新为 `synced`，表示所有 TiDB 实例已同步到该状态。
    - `running`：表示操作正在执行。
    - `synced`：表示操作已成功执行，且所有 TiDB 实例已同步到该状态。
    - `rollback done`：表示操作失败且回滚已完成。
    - `rollingback`：表示操作失败，正在回滚。
...
- 语句 565 当前正在进行中（`STATE` 为 `running`）。schema 状态当前为 `write reorganization`，但在语句完成后会切换为 `public`，表示该变更对用户会话可见。`end_time` 列为 `NULL`，说明该语句的完成时间尚未知晓。

- `job_id` 566 的 `STATE` 显示为 `queueing`，表示正在排队。当 565 语句完成并且 566 开始执行时，566 的 `STATE` 会变为 `running`。

- 对于如删除索引、删除表等破坏性变更，`SCHEMA_STATE` 在语句完成后会变为 `none`。对于新增变更，`SCHEMA_STATE` 会变为 `public`。
...
* `NUM`：查看已完成 DDL 语句队列中的最近 `NUM` 条结果。未指定时，默认 `NUM` 为 10。
* `WHERE`：添加过滤条件。

### `ADMIN SHOW DDL JOB QUERIES`

要查看指定 `job_id` 对应 DDL 语句的原始 SQL，可使用 `ADMIN SHOW DDL JOB QUERIES`：
...
你只能在 DDL 历史语句队列的最近十条结果中，查询指定 `job_id` 对应的正在运行的 DDL 语句。

### `ADMIN SHOW DDL JOB QUERIES LIMIT m OFFSET n`

要在指定范围 `[n+1, n+m]` 内查看 DDL 语句的原始 SQL，可使用 `ADMIN SHOW DDL JOB QUERIES LIMIT m OFFSET n`：

```sql
 ADMIN SHOW DDL JOB QUERIES LIMIT m;  # 检索前 m 行
 ADMIN SHOW DDL JOB QUERIES LIMIT n, m;  # 检索第 n+1 到 n+m 行
 ADMIN SHOW DDL JOB QUERIES LIMIT m OFFSET n;  # 检索第 n+1 到 n+m 行
 ```
 ...
 你可以在 DDL 历史语句队列的任意指定范围内，查询指定 `job_id` 对应的正在运行的 DDL 语句。该语法不受 `ADMIN SHOW DDL JOB QUERIES` 最近十条结果的限制。
