...
`EXPLAIN`の文は、クエリを実行せずにその実行プランを表示します。これは、クエリを実行する`EXPLAIN ANALYZE`の文を補完するものです。`EXPLAIN`出力が期待される結果と一致しない場合は、クエリ内の各テーブルに対して`ANALYZE TABLE`実行して、テーブル統計が最新であることを確認することを検討してください。
...
| estRows    | 演算子が出力すると予想される行数。この数は、統計情報と演算子のロジックに基づいて推定されます。`estRows` 、TiDB 4.0 の以前のバージョンでは`count`呼ばれていました。                                                                                                                                                                                                             |
