...
| `cost_trace`   | The `EXPLAIN` statement outputs results in an extended `row` format, with two additional columns, `estCost` for estimated cost, and `costFormula` column for the cost formula. |
