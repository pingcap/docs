...
* TiDB does not support temporary tables. Since v5.0 GA, the `CREATE TEMPORARY TABLE` syntax will raise an error if [`tidb_enable_noop_functions = 0`](/system-variables.md#tidb_enable_noop_functions-new-in-v40); TiDB simply ignores the `TEMPORARY` keyword if `tidb_enable_noop_functions = 1`.
...
>>>>>>> b0ecbe8ec (sql: fix the number of supported columns in a single table (#12499))
