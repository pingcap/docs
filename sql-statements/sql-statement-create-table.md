...
* TiDB does not support temporary tables. `CREATE TEMPORARY TABLE` syntax will raise an error if [`tidb_enable_noop_functions = 0`](/system-variables.md#tidb_enable_noop_functions-new-in-v40). TiDB simply ignores `TEMPORARY` keyword if `tidb_enable_noop_functions = 1`.
