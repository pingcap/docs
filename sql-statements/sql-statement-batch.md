...
> **注意：**
>
> `BATCH` 语句在内部会被重写并切分为多个 DML 语句进行执行。在当前版本中，表别名可能不会被保留，这可能导致 `where 子句` 中出现 `Unknown column '<alias>.<column>'` 的错误。为避免此问题，请不要使用表别名。在执行前，可以使用 `DRY RUN QUERY` 或 `DRY RUN` 预览切分后的语句。更多信息参见 [非事务型 DML 语句](/non-transactional-dml.md)。
