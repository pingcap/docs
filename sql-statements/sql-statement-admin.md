...
该语句可以从多次出现的 `SELECT` 语句中生成 SQL 计划绑定。
...
这里的 "untrusted" 指你需要手动确保原始表的元数据可以被 `CREATE TABLE STATEMENT` 操作覆盖。使用此 `REPAIR` 语句前，请启用 [`repair-mode`](/tidb-configuration-file.md#repair-mode) 配置项，并确保待修复的表在 [`repair-table-list`](/tidb-configuration-file.md#repair-table-list) 中列出。
...
* `SCHEMA_STATE`：当前的 schema 状态。如果 `JOB_TYPE` 为 `add index`，则为索引状态；如果为 `add column`，则为列状态；如果为 `create table`，则为表状态。常见状态包括：
    ...
    * `delete only`、`write only`、`delete reorganization`、`write reorganization`：这四个状态为中间状态。这些状态在普通操作中不可见，因为从中间状态到正常状态的转换非常快。只有在 `add index` 操作中可以看到 `write reorganization`，表示索引数据正在被添加。
...
* `STATE`：DDL 操作的状态。常见状态包括：
    ...
    * `cancelling`：表示操作正在被取消。此状态仅在使用 [`ADMIN CANCEL DDL JOBS`](/sql-statements/sql-statement-admin-cancel-ddl.md) 命令取消 DDL 任务时出现。
    * `paused`：表示操作已暂停。此状态仅在使用 [`ADMIN PAUSED DDL JOBS`](/sql-statements/sql-statement-admin-pause-ddl.md) 命令暂停 DDL 任务时出现。可以使用 [`ADMIN RESUME DDL JOBS`](/sql-statements/sql-statement-admin-resume-ddl.md) 命令恢复。
