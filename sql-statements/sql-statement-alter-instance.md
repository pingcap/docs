...
新加载的证书、密钥和 CA 会在成功执行该语句后建立的连接上生效。执行该语句之前建立的连接不受影响。

当在重新加载过程中发生错误时，默认会返回该错误信息，之前的密钥和证书仍然继续使用。但是，如果你添加了可选的 `NO ROLLBACK ON ERROR`，在重新加载过程中发生错误时，不会返回错误信息，后续请求将以禁用 TLS 安全连接的方式处理。
