...
`DROP STATS` は、テーブルの TopN やヒストグラムバケットなどの統計情報を削除しますが、`STATS_META` からテーブルのレコードは削除しません。そのため、`DROP STATS` を実行した後でも、`SHOW STATS_META` は引き続きテーブルの `Modify_count` と `Row_count` の情報を返します。
