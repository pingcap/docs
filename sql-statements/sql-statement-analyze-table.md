...
1. TiDB may not include very recently committed changes when running `ANALYZE TABLE`. After a batch-update of rows, you may need to `sleep(1)` before executing `ANALYZE TABLE` in order for the statistics update to reflect these changes. [#16570](https://github.com/pingcap/tidb/issues/16570).
