...
Global Kill機能を有効にすると、 `KILL`と`KILL TIDB`両方のステートメントでインスタンス間のクエリまたは接続を終了できるため、クエリや接続が誤って終了してしまう心配はありません。クライアントを使用して任意のTiDBインスタンスに接続し、 `KILL`または`KILL TIDB`文を実行すると、ステートメントは対象のTiDBインスタンスに転送されます。クライアントとTiDBクラスタの間にプロキシが存在する場合、 `KILL`と`KILL TIDB`文も対象のTiDBインスタンスに転送され、実行されます。
