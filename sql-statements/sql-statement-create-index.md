...
以下の例では、式`LOWER(col1)`に対して式インデックス`idx`を作成するとします。
...
多値インデックスは、配列列に定義されるセカンダリインデックスの一種です。通常のインデックスでは、1つのインデックスレコードが1つのデータレコードに対応します（1:1）。多値インデックスでは、複数のインデックスレコードが1つのデータレコードに対応します（N:1）。多値インデックスは、JSON配列のインデックス付けに使用されます。たとえば、 `zipcode`フィールドに定義された多値インデックスは`zipcode`配列の各要素に対して1つのインデックスレコードを生成します。
...
### 多値インデックスを作成する {#create-multi-valued-indexes}

式インデックスを作成するのと同様に、インデックス定義で[`CAST(... AS ... ARRAY)`](/functions-and-operators/cast-functions-and-operators.md#cast)関数を使用することで、多値インデックスを作成できます。
...
多値インデックスを一意インデックスとして定義することができます。
...
多値インデックスを一意インデックスとして定義した場合、重複データを挿入しようとするとエラーが報告されます。
...
多値インデックスを複合インデックスとして定義することもできます。
...
デフォルトでは、不可視インデックスとはクエリオプティマイザによって無視されるインデックスのことです。
...
TiDB v8.0.0以降では、システム変数[`tidb_opt_use_invisible_indexes`](/system-variables.md#tidb_opt_use_invisible_indexes-new-in-v800)を変更することで、オプティマイザに不可視インデックスを選択させることができます。
