...
summary: `RESTORE`ステートメントは、以前に作成されたバックアップ アーカイブから分散復元を実行します。この機能は実験的であり、本番環境での使用はお勧めできません。復元を実行するには、`RESTORE_ADMIN`または`SUPER`権限が必要です。復元タスクは完了、失敗、またはキャンセルされた後にのみ終了します。増分復元を実行するための特別な構文はありません。
---
