...
summary: ステートメントSET [GLOBAL|SESSION]は、TiDBの組み込み変数の1つを変更します。GLOBAL変数への変更は既存の接続にもローカル接続にも適用されず、新しいセッションのみが値の変更を反映します。SET GLOBALで行われた変更は、クラスター内のすべてのTiDBインスタンスに伝播され、TiDBサーバーが再起動されても保持されます。
---
