...
`SHOW CONFIG`文は、TiDB の様々なコンポーネントの現在の設定を表示するために使用されます。設定変数とシステム変数は異なる次元で作用するため、混同しないように注意してください。システム変数の情報を取得する場合は、 [SHOW [GLOBAL|SESSION] VARIABLES](/sql-statements/sql-statement-show-variables.md)構文を使用してください。
