...
`LOCK TABLES` acquires table locks for the current client session. If you have the `LOCK TABLES` and `SELECT` privilege for each object to be locked, you can acquire table locks for common tables .
