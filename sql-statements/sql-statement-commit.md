...
* By default, `UNIQUE` and `PRIMARY KEY` constraint checks are deferred until statement commit. This improves performance because checks are batched, but may result in compatibility issues for applications not expecting a commit statement to fail. This behavior can be changed by setting `tidb_constraint_check_in_place=TRUE`.
* By default, TiDB 3.0 and above uses [Pessimistic Locking](/pessimistic-transaction.md). When using [Optimistic Locking](/optimistic-transaction.md), it is important to consider that a `COMMIT` statement may fail because rows have been modified by another transaction.
