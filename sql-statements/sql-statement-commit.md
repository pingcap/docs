...
* TiDB does not currently use Metadata Locking (MDL) to prevent DDL statements from modifying tables that are in use by transactions. In the event that table a definition has changed, committing a transaction will result in an `Information schema is changed` error. In this event, the transaction is rolled back automatically.
