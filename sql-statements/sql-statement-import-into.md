...
- If an `IMPORT INTO` task scheduled by the Distributed eXecution Framework (DXF) is already running, it is not supported to be scheduled to a new TiDB node. If the TiDB node that executes the data import task is restarted, it will no longer execute the data import task, but transfers the task to another TiDB node to continue executing. However, if the imported data is a local file, The task will not be transferred to another TiDB node to continue executing.
