...
| `MAX_WRITE_SPEED='<string>'` | All file formats | Controls the write speed of each TiDB node to TiKV. By default, there is no speed limit. For example, if you have 10 TiDB nodes, specifying this option as `1MiB` limits the write speed to 10 MiB/s for each TiKV. |
