...
- If you use the system variable [`tidb_service_scope`](/system-variables.md#tidb_service_scope-new-in-v740) to limit the `IMPORT INTO` task to run only on specific TiDB nodes, it is strongly recommended to select two or more TiDB nodes in an environment with several TiDB nodes and set this system variable to `background`. If `tidb_service_scope` is set on a single TiDB node only, when the node is restarted or fails, the task will be rescheduled to other TiDB nodes that lack the `background` setting, which will affect these TiDB nodes.
