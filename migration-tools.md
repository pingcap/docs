...
|---|---|
| **Upstream (or the imported source file)** | <ul><li>Files exported from Dumpling</li><li>Parquet files exported by Amazon Aurora or Apache Hive</li><li>CSV files</li><li>Data read from local disks or Amazon S3</li></ul> |
| **Downstream (or the output file)** | TiDB |
...
| **Limitation** | <ul><li>If Local-backend is used for data import, during the import process, the TiDB cluster cannot provide services.</li><li> If you do not want the TiDB services to be impacted, perform the data import according to TiDB Lightning TiDB-backend.</li></ul> |
...
| User scenario | <span style="font-weight:normal">This tool is implemented by pulling TiKV change logs. It can restore data to a consistent state with any upstream TSO, and support other systems to subscribe to data changes.</span> |
|---|---|
| **Upstream (or the imported source file)** | TiDB |
| **Downstream (or the output file)** | TiDB, MySQL, Kafka, Confluent |
...
|---|---|
| **Upstream (or the imported source file)** | TiDB, MySQL |
| **Downstream (or the output file)** | TiDB, MySQL |
