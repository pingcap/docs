...
|[Backup & Restore (BR)](/br/backup-and-restore-overview.md) | Backup and restore for TiDB clusters with a huge data size | TiDB| SST, backup.meta files, backup.lock files|<ul><li>Suitable for restoring data to another TiDB cluster</li><li>Support backing up data to an external storage for disaster recovery</li></ul> | <ul><li>When BR restores data to the upstream cluster of TiCDC or Drainer, the restored data cannot be replicated to the downstream by TiCDC or Drainer.</li><li>BR supports operations only between clusters that have the same `new_collations_enabled_on_first_bootstrap` value.</li></ul> |
