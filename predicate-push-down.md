...
PPD は、選択演算子をデータソースに可能な限り近づけて、データのフィルタリングをできるだけ早く完了します。これにより、データ転送や計算のコストが大幅に削減されます。
...
`explain`結果から、述語が結合演算子の下にプッシュされていないことがわかります。これは、外部結合によって`on`条件が満たされない場合に内部テーブルに`NULL`値が代入され、述語`s.a is null`結合後の結果をフィルタリングするために使用されているためです。述語7が結合演算子の下の内部テーブルにプッシュダウンされると、実行計画は元のプランと同等ではなくなります。
