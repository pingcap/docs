...
- 範囲：
    ...
    - メモリサイズの値も設定できます。値の範囲はバイト単位で`0`から`[536870912, 9223372036854775807]`です。単位が「KiB|MiB|GiB|TiB」または「KB|MB|GB|TB」のメモリ形式もサポートされており、たとえば `90GiB` のように指定できます（数値と単位の間にスペースは入れません）。 `0`メモリ制限なしを意味します。
