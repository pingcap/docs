...
For example, the type `DOUBLE(10)` is now considered invalid because the precision of floating point types is not guaranteed. This can be worked around by setting `tidb_enable_strict_double_type_check` to `OFF`:
