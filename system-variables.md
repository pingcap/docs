...
- You can also use the hint [`tidb_kv_read_timeout`](/optimizer-hints.md#tidb_kv_read_timeoutn) to set the timeout for TiDB to send a TiKV RPC read request in a query statement. If both the hint and the system variable are set, the hint takes higher priority.
