...
- This variable controls the size of the schema cache in TiDB, measured in bytes. The default value is 0, which means the cache limit feature is disabled. To enable this feature, you need to set a value within the range [536870912, 9223372036854775807]. TiDB will use this value as the maximum available memory limit and apply the Least Recently Used (LRU) algorithm to cache the required tables, effectively reducing the memory used by schema information.
