...
### tidb_enable_batch_dmlv6.1.0<span class="version-mark">の新機能</span> {#tidb-enable-batch-dml-span-class-version-mark-new-in-v6-1-0-span}

-   スコープ：グローバル
-   クラスタに永続化：はい
-   タイプ：ブール
-   デフォルト値： `OFF`
-   TiDBが非トランザクションの「バッチ処理された」ステートメントを許可するかどうかを決定します。バッチDMLはACID保証を提供しないため、 `OFF`の値のみが安全であると見なすことができます。
-   この設定は、以前は`tidb.toml`オプション（ `enable-batch-dml` ）でしたが、TiDBv6.1.0以降のシステム変数に変更されました。
