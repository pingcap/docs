...
- Default value: `0.8`
- Range: `[0, 1]`
- This variable is used to set the default selectivity rate of `like`, `rlike`, and `regexp` functions in the filter condition when estimating the number of rows, and controls whether to enable TopN to help estimate these functions.
...
- If the variable is set to `0`, TiDB tries to evaluate using TopN in statistics to improve the accuracy and consider the NULL number in statistics when estimating the preceding three functions. The prerequisite is that statistics are collected when setting [`tidb_analyze_version`](#tidb_analyze_version-new-in-v510) to `2`.
