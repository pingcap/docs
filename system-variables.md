...
-   デフォルト値: `151`
-   範囲: `[0, 100000]`
-   1 つの TiDB インスタンスに対して許可される接続の最大数 (有効ではありません)。これは noop 変数であり、現在の TiDB バージョンでは有効になりません。現在のバージョンで単一の TiDB インスタンスに許可される接続の最大数を設定するには、TiDB 構成項目[`max-server-connections`](/tidb-configuration-file.md#max-server-connections)を使用します。
-   値`0`は制限なしを意味します。
...
-   デフォルト値: `151`
-   範囲: `[0, 100000]`
-   1 つの TiDB インスタンスに対して許可される接続の最大数 (有効ではありません)。これは noop 変数であり、現在の TiDB バージョンでは有効になりません。
-   値`0`は制限なしを意味します。
