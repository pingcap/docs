...
>
> - If you start a log backup task first, and then add an index. The adding index process is not accelerated even if index acceleration is enabled. But the index is added in a slow way. Since the log backup task will always run, it is equivalent to the index acceleration feature being disabled.
