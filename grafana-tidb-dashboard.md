...
- TiDB panel: クラスターの異常のトラブルシューティングのために可能な限り包括的な情報を提供します。
- TiDB Summary Panel：TiDBパネル情報のうち、ユーザーが最も関心のある部分を抽出し、若干の修正を加えています。日々のデータベース運用においてユーザーが特に関心を持つデータ（QPS、TPS、応答遅延など）を提供し、表示またはレポートする監視情報として機能します。
