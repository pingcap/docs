...
- QPS: すべての TiDB インスタンスで秒あたりに実行される SQL ステートメントの数。`SELECT` 、 `INSERT` 、 `UPDATE`およびその他のタイプのステートメントに従ってカウントされます
...
- 失敗したクエリ OPM の詳細: 各 TiDB インスタンスで 1分あたりに SQL ステートメントを実行したときに発生したエラーに応じたエラーの種類 (構文エラーや主キーの競合など) の統計
...
- DDL 実行時間 95: DDL ステートメントの処理時間の 95% 分位
