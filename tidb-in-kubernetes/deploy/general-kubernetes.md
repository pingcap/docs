...
If you set up a storage class that does not exist in the TiDB cluster that you are creating, then the cluster creation is in the Pending state. In this situation, you must [destroy the TiDB cluster in Kubernetes](/tidb-in-kubernetes/maintain/destroy-tidb-cluster.md).
