...
在数据类型规范中的 `DEFAULT` 值子句用于指定列的默认值。
...
- 如果该列不能取 `NULL` 值，TiDB 会为该列定义为没有显式的 `DEFAULT` 子句。

对于没有显式 `DEFAULT` 子句的 `NOT NULL` 列，在向其插入数据时，如果 `INSERT` 或 `REPLACE` 语句未为该列指定值，TiDB 会根据当时生效的 SQL 模式进行处理：
...
* [`CURRENT_TIMESTAMP()`](/functions-and-operators/date-and-time-functions.md)、[`CURRENT_DATE()`](/functions-and-operators/date-and-time-functions.md)：两者均使用默认的小数秒精度（fsp）
