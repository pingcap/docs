...
>
> Normally, `STATS_META` is more credible than `TABLE_KEYS`. However, after importing data through the methods like [TiDB Lightning local-backend](/tidb-lightning/tidb-lightning-backends.md#tidb-lightning-local-backend), the result of `STATS_META` is `0`. To handle this situation, you can use `TABLE_KEYS` to calculate the sampling rate when the result of `STATS_META` is much smaller than the result of `TABLE_KEYS`.
...
+ To collect statistics of some columns for the partitions in all `PartitionNameList`s in `TableName`:
