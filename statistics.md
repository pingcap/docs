...
> -   動的プルーニング モードでは、パーティションとテーブルの Analyze 構成は同じである必要があります。したがって、 `ANALYZE TABLE TableName PARTITION PartitionNameList`ステートメントの後に`COLUMNS`構成を指定したり、 `WITH`ステートメントの後に`OPTIONS`構成を指定したりすると、TiDB はそれらを無視して警告を返します。
...
TiDB v5.0 より前では、クエリを実行すると、TiDB はフィードバックを`feedback-probability`で収集し、フィードバックに基づいてヒストグラムと Count-Min Sketch を更新します。 **v5.0 以降、この機能はデフォルトで無効になっており、この機能を有効にすることはお勧めしません。**
...
この機能を使用して、ステートメントを手動で実行するときに`ANALYZE`ステートメントで指定された持続性構成を記録できます。記録されると、次に TiDB が自動的に統計を更新するか、これらの構成を指定せずに手動で統計を収集するときに、TiDB は記録された構成に従って統計を収集します。
...
現在、 `SHOW STATS_META`ステートメントは次の 6 列を返します。
...
上記のステートメントは、 `TableName`のすべての統計を削除します。パーティション化されたテーブルが指定されている場合、このステートメントは、このテーブル内のすべてのパーティションの統計と、動的プルーニング モードで生成された GlobalStats を削除します。
