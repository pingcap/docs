...
| `Last_analyzed_at` | 列統計が最後に収集されたとき        |
...
TiDB v6.0以降、TiDBは、バックグラウンドで実行されている`ANALYZE`タスクを終了するための`KILL`ステートメントの使用をサポートしています。バックグラウンドで実行されている`ANALYZE`のタスクが多くのリソースを消費し、アプリケーションに影響を与えることがわかった場合は、次の手順を実行して`ANALYZE`のタスクを終了できます。
