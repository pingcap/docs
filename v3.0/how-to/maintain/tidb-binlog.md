...
- Modify the state of Pump or Drainer node in abnormal situations
    ...
    > **Note:**
    >
    > When a Pump or Drainer node runs normally, it regularly updates its state to PD. But the above command is used to directly modify the Pump or Drainer state saved in PD, so do no use the command when the Pump or Drainer node runs normally. Refer to [TiDB Binlog FAQ](/v3.0/faq/tidb-binlog.md) to see in what situation you need to use it.
