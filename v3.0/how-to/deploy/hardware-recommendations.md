...
> **Note**:
>
> - In the test environment, the TiDB server and PD server can be deployed on the same server.
...
> **Note**:
>
> - In the production environment, the TiDB server and PD server can be deployed on the same server. If you have a higher requirement for performance and reliability, try to deploy them separately.
