...
`TidbCluster` is a custom resource defined using CRD (`CustomResourceDefinition`) to describe the desired state of the TiDB cluster. The orchestration and scheduling for TiDB clusters is the responsibility of the following components:
...
In addition, TiDB Operator provides `tkctl`, a command-line interface for TiDB clusters in Kubernetes. It is used for cluster operations and troubleshooting cluster issues.
