...
`TidbCluster` is a custom resource defined by CRD (`CustomResourceDefinition`), which is used to describe the TiDB cluster status expected by the user. The following components are responsible for the orchestration and scheduling logic for a TiDB cluster:
...
Besides, TiDB Operator also provides `tkctl`, the command-line interface for TiDB clusters in Kubernetes. It is used for cluster operations and troubleshooting cluster issues.
