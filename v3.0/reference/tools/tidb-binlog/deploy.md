...
1. Modify the `tidb-ansible/inventory.ini` file.
    ...
    2. Add the deployment machine IPs for `pump_servers`.
        ...
        Pump retains the data of the latest 7 days by default. You can modify the value of the `gc` variable in the `tidb-ansible/conf/pump.yml` file (or `pump-cluster.yml` file in TiDB 3.0.2 or earlier) and remove the related comments:
