...
TiDB servers operate in different SQL modes and apply these modes differently for different clients. SQL mode defines the SQL syntaxes that TiDB supports and the type of data validation check to perform.
...
Ensure that you have `SUPER` privilege when setting SQL mode at `GLOBAL` level. Your setting only affects the connections established afterwards, and the old connections are not affected. Changes to SQL mode at `SESSION` level only affect the current client.
...
* `ANSI`: In this mode, data is checked. If data does not comply with the defined type or length, the data type is adjusted or trimmed and a `warning` is returned.
* `STRICT_TRANS_TABLES`: In this strict mode, data is strictly checked. When any incorrect data is inserted into a table, an error is returned.
...
| `ONLY_FULL_GROUP_BY` | If a non-aggregated column that is referred to in `SELECT`, `HAVING`, or `ORDER BY` is absent in `GROUP BY`, the SQL statement is illegal, because it is abnormal for a column to be absent in `GROUP BY` but displayed by query. (full support) |
...
| `STRICT_ALL_TABLES` | For transactional tables, rolls back the entire transaction statement after an illegal value is inserted. (support) |
| `NO_ZERO_IN_DATE` | In strict mode, dates with a month or day part of `0` are not accepted. If you use the `IGNORE` option, TiDB inserts '0000-00-00' for a similar date. In non-strict mode, this date is accepted but a warning is returned. (full support)
...
| `HIGH_NOT_PRECEDENCE` | The priority of `NOT` operator is expression. For example: `NOT a BETWEEN b AND c` is parsed as `NOT (a BETWEEN b AND c)`. In some older versions of MySQL, this expression is interpreted as `(NOT a) BETWEEN b AND c`. (full support) |
