...
summary: ランタイムフィルターは、TiDB v7.3で導入された新機能で、MPPシナリオでのハッシュ結合のパフォーマンスを向上させることを目的としています。ハッシュ結合のデータを事前にフィルタリングすることで、実行時のデータスキャン量とハッシュ結合の計算量を削減し、クエリのパフォーマンスを向上させることができます。ランタイムフィルターは、クエリ計画フェーズ中に生成される動的な述語であり、ハッシュ結合ビルドプロセス中に生成された結果からパラメーター値を取得します。
---
