...
ランタイムフィルタのモードは、**フィルタ送信オペレータ**と**フィルタ受信オペレータ**の関係です。モードは`OFF`、`LOCAL`、`GLOBAL`の3つがあります。バージョン7.3.0では、モード`OFF`と`LOCAL`のみがサポートされています。ランタイムフィルタのモードは、システム変数[`tidb_runtime_filter_mode`](/system-variables.md#tidb_runtime_filter_mode-new-in-v720)によって制御されます。
