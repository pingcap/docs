...
TiProxyは[TiUP](https://github.com/pingcap/tiup)、[TiDB Operator](https://github.com/pingcap/tidb-operator)、[TiDB Dashboard](/dashboard/dashboard-intro.md)、[Grafana](/tiproxy/tiproxy-grafana.md)に統合されており、組み込みの仮想IP管理をサポートし、導入、運用、管理コストを削減できます。
...
- `STATUS`と`SHOW STATUS`文は異なるTLS情報を返す可能性があります。`STATUS`文はクライアントとTiProxy間のTLS情報を返し、 `SHOW STATUS`文はTiProxyとTiDBサーバー間のTLS情報を返します。
...
- セッションで[プリペアドステートメント](/develop/dev-guide-prepared-statement.md)が作成され、プリペアドステートメントが無効になります。たとえば、プリペアドステートメントに関連付けられたテーブルが、プリペアドステートメントの作成後に削除された場合などです。
