...
本文介绍与 TiProxy 部署和使用相关的配置参数。以下是一个示例配置：
...
> **提示：**
>
> 如果你需要调整某个配置项的值，请参考 [Modify the configuration](/maintain-tidb-using-tiup.md#modify-the-configuration)。通常修改后需要重启。由于 TiProxy 支持热重载，你可以通过执行 `tiup cluster reload --skip-restart` 来跳过重启。
...
+ 支持热重载：是
+ 单位：秒
+ 当 TiProxy 关闭时，HTTP 状态显示为不健康，但 SQL 端口仍会接受新连接，持续 `graceful-wait-before-shutdown` 秒。之后，拒绝新连接并逐步关闭客户端。建议在客户端与 TiProxy 之间没有其他代理（如 NLB）时，将其设置为 `0`。
...
+ 支持热重载：是，但仅对新连接生效
+ 范围：[1024, 16777216]
+ 该配置项决定连接缓冲区的大小。每个连接使用一个读缓冲区和一个写缓冲区。这是在内存和性能之间的权衡。缓冲区越大，性能可能越好，但占用的内存也越多。当设置为 `0` 时，TiProxy 使用默认缓冲区大小。

#### `pd-addrs`

+ 默认值：`127.0.0.1:2379`
+ 支持热重载：否
+ TiProxy 连接的 PD 地址。TiProxy 通过从 PD 获取 TiDB 列表来发现 TiDB 实例。部署 TiProxy 时，通常由 TiUP 或 TiDB Operator 自动设置。
...
+ 支持热重载：否
+ 可能的值：`""`、`"v2"`
+ 在端口启用 [PROXY protocol](https://www.haproxy.org/download/1.8/doc/proxy-protocol.txt)。
...
+ 默认值：`""`
+ 支持热重载：是
+ 日志文件路径。非空值将启用文件日志。当 TiProxy 通过 TiUP 部署时，文件名会自动设置。
...
+ `skip-ca`：跳过验证客户端或服务端的证书（取决于对象类型）
+ `min-tls-version`：设置最低 TLS 版本。可能的值为 `1.0`、`1.1`、`1.2` 和 `1.3`。默认值为 `1.2`，表示允许 v1.2 及以上版本的 TLS。
+ `rsa-key-size`：在启用 `auto-certs` 时设置 RSA 密钥长度。
+ `autocert-expire-duration`：设置自动生成证书的默认过期时间。

对象根据名称分类为客户端对象或服务端对象。

对于客户端 TLS 对象：

- 必须设置 `ca` 或 `skip-ca` 以跳过验证服务器证书。
- 可选地，设置 `cert` 或 `key` 以进行服务端客户端验证。
- `auto-certs` 无用。

对于服务端 TLS 对象：

+ 可以设置 `cert` 或 `key` 或 `auto-certs` 来支持 TLS 连接，否则 TiProxy 不支持 TLS 连接。
+ 如果 `ca` 不为空，则启用服务端客户端验证。客户端必须提供证书。或者，如果 `skip-ca` 为 true 且 `ca` 不为空，服务器只会验证客户端证书（如果客户端提供的话）。
...
+ 默认值：`false`
+ 支持热重载：是，但仅对新连接
+ 要求 TiProxy 和 TiDB 之间使用 TLS。如果 TiDB 不支持 TLS，客户端在连接 TiProxy 时会报错。
...
一个服务端 TLS 对象。用于在 SQL 端口（6000）提供 TLS。

#### `server-http-tls`

一个服务端 TLS 对象。用于在 HTTP 状态端口（3080）提供 TLS。
