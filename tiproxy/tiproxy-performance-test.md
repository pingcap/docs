...
- The QPS upper limit of TiProxy is affected by the type of workload. When the client concurrency is the same and the TiProxy CPU usage is below 80%, the QPS is less than 5% lower than that of HAProxy. At this time, the client concurrency can be increased to improve QPS. Under the same QPS, the TiProxy CPU usage is 25% higher than that of HAProxy, so more CPU resources need to be reserved.
