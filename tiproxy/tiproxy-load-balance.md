...
2. Configure at least 2 TiProxy instances. The TiProxy instance used for transaction business is configured with [`labels`](/tiproxy/tiproxy-configuration.md#labels) as `{"app"="Order"}`, and the instance used for BI business is configured with [`labels`](/tiproxy/tiproxy-configuration.md#labels) as `{"app"="BI"}`.
