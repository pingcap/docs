...
TiProxy APIにアクセスするためのアドレスは`http://${host}:${port}${path}`です。`${host}:${port}` TiProxy設定項目[`api.addr`](/tiproxy/tiproxy-configuration.md#addr-1)で指定され、 `${path}`アクセスしたい特定のAPIエンドポイントです。例：
