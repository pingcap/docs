...
クラスター トポロジに基づいて、TiKV、 TiFlash、および TiDB に`labels`を設定できます。
...
> - SQL の配置ルールを使用する場合、TiKV の場合は`labels`のみ設定する必要があります。現在、SQL の配置ルールは PD の`location-labels`設定と互換性がなく、この設定は無視されます。`location-labels` SQL の配置ルールを同時に使用することは推奨されません。予期しない結果が発生する可能性があります。
...
`location-labels`が設定されている場合は、PD 設定ファイルで`isolation-level`を設定することで、TiKV クラスターのトポロジ分離要件をさらに強化できます。
