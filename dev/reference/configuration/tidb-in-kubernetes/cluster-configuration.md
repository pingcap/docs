...
# TiDB Cluster Configuration on Kubernetes
...
The `charts/tidb-cluster/values.yaml` file of `tidb-cluster` provides a basic configuration by default with which you could quickly start a TiDB cluster. However, if you need special configuration or are deploying in a production environment, you need to manually configure the corresponding items according to the table below.
...
| `pd.logLevel` | The level of PD logs | `info` |
...
| `pd.maxStoreDownTime` | This parameter indicates how soon a store node is marked as `down` after it is disconnected. When the state changes to `down`, the store node starts migrating data to other store nodes. | `30m` |
| `pd.maxReplicas` | The number of data replica in the TiDB cluster | `3` |
...
| `tikv.logLevel` | The level of TiKV logs | `info` |
| `tikv.storageClassName` | The `storageClass` used by TiKV. `storageClassName` refers to a type of storage provided by the Kubernetes cluster, which might map to a level of service quality, a backup policy, or to any policy determined by the cluster administrator. Detailed reference: [storage-classes](https://kubernetes.io/docs/concepts/storage/storage-classes) | `local-storage` |
| `tikv.syncLog` | `SyncLog` means whether to enable the raft log synchronization. Enabling this feature ensures that data will not be lost when power is off. | `true` |
...
| `tikv.defaultcfBlockCacheSize` | This parameter specifies the size of block cache which is used to cache uncompressed blocks. Larger block cache settings speed up reads. It is recommended to set the parameter to 30%-50% of the value of `tikv.resources.limits.memory`.  | `1GB` |
| `tikv.writecfBlockCacheSize` | The parameter specifies the size of writecf block cache. It is recommended to set the parameter to 10%-30% of the value of `tikv.resources.limits.memory`.  | `256MB` |
| `tikv.readpoolStorageConcurrency` | The size of thread pool for high priority, normal priority or low priority operations in the TiKV storage | `4` |
| `tikv.readpoolCoprocessorConcurrency` | If `tikv.resources.limits.cpu` is greater than `8`, set the value of `tikv.readpoolCoprocessorConcurrency` to `tikv.resources.limits.cpu` * 0.8 | `8` |
| `tikv.storageSchedulerWorkerPoolSize` | The worker pool size of TiKV scheduler. This size must be increased in the case of rewriting but be smaller than the total CPU core. | `4` |
...
| `tidb.logLevel` | The level of TiDB logs | `info` |
...
| `tidb.separateSlowLog` | Whether to run in sidebar mode the `SlowLog` of TiDB exported via independent container | `false` |
...
| `tidb.preparedPlanCacheEnabled` | Whether to enable TiDB's prepared plan cache | `false` |
| `tidb.preparedPlanCacheCapacity` | The cache capacity of TiDB's prepared plan | `100` |
| `tidb.txnLocalLatchesEnabled` | Whether to enable the local latch of the transaction. Enable the latch when there are many conflicts among transactions. | `false` |
| `tidb.txnLocalLatchesCapacity` |  The number of slots corresponding to Hash. It will be automatically adjusted up to an exponential multiple of `2`. Each slot occupies 32 Bytes of memory. When the range of data writing is relatively wide (such as data import and export), setting this parameter too small a value will result in slower speed and lower performance. | `10240000` |
| `tidb.tokenLimit` | The restrictions on TiDB to execute concurrent sessions | `1000` |
| `tidb.memQuotaQuery` | The memory quota for TiDB queries, which is 32GB by default. | `34359738368` |
| `tidb.txnEntryCountLimit` | The limit on the number of entries in a transaction. If TiKV is used as the storage, the entry represents a key-value pair. **Warning:** Do not set this value too large. Otherwise, it might have a big impact on the TiKV cluster. Set this parameter carefully. | `300000` |
| `tidb.txnTotalSizeLimit` | The limit on byte size for each entry in a transaction. If TiKV is used as the storage, the entry represents a key-value pair. **Warning:** Do not set this value too large. Otherwise, it might have a big impact on the TiKV cluster. Set this parameter carefully. | `104857600` |
| `tidb.enableBatchDml` | This parameter enables batch submission for DML. | `false` |
| `tidb.checkMb4ValueInUtf8` | This parameter controls whether to check the `mb4` characters when the character set is `utf8`. | `true` |
| `tidb.treatOldVersionUtf8AsUtf8mb4` | This parameter is used for upgrading compatibility. After it is set to `true`, `utf8` character set in the old table is treated as `utf8mb4`. | `true` |
| `tidb.lease` | The lease time of TiDB Schema lease. It is highly risky to change this parameter. Therefore, it is not recommended to do so unless you know exactly what might be happening. | `45s` |
| `tidb.maxProcs` | The maximum available CPU cores. `0` represents the number of CPU on the machine or Pod. | `0` |
...
The disaster recovery of TiDB service is essentially based on Kubernetes' scheduling capabilities. To optimize scheduling, TiDB Operator provides a custom scheduler that can guarantee the disaster recovery of the TiDB service at the host level through the specified scheduling algorithm. Currently, TiDB Cluster uses this scheduler as the default scheduler which can be configured through the item `schedulerName` in the above table.
...
Currently, TiDB Operator only identifies a specific number of labels. Therefore, you need to use the following tags to set the topology information of a node:
...
The tags in the above command are not to be set all at once. Instead, they are to be selected according to actual situation.
