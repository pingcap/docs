...
* `TIDB_TABLE_ID`: to specify the internal ID of a table. This ID is unique in a TiDB cluster.
* `TIDDB_ROW_ID_SHARDING_INFO`: to specify the sharding type of a table. The possible values are as follows:
    - `"NOT_SHARDED"`: the table is not sharded.
    - `"NOT_SHARDED(PK_IS_HANDLE)"`: the table that defines Primary Key as integer is not sharded.
    - `"PK_AUTO_RANDOM_BITS={bit_number}"`: the table that defines Primary Key as integer is sharded because the table defines `AUTO_RANDOM`.
