...
By default, MySQL time zone is set to the server's time zone and depends on the system time zone rules including daylight saving time rules. Before loading [the time zone information tables](https://dev.mysql.com/doc/refman/8.0/en/time-zone-support.html#time-zone-installation), you cannot use the time zone name to specify the time zone. 

All the time zone name is available in TiDB without loading the time zone information tables. The calculation is based on the system time zone rules such as `tzdata` package. You cannot modify the calculation rules by loading the time zone information tables. 

> **Warning**:
>
> TiKV calculates the time-related expressions received with its own time zone rules instead of system time zone rules. When these two kinds of time zone rules do not match with each other, it may occur that the inserted time data cannot be read again. But this case is rare. In addition, if the tzdata 2019a time zone rules is installed in the system, when the time zone is set to Aisa/Shanghai, or to the server's time zone which is Asia/Shanghai, TiDB 3.0 RC.1 can be normally inserted into the time data `1988-04-17 02:00:00`. But this record cannot be read again by certain type of SQL statement because this time does not exist in the Asia/Shanghai time zone according to the tzdata 2018i rules on which TiDB 3.0 RC.1 is based.(For example daylight saving time is shifted back by an hour.)
> 
> The system time zone rules of two versions of TiKV:
> 
...
TiDB does not accept that month and day value can be "zero" up to now. You can insert this date of value 0 but it might not be read by some certain type of SQL statements.