...
| `code=32001` |   Abnormal dump processing unit                                            |  If the error message contains `mydumper: argument list too long.`, manually add the `--regex` regular expression in the Mydumper parameter `extra-args` in the `task.yaml` file according to the black-white list to configure the table to be exported in TiDB. For example, to export all the tables named `hello` in TiDB, add `--regex '.*\\.hello$'`; to export all tables, add `--regex '.*'`. |
