...
| `code=32001` |   Abnormal dump processing unit                                            |  If the error message contains `mydumper: argument list too long.`, configure the table to be exported in TiDB by manually adding the `--regex` regular expression in the Mydumper parameter `extra-args` in the `task.yaml` file according to the black-white list. For example, to export all tables named `hello` in TiDB, add `--regex '.*\\.hello$'`; to export all tables in TiDB, add `--regex '.*'`. |
