...
| `worker-addr` | Specifies the address of DM-worker which provides services. You can omit the host name and specify the port number only, such as ":8262".| |
| `source-id` | Uniquely identifies a MySQL or MariaDB instance, or a replication group | |
| `server-id` | Identifies the server ID of MySQL slave, used when pulling binlogs from MySQL. In a replication group, each instance (master and slave included) must have a unique server ID. In v1.0.2 or later versions, the `server_id` is automatically generated by DM. | |
| `flavor` | Indicates the release type of MySQL: `"Percona"`, `"mysql"` or `"mariadb"`. In v1.0.2 or later versions, DM automatically detects and fills in the release type. | `"mysql"`|
...
| `relay-binlog-name` | Specifies the file name from which DM-worker starts to pull the binlog. For example, `"mysql-bin.000002"`. Only used when `enable_gtid` is `false`. DM-worker defaults to pull the binlogs starting from the earliest one. But in v1.0.2 or later versions, DM-worker defaults to pull the binlogs starting from the latest one. | |
| `relay-binlog-gtid` | Specifies the GTID from which DM-worker starts to pull the binlog. For example, `"e9a1fc22-ec08-11e9-b2ac-0242ac110003:1-7849"`. Only used when `enable_gtid` is `true`. DM-worker defaults to pull the binlogs starting from the earliest GTID. But in v1.0.2 or later versions, DM-worker defaults to pull the binlogs starting from the latest GTID. | |
