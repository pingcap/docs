...
# sql-mode = ""
# `max-allowed-packet` sets the maximum data packet allowed for database connection, which corresponds to the `max-allowed-packet` in system parameters. If it is set to 0, the global `max-allowed-packet` for downstream is used.
