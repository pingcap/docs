...
This step shows how to edit the variable of the deployment directory, how to configure the relay log replication position and the relay log GTID replication mode, and explains the global variables in the `inventory.ini` file.
...
### Configure the relay log replication position
...
> **Note:**
>
> If `relay_binlog_name` is not set, DM-worker pulls the binlog starting from the earliest existing binlog file of the upstream MySQL or MariaDB. In this case, it can take a long period of time to pull the latest binlog for the data replication task.
...
- `enable_gtid`: to enable the relay log GTID replication mode to deal with scenarios like master-slave switch
