...
When "cluster id mismatch" message appears in the TiKV Pod log, it means that the TiKV Pod may use old data from other or previous TiKV Pod. If the data on the local disk remain uncleared when configuring local storage in the cluster, or the data is not recycled by local volume provisioner program due to a forced deletion of PV, an error may occur.

When you make sure that the TiKV should join the cluster as a new node and that the data on the PV should be deleted, you can delete the TiKV Pod and the corresponding PVC. The TiKV Pod will automatically rebuild and bind the new PV for use. When configuring local storage, local storage on the machine should be deleted to avoid Kubernetes using old data. As for operation and maintenance, the PV should be managed by the local volume provisioner program instead of being forcibly deleted. Users can manage the lifecycle of PV by creating, deleting PVCs and setting the reclaimPolicy for the PV.
