...
- tidb-scheduler has a high availability scheduling policy for TiKV and PD. Suppose that the number of TiKV or PD replicas is N, then for each node, the number of PD Pods that can be scheduled in the same TiDB cluster is `M=(N-1)/2` (if N<3 AND M=1) at most, and the number of TiKV Pods that can be scheduled in the same TiDB cluster is `M=ceil(N/3)` (if N<3 and M=1; `ceil` means rounding up) at most. If the Pod's state becomes `Pending` because the high availability scheduling policy is not satisfied, you need to add more nodes in the cluster.
