...
TiDB supports changing the priority on a [per-session](/dev/reference/configuration/tidb-server/tidb-specific-variables.md#tidb_force_priority), [global](/dev/reference/configuration/tidb-server/configuration.md#force-priority) or individual statement basis. Priority has the following meaning:
