...
+ SQL Optimizer
    ...
    - Fix the issue that query execution plan is incorrect when the partition table statistics is wrongly used [#13628](https://github.com/pingcap/tidb/pull/13628)
...
+ DDL
    ...
    - Add the new config `alter-primary-key` to support add/drop primary key on TiDB, this config is disable by default [#13522](https://github.com/pingcap/tidb/pull/13522)
...
- Avoid reducing TTL in pessimistic lock prewrite request [#6056](https://github.com/tikv/tikv/pull/6056)
- Fix the issue that may occur missing blob file in Titan [#5968](https://github.com/tikv/tikv/pull/5968)
- Fix the issue that `RocksDBOptions` may not take effect in Titan [#6009](https://github.com/tikv/tikv/pull/6009)
...
- Fix the issue that RPC receive message may exceed max size in `RegionSyncer` replication [#1952](https://github.com/pingcap/pd/pull/1952)
