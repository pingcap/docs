...
- Compatibility of the upstream MySQL table schema
    ...
    > **Warning:**
    >
    > - When the upstream uses incompatible character sets, you can still continue the replication by creating tables with the utf8mb4 character set in the downstream. However, this practice is not recommended. You are advised to replace the character set used by the upstream with another character set that is supported in downstream.
