...
> **注記：**
>
> TiUPを使用してデプロイされた DM クラスターの場合、クラスターを維持するために[`tiup dmctl`](/dm/maintain-dm-using-tiup.md#dmctl)直接使用することをお勧めします。
...
> -   v2.0.4 以降、DM は環境変数`DM_MASTER_ADDR`から`-master-addr`パラメータを読み取ることをサポートします。
