...
- **セーフモード**: セーフモード実行中、DM は各バッチに対して`foreign_key_checks=0`を設定し、主キーまたは一意キーの値を変更しない`UPDATE`ステートメントの冗長な`DELETE`ステップをスキップします。これにより、 `REPLACE INTO` (内部で`DELETE` + `INSERT`を実行) が子行に意図しない`ON DELETE CASCADE`効果を引き起こすのを防ぎます。詳細については、 [DMセーフモード](/dm/dm-safe-mode.md#foreign-key-handling-new-in-v856)を参照してください。
