...
2.  ソース MySQL インスタンスを停止して削除します。
    ...
    > **注記：**
    >
    > すべての MySQL データ ファイルを削除する場合は、MySQL データディレクトリ (通常は`/opt/homebrew/var/mysql`にあります) を削除します。
    ...
    > **注記：**
    >
    > すべての MySQL データ ファイルを削除する場合は、MySQL データディレクトリ (通常は`/var/lib/mysql`にあります) を削除します。
