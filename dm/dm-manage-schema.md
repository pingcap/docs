...
> **ノート：**
>
> -   テーブルスキーマはデータ移行中に変更される可能性があるため、予測可能なテーブルスキーマを取得するために、現在`operate-schema`コマンドは、データ移行タスクが`Paused`状態の場合にのみ使用できます。
...
-   `--flush` ：
    ...
    -   デフォルト値は`true`です。
-   `--sync` ：
    -   オプション。オプティミスティックシャーディングDDLモードでエラーが発生した場合にのみ使用されます。
