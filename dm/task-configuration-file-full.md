...
target-database:                # Configuration of the downstream database instance.
  ...
  session:                                       # The session variables of TiDB, supported since v1.0.6. For details, go to `https://pingcap.com/docs/stable/system-variables`.
    ...
    sql_require_primary_key: OFF                # During the creation of a DM task, several metadata tables are created in TiDB. Some metadata tables have no primary key. If this parameter is enabled, these metadata tables without primary keys cannot be created. Therefore, you need to set this parameter to `OFF` from the session level.
