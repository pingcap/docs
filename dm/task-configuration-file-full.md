...
各構成項目の機能と構成については、 [データ移行機能](/dm/dm-overview.md#basic-features)を参照してください。
...
| `mydumpers`        | ダンプ処理ユニットのConfiguration / コンフィグレーション引数。デフォルトの構成で十分な場合は、この項目を構成する必要はありません。または、 `mydumper-thread`のみを使用して`thread`を構成できます。                                                                                                                                                                         |
| `loaders`          | 負荷処理ユニットのConfiguration / コンフィグレーション引数。デフォルトの構成で十分な場合は、この項目を構成する必要はありません。または、 `loader-thread`のみを使用して`pool-size`を構成できます。                                                                                                                                                                         |
| `syncers`          | 同期処理ユニットのConfiguration / コンフィグレーション引数。デフォルトの構成で十分な場合は、この項目を構成する必要はありません。または、 `syncer-thread`のみを使用して`worker-count`を構成できます。                                                                                                                                                                      |
