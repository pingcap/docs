...
summary: TiDB データ移行ツールを使用してデータを移行するときに、失敗した DDL文を処理する方法を学習します。
---

# TiDB データ移行で失敗した DDL文を処理する {#handle-failed-ddl-statements-in-tidb-data-migration}

このドキュメントでは、TiDB Data Migration (DM) ツールを使用してデータを移行するときに、失敗した DDL文を処理する方法について説明します。
...
- 実際の本番環境では、失敗した DDL文が下流の TiDB でスキップされることは許容されません。
- 失敗した DDL文を他の DDL文に置き換えることはできません。
- その他の DDL文をダウンストリーム TiDB に挿入してはなりません。

たとえば、 `DROP PRIMARY KEY` 。このシナリオでは、新しいテーブルスキーマを使用してダウンストリームに新しいテーブルを作成し (DDL文を実行した後)、すべてのデータをこの新しいテーブルに再インポートすることしかできません。

## サポートされているシナリオ {#supported-scenarios}

移行中に、TiDB でサポートされていない DDL文がアップストリームで実行され、ダウンストリームに移行され、その結果、移行タスクが中断されます。

- この DDL文がダウンストリーム TiDB でスキップされることが許容される場合は、 `binlog skip <task-name>`を使用してこの DDL文の移行をスキップし、移行を再開できます。
- この DDL文を他の DDL文に置き換えても問題ない場合は、 `binlog replace <task-name>`を使用してこの DDL文を置き換え、移行を再開できます。
- 他の DDL文がダウンストリーム TiDB に挿入されることが許容される場合は、 `binlog inject <task-name>`を使用して他の DDL文を挿入し、移行を再開できます。
...
移行が中断されたときに DDL文をスキップする必要がある場合は、 `binlog skip`コマンドを実行します。
...
ここで、アップストリームで次の DDL文が実行され、テーブルスキーマが変更されます (つまり、c2 の DECIMAL(11, 3) が DECIMAL(10, 3) に変更されます)。
...
1. `binlog skip <task-name>`を実行して、現在失敗している DDL文をスキップします。
...
次に、すべてのアップストリーム シャードテーブルに対して次の DDL文を実行して、文字セットを変更します。
...
1. `binlog skip <task-name>`を実行して、MySQL インスタンス 1 と 2 で現在失敗している DDL文をスキップします。
...
3. `binlog skip <task-name>`を再度実行して、MySQL インスタンス 1 と 2 で現在失敗している DDL文をスキップします。
...
4. タスクのステータスを表示するには`query-status <task-name>`を使用します。
    ...
    タスクはエラーなしで正常に実行され、4つの間違った DDL文はすべてスキップされていることがわかります。

### 移行が中断された場合はDDLを置き換える {#replace-ddl-if-the-migration-gets-interrupted}

移行が中断されたときに DDL文を置き換える必要がある場合は、 `binlog replace`コマンドを実行します。
...
2. タスクのステータスを表示するには`query-status <task-name>`を使用します。
    ...
    タスクが正常に実行され、間違った DDL文が正常に実行される新しい DDL文に置き換えられていることがわかります。
...
1. 次のコマンドを使用して、MySQL インスタンス 1 と MySQL インスタンス 2 の間違った DDL文をそれぞれ置き換えます。
...
3. `handle-error <task-name> replace`を再度実行して、MySQL インスタンス 1 と 2 の間違った DDL文を置き換えます。
...
4. タスクのステータスを表示するには`query-status <task-name>`を使用します。
    ...
    タスクはエラーなしで正常に実行され、4つの誤った DDL文がすべて置き換えられていることがわかります。
