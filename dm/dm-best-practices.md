...
TiDBの`AUTO_INCREMENT`はMySQLの`AUTO_INCREMENT`と互換性があります。ただし、分散データベースであるTiDBは、通常複数のコンピューティングノード（クライアント側のエントリ）を備えています。アプリケーションデータの書き込み時には、ワークロードが均等に分散されます。そのため、テーブルに`AUTO_INCREMENT`列目がある場合、その列のAUTO_INCREMENT IDが不連続になる可能性があります。詳細については、 [AUTO_INCREMENT](/auto-increment.md#implementation-principles)参照してください。
