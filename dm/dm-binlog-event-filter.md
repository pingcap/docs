...
- `events`: the binlog event array. You can only select one or more `Event`s from the following table:
    ...
    | `value range decrease` | incompatible DDL  | A DDL statement that makes a column field smaller, such as the `ALTER TABLE MODIFY COLUMN` statement that changes `VARCHAR(20)` to `VARCHAR(10)`  |
...
If you need to report errors on DDL statements for some upstream operations, use the following settings:
