...
DM v2.0.2以降では、ソース設定ファイルでbinlogイベントフィルターを設定できます。詳細については、 [上流データベース設定ファイル](/dm/dm-source-configuration-file.md)参照してください。
...
-   `events` : binlogイベント配列。次の表から1つ以上の`Event`のみを選択できます。
    ...
    | `modify ストレージ engine`      | 互換性のないDDL | テーブルストレージエンジンを変更するDDL文（ `ALTER TABLE ENGINE = MyISAM`文など）                                |
