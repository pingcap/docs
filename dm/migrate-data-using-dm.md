...
- 上流および下流のデータベースインスタンスの情報：
    ...
    | ---------------- | ------------ | ---- | ----- | -------------------------------- |
    | アップストリームのMySQL-1 | 172.16.10.81 | 3306 | root     | VjX8cEeTX+qcvZ3bPaO4h0C80pe/1aU= |
    | アップストリームのMySQL-2 | 172.16.10.82 | 3306 | root     | VjX8cEeTX+qcvZ3bPaO4h0C80pe/1aU= |
    | ダウンストリーム TiDB    | 172.16.10.83 | 4000 | root     |                                  |
...
DMクラスタの実行中、DM-master、DM-worker、およびdmctlは、監視メトリクス情報をログに出力します。各コンポーネントのログディレクトリは以下のとおりです。

- DM-masterのログディレクトリ：これは`--log-file` DM-masterプロセスパラメータで指定されます。DMがTiUPを使用してデプロイされている場合、ログディレクトリはDM-masterノードの`{log_dir}`になります。
- DM-workerのログディレクトリ：これは`--log-file` DM-workerプロセスパラメータで指定されます。DMがTiUPを使用してデプロイされている場合、ログディレクトリはDM-workerノードの`{log_dir}`になります。
