...
5.  移行が完了すると、元のテーブルと`_gho`テーブルの両方の名前が変更され、オンライン DDL 操作が完了します。
    ...
    -   DMは`rename to _test4_del`を実行しません。`rename ghost_table to origin table`実行する場合、DMは以下の手順を実行します。
...
5.  データ移行が完了すると、元のテーブルと`_new`テーブルの名前が変更され、オンライン DDL 操作が完了します。
    ...
    -   DMは`rename to _test4_old`を実行しません。`rename ghost_table to origin table`実行する場合、DMは以下の手順を実行します。
