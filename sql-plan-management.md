...
You can execute this statement to change the status of a binding. The default status is ENABLED. The effective scope is GLOBAL and cannot be modified.
...
| status | Status including Enabled (available, replacing Using from v6.0), Disabled (unavailable), Deleted, Invalid, Rejected, and Pending verification|
...
This function allows you to configure a blocklist to filter out queries that meet the requirements. A blocklist has three dimensions, table name, frequency, and user name.
...
| User name       | user         | Statements executed by blocklisted users are not captured.                           | If multiple users execute the same statement, this statement is not captured only if the user names are all in the blocklist. |
