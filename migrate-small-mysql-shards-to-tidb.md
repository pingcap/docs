...
`id`列目は主キー、 `sid`列目はシャーディングキーです。`id`列目はAUTO_INCREMENTであり、複数のシャーディングテーブル範囲が重複するとデータ競合が発生します。`sid`はインデックスのグローバル一意性を保証するため、 [AUTO_INCREMENT主キーの主キー属性を削除します](/dm/shard-merge-best-practices.md#remove-the-primary-key-attribute-from-the-column)の手順に従って`id`列目をバイパスできます。
