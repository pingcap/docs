...
このチュートリアルでは、 [TiDBベクトル検索](/ai/concepts/vector-search-overview.md)と[ランチェーン](https://python.langchain.com/)を統合する方法を説明します。
...
> **ヒント**
>
> 完全な[サンプルコード](https://github.com/langchain-ai/langchain/blob/master/docs/docs/integrations/vectorstores/tidb_vector.ipynb) Jupyter Notebook で表示することも、 [コラボ](https://colab.research.google.com/github/langchain-ai/langchain/blob/master/docs/docs/integrations/vectorstores/tidb_vector.ipynb)オンライン環境で直接実行することもできます。
...
接続パラメータの値は、TiDB クラスターに合わせて変更する必要があります。ローカルマシンで TiDB を実行している場合、デフォルトでは`<HOST>`が`127.0.0.1`です。初期の`<PASSWORD>`は空なので、クラスターを初めて起動する場合はこのフィールドを省略できます。
...
プロジェクト ディレクトリに`data/how_to/`という名前のディレクトリを作成し、 [ランチェーン-ai/ランチェーン](https://github.com/langchain-ai/langchain) GitHub リポジトリからサンプル ドキュメント[`state_of_the_union.txt`](https://github.com/langchain-ai/langchain/blob/master/docs/docs/how_to/state_of_the_union.txt)ダウンロードします。
...
TiDBベクトルストアは、ベクトル間の類似性を測定するために、コサイン距離（ `cosine` ）とユークリッド距離（ `l2` ）の両方をサポートしています。デフォルトの戦略はコサイン距離です。
...
LangChainにおいて、 [レトリーバー](https://python.langchain.com/v0.2/docs/concepts/#retrievers)非構造化クエリのドキュメントを取得するインターフェースであり、ベクターストアよりも多くの機能を提供します。以下のコードは、TiDBベクターストアを取得子として使用する方法を示しています。
