...
> - ベクトル検索機能は、[TiDB Self-Managed](/overview.md)、[{{{ .starter }}}](/tidb-cloud/select-cluster-tier.md#starter)、[{{{ .essential }}}](/tidb-cloud/select-cluster-tier.md#essential)、および [TiDB Cloud Dedicated](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) で利用できます。TiDB Self-Managed と TiDB Cloud Dedicated では、TiDB バージョンが v8.4.0 以降である必要があります（v8.5.0 以降を推奨します）。
