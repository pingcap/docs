...
<sup>2</sup> BYOK（Bring Your Own Key）モデルでは、対応する埋め込みプロバイダーから独自のAPIキーを提供する必要があります。TiDB TiDB CloudはBYOKモデルの使用料を請求しません。これらのモデルの使用に伴うコストの管理と監視はお客様の責任となります。
...
-   `use_server` *(オプション)* : TiDB Cloud のホスト型埋め込みサービスを使用するかどうか。TiDB TiDB Cloud Starterの場合は、デフォルトで`True`になります。
