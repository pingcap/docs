...
> **Note:**
>
> Vector data types are available on TiDB Self-Managed, [{{{ .starter }}}](/tidb-cloud/select-cluster-tier.md#starter), [{{{ .essential }}}](/tidb-cloud/select-cluster-tier.md#essential), and [TiDB Cloud Dedicated](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated). For TiDB Self-Managed and TiDB Cloud Dedicated, the TiDB version must be v8.4.0 or later (v8.5.0 or later is recommended).
...
- Vector index support: You can build a [vector search index](/ai/vector-search-index.md) to speed up vector searching.
...
For available functions and operators over the vector data types, see [Vector Functions and Operators](/ai/vector-search-functions-and-operators.md).

For more information about building and using a vector search index, see [Vector Search Index](/ai/vector-search-index.md).
...
However, note that you cannot build a [vector search index](/ai/vector-search-index.md) for this column, as vector distances can be only calculated between vectors with the same dimensions.

## Comparison

You can compare vector data types using [comparison operators](/functions-and-operators/operators.md) such as `=`, `!=`, `<`, `>`, `<=`, and `>=`. For a complete list of comparison operators and functions for vector data types, see [Vector Functions and Operators](/ai/vector-search-functions-and-operators.md).
...
For additional cast functions, see [Vector Functions and Operators](/ai/vector-search-functions-and-operators.md).
...
For restrictions on vector data types, see [Vector search limitations](/ai/vector-search-limitations.md) and [Vector index restrictions](/ai/vector-search-index.md#restrictions).
