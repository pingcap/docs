---
title: tdc fs import-file-system-token
...
- `--fs-token <string>`: Supply the token directly. Prefer `TDC_FS_TOKEN` or `--from-file` to avoid process argument exposure.
...
- Import a token from standard input:
    ...
    cat ./fs-token | tdc fs import-file-system-token --from-file - --region aws-us-east-1
    ```
