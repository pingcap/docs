...
[ベクター検索インデックス](/ai/reference/vector-search-index.md) 、リコール率がわずかに低下するだけのトレードオフで、ベクトル検索クエリのパフォーマンスを通常 10 倍以上大幅に向上させます。

## ベクトルインデックスが完全に構築されていることを確認する {#ensure-vector-indexes-are-fully-built}

大量のベクターデータを挿入すると、一部のデータがデルタレイヤーに保持され、永続化を待機している場合があります。TiDBは、データが永続化された後、このようなデータに対してベクターインデックスを構築します。すべてのベクターデータがインデックス化されるまで、ベクター検索のパフォーマンスは最適ではありません。インデックス構築の進行状況を確認するには、 [インデックス構築の進行状況をビュー](/ai/reference/vector-search-index.md#view-index-build-progress)参照してください。
