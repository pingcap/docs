...
> -   ベクトル検索機能は[TiDB Self-Managed](/overview.md) 、 [TiDB Cloud Starter](/tidb-cloud/select-cluster-tier.md#starter) 、 [TiDB Cloud Essential](/tidb-cloud/select-cluster-tier.md#essential) 、 [TiDB Cloud Dedicated](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated)で利用可能です。TiDB Self-ManagedおよびTiDB Cloud Dedicatedの場合、TiDBバージョンはv8.4.0以降である必要があります（v8.5.0以降を推奨）。

## ベクトル列にベクトル検索インデックスを追加する {#add-vector-search-index-for-vector-columns}

[ベクトル検索インデックス](/ai/reference/vector-search-index.md) 、リコール率がわずかに低下するだけのトレードオフで、ベクトル検索クエリのパフォーマンスを通常 10 倍以上大幅に向上させます。

## ベクトルインデックスが完全に構築されていることを確認する {#ensure-vector-indexes-are-fully-built}

大量のベクトルデータを挿入すると、一部のデータがデルタレイヤーに保持され、永続化を待機している場合があります。TiDBは、データが永続化された後、このようなデータに対してベクトルインデックスを構築します。すべてのベクトルデータがインデックス化されるまで、ベクトル検索のパフォーマンスは最適ではありません。インデックス構築の進行状況を確認するには、 [インデックス構築の進行状況を確認する](/ai/reference/vector-search-index.md#view-index-build-progress)参照してください。
...
## 結果からベクトル列を除外する {#exclude-vector-columns-from-the-results}
...
一度も使用されていない、または長期間アクセスされていないインデックス（コールドアクセス）にアクセスする場合、TiDBはインデックス全体をメモリではなくクラウドストレージまたはディスクから読み込む必要があります。このプロセスには時間がかかり、多くの場合、クエリのレイテンシーが長くなります。さらに、長期間（例えば数時間）SQLクエリが実行されない場合、コンピューティングリソースが再利用されるため、以降のアクセスはコールドアクセスになります。
