...
大量のベクターデータを挿入すると、一部のデータがデルタレイヤーに保持され、永続化を待機している場合があります。TiDBは、データが永続化された後、このようなデータに対してベクターインデックスを構築します。すべてのベクターデータがインデックス化されるまで、ベクター検索のパフォーマンスは最適ではありません。インデックス構築の進行状況を確認するには、 [インデックス構築の進行状況を確認する](/ai/reference/vector-search-index.md#view-index-build-progress)参照してください。
