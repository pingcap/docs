...
> -   ベクトルデータ型は[TiDB Self-Managed](/overview.md) 、 [TiDB Cloud Starter](/tidb-cloud/select-cluster-tier.md#starter) 、 [TiDB Cloud Dedicated](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated)で使用できます。TiDB Self-Managed およびTiDB Cloud Dedicated の場合、TiDB バージョンは v8.4.0 以降である必要があります（v8.5.0 以降を推奨）。
...
ベクトル データ型で使用できる関数と演算子については、 [ベクトル関数と演算子](/ai/reference/vector-search-functions-and-operators.md)参照してください。

ベクトル検索インデックスの構築と使用の詳細については、 [ベクトル検索インデックス](/ai/reference/vector-search-index.md)参照してください。
...
[比較演算子](/functions-and-operators/operators.md)を使用して、 `=` 、 `!=` 、 `<` 、 `>` 、 `<=` 、 `>=`などのベクトルデータ型を比較できます。ベクトルデータ型の比較演算子と関数の完全なリストについては、 [ベクトル関数と演算子](/ai/reference/vector-search-functions-and-operators.md)参照してください。
...
複数のデータ型を受け入れる演算子または関数を使用する場合、文字列をその演算子または関数に渡す前に、文字列型を明示的にベクトル型にキャストする必要があります。これは、TiDBが暗黙的なキャストを行わないためです。例えば、比較演算を実行する前に、文字列をベクトルに明示的にキャストする必要があります。そうしないと、TiDBは文字列をベクトル数値ではなく文字列値として比較します。
...
追加のキャスト関数については、 [ベクトル関数と演算子](/ai/reference/vector-search-functions-and-operators.md)参照してください。

### ベクトルと他のデータ型間のキャスト {#cast-between-vector-other-data-types}
...
ベクトル データ型の制限については、 [ベクトル検索の制限](/ai/reference/vector-search-limitations.md)および[ベクトルインデックスの制限](/ai/reference/vector-search-index.md#restrictions)参照してください。
...
-   [ベクトル検索のパフォーマンスを向上させる](/ai/reference/vector-search-improve-performance.md)
