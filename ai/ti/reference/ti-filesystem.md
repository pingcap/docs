...
オーナートークンがトークン管理を認可している場合、そのトークンはトークンの一覧表示、スコープ付きトークンの作成、およびオーナートークンまたはスコープ付きトークンの失効を実行できます。有効化または無効化できるのはスコープ付きトークンのみです。TiDB Cloud API credentials は、どちらの種類のトークンでも有効化、無効化、または失効できます。
