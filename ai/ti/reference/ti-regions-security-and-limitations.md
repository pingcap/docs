...
| `ti fs delete-file-system` | TiDB Cloud API key, local resource registration, and owner resource credential |
...
- Do not copy the complete `~/.ti/` directory into an agent sandbox. For an existing Filesystem, pass only `TI_FS_TOKEN`, `TI_REGION_CODE`, and `TI_FS_FILE_SYSTEM_NAME`.
