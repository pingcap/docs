...
summary: TiDB Cloud Filesystem トークンを完全に取り消します。
---

# ti fs delete-file-system-token

Filesystem トークンを完全に取り消します。変更が反映されると、そのトークンは認証に使用できなくなり、一覧結果にも表示されなくなります。オーナートークンは同じ Filesystem 内のいずれの種類のトークンも取り消せますが、スコープ付きトークンではこのコマンドを使用できません。
