...
- Create a layer:
    ...
    ti fs create-layer --file-system-name workspace --base-root-path /workspace --layer-name agent-task
    ```
...
- Create a restore-safe tagged layer:
    ...
    ti fs create-layer --file-system-name workspace --base-root-path /workspace --durability-mode restore-safe --tag task=review
    ```
