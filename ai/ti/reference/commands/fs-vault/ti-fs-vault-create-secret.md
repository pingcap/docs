...
- Create a secret from values and a file:
    ...
    ti fs-vault create-secret --file-system-name workspace --secret-name db-prod --field DB_URL=mysql://example --field PASSWORD=@./password.txt
    ```
...
- Read a secret field from standard input:
    ...
    printf '%s' "$API_TOKEN" | ti fs-vault create-secret --file-system-name workspace --secret-name api-dev --field TOKEN=-
    ```
...
- Preview secret creation:
    ...
    ti fs-vault create-secret --file-system-name workspace --secret-name api-dev --field TOKEN=@./token.txt --dry-run
    ```
