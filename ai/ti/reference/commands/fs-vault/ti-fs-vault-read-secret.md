...
- Read one secret field as raw text:
    ...
    ti fs-vault read-secret --file-system-name workspace --secret-name db-prod --field PASSWORD --format raw
    ```
...
- Format a field as an environment assignment:
    ...
    ti fs-vault read-secret --file-system-name workspace --secret-name db-prod --field DB_URL --format env
    ```
...
- Read with a delegated Vault token:
    ...
    ti fs-vault read-secret --file-system-name workspace --secret-name db-prod --field DB_URL --vault-token "$TI_VAULT_TOKEN" --format raw
    ```
