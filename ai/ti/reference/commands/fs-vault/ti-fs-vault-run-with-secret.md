...
- Run a process with secret fields:
    ...
    ti fs-vault run-with-secret --file-system-name workspace --secret-path /n/vault/db-prod -- env
    ```
...
- Use an injected field in a shell command:
    ...
    ti fs-vault run-with-secret --file-system-name workspace --secret-path /n/vault/db-prod -- sh -c 'test -n "$DB_URL"'
    ```
