...
- Mount a delegated Vault view:
    ...
    ti fs-vault mount-vault --file-system-name workspace --mount-path ./vault --vault-token "$TI_VAULT_TOKEN"
    ```
...
- Run the Vault mount in the foreground:
    ...
    ti fs-vault mount-vault --file-system-name workspace --mount-path ./vault --foreground --ready-timeout 60s
    ```
