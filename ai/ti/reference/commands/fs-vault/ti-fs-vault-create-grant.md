...
- Create a temporary read grant:
    ...
    ti fs-vault create-grant --file-system-name workspace --agent-id deploy-agent --scope db-prod/DB_URL --permission read --ttl 10m
    ```
...
- Return only the delegated token:
    ...
    ti fs-vault create-grant --file-system-name workspace --agent-id ci-agent --scope api-dev/TOKEN --permission read --ttl 5m --token-only
    ```
