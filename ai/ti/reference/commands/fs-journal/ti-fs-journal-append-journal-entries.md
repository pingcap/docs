...
- Append one JSON entry:
    ...
    ti fs-journal append-journal-entries --file-system-name workspace --journal-id jrn-demo --entry-json '{"type":"task.started"}'
    ```
...
- Append an idempotent typed entry:
    ...
    ti fs-journal append-journal-entries --file-system-name workspace --journal-id jrn-demo --entry-type task.completed --subject issue-42 --idempotency-key issue-42-complete
    ```
...
- Append a JSON array from standard input:
    ...
    printf '[{"type":"step.started"},{"type":"step.completed"}]' | ti fs-journal append-journal-entries --file-system-name workspace --journal-id jrn-demo --json-array
    ```
