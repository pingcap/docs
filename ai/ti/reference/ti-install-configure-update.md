...
以前に `tdc` v0.1.x を使用していた場合、`ti` は `~/.tdc/` から `~/.ti/` へ、サポートされているローカルプロファイル、認証情報、設定、Preferences、および Filesystem の状態を移行できます。`ti` をインストールする前に、`tdc` によって開始された Filesystem または Vault のマウントをすべてアンマウントしてください。
