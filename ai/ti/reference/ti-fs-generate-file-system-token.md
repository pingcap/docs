...
summary: 1 つの TiDB Cloud Filesystem 用の追加の owner token を生成します。
---

# ti fs generate-file-system-token

TiDB Cloud API 認証情報を使用して、Filesystem の owner token を生成します。トークン値はコマンド出力にのみ表示され、後から取得することはできません。トークンをローカルの認証情報ストアに保存するには、`--store-locally` を使用します。既存の Filesystem token を使用して owner token を生成することはできません。
...
- `--file-system-id <string>`: トークンを所有する Filesystem を指定します。FS token ではこのオプションを置き換えることも、owner-token の生成を認可することもできません。このオプションは必須です。
