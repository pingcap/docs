...
Filesystem オーナーはシークレットを一度だけ保存し、必要なフィールドにスコープを限定した短期間有効な grant を作成します。エージェントは委任された vault トークンのみを受け取り、許可された値を子プロセスに注入できます。オーナーは監査イベントを確認し、Filesystem オーナー認証情報をローテーションしたり公開したりすることなく grant を取り消せます。
