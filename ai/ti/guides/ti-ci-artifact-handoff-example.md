...
パイプラインは、1 つの Filesystem トークンとリージョンを両方のジョブに注入します。トークンは Filesystem を識別します。生成側ジョブは `/ci/${RUN_ID}/` のような今回の実行に固有のパス配下に出力をアップロードし、コンシューマー側ジョブは別のランナー上でその正確なパスからデータをダウンロードまたはストリーミングします。どちらのジョブにも TiDB Cloud API キーやコピーされた `~/.ti/` ディレクトリは不要です。
