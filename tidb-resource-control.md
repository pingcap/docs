...
You can use [`SET RESOURCE GROUP`](/sql-statements/sql-statement-set-resource-group.md) statement to change the bind resource group of current session. By binding a session to a resource group, the resource usage of the corresponding session is limited by the specified usage (RU).
...
When system variable [`tidb_resource_control_strict_mode`](/system-variables.md#tidb_resource_control_strict_mode--new-in-v820) is set to `ON`, you need to have `SUPER` or `RESOURCE_GROUP_ADMIN` or `RESOURCE_GROUP_USER` privilege to use this hint.
